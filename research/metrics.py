"""Frozen, dependency-light metrics for the HS-TasNet research evaluator.
...
def _self_test() -> None:
    config = MetricConfig(sample_rate=8_000, window_samples=8_000,
                          hop_samples=8_000,
                          bands_hz={
                              "low_20_250": (20.0, 250.0),
                              "low_20_80": (20.0, 80.0),
    ...
    time = np.arange(8_000) / config.sample_rate
    refs = np.stack([
        np.stack([np.sin(2 * np.pi * frequency * time)] * 2)
        for frequency in (50.0, 110.0, 440.0, 880.0)
    ])
    estimates = refs + 0.01 * np.roll(refs, 1, axis=0)
    exact = windowed_sdr(refs[0], refs[0], config)["db"]
    assert exact == config.db_ceiling
    projection = projection_metrics(refs, estimates, config)
    assert projection["sir_db"] is not None and projection["sir_db"] > 30.0
    silent_projection = projection_metrics(refs, np.zeros_like(refs), config)
    assert silent_projection["sir_db"] == config.db_floor
    consistency = mixture_consistency(refs.sum(axis=0), refs, config)
    assert consistency["db"] == config.db_ceiling
    probe = single_source_probe_metrics(refs[0], estimates, 0, config)
    assert len(probe["output_to_input_db"]) == 4
    print(json.dumps({"self_test": "pass", "sir_db": projection["sir_db"]},
                     sort_keys=True))

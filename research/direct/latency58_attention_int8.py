"""Ten signed integer projections from the selected saved attention checkpoint.
...
VERSION = "temporal-attention-ten-u8s8-reduced-precise-v1"
...
def build(native, payload, training, checkpoint, quality_path):
    import onnx
    ...
    from research.direct.latency58_best_onnx_export import build as float_build
    from research.direct.latency58_conv_gemm import convert
    from research.direct.latency58_quadrature_all_s8 import source_matrices
    from research.direct.latency58_attention_int8_precision import rewrite

    fingerprint = state_sha256(native.state_dict())
    _, graph = float_build(native, payload, training, checkpoint, quality_path)
    require(len(graph.graph.input) == len(graph.graph.output) == 7,
            "Require the selected attention model's six-state interface")
    ...
    properties["hs_tasnet.source_checkpoint_full14_sdr_db"] = properties.pop("hs_tasnet.full14_sdr_db")
    properties["hs_tasnet.source_checkpoint_full14_result_sha256"] = properties.pop("hs_tasnet.full14_result_sha256")
    properties.update({"hs_tasnet.runtime_variant": VERSION,
                       ...
                       "hs_tasnet.floating_additional_layers": "phase factors, fusion refinement and temporal attention",
...
def make_reference(native, integer_graph, proof):
    ...
    from research.direct.latency58_best_onnx import make_export_copy
    from research.direct.latency58_int8_reference import IntegerGRU
    ...
    return AttentionIntegerReference(copied.double()).eval().requires_grad_(False), independent


class AttentionIntegerReference(nn.Module):
    ...
    def forward(self, audio, history, hidden, spectral_tail, waveform_tail, past_keys, past_values):
        ...
        require(all(v.dtype == torch.float32 and v.device.type == "cpu" for v in
                    (audio, history, hidden, spectral_tail, waveform_tail, past_keys, past_values)),

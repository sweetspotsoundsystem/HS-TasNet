"""Retire one closed failed Adam state, first retaining its exact RNG payload."""
...
def main():
    ...
    out = PHASE / "magnitude-storage-001"
    ...
    for name in ("direct-sdr-001", "direct-sdr-musdb-001"):
        ...
        require(result["status"] == "training_audit_and_full14_complete" and not result["target_reached"]
                ...
                "Both direct-SDR pilots must be closed lower-SDR endpoints")
        ...
        for path in (root / "production-run/checkpoint").iterdir():
            if not (name == "direct-sdr-001" and path.name == "optimizer.pt"):
                protected[str(path)] = sha(path)
    ...
    generation = PHASE / "direct-sdr-001/production-run/checkpoint"
    ...
    require(before - binding["bytes"] + 800_000_000 + 180_000_000 + 1_000_000 < 80_000_000_000,
            "Retirement cannot fund magnitude training")
    ...
    write(out / "receipt.json", {"status": "complete", "intent_sha256": sha(out / "intent.json"), "retired": retired,
          ...
          "counted_bytes_after": after, "forecast_including_outside_and_magnitude": after + 800_000_000 + 180_000_000})

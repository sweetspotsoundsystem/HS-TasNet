"""Verify long trajectories after replacing only the quadrature encoder quantization."""
import hashlib
import json
import os
...
def main():
    ...
    from research.direct.latency58_quadrature_encoder_s8 import build, make_reference
    ...
    screen_execution_path = PHASE / "quadrature-encoder-s8-screen-stage-002/execution.json"
    ...
    out = PHASE / "quadrature-encoder-s8-long-001"
    require(not out.exists(), "Preserve long diagnostics")
    out.mkdir()
    paths = [original_path, original_proof_path, source_path, Path(__file__).resolve(), ROOT / "research/direct/latency58_quadrature_int8.py",
             screen_path, screen_path.parent / "plan.json", screen_execution_path, Path(checkpoint["path"]), music_path]
    paths.extend(ROOT / "research/direct" / name for name in ("latency58_quadrature_encoder_s8.py", "latency58_quadrature_magint8.py"))
    ...
    write(out / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
          ...
          "tolerances": TOLERANCES, "pending_training_reservation_bytes": 0,

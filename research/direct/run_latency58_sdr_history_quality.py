...
def main():
    ...
    require(training["schema"] == "latency58-sdr-history-training-v1", "Require a context trial plan")
    ...
    require(audit["status"] == "pass" and audit["source_bindings_unchanged"]
            and audit["normalized_drum_objective_journal_verified"]
            ...
            and audit["history_version"] == training["history_version"]
            and audit["warmup_samples"] == training["warmup_samples"]
            and audit["teacher_history_samples"] == training["teacher_history_samples"]
    ...
    paths = [audit_path, audit_exec_path, stage / "execution.json", stage / "stage.json", monitor_path,
             ...
             Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_sdr_history.py",
             ROOT / "research/direct/compare_latency58_sdr.py", ROOT / "research/direct/compare.py"]
    if "probes" in args.modes:
        paths.append(ROOT / "research/direct/evaluate_latency58_sdr_history_probes.py")
    if "full14" in args.modes:
        ...
        paths += [proof_path, ROOT / "research/direct/evaluate_latency58_sdr_history_parallel.py"]
    ...
    for mode in args.modes:
        ...
        plan.update(schema="latency58-sdr-history-" + ("probe" if mode == "probes" else "music") + "-plan-v1",
        ...
        if mode == "full14":
            plan.update(schema="latency58-sdr-history-parallel-music-plan-v1", workers=2,
                        model_kind="history_candidate", track_indices=list(range(14)),
    ...
    for mode, out, plan in prepared:
        ...
        module = "research.direct.evaluate_latency58_sdr_history" + ("_probes" if mode == "probes" else "_parallel" if mode == "full14" else "")

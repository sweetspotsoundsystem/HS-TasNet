"""Diagnose the rejected signed kernel against exact and saturated-pair arithmetic."""
...
def build(native, saved_graph):
    ...
    signed = [quantize_data(np.ascontiguousarray(weight[:, column]), T.INT8,
                            symmetric=True, reduce_range=False) for column in range(3000)]
    ...
    models = {"u8u8": graph(nodes8, [nh.from_array(v, k) for k, v in arrays8.items()]),
              "u8s8_symmetric": graph(nodes_s8, [nh.from_array(v, k) for k, v in arrays_s8.items()])}
    proof = {"weight_shape": list(weight.shape), "source_matrix_sha256": fingerprint(weight),
             ...
             "bits": 8, "symmetric": True, "reduce_range": False, "weight_zero_points_all_zero": True,
...
def references(inputs, weights):
    ...
    rows = {"u8u8": [], "u8s8_symmetric": [], "u8s8_saturated_pairs": []}
    for row in inputs:
        ...
        for label, values, scales, zeros in (("u8u8", q8, s8, z8), ("u8s8_symmetric", qs8, ss8, zs8)):
        ...
        products = quantized.astype(np.int64)[:, None] * qs8.astype(np.int64)
        pairs = products[::2] + products[1::2]
        saturated = np.clip(pairs, -32768, 32767).sum(0) - zero.astype(np.int64) * qs8.astype(np.int64).sum(0)
        rows["u8s8_saturated_pairs"].append(saturated.astype(np.float32) * (scale * ss8) + bias)
...
def main():
    ...
    failed_root = PHASE / "encoder-s8-kernel-screen-001"
    failed_stage = PHASE / "encoder-s8-kernel-screen-stage-001/execution.json"
    failed = read(failed_stage)
    require(failed["actual_exit_code"] == 1 and failed["source_bindings_unchanged"], "Require the recorded numerical rejection")
    verify_inputs(read(failed_root / "plan.json"))
    paths = (failed_root / "plan.json", failed_stage, Path(__file__).with_name("check_latency58_encoder_s8_kernel.py"),
    ...
    out = PHASE / "encoder-s8-saturation-diagnostic-001"
    ...
    write(out / "plan.json", {"source_bindings": bindings, "source_checkpoint": parent_binding,
          ...
          "recipe": {"weight_bits": 8, "signed_weights": True, "symmetric": True, "reduce_range": False},
    ...
    with tempfile.TemporaryDirectory(prefix="latency58-encoder-kernel-", dir=PHASE) as temporary:
        ...
        for cycle, label in enumerate(("u8u8", "u8s8_symmetric", "u8s8_symmetric", "u8u8"), 1):
            ...
            saturation_error = float(np.max(np.abs(actual - expected["u8s8_saturated_pairs"]))) if label == "u8s8_symmetric" else None
            write(out / f"cycle-{cycle}-numeric.json", {"variant": label, "exact_integer_error": error,
                  "saturated_pair_reference_error": saturation_error,
                  "per_case_exact_integer_error": np.max(np.abs(actual - expected[label]), axis=1).tolist()})
            np.savez_compressed(out / f"cycle-{cycle}-values.npz", inputs=inputs, actual=actual,
                                exact=expected[label], saturation=expected["u8s8_saturated_pairs"])
            require((error < 1e-4 if label == "u8u8" else error > 1e-4 and saturation_error < 1e-4),
                    "Observed signed-kernel error is not explained by saturated adjacent products")
            ...
            numerical[label] = {"native_reference_max_abs": error, "saturated_pair_reference_error": saturation_error, "native_outputs_sha256": fingerprint(actual),
    ...
    write(out / "result.json", {"status": "diagnostic_complete", "s8_kernel_selected": False, "source_bindings_unchanged": True,
          ...
          "median_of_process_p50_us": medians, "s8_over_u8_ratio": medians["u8s8_symmetric"] / medians["u8u8"],
          ...
          "conclusion": "Signed full-range weights are rejected: native outputs match saturated adjacent U8*S8 products rather than exact integer dot products on this x86 CPU. Preserve the unsigned graph. This is not an M4 result.",
          "limitation": "Synthetic kernel diagnosis on the local x86 CPU during GPU training. No full-model quality score or speed claim."})
    print(json.dumps({"status": "diagnostic_complete", "medians_us": medians, "numerical": numerical}), flush=True)

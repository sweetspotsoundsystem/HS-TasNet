"""Qualify two-second B4 context gradients and B16 loss accumulation on CUDA."""
import torch
...
def check_loss(device, generator):
    ...
    pieces = [objective(raw[i:i + 4], deployed[i:i + 4], target[i:i + 4], mixture[i:i + 4], reduction)
    ...
    return {"status": "pass", "logical_batch_size": 16, "microbatch_size": 4,
...
def check_gpu(model):
    ...
    try:
        with torch.random.fork_rng(devices=[parameter.device.index]):
            audio = .03 * torch.randn(4, 2, CROP_SAMPLES, device=parameter.device, generator=generator)
    ...
    return {**context, "logical_batch_loss": loss, "original_model_state_sha256": fingerprint,
            ...
            "precision": "BF16 learned operations; FP32 FFT, states and loss", "microbatch_size": 4,

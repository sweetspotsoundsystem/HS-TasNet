...
def check():
    ...
    for case, batch, samples in (("heterogeneous_b16_two_seconds", 16, 88320),
        ...
        pieces = [objective(raw_micro[i:i + micro], deployed_micro[i:i + micro], target[i:i + micro],
                            mixture[i:i + micro], reduction) for i in range(0, batch, micro)]
        ...
        with torch.no_grad():
            naive = sum(full_objective(raw[i:i + micro], deployed[i:i + micro], target[i:i + micro],
                                       mixture[i:i + micro]).total for i in range(0, batch, micro)) / 4
        ...
        rows.append({"case": case, "batch_size": batch, "microbatch_size": micro, "scored_samples": samples,

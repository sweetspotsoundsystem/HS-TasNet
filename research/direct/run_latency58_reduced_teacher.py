"""Prepare or run a bounded reduced-teacher SDR trial under the existing GPU supervisor."""
...
from research.direct.latency58_reduced_teacher_checkpoint import validate_recipe, validate_journal, require_space, load_parent, LOSS_KEYS
...
def launch(plan_path, previous_execution_path, name):
    ...
    stage = {"schema": "latency58-reduced-teacher-stage-v1", "plan_sha256": sha(plan_path),
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_reduced_teacher",
    ...
    monitor_out = Path(plan["watchdog_source"]).parent / ("latency58-reduced-teacher-001-" + name)
...
def prepare():
    ...
    reference_path = PHASE / "cleanup-rebound-prep-001/training-plan.json"
    ...
    sources = [Path(__file__).resolve(), reference_path,
               ROOT / "research/direct/latency58_reduced_teacher_loss.py",
               ROOT / "research/direct/check_latency58_reduced_teacher_loss.py",
               PHASE / "reduced-teacher-functional-001.json",
               generation / "metrics.jsonl", generation / "receipt.json",
               ROOT / "research/direct/train_latency58_reduced_teacher.py",
               ROOT / "research/direct/latency58_reduced_teacher_checkpoint.py",
               ROOT / "research/direct/audit_latency58_reduced_teacher.py",
               ROOT / "research/direct/hare_loss_ablation_checkpoint.py",
               ROOT / "research/direct/score_latency58_reduced_teacher.py",
               ROOT / "research/direct/evaluate_latency58_reduced_teacher.py",
               ROOT / "research/direct/evaluate_latency58_reduced_teacher_parallel.py"]
    bindings = {**original_sources, **{str(p): sha(p) for p in sources}}
    OUT.mkdir()
    decision = {"schema": "latency58-reduced-teacher-decision-v1", "comparison_variable": "teacher_weight",
                "candidate_weight": .25, "control_weight": .5,
                ...
                "selection_scope": "Improve full14 SDR from C204; the previous half-teacher continuation is the training control.",
                "evaluation": "Unchanged primary full14; compare C204 and the matched half-teacher continuation, with per-stem costs.",
    ...
    for key in ("resource_plan", "full_resource", "full_resource_execution", "resource_comparison",
        ...
        plan.pop(key, None)
    plan.update(schema="latency58-reduced-teacher-training-v1", resource_only=True,
                comparison_variable="teacher_weight", teacher_weight=.25, additional_loss_weight=.5,
                reduced_teacher_version="latency58-ordinary-teacher-quarter-v1",
                functional_check=binding(PHASE / "reduced-teacher-functional-001.json"),
                matched_positive_training_plan=binding(reference_path),
                ...
                run_dir=str(PHASE / "latency58-reduced-teacher-resource-run-001"),
    ...
    for row in fixture:
        row.update(teacher_weight=.25, reduced_teacher_version=plan["reduced_teacher_version"])
        for micro in row["microbatches"]:
            base = micro["supervised_loss"] + .25 * micro["teacher_l1"]
            micro.update(teacher_weight=.25, reduced_teacher_version=plan["reduced_teacher_version"],
                         base_loss=base, loss=base + micro["controlled_deployed_contribution"])
        row.update({key: sum(m[key] for m in row["microbatches"]) / 4 for key in LOSS_KEYS})
    def validate(rows):
        return validate_journal(("\n".join(json.dumps(row) for row in rows) + "\n").encode(), 2, plan, helpers)
    validate(fixture)
    rejected = 0
    for field, value in (("augmented_batch_sha256", "0" * 64), ("teacher_weight", .5),
    ...
    write(OUT / "preflight.json", {"status": "pass", "cpu_parent_state_exact": True,
          "quarter_weight_arithmetic_fixture_pass": True, "negative_journal_fixtures_rejected": rejected,
    ...
    print(json.dumps({"event": "prepared", "resource_plan": str(OUT / "resource-plan.json"),
                      "training_launched": False}), flush=True)


def run():
    require(Path.cwd() == ROOT, "Unexpected cwd")
    summary_path = PHASE / "hare-loss-ablation-001/sdr-comparison.json"
    summary = read(summary_path)
    require(summary["status"] == "pass" and summary["source_bindings_unchanged"], "Prior SDR scoring is incomplete")
    verify_inputs(summary)
    require(summary["full_mixture_aggregate"]["full_sdr_db"] <= 4.069078803302578,
            "Prior trial already improved SDR; review it before launching another experiment")
    plan = read(OUT / "resource-plan.json")
    verify_inputs(plan)
    validate_recipe(plan)
    require_space(plan, 400_000_000)
    write(OUT / "launch-prerequisite.json", {"prior_sdr_comparison": binding(summary_path),
          "prior_full_sdr_db": summary["full_mixture_aggregate"]["full_sdr_db"],
          "parent_full_sdr_db": 4.069078803302578})
    previous = PHASE / "hare-loss-ablation-001/production/execution.json"
    ...
    production.update(resource_only=False, run_dir=str(PHASE / "latency58-reduced-teacher-quarter-b16-250-001"),
    ...
    execute([PYTHON, "-u", "-m", "research.direct.audit_latency58_reduced_teacher", "--plan", str(OUT / "training-plan.json"),
    ...
    from research.direct.score_latency58_reduced_teacher import main as score
    score()

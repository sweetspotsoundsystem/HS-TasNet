...
saving two experimental ONNX graphs while a training checkpoint is reserved.
...
def main():
    ...
    require_cpu_space(source, 2_000_000)
    training_root = Path(source["output_directory"])
    training_full14_started = (training_root / "full14").exists()
    if training_full14_started:
        closed = read(training_root / "full14/execution.json")
        require(closed["actual_exit_code"] == 0 and closed["source_bindings_unchanged"],
                "Wait for the training endpoint's CPU scoring to close")
    training_active = not (training_root / "production-stage/execution.json").exists()
    ...
    execution_path = PHASE / "m4-quadrature-magint8-full14-memory-stage-001/execution.json"
    ...
    screen_path = PHASE / "m4-quadrature-magint8-screen-001/result.json"
    screen = read(screen_path)
    parent = source["parent_checkpoint"]
    require(parent == screen["checkpoint"], "Wrong saved parent")
    native, _ = load_model(parent)
    fingerprint = state_sha256(native.state_dict())
    floating, integer, graph, proof, nine = build(native, return_baseline=True)
    ten = graph.SerializeToString()
    digests = {"quadrature_nine": hashlib.sha256(nine).hexdigest(), "quadrature_ten": hashlib.sha256(ten).hexdigest()}
    require(digests["quadrature_ten"] == screen["graph_sha256"] == quality["results"][0]["checkpoint"]["sha256"]
            and digests["quadrature_nine"] == proof["baseline_nine_projection_graph_sha256"]
            == read(PHASE / "m4-quadrature-int8-screen-001/result.json")["graph_sha256"]
    ...
    del native, floating, integer, graph
    ...
    paths = [source_path, review_path, execution_path, quality_root / "result.json", quality_root / "plan.json",
             screen_path, cpp, c204, Path(__file__).resolve(), Path(compiler), *libraries, *headers]
    ...
    paths.extend(ROOT / "research/direct" / name for name in (
        "latency58_quadrature_magint8.py", "latency58_int8_precise_ten.py",
    ...
    out = PHASE / "m4-quadrature-memory-native-001"
    ...
    write(out / "plan.json", {"schema": "latency58-native-sealed-memory-three-model-v1", "source_bindings": bindings,
          "runtime_graphs": {"c204_integer": {"path": str(c204), "sha256": sha(c204)},
              **{label: {"sha256": digests[label], "bytes": len(data), "saved": False, "sealed_memfd": True}
                 for label, data in (("quadrature_nine", nine), ("quadrature_ten", ten))}},
          ...
          "scope": "Linux native preallocated CPU1 inference; no concurrent quality scoring",
          "gpu_training_active_at_start": training_active,
          "speed_gate": {"maximum_ten_over_nine_median_ratio": .97, "minimum_faster_cycles": 4},
    ...
    with ExitStack() as resources, tempfile.TemporaryDirectory(prefix="latency58-native-") as temporary:
        fd9, path9 = resources.enter_context(sealed_graph("quadrature-nine", nine))
        fd10, path10 = resources.enter_context(sealed_graph("quadrature-ten", ten))
        models = {"c204_integer": str(c204), "quadrature_nine": path9, "quadrature_ten": path10}
    ...
    require((training_root / "full14").exists() == training_full14_started,
            "Training endpoint began CPU scoring during timing; retain rows without qualifying this run")
    medians = {label: statistics.median(row["run"]["p50_ms"] for row in rows if row["variant"] == label) for label in models}
    ratio = medians["quadrature_ten"] / medians["quadrature_nine"]
    faster = sum(next(row["run"]["p50_ms"] for row in rows if row["cycle"] == cycle and row["variant"] == "quadrature_ten")
                 < next(row["run"]["p50_ms"] for row in rows if row["cycle"] == cycle and row["variant"] == "quadrature_nine")
    ...
    print(json.dumps({"status": "pass", "median_of_cycle_p50_ms": medians, "ten_over_nine_median_ratio": ratio,

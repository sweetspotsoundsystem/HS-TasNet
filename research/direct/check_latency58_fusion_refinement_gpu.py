"""Resource-stage check of fusion_refinement warmup at the full training batch size."""
import torch

from research.direct.check_latency58_fusion_refinement import compare_context
...
def check_gpu(model):
    ...
    require(type(model) is Latency58FusionRefinementModel and parameter.device.type == "cuda"
            and model.training and model.training_precision == "bf16"
            and all(p.requires_grad and p.grad is None for p in model.parameters()),
            "Require a fresh BF16 fusion-refinement training model before its first optimizer update")
    fingerprint = state_sha256(model.state_dict())
    saved_expansion = model.fusion_refine_expand.weight.detach().clone()
    generator = torch.Generator(device=parameter.device).manual_seed(202609143)
    # A nonzero local fixture exercises both factors. Restoring the exact original
    # expansion keeps resource and production prefixes identical from zero refinement init.
    ...
    try:
        with torch.random.fork_rng(devices=[parameter.device.index]), torch.no_grad():
            model.fusion_refine_expand.weight.copy_(.002 * torch.randn(
                model.fusion_refine_expand.weight.shape, device=parameter.device, generator=generator))
        ...
        with torch.no_grad():
            model.fusion_refine_expand.weight.copy_(saved_expansion)

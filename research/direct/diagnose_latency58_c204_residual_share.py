...
Each CPU1 worker streams an unchanged C204 model once per full track.
...
def initialize_worker(plan):
    ...
    _PLAN = plan
    binding = plan["parent_quality_plan"]
    require(sha(binding["path"]) == binding["sha256"], "Parent loading plan changed")
    _MODEL, _ = load_evaluation_model(read(binding["path"]))
    _MODEL = _MODEL.eval().requires_grad_(False)
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-c204-residual-share-parallel-diagnostic-v1"
    ...
    for key in ("functional_check", "parent_quality_plan", "parent_full14_result"):
        binding = plan[key]
        require(sha(binding["path"]) == binding["sha256"]
                and plan["source_bindings"].get(binding["path"]) == binding["sha256"], "Evidence binding differs")
    functional = read(plan["functional_check"]["path"])
    require(functional["status"] == "pass" and functional["primary_share"] == 1 / 16
            and functional["zero_identity_and_prefix_exact"]
            and plan["parent_model_state_sha256"] == "c204b0fcb9627ca7fecd287db42fb869a1ae6783a1bc24cf2d8864c3b4a565fb",
            "Different C204 model or unchecked fixed correction")
    ...
    cells = music_cells(policies["working_policy"], policies["fixed_share"])
    require(all(sha(p) == s for p, s in plan["source_bindings"].items()), "Diagnostic input changed")
    result = {"schema": "latency58-c204-residual-share-full14-result-v1", "status": "pass",
              ...
              "primary_share": 1 / 16, "policies": policies, "comparison": comparison, "all_track_stem_cells": cells,
              ...
              "limitations": ["Previously used primary development panel; no new confirmation or unseen-song claim.",
                              ...
                              "Correction is not yet integrated into a saved inference model or plugin."]}

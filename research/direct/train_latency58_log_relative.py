"""One monitored resource check or bounded weak log-relative training stage."""
...
def main():
    ...
    require(plan["schema"] == "latency58-log-relative-training-v1" and stage["schema"] == "latency58-log-relative-stage-v1"
            ...
            and plan["log_relative_weight"] == 0.01,
    ...
    from research.direct.latency58_log_relative_checkpoint import (
    ...
    from research.direct.latency58_log_relative import AUXILIARY_WEIGHT, VERSION, log_relative_error
    require(plan["log_relative_weight"] == AUXILIARY_WEIGHT and plan["log_relative_version"] == VERSION,
    ...
    try:
        for mixture, targets in loader:
            ...
            relative = log_relative_error(deployed, targets[..., plan["warmup_samples"]:])
            loss = terms.total + plan["teacher_weight"] * auxiliary + plan["log_relative_weight"] * relative.total
            loss.backward()
            ...
            row = {"step": step, "lr": lr, "loss": float(loss.detach()), "supervised_loss": float(terms.total.detach()),
                   ...
                   "log_relative_loss": float(relative.total.detach()), "log_relative_weight": plan["log_relative_weight"],
                   "log_relative_per_stem": relative.per_stem.detach().cpu().tolist(),
                   "log_relative_active_windows": relative.active_window_counts.cpu().tolist(),
                   "log_relative_active_examples_per_stem": relative.active_examples_per_stem.cpu().tolist(),
        ...
        if plan["resource_only"]:
            ...
            helpers.atomic_json(run / "resource.json", {
                "schema": "latency58-log-relative-resource-result-v1", "status": "pass",
                ...
                "log_relative_weight": plan["log_relative_weight"], "log_relative_version": VERSION,

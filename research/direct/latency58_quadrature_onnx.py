"""Literal-hop ONNX export of the saved quadrature model with all four public states."""
...
from research.direct.latency58_asymmetric_onnx import verify_onnx

STATE_NAMES = tuple(QuadratureState._fields)
...
def make_export_copy(model):
    ...
    require(type(model) is Latency58QuadratureModel, "Require a saved quadrature model")
    ...
    class Latency58StreamingWrapper(torch.nn.Module):
        ...
        def forward(self, audio_chunk, audio_history, fusion_hidden, spectral_numerator_tail, waveform_tail):
            ...
            spec_features = copied.spec_norm(spec)
...
def run_float_case(model, wrapper, session, case):
    """Reuse the physical-sample verifier with the quadrature state constructor.

    The shared verifier constructs its first state with the older NamedTuple.
    This adapter changes only that Python container; all four tensors and every
    subsequent native forward/flush operation use the quadrature model.
    """
    from research.direct.latency58_asymmetric_onnx import _run_case

    class StateAdapter:
        def forward_chunk(self, audio, state):
            return model.forward_chunk(audio, QuadratureState(*state))

        def flush(self, state):
            return model.flush(QuadratureState(*state))

    require(type(model) is Latency58QuadratureModel, "Require native quadrature verification")
    return _run_case(StateAdapter(), wrapper, session, case)

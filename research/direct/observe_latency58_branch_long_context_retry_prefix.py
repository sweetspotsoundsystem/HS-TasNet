...
def main():
    ...
    root = PHASE / "branch-long-context-002"
    ...
    for entry in Path("/proc").iterdir():
        ...
        try:
            ...
            continue
        if len(argv) > 3 and argv[:4] == [PYTHON, "-u", "-m", "research.direct.run_latency58_branch_long_context_retry"]:
    ...
    paths = [plan_path, Path(__file__).resolve(), ROOT / "research/direct/report_latency58_branch_long_context_retry.py",
             root / "resource-stage/root-execution.json", root / "resource-stage/execution.json",
             root / "resource-run/result.json", PHASE / "branch-long-context-preparation-stage-002/execution.json",
             PHASE / "branch-long-context-preparation-stage-002/root-execution.json"]

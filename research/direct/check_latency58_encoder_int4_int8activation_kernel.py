"""Screen the four-bit encoder kernel with blockwise integer activations."""
...
def build(native, saved_graph):
    ...
    config = DefaultWeightOnlyQuantConfig(block_size=32, is_symmetric=True, accuracy_level=4, bits=4)
    ...
    nodes4 = [h.make_node("MatMulNBits", ["x", "w", "s"], ["linear"], name="matrix_int4",
                         domain="com.microsoft", K=2048, N=3000, block_size=32, bits=4, accuracy_level=4),
              h.make_node("Add", ["linear", "bias"], ["y"], name="bias")]
    ...
    proof = {"weight_shape": list(weight.shape), "source_matrix_sha256": fingerprint(weight),
             ...
             "block_size": 32, "accuracy_level": 4, "default_zero_point": 8,
...
def block_reference(inputs, decoded, bias):
    blocks = inputs.reshape(16, 64, 32)
...
def main():
    ...
    previous_root = PHASE / "encoder-int4-kernel-screen-001"
    previous = read(previous_root / "result.json")
    execution_path = PHASE / "encoder-int4-kernel-screen-stage-001/execution.json"
    ...
    require(previous["status"] == "pass" and previous["int4_over_u8_ratio"] > 1
            ...
            "Require the closed slower FP32-activation screen")
    ...
    out = PHASE / "encoder-int4-int8activation-kernel-screen-001"
    ...
    write(out / "plan.json", {"source_bindings": bindings, "source_checkpoint": parent_binding,
          ...
          "recipe": {"bits": 4, "block_size": 32, "symmetric": True, "accuracy_level": 4},

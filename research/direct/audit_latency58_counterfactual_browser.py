...
from research.direct.prepare_latency58_counterfactual_listening_v2 import ORDER
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-counterfactual-browser-inventory-plan-v1"
    ...
    expected_schema = "latency58-counterfactual-listening-preparation-v1"
    require(saved["status"] == browser["status"] == "pass"
            and saved["schema"] == expected_schema
            and browser["schema"] == "latency58-counterfactual-listening-browser-check-v1"
    ...
    verify_inputs(plan)
    write(Path(plan["output"]), {"schema": "latency58-counterfactual-browser-inventory-v1",

"""Replay derivatives of the unchanged whole-group loss through B4/B2 renders.
...
groups. This removes floating-point reassociation of the ordinary objective.
"""
import torch
...
VERSION = "latency58-whole-group-output-vjp-replay-v1"


def policy():
    return {"version": VERSION, "loss": "Unchanged original whole ordinary16 plus 0.1 whole source-view2 objective",
            "capture": "Grad-enabled B4/B2 renders, detached output coordinates only; release each neural graph",
            "output_derivatives": "One original FP32 whole-group objective per independent group",
...
def accumulate_group(model, mixture, targets, *, group, microbatch, warmup_samples,
    ...
    require(group in ("ordinary", "auxiliary") and len(mixture) == (16 if group == "ordinary" else 2),
    ...
    del captured
    terms = whole_objective(raw, deployed, targets[..., warmup_samples:], mixture[..., warmup_samples:])
    ...
    return {"examples": len(mixture), "weighted_loss": loss, "active_windows": active,
            ...
            "whole_group_objective_evaluations": 1}

"""Authenticated generations for an isolated counterfactual teacher policy."""
...
from research.direct.latency58_counterfactual_journal import (
    UNIFORM_KEYS, validate_microbatch, compare_control_prefix, rng_state_sha256)

LOSS_KEYS = (*LOSS_KEYS, *UNIFORM_KEYS)
...
def validate_recipe(plan):
    config = plan["config"]
    require(plan["schema"] == "latency58-counterfactual-training-v1"
    ...
    require(plan["arm"] == "focused" and plan["focused_augmentation"] and not plan["local_mask_mixer"]
            and plan["counterfactual_version"] == COUNTERFACTUAL_VERSION
            and plan["teacher_mode"] in ("all_views", "ordinary_only")
            and (plan["resource_only"] or plan["teacher_mode"] == "ordinary_only"),
            "Unsupported counterfactual architecture or teacher policy")
    item = plan["matched_control_training_plan"]
    require(plan["source_bindings"].get(item["path"]) == item["sha256"] == sha(item["path"]),
            "Unbound original focused control")
    control = read(item["path"])
    require(control["schema"] == "latency58-vocal-focus-training-v1" and not control["resource_only"]
            and all(plan[key] == control[key] for key in SHARED_CONTROL_KEYS),
            "The counterfactual recipe changes more than teacher participation")
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-counterfactual-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-counterfactual-inference-v1" and payload["step"] == step
...
def validate_journal(journal, step, plan, helpers):
    ...
    for row in rows:
        ...
        for micro in row["microbatches"]:
            validate_microbatch(micro, plan["teacher_mode"])
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-counterfactual-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-counterfactual-generation-v1", "step": step, "plan_sha256": plan_sha,

...
def main():
    ...
    require(payload["schema"] == "latency58-asymmetric-resume-v1" and type(payload["step"]) is int
    ...
    require(plan["schema"] == "latency58-asymmetric-training-plan-v1"
            ...
            and provenance["parent_model_state_sha256"] == plan["parent_model_state_sha256"]
            and provenance["initialized_model_state_sha256"] == plan["initial_model_state_sha256"]
            and provenance["asymmetric_training_updates"] == step
    ...
    require(parent_step == 2000 and state_sha256(parent.state_dict()) == plan["parent_model_state_sha256"],
            "Frozen parent differs")
    model = Latency58AsymmetricModel.from_hann_model(parent)
    require(state_sha256(model.state_dict()) == plan["initial_model_state_sha256"]
            and torch.__version__ == plan["torch_version"], "Transferred model or runtime differs")
    del parent
    ...
    with torch.inference_mode():
        ...
        require(isinstance(a.state, AsymmetricState) and torch.isfinite(a.deployed).all().item()
    ...
    if pointer["inference"] is not None:
        ...
        require(saved["schema"] == "latency58-asymmetric-inference-v1" and saved["step"] == step
    ...
    result = {"status": "pass", "step": step, "resume_sha256": args.resume_sha256,
              ...
              "cumulative_hop128_updates": provenance["pilot_updates"],
              "asymmetric_training_updates": provenance["asymmetric_training_updates"], "batch_identity_rows": len(rows)}

...
def main():
    ...
    def walk(v,tracks,path,key=''):
        if isinstance(v,dict):
            ...
            for a,b in [('reference_start','reference_end'),('physical_start','physical_end')]:
            ...
            for start_key in ('start_frame','start_sample','physical_start_sample','physical_reference_start','requested_start_frame'):
                ...
                end=v.get('end_frame' if 'frame' in start_key else 'end_sample')
                duration=next((v[k] for k in ('duration_samples','frames','samples','requested_frames','rendered_frames') if numeric(v.get(k))),None)
                ...
                else:omitted.add((str(path),key+'/'+start_key,str(v[start_key])))
            for k,x in v.items():
                ...
                next_tracks={k} if k in known else tracks
    ...
    inventory={'schema':'latency58-recorded-interval-coverage-v1','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
        'search_roots':list(map(str,roots)),'search_command':command,'metadata_files':len(paths),'metadata_bindings_sha256':sha(binding_path.read_bytes()),

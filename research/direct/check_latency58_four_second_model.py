...
ordinary microbatches of two. The original absolute, relative and relative-L2
...
def compare_group_gradients(model, mixture_cpu, targets_cpu, *, warmup_samples, progress=None):
    ...
    for group, microbatch in (("ordinary", 2), ("auxiliary", 2)):
    ...
    try:
        ...
        return {"status": "pass", "model_state_sha256": fingerprint, "device": str(device),
            "precision": model.training_precision, "ordinary_microbatch": 2, "auxiliary_microbatch": 2,
...
def main():
    ...
    model.train().requires_grad_(True); model.training_precision = "float32"
    fingerprint = state_sha256(model.state_dict())

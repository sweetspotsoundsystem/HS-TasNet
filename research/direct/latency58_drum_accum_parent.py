"""Load an authenticated accumulated drum endpoint as a fresh fine-tuning parent.

Only inference weights are inherited. The original accumulated drum loader checks
...
def load_parent(plan):
    ...
    require(not torch.cuda.is_initialized(), "Load accumulated drum parent before CUDA")
    parent = plan["parent"]
    require(parent["kind"] == "drum_accum_candidate" and type(parent["step"]) is int
            and parent["step"] in (250, 500, 1000), "Require a planned accumulated drum endpoint")
    ...
    require(training["schema"] == "latency58-sdr-drum-accum-training-v1"
            ...
            "Parent accumulated drum recipe or directory differs")
    ...
    require(receipt["step"] == parent["step"] and receipt["carry_state"]
            ...
            "Wrong accumulated drum parent endpoint")

// Standalone CPU1 timing probe for the four- or six-state hop128 ABI. This is
// not a DAW test.
...
constexpr std::array<const char*, 7> inputs{
    "audio_chunk",     "audio_history",
    "fusion_hidden",   "spectral_numerator_tail",
    "waveform_tail",   "attention_keys",
    "attention_values"};
constexpr std::array<const char*, 7> outputs{
    "separated_chunk",      "next_audio_history",
    "next_fusion_hidden",   "next_spectral_numerator_tail",
    "next_waveform_tail",   "next_attention_keys",
    "next_attention_values"};
const std::array<std::vector<int64_t>, 7> inputShapes{
    std::vector<int64_t>{1, 2, 128},
    {1, 2, 896},
    {2, 1, 1000},
    {1, 4, 2, 128},
    {1, 4, 2, 128},
    {1, 31, 64},
    {1, 31, 128}};
const std::array<std::vector<int64_t>, 7> outputShapes{
    std::vector<int64_t>{1, 4, 2, 128},
    {1, 2, 896},
    {2, 1, 1000},
    {1, 4, 2, 128},
    {1, 4, 2, 128},
...
void statistics(std::vector<double> values) {
  ...
  std::cout << "{\"mean_ms\":" << mean << ",\"p50_ms\":" << percentile(.5)
            ...
            << ",\"hop_budget_ms\":" << budget << ",\"calls_over_hop_budget\":"
...
int main(int argc, char** argv) {
  try {
    ...
    require(
        warmup >= 16 && warmup <= 16384 && measured >= 64 && measured <= 65536,
    ...
    const size_t tensorCount = session.GetInputCount();
    require((tensorCount == 5 || tensorCount == 7) &&
                session.GetOutputCount() == tensorCount,
            "Expected five or seven inputs and outputs");
    auto memory =
        Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
    std::array<std::vector<float>, 7> in, out;
    ...
    for (size_t i = 0; i < tensorCount; ++i) {
      ...
      require(std::string(inputName.get()) == inputs[i] &&
                  std::string(outputName.get()) == outputs[i],
              "Tensor names changed");
      ...
      require(
          inputInfo.GetElementType() == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT &&
              ...
              outputInfo.GetShape() == outputShapes[i],
          "Static FP32 ABI changed");
      ...
      inputValues.push_back(Ort::Value::CreateTensor<float>(
          memory, in[i].data(), in[i].size(), inputShapes[i].data(),
          inputShapes[i].size()));
      outputValues.push_back(Ort::Value::CreateTensor<float>(
          memory, out[i].data(), out[i].size(), outputShapes[i].data(),
          outputShapes[i].size()));
    }
      ...
      for (int channel = 0; channel < 2; ++channel) {
        for (int sample = 0; sample < 128; ++sample) {
          ...
          audio[static_cast<size_t>(hop) * 256 + channel * 128 + sample] =
              static_cast<float>(.07 * noise +
                                 .12 * std::sin((220. + 31. * channel) *
                                                6.283185307179586 * t));
        }
      }
    }
    ...
    for (int hop = 0; hop < warmup + measured; ++hop) {
      ...
      std::copy_n(audio.data() + static_cast<size_t>(hop) * 256, 256,
                  in[0].data());
      const auto runBegan = Clock::now();
      session.Run(runOptions, inputs.data(), inputValues.data(), tensorCount,
                  outputs.data(), outputValues.data(), tensorCount);
      const auto runEnd = Clock::now();
      for (size_t i = 0; i < tensorCount; ++i) {
        for (float value : out[i])
          require(std::isfinite(value), "Nonfinite output/state");
        if (i != 0)
          std::copy(out[i].begin(), out[i].end(), in[i].begin());
      }
      for (size_t sample = 0; sample < 256; ++sample) {
        const float sum = out[0][sample] + out[0][256 + sample] +
                          out[0][512 + sample] + out[0][768 + sample];
        maximumClosure =
            std::max(maximumClosure,
                     std::abs(static_cast<double>(sum - previous[sample])));
      }
      std::copy(in[0].begin(), in[0].end(), previous.begin());
      const auto ended = Clock::now();
      if (hop >= warmup) {
        runTimes[static_cast<size_t>(hop - warmup)] =
            milliseconds(runEnd - runBegan);
        loopTimes[static_cast<size_t>(hop - warmup)] =
    ...
    std::cout << "{\"status\":\"pass\",\"onnxruntime_version\":\""
              ...
              << "\",\"intra_op_threads\":1,\"inter_op_threads\":1,"
                 ...
                 "\"warmup_hops\":"
              << warmup << ",\"measured_hops\":" << measured
              << ",\"persistent_states\":" << (tensorCount - 1)
    ...
    std::cout << ",\"thread_priority\":\"" << priority
              << "\",\"plugin_queue_measured\":false,\"native_host_qualified\":"
                 "false}\n";
    return 0;

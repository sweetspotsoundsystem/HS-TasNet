...
def run(plan_path, plan_sha):
    ...
    require(plan["schema"] == "latency58-residual-native-plan-v1"
    ...
    model, _ = load_checkpoint(checkpoint["path"], checkpoint["sha256"])
...
def main():
    ...
    if args.plan is not None:
        ...
        return
    export_path = PHASE / "c204-residual-model-onnx-001/verification.json"
    export = read(export_path)
    require(export["status"] == "pass", "Export parity must pass before native execution")
    source = read(BASE / "source-preparation.json")["source"]["path"]
    base_plan = read(BASE / "qualification-plan-001.json")
    paths = [Path(__file__).resolve(), export_path, Path(export["onnx"]["path"]), Path(source),
    ...
    out = PHASE / "c204-residual-model-native-001"
    require(not out.exists(), "Preserve native results")
    plan = {"schema": "latency58-residual-native-plan-v1", "checkpoint": export["checkpoint"],
            "export_verification": str(export_path), "source_bindings": bindings, "output_directory": str(out)}
    out.mkdir()
    write(out / "plan.json", plan)
    execute([PYTHON, "-u", "-m", "research.direct.qualify_latency58_residual_native", "--plan", str(out / "plan.json"),

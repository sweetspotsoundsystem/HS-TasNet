...
def require_resource_result(result, parity, plan, plan_sha):
    ...
    recovered = read(Path(plan["output_directory"]) / "resource-run-003/recovery-gpu-parity.json")
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    require(previous_execution_path == (Path(plan["previous_execution_for_resource"]) if resource
                                        else root / "resource-stage-003/execution.json"), "Wrong GPU continuity predecessor")
    if not resource:
        enclosing = read(root / "resource-root-execution-003.json")
        require(type(enclosing["actual_exit_code"]) is int and enclosing["actual_exit_code"] == 0
                ...
                and enclosing["stage_execution_sha256"] == sha(root / "resource-stage-003/execution.json")
                and enclosing["result_sha256"] == sha(root / "resource-run-003/result.json"),
    ...
    out, run = root / (name + ("-stage-003" if resource else "-stage")), root / (name + ("-run-003" if resource else "-run"))
    ...
    if not resource:
        result_path, parity_path = root / "resource-run-003/result.json", root / "resource-run-003/grouped-vocal-gpu-parity.json"
        require_resource_result(read(result_path), read(parity_path), plan, sha(plan_path))
        stage["resource_result"] = binding(result_path)
        for path in (result_path, parity_path, root / "resource-run-003/recovery-gpu-parity.json"):
    ...
    spec = {"schema": "gpu-watchdog-launch-finalization-v1", "expected_final_step": stop,
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_four_second_v2", "--plan", str(plan_path),
                     "--plan-sha256", sha(plan_path), "--stage", str(out / "stage.json"),
                     "--stage-sha256", sha(out / "stage.json")]}
    write(out / "watchdog-spec.json", spec)
    monitor_out = ARTIFACT_ROOT / "monitors" / (root.name + "-" + name + ("-003" if resource else ""))

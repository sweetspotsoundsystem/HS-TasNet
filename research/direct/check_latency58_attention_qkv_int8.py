"""Screen one fused integer QKV product against the exact PR #17 parent."""
...
OUT = ROOT / "research/m4_followup_20260916/attention-qkv-int8-screen-001"
...
def main():
    ...
    import torch
    from research.direct.latency58_attention_qkv_int8 import build, PARENT_SHA
    ...
    write(OUT / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
        ...
        "purpose": "One fused integer QKV product; retain FP64 temporal output and refinement",
    ...
    write(OUT / "result.json", {"status": "pass" if passed else "fail", "strict_parity_passed": passed,
        ...
        "limitations": "Independent seventeen-product synthetic parity only. Quantization changes model arithmetic. Long-stream numerical checks, full14 and source-view quality, native timing and physical M4 acceptance remain required. Linux Python timings include allocation and concurrent GPU training load."})
    print(json.dumps({"status": "pass" if passed else "fail", "local_speed_gate": gate}), flush=True)
    require(passed, "Integer QKV candidate failed independent numerical checks")

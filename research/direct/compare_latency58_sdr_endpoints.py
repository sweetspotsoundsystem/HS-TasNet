"""Compare two complete quality bundles from one unchanged training plan."""
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-sdr-endpoint-comparison-plan-v1"
    ...
    require(0 < plan["reference_step"] < plan["candidate_step"] <= training["config"]["steps"],
            "Require increasing saved endpoints from one trial")
    ...
    write(out / "result.json", {
        "schema": "latency58-sdr-endpoint-comparison-v1", "status": "pass",
        "plan_sha256": args.plan_sha256, "source_bindings": plan["source_bindings"],
        "source_bindings_unchanged": True, "training_plan": training_binding,

...
try {
    ...
    while ($began.Elapsed.TotalSeconds -lt 20 -and $null -eq $target) {
        try {
            $target = Invoke-RestMethod "$endpoint/json/list" -TimeoutSec 2 |
...
(async () => {
  ...
  require(data.sources.length === 6 && stem.options.length === 4, 'Unexpected player inventory');
  ...
  for (const source of data.sources) {
    for (const name of source.kind === 'mixture' ? ['bass'] : ['drums', 'bass', 'vocals', 'other']) {
      ...
      audio.currentTime = 3;
      await until(() => !audio.seeking, 'seek');
      await audio.play();
  ...
  require(files.length === 21 && new Set(files.map(x => x.url)).size === 21, 'Incomplete WAV coverage');
  stem.value = 'bass'; stem.dispatchEvent(new Event('change'));
  button('working').click(); await until(ready, 'initial switching clip');
  audio.currentTime = 4.25; await until(() => !audio.seeking, 'switching seek');
  await audio.play();
  const beforeSource = audio.currentTime;
  button('warm').click();
  await until(() => ready() && !audio.paused && audio.currentTime > beforeSource + 0.05, 'source resume');
  const afterSource = audio.currentTime;
  require(afterSource - beforeSource < 1, 'Source switch lost position');
  const beforeStem = audio.currentTime;
  stem.value = 'vocals'; stem.dispatchEvent(new Event('change'));
  await until(() => ready() && !audio.paused && audio.currentTime > beforeStem + 0.05, 'stem resume');
  const afterStem = audio.currentTime;
  require(afterStem - beforeStem < 1, 'Stem switch lost position');
  const beforeRapid = audio.currentTime;
  button('reset').click(); button('parent').click(); button('working').click();
  await until(() => ready() && !audio.paused && audio.currentTime > beforeRapid + 0.05, 'rapid source resume');
  require(audio.currentTime - beforeRapid < 1 && button('working').getAttribute('aria-pressed') === 'true',
    'Rapid switches lost the final source or position');
  document.dispatchEvent(new KeyboardEvent('keydown', {key:'4', bubbles:true}));
  await until(() => ready() && button('warm').getAttribute('aria-pressed') === 'true', 'keyboard source');
  ...
  return {status:'pass', files, source_switch:{before:beforeSource, after:afterSource},
    stem_switch:{before:beforeStem, after:afterStem}, rapid_source_switch:true,

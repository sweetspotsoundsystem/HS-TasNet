...
def main():
    ...
    require(outside < 500_000_000 and counted + 400_000_000 < 79_500_000_000, "Combined artifact cap exceeded")
    prep = PHASE / "cleanup-rebound-prep-001"
    require(not prep.exists(), "Preserve existing preparation")
    prep.mkdir()
    config = {**control["config"], "lr": 3e-5, "min_lr": 3e-6}
    decision = {"schema": "latency58-cleanup-rebound-decision-v1", "status": "train_rebound_independent_of_deployment",
                ...
                "hypothesis": "The two lower-learning-rate continuation endpoints do not improve the accepted panel score. Test whether restoring the earlier successful learning-rate schedule improves adaptation from the accepted checkpoint. All 4000 crop addresses, augmentation RNG, teacher targets, model initialization, loss and context match the completed lower-rate successor; only peak and floor learning rate change.",
                ...
                "storage": {"counted_bytes": counted, "outside_counted_roots_bytes": outside, "reserve_bytes": 400_000_000,
    ...
    for k in ("resource_plan", "full_resource", "full_resource_execution"):
        plan.pop(k, None)
    plan.update(schema="latency58-cleanup-rebound-training-v1", resource_only=True, config=config,
                run_dir=str(PHASE / "cleanup-rebound-resource-run-001"), comparison_variable="matched_learning_rate_schedule",
    ...
    paths = [control_path, prep / "decision.json", Path(__file__).resolve(),
             *(ROOT / "research/direct").glob('*cleanup_rebound*.py'),
             *(Path(control["run_dir"]) / 'checkpoints/step-000250').glob('*')]
    plan["source_bindings"].update({str(p):sha(p) for p in paths if p.is_file()})
    from research.direct.latency58_cleanup_rebound_checkpoint import validate_recipe, load_parent
    verify_inputs(plan)

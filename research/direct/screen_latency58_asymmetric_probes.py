"""Screen the untrained asymmetric carrier with the unchanged hop128 bass probes.
...
def main():
    ...
    require(plan["schema"] == "latency58-asymmetric-probe-plan-v1"
    ...
    from research.direct.latency58 import HOP, Latency58Model, SOURCE_ORDER
    ...
    model = Latency58AsymmetricModel.from_hann_model(parent).eval().requires_grad_(False)
    step, fingerprint = 0, model_state_sha256(model)
    ...
    require(proof["status"] == "pass" and proof["parent_step"] == parent_step
            ...
            and proof["model_state_sha256"] == fingerprint, "Functional model identity differs")
    ...
    report = {"schema": "latency58-asymmetric-probe-result-v1", "status": "pass", "checkpoint": None,

"""Audit the obsolete step-825 full resume to preserve continued training space."""
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-quarter-controlled-training-storage-plan-v1"
            and plan["steps"] == [825]
            and plan["reserve_after_bytes"] == 1_250_000_000, "Different bounded retirement")
    verify_inputs(plan)
    import torch
    ...
    require(counted - freed + plan["reserve_after_bytes"] < plan["stop_counted_bytes"],
            "Retirement does not provide the bounded training reserve")
    ...
    write(out / "intent.json", {
        "schema": "latency58-quarter-controlled-training-storage-intent-v1", "status": "audited",
        "plan_sha256": args.plan_sha256, "remove": remove, "protected_bindings": protected,
        "reason": "Reserve current and queued learning-rate and view-frequency checkpoints plus CPU quality and continued-training space under the existing cap. Retire only the obsolete intermediate full resume at step 825; preserve every corresponding inference tensor and the final step-2000 full resume. Same-Adam/RNG restart at this old intermediate step is intentionally retired.",

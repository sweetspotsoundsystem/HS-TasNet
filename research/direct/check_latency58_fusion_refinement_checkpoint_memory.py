"""Exercise the fusion-refinement serializer and optimizer restart in RAM only."""
import hashlib
import io
import json
import os
...
def check(source, evidence):
    ...
    from research.direct.latency58_fusion_refinement_checkpoint import make_payloads, load_payload, audit_resume, audit_live
    ...
    model = Latency58FusionRefinementModel.from_parent(parent).train().requires_grad_(True)
    ...
    plan = {**source, "schema": "latency58-fusion-refinement-training-plan-v1",
    ...
    invalid = {**recovered, "model": {**recovered["model"],
               "fusion_refine_expand.weight": recovered["model"]["fusion_refine_expand.weight"].double()}}
    wrong_step = {**loaded["optimizer"], "step": 3}
    for name, operation, message in (
        ("wrong_tensor_dtype", lambda: load_payload(invalid), "Invalid fusion_refinement inference tensors"),
...
def main():
    ...
    functional = read(PHASE / "fusion-refinement-functional-001/result.json")
    execution = read(PHASE / "fusion-refinement-functional-stage-001/execution.json")
    ...
    functional_plan = read(PHASE / "fusion-refinement-functional-001/plan.json")
    ...
    paths = [source_path, Path(__file__).resolve(), PHASE / "fusion-refinement-functional-001/result.json",
             PHASE / "fusion-refinement-functional-stage-001/execution.json",
             ROOT / "research/direct/latency58_fusion_refinement_checkpoint.py"]
    bindings = {**read(PHASE / "fusion-refinement-functional-001/plan.json")["source_bindings"],
                **{str(path): sha(path) for path in paths}}
    verify_inputs({"source_bindings": bindings})
    out = PHASE / "fusion-refinement-checkpoint-memory-functional-001"

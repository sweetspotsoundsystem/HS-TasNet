"""Run a monitored resource rehearsal or an audited stronger soft-capped SDR stage."""
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-sdr-softcap-strong-training-v1", "Unknown training plan")
    from research.direct.latency58_sdr_softcap_strong_checkpoint import read_generation, require_space
    ...
    require(proof["status"] == "pass" and proof["source_bindings_unchanged"]
            and proof["version"] == plan["sdr_softcap_version"] and 10 * proof["auxiliary_weight"] == plan["sdr_softcap_weight"]
    ...
    if plan["resource_only"]:
        ...
        require(resource["schema"] == "latency58-sdr-softcap-strong-resource-result-v1" and resource["status"] == "pass"
    ...
    if start:
        ...
        require(decision["schema"] == "latency58-sdr-softcap-strong-continuation-v1"
        ...
        require(args.continuation_decision is None and args.stop_step == (2 if plan["resource_only"] else 250),
                "Require the rehearsal or first quality endpoint before further updates")
    stage = {"schema": "latency58-sdr-softcap-strong-stage-v1", "plan_sha256": args.plan_sha256,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_sdr_softcap_strong", "--plan", str(args.plan.resolve()),
    ...
    if plan["resource_only"]:
        ...
        write(out / "resource-qualification.json", {
            "schema": "latency58-sdr-softcap-strong-resource-qualification-v1", "status": "pass",
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.audit_latency58_sdr_softcap_strong", "--plan", str(args.plan.resolve()),
    ...
    if previous_generation is not None:
        ...
        retirement = {"schema": "latency58-sdr-softcap-strong-replaced-optimizer-v1", "path": str(old_optimizer),

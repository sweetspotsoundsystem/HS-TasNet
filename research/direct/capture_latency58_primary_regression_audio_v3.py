...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-primary-regression-audio-plan-v3"
    ...
    require(saved["schema"] == "latency58-primary-regression-audio-v2" and saved["status"] == "pass"
    ...
    for path, old_row in saved["audio_files"].items():
        ...
        inventory[path] = row
        reused.append(path)
    require(len(reused) == 34 and all(len(v) == 8 for v in replay.values()), "Incomplete reused inventory")
    for source_id, audio in replay.items():
        estimates = [np.stack([audio[i, stem] for stem in SOURCE_ORDER]) for i in range(2)]
    ...
    html = html.replace(
        ...
        "Compare the working model, training parent, previous best and current candidate on the two primary excerpts from Skelpolu – Human Mistakes.")
    ...
    write(out / "result.json", {"schema": "latency58-primary-regression-audio-v3", "status": "pass",

...
from research.direct.report_latency58_cleanup_successor import REFERENCES, load_views, reference_views
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-cleanup-successor-queued-quality-plan-v1"
    ...
    require(training["schema"] == "latency58-cleanup-successor-training-v1" and not training["resource_only"]
            and training["teacher_mode"] == "ordinary_only",
            "Only a production pilot may queue quality")
    prefix = plan["quality_prefix"]
    require(prefix == "cleanup-successor-250" and training["additional_loss_weight"] == .5, "Different quality prefix")
    waited = 0.0
    ...
    if not ((stage / "execution.json").is_file() and (stage / "audit-execution.json").is_file()):
        ...
        try:
            ...
            require(int(stat[19]) == root_identity["start_ticks"] and command == root_identity["argv"]
                    and command[command.index("-m") + 1] == "research.direct.run_latency58_cleanup_successor_v2"
    ...
    commands = [
        [PYTHON, "-u", "-m", "research.direct.run_latency58_cleanup_successor_quality",
        ...
        [PYTHON, "-u", "-m", "research.direct.run_latency58_cleanup_successor_views", "--prefix", prefix,
    ...
    summary_plan = {"schema": "latency58-cleanup-successor-summary-plan-v1", "candidate_prefix": prefix,
    ...
    execute([PYTHON, "-u", "-m", "research.direct.report_latency58_cleanup_successor",
    ...
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-cleanup-successor-queued-quality-result-v1", "status": "pass",

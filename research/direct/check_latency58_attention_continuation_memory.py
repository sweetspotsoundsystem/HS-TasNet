"""Check the trained attention context, serializer and new-schedule Adam restart in RAM."""
import copy
import hashlib
import io
import json
...
def check(source, evidence):
    ...
    return {"status": "pass", "trained_parent_context": context, "serialized_in_memory": serialized, "all_30_optimizer_states_checked": True,
...
def main():
    ...
    out = PHASE / "attention-continuation-memory-functional-001"
    ...
    write(out / "plan.json", {"source_bindings": bindings, "parent_checkpoint": terminal["checkpoint"],
          "scope": "Trained attention context parity, in-memory serialization and exact fresh-schedule Adam restart",

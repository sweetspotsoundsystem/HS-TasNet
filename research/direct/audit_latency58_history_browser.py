"""Match muted browser playback and HTTP bytes to a completed history player."""
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-history-browser-inventory-plan-v1"
            and plan["panel"] in ("skelpolu", "actions"), "Unexpected audit scope")
    verify_inputs(plan)
    binding = plan["source_bindings"]
    browser_path, player_path = Path(plan["browser_directory"]), Path(plan["player_directory"])
    require(browser_path.parent == player_path.parent == PHASE, "Use the history phase")
    ...
    source_stem = "capture" if plan["panel"] == "skelpolu" else "preparation"
    source_execution_path = player_path / (source_stem + "-execution.json")
    ...
    expected_schema = "latency58-history-primary-regression-audio-v1" if plan["panel"] == "skelpolu" \
        else "latency58-history-listening-preparation-v1"
    ...
    argv = execution["argv"]
    page = plan["page_url"]
    require(argv[argv.index("-Panel") + 1] == plan["panel"]
    ...
    clips = 2 if plan["panel"] == "skelpolu" else 1
    require(checks["status"] == "pass" and checks["panel"] == plan["panel"]
    ...
    for key in ("source_switch", "history_switch", "stem_switch", "rapid_source_switch"):
    ...
    require([s["id"] for s in data["sources"]] == ["working", "previous", "parent", "short", "long", "reference", "mixture"]
            and data["human_listening_verdict"] is None, "Player identities differ")
    expected = {}
    ...
    require(len(expected) == len(checks["files"]) == clips * 25, "Incomplete playback inventory")
    inventory = {}
    ...
    verify_inputs(plan)
    write(Path(plan["output"]), {"status": "pass", "plan_sha256": args.plan_sha256,
    ...
    print({"status": "pass", "panel": plan["panel"], "files": len(inventory)}, flush=True)

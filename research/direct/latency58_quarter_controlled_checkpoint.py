"""Authenticate a matched reduction in forced views from the accepted checkpoint."""
...
from research.direct.latency58_variable_counterfactual_journal import rng_state_sha256
...
def bound(plan, item):
    require(plan["source_bindings"].get(item["path"]) == item["sha256"] == sha(item["path"]),
            "Unbound quarter-controlled prerequisite")
    return read(item["path"])


def validate_recipe(plan):
    require(plan["schema"] == "latency58-quarter-controlled-training-v1"
            and type(plan["resource_only"]) is bool
            and plan["comparison_variable"] == "matched_controlled_view_frequency"
            and plan["optimizer_initialization"] == "fresh_adam"
            and plan["checkpoint_and_quality_reserve_bytes"] == 400_000_000,
            "Different bounded quarter-controlled recipe")
    ...
    excluded = {"config", "parent", "initialized_model_state_sha256", "stop_counted_bytes", "augmentation_version"}
    ...
    require(plan["config"] == config and plan["stop_counted_bytes"] == 79_500_000_000
    ...
    require(decision["status"] == "train_quarter_controlled_independent_of_deployment"
    ...
    control = bound(plan, plan["matched_control_training_plan"])
    from research.direct.latency58_cleanup_successor_checkpoint_v2 import validate_recipe as validate_control
    validate_control(control)
    require(not control["resource_only"] and control["config"] == config
            and all(plan[k] == control[k] for k in RECIPE_KEYS if k not in {"stop_counted_bytes", "augmentation_version"})
            and plan["parent"] == control["parent"]
            and decision["matched_control_training_plan"] == plan["matched_control_training_plan"],
            "Matched control differs beyond controlled-view frequency")
    from research.direct.latency58_quarter_controlled_augmentation import VERSION, VIEW_CYCLE
    from research.direct.latency58_vocal_focus_augmentation import VERSION as CONTROL_VERSION
    proof = bound(plan, plan["functional_proof"])
    require(plan["augmentation_version"] == VERSION and control["augmentation_version"] == CONTROL_VERSION
            and plan["view_cycle"] == list(VIEW_CYCLE)
            and plan["controlled_examples_per_update"] == 4 and plan["ordinary_examples_per_update"] == 12
            and proof["status"] == "pass" and proof["source_bindings_unchanged"]
            and proof["controlled_per_update"] == 4 and proof["ordinary_per_update"] == 12
            and not proof["cuda_initialized"] and proof["training_updates"] == proof["quality_scores"] == 0,
            "Prospective view policy or CPU functional proof differs")
    return reference
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="selected_cleanup_quarter_controlled_views",
                 training_objective="ordinary_only_teacher_plus_controlled_deployed_truth",
                 quarter_controlled_trial_updates=step,
                 ...
                 comparison_variable="matched_controlled_view_frequency",
                 controlled_examples_per_update=4, ordinary_examples_per_update=12,
                 view_cycle=plan["view_cycle"],
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-quarter-controlled-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-quarter-controlled-inference-v1" and payload["step"] == step
...
from research.direct.latency58_quarter_controlled_augmentation import VERSION as AUGMENTATION_VERSION, VIEW_CYCLE
from research.direct.latency58_counterfactual_teacher import VERSION as COUNTERFACTUAL_VERSION
from research.direct.latency58_variable_counterfactual_journal import UNIFORM_KEYS, validate_microbatch as validate_counterfactual_microbatch, compare_control_prefix
from research.direct.latency58_quarter_controlled_journal import EXTRA_LOSS_KEYS, validate_microbatch as validate_controlled_microbatch
...
def validate_counterfactual_journal(journal, step, plan, helpers):
    ...
    for row in rows:
        ...
        for micro in row["microbatches"]:
            codes = [VIEW_CYCLE[(micro["first_sample_index"] + i) % 8] for i in range(4)] if plan["focused_augmentation"] else [2] * 4
        ...
        require(row["controlled_examples"] == sum(m["controlled_examples"] for m in row["microbatches"]) == 4
                and row["ordinary_examples"] == sum(m["ordinary_examples"] for m in row["microbatches"]) == 12,
...
def validate_journal(journal, step, plan, helpers):
    ...
    control = bound(plan, plan["matched_control_training_plan"])
    path = str(Path(control["run_dir"]) / "checkpoints/step-000250/metrics.jsonl")
    require(plan["source_bindings"].get(path) == sha(path), "Unbound matched control journal")
    reference_rows = [json.loads(line) for line in Path(path).read_bytes().splitlines()]
    require(len(reference_rows) == 250 and len(rows) == step, "Incomplete view-frequency comparison")
    validate_matched_inputs(rows, reference_rows[:step])
    return rows


def validate_matched_inputs(rows, reference_rows):
    """Match data and RNG, and require teacher identity only on unchanged views."""
    require(len(rows) == len(reference_rows) and len(rows) in (2, 250), "Incomplete matched input fixture")
    ...
    changed = shared = ordinary = 0
    for row, reference_row in zip(rows, reference_rows, strict=True):
        require(row["step"] == reference_row["step"], "Matched update order differs")
        for actual, reference_micro in zip(row["microbatches"], reference_row["microbatches"], strict=True):
            require(all(actual[k] == reference_micro[k] for k in identity_keys), "Matched pristine data or RNG differs")
            codes = [VIEW_CYCLE[(actual["first_sample_index"] + i) % 8] for i in range(4)]
            require(actual["view_codes"] == codes, "Quarter-controlled absolute view assignment differs")
            if any(code < 2 for code in codes):
                require(actual["view_codes"] == reference_micro["view_codes"]
                        and actual["augmented_batch_sha256"] == reference_micro["augmented_batch_sha256"]
                        and actual["teacher_targets_sha256"] == reference_micro["teacher_targets_sha256"],
                        "Shared controlled microbatch or its teacher targets changed")
                shared += 1
            else:
                require(actual["augmented_batch_sha256"] == actual["original_augmentation_sha256"],
                        "Restored ordinary microbatch differs from original augmentation")
                ordinary += 1
                changed += actual["augmented_batch_sha256"] != reference_micro["augmented_batch_sha256"]
    require(shared == ordinary == len(rows) * 2 and changed > 0, "View-frequency intervention was not exercised")
    return {"updates": len(rows), "pristine_and_rng_matched_microbatches": shared + ordinary,
            "shared_input_and_teacher_microbatches": shared, "restored_ordinary_microbatches": ordinary,
            "changed_augmented_microbatches": changed,
            "first_gradient_identity_required": False}
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-quarter-controlled-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-quarter-controlled-generation-v1", "step": step, "plan_sha256": plan_sha,

...
def main():
    ...
    require(training["schema"] == "latency58-sdr-drum-training-v2", "Require a context trial plan")
    ...
    paths = [audit_path, audit_exec_path, stage / "execution.json", stage / "stage.json", monitor_path,
             ...
             Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_sdr_drum_v2.py",
             ROOT / "research/direct/compare_latency58_sdr.py", ROOT / "research/direct/compare.py"]
    if "probes" in args.modes:
        paths.append(ROOT / "research/direct/evaluate_latency58_sdr_drum_v2_probes.py")
    if "full14" in args.modes:
        ...
        paths += [proof_path, ROOT / "research/direct/evaluate_latency58_sdr_drum_v2_parallel.py"]
    ...
    for mode in args.modes:
        ...
        plan.update(schema="latency58-sdr-drum-" + ("probe" if mode == "probes" else "music") + "-plan-v2",
        ...
        if mode == "full14":
            plan.update(schema="latency58-sdr-drum-parallel-music-plan-v2", workers=2,
                        model_kind="drum_candidate", track_indices=list(range(14)),
    ...
    for mode, out, plan in prepared:
        ...
        module = "research.direct.evaluate_latency58_sdr_drum_v2" + ("_probes" if mode == "probes" else "_parallel" if mode == "full14" else "")

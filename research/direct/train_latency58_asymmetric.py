"""Matched GPU-only hop128 asymmetric-window adaptation from an explicit inference parent.
...
def save(run, model, optimizer, step, plan, plan_sha, helpers, torch, np, frozen, save_inference):
    ...
    provenance.update(pilot_updates=plan["parent_hop128_updates"] + step,
                      training_updates=plan["parent_training_updates"] + step, tail_updates=step,
                      parent_checkpoint=plan["parent_checkpoint"],
                      parent_model_state_sha256=plan["parent_model_state_sha256"],
                      initialized_model_state_sha256=plan["initial_model_state_sha256"],
                      asymmetric_training_updates=step,
                      parent_training_updates=plan["parent_training_updates"],
                      training_objective=plan["objective"],
    ...
    payload = {"schema": "latency58-asymmetric-resume-v1", "step": step, "model": tensors,
    ...
    receipt = {"schema": "latency58-asymmetric-checkpoint-receipt-v1", "step": step,
    ...
    if save_inference:
        ...
        inference["schema"] = "latency58-asymmetric-inference-v1"
...
def main():
    ...
    require(plan["schema"] == "latency58-asymmetric-training-plan-v1"
    ...
    config = plan["config"]
    require(plan["objective"] == "raw4_l1"
            and plan["parent_checkpoint"].get("kind", "inference") == "inference"
            and plan["parent_training_updates"] == 4250 and plan["parent_hop128_updates"] == 2000,
            "Use the explicit audited Hann +2000 parent and a declared matched objective")
    ...
    from research.direct.latency58_asymmetric import Latency58AsymmetricModel, PRECISION_POLICY as POLICY
    from research.direct.latency58 import HOP, Latency58Model
    ...
    # Authenticate the CPU parent before any CUDA initialization. The production
    # seed is installed afterwards identically in both objective arms.
    ...
    require(parent_step == 2000 and state_sha256(parent.state_dict()) == plan["parent_model_state_sha256"]
            and parent.provenance["training_updates"] == plan["parent_training_updates"]
            and parent.provenance["pilot_updates"] == plan["parent_hop128_updates"],
            "Loaded inference parent or cumulative update lineage differs")
    model = Latency58AsymmetricModel.from_hann_model(parent).eval()
    require(state_sha256(model.state_dict()) == plan["initial_model_state_sha256"],
            "Transferred model differs from functional and resource evidence")
    del parent
    ...
    if resuming:
        ...
        require(stored["schema"] == "latency58-asymmetric-resume-v1" and stored["step"] == step
    ...
    try:
        for mixture, targets in loader:
            ...
            for name, tensor in (("mixture", mixture), ("targets", targets), ("deranged", flags)):
                batch_digest.update(name.encode())
                batch_digest.update(str((tuple(tensor.shape), tensor.dtype)).encode())
            ...
            deployed = output.deployed[..., HOP:HOP + config["crop_samples"]]
            if plan["objective"] == "raw4_l1":
                loss = raw4_native_objective(raw, targets, flags, projection=True)
                raw_l1 = loss.waveform_l1
            else:
                loss = deployed_l1_objective(raw, deployed, targets, flags, projection=True)
                raw_l1 = loss.raw_waveform_l1
            deployed_l1 = torch.nn.functional.l1_loss(deployed.detach(), targets)
            loss.total.backward()
            ...
            metrics = {"step": step, "lr": lr, "loss": float(loss.total.detach()),
                ...
                "raw_waveform_l1": float(raw_l1.detach()), "deployed_waveform_l1": float(deployed_l1),
                "augmented_batch_sha256": batch_digest.hexdigest(), "objective": plan["objective"],
            ...
            del output, raw, deployed, loss, raw_l1, deployed_l1, padded

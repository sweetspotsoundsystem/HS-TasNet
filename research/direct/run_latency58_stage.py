...
def main():
    ...
    parser.add_argument("--previous-stage", type=Path, required=True)
    ...
    previous_command, previous_execution = read(previous / "root-command.json"), read(previous / "root-execution.json")
    command = previous_command["argv"]
    monitor_directory = Path(command[command.index("--output-dir") + 1])
    monitor_path = monitor_directory / "result.json"
    monitor = read(monitor_path)
    require(audit_execution["actual_exit_code"] == previous_execution["actual_exit_code"] == 0
            and audit["status"] == monitor["status"] == monitor["supervisor_health"] == "pass"
            and monitor["child_exit_code"] == 0 and monitor["post_exit_quiet_completed"]
            and previous_execution["source_bindings_unchanged"], "Previous stage or separate audit failed")
    require(audit["step"] == latest["step"]
    ...
    stage = {"schema": "latency58-training-stage-v1", "plan_sha256": args.plan_sha256,
             ...
             "previous_checkpoint_audit": {"path": str(audit_path), "sha256": sha(audit_path)},
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT),
            ...
            "argv": [python, "-u", str(ROOT / "research/direct/train_latency58.py"),
    ...
    argv = [python, "-u", "-m", "research.direct.audit_latency58_checkpoint",
            "--plan", str(args.plan.absolute()), "--plan-sha256", args.plan_sha256,

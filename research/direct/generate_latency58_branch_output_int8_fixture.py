...
def main():
    ...
    parser.add_argument("--output-prefix", default="branch-output-int8-fixture-001")
    ...
    short_root, long_root = PHASE / "branch-output-int8-screen-003", PHASE / "branch-output-int8-long-001"
    ...
    require(short["strict_parity_passed"] and short["graph_sha256"] == long["graph_sha256"]
            == "c7ea50ac67bf4bfddf1f5ff41c6eb419af00fe420ce1a0b0eaeef11a1861cd61", "Wrong candidate graph")
    ...
    paths = [ten_path, fourteen_proof_path, ROOT / "research/direct/latency58_branch_output_int8_reference.py", Path(__file__).resolve(), graph_path, parent_path, parent_proof_path,
    ...
    reference, independent = make_reference(model, onnx.load(ten_path),
        read(parent_proof_path)["conversion"], onnx.load(parent_path), read(fourteen_proof_path)["conversion"], onnx.load(graph_path), read(short_root / "graph.json")["conversion"])
    require(len(independent) == 16, "Incomplete independently reconstructed projections")
    ...
    out.mkdir()
    plan = {"schema": "latency58-sixteen-projection-fixture-plan-v1", "source_bindings": bindings,
    ...
    provenance = {"format": "SGRTG001: LE uint32 case count, then per case LE uint32 frames, planar input[2,T] and deployed[4,2,T] LE float32",
        ...
        "reference": "Independent CPU PyTorch declared sixteen-projection signed integer inference, reconstructed from source checkpoint weights. Eight FP32 public states; zero initial states; one zero flush; crop to real length. ONNX parsing authenticates reconstructed weights; runtime imports are actively rejected."}

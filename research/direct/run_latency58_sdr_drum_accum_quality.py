...
def main():
    ...
    require(training["schema"] == "latency58-sdr-drum-accum-training-v1", "Require a context trial plan")
    from research.direct.latency58_sdr_drum_accum_checkpoint import read_generation, require_space
    ...
    paths = [audit_path, audit_exec_path, stage / "execution.json", stage / "stage.json", monitor_path,
             ...
             Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_sdr_drum_accum.py",
             ROOT / "research/direct/compare_latency58_sdr.py", ROOT / "research/direct/compare.py"]
    if "probes" in args.modes:
        paths.append(ROOT / "research/direct/evaluate_latency58_sdr_drum_accum_probes.py")
    if "full14" in args.modes:
        ...
        paths += [proof_path, ROOT / "research/direct/evaluate_latency58_sdr_drum_accum_parallel.py"]
    ...
    for mode in args.modes:
        ...
        plan.update(schema="latency58-sdr-drum-accum-" + ("probe" if mode == "probes" else "music") + "-plan-v1",
        ...
        if mode == "full14":
            plan.update(schema="latency58-sdr-drum-accum-parallel-music-plan-v1", workers=2,
                        model_kind="drum_accum_candidate", track_indices=list(range(14)),
    ...
    for mode, out, plan in prepared:
        ...
        module = "research.direct.evaluate_latency58_sdr_drum_accum" + ("_probes" if mode == "probes" else "_parallel" if mode == "full14" else "")

...
def selected_rows(path, steps):
    text = path.read_text()
    # The production journal can be appending its final line during this read.
    complete = text[:text.rfind("\n") + 1]
    rows = {row["step"]: row for line in complete.splitlines() if line.strip()
...
def main():
    ...
    training_root = PHASE / "quadrature-continuation-001"
    training_plan = training_root / "plan.json"
    source = read(training_plan)
    verify_inputs(source)
    config = source["config"]
    require(config["batch_size"] == config["microbatch_size"] == 16 and source["accumulation_steps"] == 1,
            "Compare the whole logged microbatch")
    out = PHASE / "quadrature-continuation-learning-001"
    require(not out.exists(), "Preserve completed diagnostics")
    require_cpu_space(source, 2_000_000)
    steps = [1, *range(250, 2001, 250)]
    ...
    plan = {"source_bindings": bindings, "training_steps": steps, "examples_scored_per_step": 16,
            "all16_augmented_input_hash_verified": True,
            "step_selection": "Initial update and every 250th update through 2000, independent of observed losses", "parent_precision": "CPU FP32",
        ...
        for row in selected:
            ...
            for offset in range(0, 16, 16):
                for tensor in (mixture, truth, changed, factors):
                    digest.update(tensor[offset:offset + 16].contiguous().numpy().tobytes())

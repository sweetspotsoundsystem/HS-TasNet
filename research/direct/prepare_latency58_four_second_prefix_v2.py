"""Unpack the established four-value remix result; retain the first failed preparation."""
from research.direct.prepare_latency58_four_second_prefix import *

def main():
    require(Path.cwd() == ROOT and os.environ.get('CUDA_VISIBLE_DEVICES') == '', 'CPU-only prefix preparation')
    out = ARTIFACT_ROOT / 'selected-data-prefix-002'
    ...
    paths = [Path(__file__).resolve(), decision_path, previous / 'inputs.json', previous / 'result.json',
             ...
             ARTIFACT_ROOT / 'selected-data-prefix-001/plan.json',
             ARTIFACT_ROOT / 'selected-data-prefix-001/root-execution.json']
    ...
    for index, (mixture, targets) in enumerate(loader):
        ...
        mixture, targets, _, _ = augment(mixture, targets, seed=config['seed'], first_sample_index=first)

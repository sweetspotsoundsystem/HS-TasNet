"""Score the same fixed vocal cleanup_followups for a completed cleanup_followup pilot."""
...
def initialize_worker(plan):
    ...
    spec = plan["model"]
    require(spec["kind"] == "cleanup_followup", "Require the new cleanup_followup checkpoint family")
    binding = spec["quality_plan"]
    require(sha(binding["path"]) == binding["sha256"], "Diagnostic quality plan changed")
    from research.direct.evaluate_latency58_cleanup_followup import load_evaluation_model
...
def main():
    ...
    require(sha(args.plan) == args.plan_sha256, "Vocal cleanup_followup plan changed")
    plan = read(args.plan)
    require(plan["schema"] == "latency58-cleanup-followup-views-evaluation-plan-v1" and plan["workers"] == 2

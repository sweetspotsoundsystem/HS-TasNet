"""Compare the lower learning rate with the middle and higher rates and accepted parent."""
...
def reference_views(label):
    return PHASE / {"working": "vocal-views-working-001", "parent": "leader-cleanup-250-views-001",
                    "matched_control": "cleanup-successor-250-views-001", "higher_rate": "cleanup-rebound-250-views-001"}[label]
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-cleanup-lr3e6-summary-plan-v1", "Unexpected summary plan")
    ...
    require(training["schema"] == "latency58-cleanup-lr3e6-training-v1" and not training["resource_only"]
    ...
    from research.direct.latency58_cleanup_lr3e6_checkpoint import validate_journal
    from research.direct.train_latency58 import load_source
    helpers = load_source("cleanup_lr3e6_summary_helpers", training["helper_source"])
    ...
    write(out / "result.json", {
        "schema": "latency58-cleanup-lr3e6-quality-summary-v1", "status": "pass", "plan_sha256": args.plan_sha256,

"""Authenticated warm500 parent and split generations for higher-LR adaptation."""
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="higher_lr_context_fine_tune", parent_provenance=old,
                 ...
                 lr_restart_trial_updates=step, carry_state=plan["carry_state"],
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-sdr-lrrestart-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-sdr-lrrestart-inference-v1" and payload["step"] == step
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-sdr-lrrestart-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-sdr-lrrestart-generation-v1", "step": step, "plan_sha256": plan_sha,

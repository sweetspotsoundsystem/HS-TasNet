"""Run a monitored resource rehearsal or an audited drum-weighted context stage."""
...
def main():
    ...
    parser.add_argument("--stop-step", type=int, choices=(2, 250, 500, 1000), required=True)
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-sdr-drum-training-v1", "Unknown training plan")
    from research.direct.latency58_sdr_drum_checkpoint import read_generation, require_space
    ...
    for name in ("drum_functional", "drum_functional_execution"):
        require(sha(plan[name]["path"]) == plan[name]["sha256"], "Drum proof binding changed")
    functional = read(plan["drum_functional"]["path"])
    functional_execution = read(plan["drum_functional_execution"]["path"])
    require(functional["schema"] == "latency58-drum-emphasis-functional-result-v1"
            and functional["status"] == "pass" and functional["source_bindings_unchanged"]
            and functional["version"] == plan["objective_version"] == VERSION
            and plan["drum_weight"] == DRUM_WEIGHT == 2
            and functional["stem_weights"] == [2, 1, 1, 1] and functional["weight_denominator"] == 5
            and functional["teacher_weight"] == plan["teacher_weight"] == TEACHER_WEIGHT == .5
            ...
            and functional["all_21_parameter_gradients_match"] and functional["weight_one_loss_bit_exact"]
            and functional["original_projection_and_cap_unchanged"] and functional["silent_loss_zero"]
            and functional["inputs_model_teacher_and_rng_unchanged"]
            ...
            "Drum functional qualification differs")
    if plan["resource_only"]:
        ...
        shared = ("config", "parent", "warmup_samples", "scored_samples", "carry_state",
                  ...
                  "drum_weight", "objective_version", "drum_functional", "drum_functional_execution")
        require(resource["schema"] == "latency58-sdr-drum-resource-result-v1" and resource["status"] == "pass"
    ...
    if start:
        ...
        require(decision["schema"] == "latency58-sdr-drum-continuation-v1"
    ...
    stage = {"schema": "latency58-sdr-drum-stage-v1", "plan_sha256": args.plan_sha256,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_sdr_drum", "--plan", str(args.plan.resolve()),
    ...
    if plan["resource_only"]:
        ...
        write(out / "resource-qualification.json", {
            "schema": "latency58-sdr-drum-resource-qualification-v1", "status": "pass",
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.audit_latency58_sdr_drum", "--plan", str(args.plan.resolve()),
    ...
    if previous_generation is not None:
        ...
        retirement = {"schema": "latency58-sdr-drum-replaced-optimizer-v1", "path": str(old_optimizer),

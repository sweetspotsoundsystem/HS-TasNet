// CPU1 operator profiling for the hop128 ABI. Instrumented times are not a benchmark.
...
int main(int argc, char** argv) {
  try {
    require(argc == 3, "Usage: profile MODEL.onnx PROFILE_PREFIX");
    constexpr int warmup = 16;
    constexpr int measured = 16;
    disableDenormals();
    ...
    options.EnableProfiling(argv[2]);
    ...
    const auto profile = session.EndProfilingAllocated(allocator);
    ...
    std::cout << ",\"profile_path\":" << std::quoted(profile.get())
              << ",\"instrumented_diagnostic_only\":true"

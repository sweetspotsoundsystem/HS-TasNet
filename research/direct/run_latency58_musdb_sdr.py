"""Prepare, monitor, audit and score direct SDR adaptation on recorded MUSDB training music."""
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_musdb_sdr", "--plan", str(plan_path),
...
def prepare(args):
    ...
    paths = [Path(__file__).resolve(), ROOT / "research/direct/latency58_direct_sdr.py",
             ROOT / "research/direct/latency58_direct_sdr_checkpoint.py", ROOT / "research/direct/train_latency58_musdb_sdr.py",
    ...
    config = {**template["config"], "steps": args.steps, "seed": args.seed, "data_start": args.data_start,
              "crop_samples": 176384, "lr": args.lr, "min_lr": args.lr / 10, "warmup": 10,
    ...
    write(out / "preparation.json", {"status": "pass", "parent_model_state_sha256": plan["parent_model_state_sha256"],
          ...
          "counted_bytes_before": counted, "forecast_including_outside_and_pilot": counted + 800_000_000 + 400_000_000,
          "rationale": "Direct deployed SDR on the 83 frozen MUSDB training tracks using ordinary recorded mixtures and recorded stems only; no synthetic subset/remix views or teacher-generated labels in this stage.",
...
def main():
    ...
    parser.add_argument("--name", default="direct-sdr-musdb-001")
    parser.add_argument("--steps", type=int, default=250)
    parser.add_argument("--lr", type=float, default=1e-5)
    parser.add_argument("--seed", type=int, default=20260926)
    parser.add_argument("--data-start", type=int, default=1_010_000)
    ...
    resource = launch(plan_path, PHASE / "direct-sdr-001/production-stage/execution.json", resource=True)

"""Bind the audited step-100 recovery to the qualified persistent NVML monitor."""
...
from research.direct.recover_latency58_nvml_idle import (
...
SOURCE = PHASE / "branch-grouped-vocal-010"
OUT = PHASE / "branch-grouped-vocal-011"
...
def prepare():
    ...
    require(sha(source_path) == "e096e1424c93043a97358c1b0d732e439c7ca2ae3f232ae863b487661549041a",
    ...
    require(failed_root["actual_session_id"] == 51274 and failed_root["actual_tool_chunk_id"] == "3285b0"
            ...
            and failed["latest_completed_step_seen"] == failed_root["completed_training_updates"] == 141
    ...
    require(audit_execution["actual_exit_code"] == 0 and not audit_execution["timed_out"]
            and audit_execution["actual_session_id"] == 83998 and audit_execution["actual_tool_chunk_id"] == "99a78f"
    ...
    require(audit["status"] == "pass" and audit["checkpoint_step"] == snapshot["step"] == 100
            and audit["unchanged_from_pre_alert_observation"] and audit["cpu_loaded_and_audited"]
            and audit["saved_optimizer_states"] == 40 and audit["all_saved_adam_steps"] == 100
            ...
            and audit["absolute_next_sample_index"] == 4093600 and audit["algorithmic_latency_samples"] == 256
            and snapshot == failed_root["recovery_binding"]
            and sha(snapshot["path"]) == snapshot["sha256"]
            == "a033a40aaa9c13932c13091812c0f62beff562a4d7e7328f360fa970820523d4"
    ...
    require(idle_root["actual_session_id"] == 94980 and idle_root["actual_tool_chunk_id"] == "3a476e"
    ...
    old_trainer = ROOT / "research/direct/train_latency58_grouped_vocal_recovery.py"
    new_trainer = ROOT / "research/direct/train_latency58_grouped_vocal_nvml.py"
    ...
    paths = [Path(__file__).resolve(), new_trainer, ROOT / "research/direct/run_latency58_grouped_vocal_nvml.py",
    ...
    require(before["headroom_bytes"] > 1_200_000_000, "Reserve rolling replacement and the final checkpoint")
    ...
    plan.update(name=OUT.name, output_directory=str(OUT), source_bindings=bindings,
        ...
        retry_reason="Resume the audited step-100 snapshot using qualified persistent NVML telemetry; first reproduce updates 101 through 103 under the original full-500 schedule.",
        ...
        resume_checkpoint={"step": 100, "snapshot": snapshot, "training_plan": binding(source_path)},
        resume_prefix_qualification={"start_step": 100, "stop_step": 103,
        ...
        prior_unsaved_training_updates=41, replay_from_saved_selected_parent=False)
    ...
    print(json.dumps({"status": "prepared", "plan_sha256": sha(OUT / "plan.json"),
        "resumed_from_step": 100, "full_schedule_steps": 500,

"""Compare the higher learning rate with its matched control and accepted parent."""
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-cleanup-rebound-summary-plan-v1", "Unexpected summary plan")
    ...
    require(training["schema"] == "latency58-cleanup-rebound-training-v1" and not training["resource_only"]
    ...
    from research.direct.latency58_cleanup_rebound_checkpoint import validate_journal
    from research.direct.train_latency58 import load_source
    helpers = load_source("cleanup_rebound_summary_helpers", training["helper_source"])
    ...
    require(reference_states["matched_control"] == control_receipt["model_state_sha256"],
            "Learning-rate reference is not the matched training control")
    require(all(plan["source_bindings"].get(p) == s for p, s in evidence.items()), "Unbound completed score evidence")
    verify_inputs(plan)
    write(out / "result.json", {
        "schema": "latency58-cleanup-rebound-quality-summary-v1", "status": "pass", "plan_sha256": args.plan_sha256,
        ...
        "all_metrics_compared": True, "comparison_variable": "matched_learning_rate_schedule",
        "matched_training_effect_claimed": True, "matched_comparison_scope": "Learning-rate schedule only; one seed and the same 4000 training examples", "matched_input_teacher_rng_updates": len(matched_rows), "quality_selected": False, "human_listening_completed": False,

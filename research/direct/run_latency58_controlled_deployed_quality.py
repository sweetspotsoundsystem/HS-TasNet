...
Generation bindings exclude the optimizer. A completed and reviewed pilot may
retire its Adam file while preserving all inference weights and RNG records.
"""
...
def main():
    ...
    parser.add_argument("--match-directory", type=Path, required=True)
    ...
    require(training["schema"] == "latency58-controlled-deployed-training-v1"
            and training["teacher_mode"] == "ordinary_only" and training["additional_loss_weight"] == .5
            and not training["resource_only"],
            "Require the selective production trial plan")
    ...
    require(audit["status"] == "pass" and audit["source_bindings_unchanged"]
            ...
            and audit["teacher_mode"] == training["teacher_mode"]
            and audit["counterfactual_version"] == training["counterfactual_version"]
            and audit["controlled_deployed_truth_and_full_batch_divisor_verified"]
            and audit["additional_loss_weight"] == training["additional_loss_weight"]
            and audit["additional_loss_version"] == training["additional_loss_version"]
            and audit["normalized_drum_objective_journal_verified"] and audit["matched_input_journal_verified"]
            and audit["arm"] == training["arm"] and audit["parameter_tensors"] == training["parameter_tensors"]
            and audit["drum_weight"] == training["drum_weight"]
            and audit["objective_version"] == training["objective_version"]
            and audit["microbatch_size"] == training["microbatch_size"]
            and audit["accumulation_steps"] == training["accumulation_steps"]
            and audit["teacher_kind"] == training["teacher_kind"]
            and audit["step"] == receipt["step"] and receipt["step"] == 250
    ...
    match_binding = {key: {"path": str(args.match_directory / filename),
                           "sha256": sha(args.match_directory / filename)}
                     for key, filename in (("plan", "plan.json"), ("result", "result.json"),
                                           ("execution", "execution.json"))}
    match = load_completed_match(match_binding, bindings)
    require(match["training_plans"]["candidate"] == {"path": str(args.training_plan.resolve()),
                                                      "sha256": args.training_plan_sha256}
            and match["model_states"]["candidate"] == receipt["model_state_sha256"],
            "Quality endpoint differs from matched training evidence")
    paths = [audit_path, audit_exec_path, stage / "execution.json", stage / "stage.json", monitor_path,
             ...
             Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_controlled_deployed.py",
             ROOT / "research/direct/audit_latency58_controlled_deployed_training_match.py",
             ROOT / "research/direct/compare_latency58_sdr.py", ROOT / "research/direct/compare.py"]
    if "probes" in args.modes:
        paths.append(ROOT / "research/direct/evaluate_latency58_controlled_deployed_probes.py")
    if "full14" in args.modes:
        ...
        paths += [proof_path, ROOT / "research/direct/evaluate_latency58_controlled_deployed_parallel.py"]
    ...
    for mode in args.modes:
        ...
        plan.update(schema="latency58-controlled-deployed-" + ("probe" if mode == "probes" else "music") + "-plan-v1",
        ...
        if mode == "full14":
            plan.update(schema="latency58-controlled-deployed-parallel-music-plan-v1", workers=2,
                        model_kind="controlled_deployed_candidate", track_indices=list(range(14)),
    ...
    for mode, out, plan in prepared:
        ...
        module = "research.direct.evaluate_latency58_controlled_deployed" + ("_probes" if mode == "probes" else "_parallel" if mode == "full14" else "")

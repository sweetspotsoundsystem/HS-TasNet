...
def exercise(parent, payload, source, out, progress):
    ...
    expected_count = 2 * len(model.state_dict()) + 3 * len(list(model.parameters())) + 2 + len(snapshot["resume"]["cuda_rng"])
    progress("decoded_tensor_inventory", original_and_restored_count=count, expected_count=expected_count,
             encoder_count=first_stats["tensor_count"], decoder_count=stats["tensor_count"])
    require(count == stats["tensor_count"] == first_stats["tensor_count"] == expected_count == 216
    ...
    with tempfile.TemporaryDirectory(prefix="packed-fixture-", dir=out) as temporary:
        ...
        require(equal_tree(snapshot, loaded) == expected_count and fingerprint(model, optimizer, ema) == before,
...
def main():
    ...
    out = PHASE / "lossless-recovery-cpu-003"
    ...
    failed_two = PHASE / "lossless-recovery-cpu-002"
    verify_inputs(read(failed_two / "plan.json")); bindings.update(read(failed_two / "plan.json")["source_bindings"])
    paths.extend([failed_two / "plan.json", PHASE / "lossless-recovery-cpu-002-stage/command.json",
                  PHASE / "lossless-recovery-cpu-002-stage/execution.json",
                  PHASE / "lossless-recovery-cpu-002-stage/evaluation-console.log"])
    ...
    plan = {"schema": "latency58-lossless-packed-recovery-cpu-qualification-v1", "source_bindings": bindings,
            ...
            "cpu_tensor_inventory": "216 leaves: 94 raw/EMA tensors, 120 Adam tensors and two CPU RNG tensors; CUDA adds one separate RNG tensor",

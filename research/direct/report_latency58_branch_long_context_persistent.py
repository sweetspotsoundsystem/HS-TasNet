...
def check_journal(plan, rows, training, resource):
    ...
    require(len(rows) == config["steps"] and plan["logical_batch_loss"] == reduction_policy()
            and plan["accumulation_steps"] == 4 and config["microbatch_size"] == 4
    ...
    for index, row in enumerate(rows):
        ...
        require(row["step"] == row["ema_updates"] == index + 1 and row["lr"] == rate
                ...
                and len(row["microbatches"]) == 4, "Update schedule or batch geometry changed")
        ...
        for micro, terms in enumerate(row["microbatches"]):
            require(terms["augmented_examples"] == (0 if micro == 0 else 4)
                    and len(terms["active_windows"]) == len(terms["absent_windows"]) == 4
                    and all(a + b == 8 for a, b in zip(terms["active_windows"], terms["absent_windows"], strict=True)),
...
def main():
    ...
    root = PHASE / "branch-long-context-006"
    ...
    require(parity["status"] == parity["logical_batch_loss"]["status"] == "pass"
            ...
            and parity["microbatch_size"] == 4 and parity["scored_samples"] == 88320
    ...
    for path in (root / "resource-stage/execution.json", root / "production-stage/execution.json",
                 ...
                 PHASE / "branch-long-context-preparation-stage-006/execution.json",
                 PHASE / "branch-long-context-preparation-stage-006/root-execution.json"):
        execution = read(path)
        ...
        if "monitor_result" in execution:
            ...
            require(monitor["status"] == monitor["supervisor_health"] == "pass"
                    ...
                    and monitor["event_transport_sha256"] == plan["event_query_transport"]["helper_sha256"]
                    and monitor["event_worker_close"]["closed"] and not monitor["event_worker_close"]["forced"]
                    and monitor["event_worker_close"]["actual_exit_code"] == 0, "GPU monitor or event worker failed")
            paths.append(path)
    ...
    require(command["actual_root_session"] == prefix["actual_root_session"] == args.actual_root_session
            ...
            and command["argv"][3] == "research.direct.run_latency58_branch_long_context_b4"

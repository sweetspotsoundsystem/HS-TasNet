"""Train the controlled quarter-vocal objective with allocated lossless recovery."""
...
from research.direct.latency58_weighted_storage import snapshot as budget_snapshot, policy as storage_policy
from research.direct.latency58_grouped_vocal_recovery import (
    policy as recovery_policy, make_snapshot, restore_training)
from research.direct.latency58_lossless_recovery_files_v3 import publish_snapshot, read_snapshot, finalize
...
def runtime_policy():
    return {"version": "quarter-vocal-packed-save-runtime-v1", "resource_max_seconds": 1200,
...
def validate_recipe(plan):
    require(plan.get("packed_recovery") == packed_policy() and plan.get("weighted_storage") == storage_policy()
            and plan.get("runtime_allowance") == runtime_policy(), "Require qualified packed recovery and explicit storage/runtime allocations")
    decision_binding = plan["scientific_decision"]
    require(sha(decision_binding["path"]) == decision_binding["sha256"]
            == "f258eb12e90c5212dae95ab7cb96e3eeee750415a01491bdf291c8b5d631d33e", "Scientific decision changed")
    ...
    require(plan["config"] == decision["proposed_training_config"]
            ...
            and plan["parameter_names"] == decision["trainable_parameter_names"], "Controlled parent, schedule, data or parameter set changed")
...
def main():
    ...
    if stage["resource_only"]:
        from research.direct.check_latency58_weighted_vocal_device import check_gpu

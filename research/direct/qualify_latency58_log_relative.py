...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-log-relative-functional-v1"
    ...
    import numpy as np
    import torch
    from research.direct.latency58_log_relative import (
        ACTIVITY_POWER, AUXILIARY_WEIGHT, EPSILON, VERSION, WINDOW_SAMPLES, log_relative_error,
    )
    ...
    result = log_relative_error(estimate, target)
    ...
    for start, end in slices:
        ...
        losses.append(np.where(active, np.log1p(error / (np.maximum(power, ACTIVITY_POWER) + EPSILON)), 0.))
        masks.append(active)
        powers.append(power)
        errors.append(error)
    ...
    for index, (start, end) in enumerate(slices):
        ...
        derivative /= (np.maximum(powers[index], ACTIVITY_POWER) + EPSILON + errors[index])[..., None, None]
    ...
    perfect_loss = log_relative_error(perfect, target).total
    ...
    absent_loss = log_relative_error(absent, torch.zeros_like(target)).total
    ...
    with torch.autocast("cpu", dtype=torch.bfloat16):
        autocast_loss = log_relative_error(estimate, target).total
    ...
    auxiliary = log_relative_error(deployed, target)
    ...
    for label, prediction, reference in (
        ...
        try:
            log_relative_error(prediction, reference)
        except ValueError:
    ...
    report = {"schema": "latency58-log-relative-functional-result-v1", "status": "pass",

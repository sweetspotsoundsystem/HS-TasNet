"""Score the same fixed vocal leader_cleanups for a completed leader_cleanup pilot."""
...
def initialize_worker(plan):
    ...
    spec = plan["model"]
    require(spec["kind"] == "leader_cleanup", "Require the new leader_cleanup checkpoint family")
...
def main():
    ...
    require(sha(args.plan) == args.plan_sha256, "Vocal leader_cleanup plan changed")
    plan = read(args.plan)
    require(plan["schema"] == "latency58-leader-cleanup-views-evaluation-plan-v1" and plan["workers"] == 2

"""Self-contained inference snapshots for full-model trailing4096 magnitude training."""
...
SCHEMA = "latency58-long-magnitude-inference-v1"


def load_model(binding):
    import torch
    from research.direct.latency58_full_magnitude_checkpoint import SCHEMA as PARENT_SCHEMA, load_model as load_parent
    from research.direct.latency58_long_magnitude import Latency58LongMagnitudeModel
    require(sha(binding["path"]) == binding["sha256"], "Long-magnitude checkpoint file changed")
    ...
    if payload["schema"] == PARENT_SCHEMA:
        ...
        return Latency58LongMagnitudeModel.from_parent(parent), payload
    require(payload["schema"] == SCHEMA, "Unknown direct-SDR inference schema")
    with torch.random.fork_rng(devices=[]), torch.device("cpu"):
        model = Latency58LongMagnitudeModel()
    ...
    require(model.provenance["direct_sdr_updates"] == model.provenance["magnitude_updates"] == model.provenance["long_magnitude_updates"] == payload["step"]
...
def audit_live(model, optimizer, step, frozen):
    ...
    require(len(parameters) == 23 and all(p.requires_grad for p in parameters)
...
def save_generation(model, optimizer, step, plan, plan_sha, run):
    ...
    provenance = {**model.provenance, "direct_sdr_parent_checkpoint": plan["parent_checkpoint"],
                  ...
                  "magnitude_updates": step, "long_magnitude_updates": step, "all_neural_parameters_trained": True,
    ...
    write(pending / "receipt.json", {"schema": "latency58-long-magnitude-generation-v1", "step": step,

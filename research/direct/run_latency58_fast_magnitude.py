...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_fast_magnitude", "--plan", str(plan_path),
...
def prepare(args):
    ...
    paths = [Path(__file__).resolve(), ROOT / "research/direct/latency58_direct_sdr.py",
             ROOT / "research/direct/latency58_full_magnitude_checkpoint.py", ROOT / "research/direct/train_latency58_fast_magnitude.py",
    ...
    paths.extend(ROOT / "research/direct" / name for name in (
        "latency58_fast_context.py", "latency58_state_warmup.py", "check_latency58_fast_context.py"))
    ...
    config = {**template["config"], "steps": args.steps, "seed": args.seed, "data_start": args.data_start,
              ...
              "microbatch_size": 8, "checkpoint_every": args.steps,
    ...
    plan = {"schema": "latency58-direct-sdr-plan-v1", "name": args.name, "output_directory": str(out), "config": config,
            ...
            "warmup_samples": 88064, "scored_samples": 44160, "accumulation_steps": 2,
    ...
    write(out / "preparation.json", {"status": "pass", "parent_model_state_sha256": plan["parent_model_state_sha256"],
          ...
          "rationale": "Continue all 22 neural tensors from the saved 4.114394535 dB checkpoint on the same 301 recorded training tracks. Keep two seconds of detached warmup; score one second per crop with microbatch eight and two accumulation passes. Omit unused warmup decoder frames, with exact CPU and required GPU scored-waveform/gradient checks. Four thousand updates expose approximately the same scored duration as the previous two thousand. Inference geometry and validation remain unchanged.",
...
def main():
    ...
    parser.add_argument("--name", default="full-magnitude-fast-001")
    ...
    resource = (plan_path.parent / "resource-stage/execution.json" if args.after_resource else
                launch(plan_path, PHASE / "full-magnitude-001/production-stage/execution.json", resource=True))

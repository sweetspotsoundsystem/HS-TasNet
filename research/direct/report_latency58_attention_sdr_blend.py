"""Review the completed fixed SDR-blend trial and every track/stem regression."""
...
def main():
    ...
    from research.direct.prepare_latency58_attention_sdr_blend import RECIPE
    ...
    root = PHASE / "attention-sdr-blend-001"
    ...
    require(plan["objective_version"] == VERSION and plan["direct_sdr_weight"] == SDR_WEIGHT == .1
            and all(plan["config"][key] == RECIPE[key] for key in
                    ("steps", "lr", "min_lr", "warmup", "data_start", "seed")), "Frozen blend recipe changed")
    ...
    require(root_command["record_kind"] == "observed_live_root_command"
            ...
            and root_command["argv"][3] == "research.direct.run_latency58_attention_sdr_blend"
    ...
    require(parent["checkpoint"] == plan["parent_checkpoint"]
            and parent["aggregate"]["full_sdr_db"] == 4.330200584072513
            and plan["parent_kind"] == "saved_temporal_attention"
            and plan["parent_training_updates"] == 25250
    ...
    review = {"status": "selected_for_research" if selected else "not_selected",
              ...
              "graph_plus_host_delay_samples": 256, "total_training_updates": 26250,
              "parent_training_updates": 25250, "objective_version": VERSION,
              ...
              "interpretation": "Single-seed trial with a new blended objective, fresh Adam, a new schedule and a new sample range; not an exact optimizer restart or a matched causal ablation. Track-bootstrap uncertainty excludes training-seed uncertainty and selection effects.",

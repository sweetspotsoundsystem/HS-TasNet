"""Authenticated warm500 parent and split generations for accumulated-gradient adaptation."""
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="accumulated_gradient_context_fine_tune", parent_provenance=old,
                 ...
                 accum_trial_updates=step, carry_state=plan["carry_state"],
                 ...
                 training_objective="mean_four_b4_raw4_l1_plus_context_teacher_l1",
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-sdr-accum-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-sdr-accum-inference-v1" and payload["step"] == step
...
def validate_journal(journal, step, plan, helpers):
    ...
    for row in rows:
        ...
        require(plan["microbatch_size"] == row["microbatch_size"] == 4
                ...
                and len(row["microbatches"]) == 4, "Malformed accumulated update")
        ...
        for index, micro in enumerate(micros):
            ...
            for key in ("loss", "supervised_loss", "waveform_l1", "projection", "projection_contribution", "teacher_l1"):
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-sdr-accum-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-sdr-accum-generation-v1", "step": step, "plan_sha256": plan_sha,

"""Qualify B1 weighted updates and exact packed disk restart for the 2000-step plan.
    ...
    try:
        with patch("torch.nn.utils.clip_grad_norm_", wraps=torch.nn.utils.clip_grad_norm_) as clips, \
            ...
            require(result["raw_model_state_sha256"] != before["model"]
                    ...
                    and len(result["groups"]["ordinary"]["microbatches"]) == 16
                    and len(result["groups"]["auxiliary"]["microbatches"]) == 2,
...
def run_fixture(parent, plan, out, progress):
    ...
    fixture = {'schema': 'latency58-four-second-proposal-short-cpu-restart-fixture-v1',
        ...
        **{key: decision[key] for key in ('parent_checkpoint', 'parent_model_state_sha256',
        ...
        'precision_policy': 'CPU FP32 synthetic weighted B1 restart fixture',
    ...
    require(fixture['config']['steps'] == 2000 and fixture['config']['microbatch_size']
            == fixture['config']['auxiliary_microbatch_size'] == 1, 'Retain the selected complete schedule and B1 grouping')
    ...
    reject(rejected, 'noncontiguous_step', lambda: grouped_update(model, optimizer, ema, *inputs,
        step=2, warmup_samples=512, ordinary_microbatch=1, auxiliary_microbatch=1))
    ...
    require(continuous == replay and fingerprint(model, optimizer, ema) == fingerprint(restored, adam, average),
            'Weighted B1 update or raw/Adam/EMA endpoint differs after disk restart')
    ...
    return {'status': 'pass', 'parent_model_state_sha256': parent_sha, 'ordinary_microbatch': 1,
        'auxiliary_microbatch': 1, 'logical_batch_size': 16, 'warmup_samples': 512, 'scored_samples': 44160,
        ...
        'all_tensor_and_metadata_bytes_exact': True, 'third_weighted_b1_update_and_accounting_bit_exact': True,

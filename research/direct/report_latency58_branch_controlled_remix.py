"""Review a saved source-controlled endpoint, its complete recipe and every regression."""
...
def main():
    ...
    from research.direct.prepare_latency58_branch_controlled_remix import RECIPE
    from research.direct.latency58_branch_controlled_remix import AUGMENTATION, recipe
    from research.direct.latency58_attention_sdr_blend import VERSION as TENTH_VERSION
    from research.direct.latency58_branch_sdr_blend import VERSION as FIFTH_VERSION
    ...
    root = PHASE / "branch-controlled-remix-001"
    ...
    SDR_WEIGHT = plan["direct_sdr_weight"]
    require(SDR_WEIGHT in (.1, .2), "Unsupported inherited SDR objective")
    VERSION = { .1: TENTH_VERSION, .2: FIFTH_VERSION }[SDR_WEIGHT]
    require(plan["objective_version"] == VERSION and plan["config"]["augmentation"] == AUGMENTATION
    ...
    require(root_command["record_kind"] == "observed_live_root_command"
            ...
            and root_command["argv"][3] == "research.direct.run_latency58_branch_controlled_remix"
    ...
    preparation_execution = PHASE / "branch-controlled-remix-preparation-stage-001/execution.json"
    ...
    for index, row in enumerate(all_rows):
        ...
        require(row["augmentation_recipe"] == expected_recipe
                and terms["source_view_counts"] == {"ordinary": 12, "instrumental": 2, "vocals_only": 2}
                and min(terms["absent_windows"]) >= 2
                and all(a + b == 16 for a, b in zip(terms["active_windows"], terms["absent_windows"], strict=True)),
                "Source-controlled recipes, view counts or silent target windows differ")
    ...
    require(parent["checkpoint"] == plan["parent_checkpoint"]
            == selection["best_research_checkpoint"]
            and parent["aggregate"]["full_sdr_db"] == plan["parent_full_sdr_db"]
            == selection["best_full_sdr_db"]
            and plan["parent_kind"] == "saved_trained_branch_memory"
            and plan["parent_training_updates"] == parent_plan["parent_training_updates"] + parent_plan["config"]["steps"]
            and plan["direct_sdr_weight"] == parent_plan["direct_sdr_weight"]
            and plan["config"]["steps"] == 1000
    ...
    review = {"status": "selected_for_research" if selected else "not_selected",
              ...
              **completion_detail,
              ...
              "direct_sdr_weight": SDR_WEIGHT, "all_1000_blend_journal_identities_checked": True,
              "all_1000_controlled_recipes_and_silent_window_counts_checked": True,
              ...
              "interpretation": "Single-seed continuation retaining the selected parent objective and adding two instrumental and two vocal-only examples to each sixteen-crop batch. Fresh Adam, a new schedule and sample range prevent matched causal attribution to augmentation alone. Track-bootstrap uncertainty excludes training-seed uncertainty and selection effects.",

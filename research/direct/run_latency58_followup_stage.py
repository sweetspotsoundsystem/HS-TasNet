...
def main():
    ...
    require(plan["schema"] == "latency58-followup-training-plan-v1", "Use a follow-up plan")
    ...
    if args.previous_stage is not None:
        ...
        require(sha(plan["parent_snapshot_receipt"]["path"]) == plan["parent_snapshot_receipt"]["sha256"]
                ...
                and snapshot["model_state_sha256"] == plan["initial_model_state_sha256"],
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT),
            ...
            "argv": [python, "-u", str(ROOT / "research/direct/train_latency58_followup.py"),
    ...
    argv = [python, "-u", "-m", "research.direct.audit_latency58_followup",
            "--plan", str(args.plan.absolute()), "--plan-sha256", args.plan_sha256,

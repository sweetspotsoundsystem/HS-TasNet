"""Run one monitored vocal-focus rehearsal or independently audited 250-update pilot."""
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-vocal-focus-training-v1", "Unknown training plan")
    ...
    require(decision["schema"] == "latency58-vocal-focus-preparation-decision-v1"
            and decision["status"] == "prepare_pilots" and decision["history_review_complete"]
            and decision["baseline_vocal_views_review_complete"]
            and decision["parent_model_state_sha256"] == plan["parent"]["model_state_sha256"]
            and decision["pilot_updates_per_arm"] == 250 and not decision["automatic_continuation"]
            and decision["priority"] == "vocal_cleanliness", "New pilot lacks the completed parent and vocal review")
    if plan["resource_only"]:
        ...
        require(resource["schema"] == "latency58-vocal-focus-resource-result-v1" and resource["status"] == "pass"
        ...
        require(pairing["schema"] == "latency58-vocal-focus-resource-pairing-v1" and pairing["status"] == "pass"
                ...
                and pairing["focused_inputs_and_teacher_targets_exact"]
                and pairing["initial_inherited_gradients_exact"]
                and pairing["source_bindings_unchanged"]
                and pairing["resource_results"][plan["arm"]] == plan["full_resource"]
                and pairing_execution["actual_exit_code"] == 0 and not pairing_execution["timed_out"]
                and pairing_execution["source_bindings_unchanged"], "Three-arm GPU matching is not qualified")
    start = 0
    ...
    stage = {"schema": "latency58-vocal-focus-stage-v1", "plan_sha256": args.plan_sha256,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_vocal_focus", "--plan", str(args.plan.resolve()),
    ...
    if plan["resource_only"]:
        ...
        write(out / "resource-qualification.json", {
            "schema": "latency58-vocal-focus-resource-qualification-v1", "status": "pass",
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.audit_latency58_vocal_focus", "--plan", str(args.plan.resolve()),

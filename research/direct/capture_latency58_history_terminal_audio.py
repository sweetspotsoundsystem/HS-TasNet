"""Capture both matched history arms with three authenticated reused models.
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-history-primary-regression-audio-plan-v1"
            and plan["track_index"] == 10 and plan["track_name"] == "Skelpolu - Human Mistakes"
            and [m["id"] for m in plan["models"]] == ["short", "long"]
            and [m["kind"] for m in plan["models"]] == ["history_candidate", "history_candidate"]
    ...
    require(out.parent == PHASE and old.parent == PHASE and out.is_dir()
            and all(not (out / name).exists() for name in ("result.json", "index.html", "short", "long")),
    ...
    require(set(saved["models"]) == {"working", "parent", "previous", "candidate"}
            ...
            and plan["omitted_capture_model_id"] == "parent"
            and all(plan["source_bindings"].get(p) == h for p, h in saved["source_bindings"].items()),
            "Prior capture inventory or provenance differs")
    comparison_binding, comparison_execution_binding = plan["matched_comparison"], plan["matched_comparison_execution"]
    for binding in (comparison_binding, comparison_execution_binding):
        require(plan["source_bindings"].get(binding["path"]) == binding["sha256"] == sha(binding["path"]),
                "Unbound matched comparison")
    comparison, comparison_execution = read(comparison_binding["path"]), read(comparison_execution_binding["path"])
    command = comparison_execution["argv"]
    comparison_plan_path = Path(command[command.index("--plan") + 1])
    comparison_plan = read(comparison_plan_path)
    require(comparison["schema"] == "latency58-history-comparison-v1" and comparison["status"] == "pass"
            and comparison["source_bindings_unchanged"] and comparison_execution["actual_exit_code"] == 0
            and not comparison_execution["timed_out"] and comparison_execution["source_bindings_unchanged"]
            and comparison["reference_step"] == comparison["candidate_step"] == plan["step"]
            and plan["step"] in (250, 500)
            and command[command.index("-m") + 1] == "research.direct.compare_latency58_history"
            and comparison["plan_sha256"] == comparison_execution["plan_sha256"] == sha(comparison_plan_path)
            and Path(comparison_plan["output_directory"]) / "result.json" == Path(comparison_binding["path"])
            and plan["source_bindings"].get(str(comparison_plan_path)) == sha(comparison_plan_path)
            and all(plan["source_bindings"].get(p) == s for p, s in comparison["source_bindings"].items()),
            "Require an executed matched endpoint comparison")
    verify_inputs(comparison)
    for item, side in zip(plan["models"], ("reference", "candidate"), strict=True):
        require(item["prefix"] == comparison[side + "_prefix"]
                and item["model_state_sha256"] == comparison[side + "_model_state_sha256"]
                and item["training_plan"] == comparison[side + "_training_plan"], "Different matched endpoint")
    ...
    require(plan["step"] == 500 and plan["new_audio_reserve_bytes"] == 90_000_000
            and plan["concurrent_checkpoint_reserve_bytes"] == 0,
            "Reserve only capture output after both original training schedules finish")
    for item in plan["models"]:
        training = read(item["training_plan"]["path"])
        run = Path(training["run_dir"])
        status = read(run / "status.json")
        pointer = read(run / "audit-latest.json")
        audit = read(pointer["audit"]["path"])
        execution = read(pointer["execution"]["path"])
        require(training["config"]["steps"] == training["continuation_rules"]["maximum_step"] == 500
                and status["status"] == "complete" and status["step"] == pointer["step"] == audit["step"] == 500
                and audit["status"] == "pass" and audit["source_bindings_unchanged"]
                and audit["model_state_sha256"] == item["model_state_sha256"]
                and execution["actual_exit_code"] == 0 and not execution["timed_out"]
                and execution["source_bindings_unchanged"], "Original training is not complete and audited")
        for path in (run / "status.json", run / "audit-latest.json", Path(pointer["audit"]["path"]),
                     Path(pointer["execution"]["path"])):
            require(plan["source_bindings"].get(str(path)) == sha(path), "Unbound terminal training evidence")
    require_space(space_plan, 90_000_000)
    import numpy as np
    ...
    require([m["id"] for m in plan["reuse_models"]] == ["working", "previous", "parent"]
            and [m["capture_id"] for m in plan["reuse_models"]] == ["working", "previous", "candidate"],
    ...
    with (out / "progress.jsonl").open("x", buffering=1) as progress:
        for item in plan["models"]:
            ...
            from research.direct.evaluate_latency58_sdr_history import load_evaluation_model
            ...
            training = read(item["training_plan"]["path"])
            require(training["warmup_samples"] == {"short": 88064, "long": 352256}[item["id"]]
                    and training["teacher_history_samples"] == 352256
                    and training["parent"]["model_state_sha256"] == models["parent"]["model_state_sha256"],
                    "History geometry or training parent differs")
    ...
    require(len(inventory) == 50 and len(new_files) == 16
            and sum(inventory[p]["bytes"] for p in new_files) < 90_000_000, "Wrong audio inventory or allowance")
    ...
    html = html.replace("Actions · training comparison", "Skelpolu · training history comparison")
    html = html.replace(
        ...
        "Compare the working model, previous SDR leader, training parent and matched short- and long-history models on the two primary excerpts from Skelpolu – Human Mistakes.")
    ...
    with (out / "README.md").open("x") as stream:
        stream.write("# Skelpolu primary-excerpt comparison\n\n[Open the player](index.html). "
                     ...
                     "Both history models receive new captures; the other models reuse authenticated audio.\n\n"
                     ...
                     "34 are reused and 16 are new. [Capture details](result.json) record hashes and per-excerpt diagnostics.\n\n"
    ...
    write(out / "result.json", {"schema": "latency58-history-primary-regression-audio-v1", "status": "pass",
        ...
        "step": plan["step"], "matched_comparison": comparison_binding,

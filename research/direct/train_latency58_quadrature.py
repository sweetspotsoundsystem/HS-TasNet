"""Train all 24 quadrature tensors from an authenticated magnitude parent."""
...
def main():
    ...
    require(state_sha256(parent.state_dict()) == plan["parent_model_state_sha256"], "Magnitude parent changed")
        ...
        for mixture_cpu, truth_cpu in loader:
            ...
            if step < 2:
                require(phase_gradients["phase_expand.weight"] > 0
                        and (phase_gradients["phase_reduce.weight"] == 0 if step == 0 else
                             phase_gradients["phase_reduce.weight"] > 0),
                        "Quadrature factors did not activate from zero expansion")

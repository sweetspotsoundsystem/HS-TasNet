...
def main():
    ...
    require(quality_plan["schema"] == "latency58-vocal-focus-parallel-music-plan-v1"
    ...
    require(sha(training_binding["path"]) == training_binding["sha256"]
            and training["schema"] == "latency58-vocal-focus-training-v1" and not training["resource_only"],
    ...
    for path in (template_path, template_path.parent / "execution.json", args.reservation,
                 Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_vocal_focus_views.py",
                 ROOT / "research/direct/evaluate_latency58_vocal_focus.py",
                 ROOT / "research/direct/latency58_vocal_views.py"):
        bindings[str(path)] = sha(path)
    plan = copy.deepcopy(template)
    plan.update(schema="latency58-vocal-focus-views-evaluation-plan-v1", output_directory=str(out),
                model={"kind": "vocal_focus", "label": args.prefix, "arm": training["arm"],
    ...
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_vocal_focus_views",

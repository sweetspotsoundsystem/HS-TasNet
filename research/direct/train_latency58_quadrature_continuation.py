"""Continue all 24 tensors from a saved quadrature parent with a fresh Adam schedule."""
...
def main():
    ...
    verify_inputs(stage)
    config = plan["config"]
    require(plan["schema"] == "latency58-quadrature-training-plan-v1" and stage["plan_sha256"] == args.plan_sha256
    ...
    from research.direct.latency58_quadrature import Latency58QuadratureModel, ADAPTERS
    from research.direct.latency58_quadrature_checkpoint import audit_live, save_generation
    from research.direct.latency58_quadrature_context import render_scored_context
    ...
    require(plan["parent_kind"] == "saved_quadrature"
            and plan["optimizer_initialization"] == "fresh_adam"
            and state_sha256(parent.state_dict()) == plan["parent_model_state_sha256"], "Quadrature parent changed")
    model = parent
    model.provenance = {**model.provenance,
                        "quadrature_previous_provenance": dict(model.provenance),
    ...
    model.provenance = {**model.provenance, "quadrature_current_stage_corpus": "301 recorded MUSDB18-HQ and MoisesDB training tracks",
                        "quadrature_current_stage_augmentation": AUGMENTATION,
                        "quadrature_online_teacher_used": False,
                        "quadrature_teacher_generated_targets_in_current_stage": False,
                        "quadrature_training_selection": plan["training_selection"]}
    ...
    require(config["microbatch_size"] == 16 and plan["accumulation_steps"] == 1
            ...
            "Fast continuation geometry changed")
    if stage["resource_only"]:
        from research.direct.check_latency58_quadrature_gpu import check_gpu
        write(run / "quadrature-context-gpu-parity.json", check_gpu(model))
    ...
    with (run / "metrics.jsonl").open("x", buffering=1) as journal:
        for mixture_cpu, truth_cpu in loader:
            ...
            for offset in range(0, config["batch_size"], config["microbatch_size"]):
                ...
                require(all(p.grad is not None for p in model.parameters()), "A quadrature parameter lacks a gradient")
            ...
            phase_gradients = {name: float(dict(model.named_parameters())[name].grad.norm()) for name in ADAPTERS}
            if step < 2:
                require(phase_gradients["phase_expand.weight"] > 0
                        and phase_gradients["phase_reduce.weight"] > 0,
                        "Trained quadrature factors must both receive nonzero gradients")
            ...
            row = {"step": step, "lr": lr, "first_sample_index": config["data_start"] + (step - 1) * config["batch_size"],
                   ...
                   "quadrature_gradient_norms": phase_gradients}
            if step <= 2:
                prefix.append(row)

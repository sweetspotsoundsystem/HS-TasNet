"""Run one frozen context stage, audit it, and retire replaced Adam state."""
...
def main():
    ...
    parser.add_argument("--stop-step", type=int, choices=(2, 25, 250, 500), required=True)
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-context-training-v1", "Unknown training plan")
    from research.direct.latency58_context_checkpoint import read_generation, require_space
    ...
    require(resource["status"] == "pass" and resource["teacher_kind"] == plan["teacher_kind"]
            and resource["scored_samples"] == resource["warmup_samples"] == 88064
            and resource["arms"] == [False, True] and resource["teacher_weight"] == plan["teacher_weight"]
    ...
    stage = {"schema": "latency58-context-stage-v1", "plan_sha256": args.plan_sha256,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_context", "--plan", str(args.plan.resolve()),
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.audit_latency58_context", "--plan", str(args.plan.resolve()),
    ...
    if previous_generation is not None:
        ...
        retirement = {"schema": "latency58-context-replaced-optimizer-v1", "path": str(old_optimizer),

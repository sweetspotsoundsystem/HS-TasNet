"""Qualify six-second pitch/tempo crops for a possible four-second scored context."""
...
from research.direct.latency58_m4_followup_budget import snapshot as budget_snapshot, POLICY as BUDGET_POLICY
from research.direct.latency58_recorded301_data import ROOT_WEIGHTS, select_tracks, selection_contract
from research.direct.latency58_four_second_data import (
...
def main():
    ...
    policy_path = PHASE / "branch-weighted-vocal-014/plan.json"
    policy = read(policy_path)
    counted = budget_snapshot()
    require(counted["frozen_training_forecast"]["projected_peak_bytes"] + 1_000_000 < 90_000_000_000
            and counted["combined_peak_bytes"] + 1_000_000 < counted["authorized_cap_bytes"],
            "Reserve the complete one-megabyte data-qualification allowance")
    source_paths = [Path(__file__), ROOT / "research/direct/latency58_four_second_data.py",
                    ...
                    ROOT / "research/direct/latency58_weighted_storage.py", BUDGET_POLICY, PRODUCTION / "train_production.py",
    ...
    first, stop, seed = 4132000, 4132064, 20261102
    ...
    snapshot = {"schema": "latency58-four-second-score-pitch-tempo-cpu-inputs-v1", "version": VERSION, "selection": selection,
                "first_sample_index": first, "stop_sample_index": stop, "augmentation_seed": seed,
                "source_bindings": bindings, "input_rows": input_rows,
                "artifact_policy": {"maximum_output_bytes": 1_000_000, "budget_before": counted},
                "new_training_recipe_selected": False}
    ...
    require(sum(p.stat().st_size for p in out.rglob("*") if p.is_file()) + 100_000 < 1_000_000,
            "Data-qualification allowance exceeded")
    result = {"status": "pass", "version": VERSION, "inputs_sha256": sha(out / "inputs.json"),
              ...
              "elapsed_seconds": time.monotonic() - started, "budget_before": counted, "budget_after": after,
              "forecast_bytes": after["combined_peak_bytes"], "new_training_recipe_selected": False,
              "versions": {"torch": torch.__version__, "numpy": np.__version__,
                           "ffmpeg": subprocess.check_output([FFMPEG, "-version"], text=True).splitlines()[0]},
              "limitations": "64 six-second training crops (two seconds of warmup and four seconds scored), four B16 batches per loader. Timings include startup/shutdown; no steady-state GPU throughput or SDR gain is established. Selected crops use longer reads with different offsets; unselected crops retain original offsets. Drum transient fidelity, vocal formant quality, and audible quality require separate assessment. This prototype is not connected to a trainer."}

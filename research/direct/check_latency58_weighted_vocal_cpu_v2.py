...
from research.direct.latency58_direct_sdr import WINDOW, ACTIVITY_POWER, ABSENCE_WEIGHT
...
def independent_scalar(raw, deployed, targets, mixture, *, weights):
    ...
    Exact predictions contribute zero reconstruction/anchor loss and -60 dB
    on active windows. Quiet nonzero targets may be inactive, so their
    perfect-prediction absence term must also be retained. Removing the
    complete known constant isolates each
    ...
    eligible = reference.square().sum((2, 4)) / (2 * WINDOW) > ACTIVITY_POWER
    leakage = reference.square().mean((2, 4))
    mixture_power = physical.square().mean((1, 3))
    perfect_absence = 10 * torch.log10(1 + leakage / mixture_power[:, None].clamp_min(ACTIVITY_POWER))
    by_view_and_stem = torch.where(~eligible, perfect_absence, 0).sum(2) / absent.clamp_min(1)
    for view in (0, 1):
        constants[view] = constants[view] + SDR_WEIGHT * ABSENCE_WEIGHT * by_view_and_stem[view].sum() / (absent > 0).sum().clamp_min(1)
    values = []
    for view in (0, 1):
...
def main():
    ...
    out = PHASE / "weighted-vocal-quarter-cpu-002"
    ...
    failed = PHASE / "weighted-vocal-quarter-cpu-001"
    failed_plan = read(failed / "plan.json")
    verify_inputs(failed_plan)
    bindings.update(failed_plan["source_bindings"])
    paths = [Path(__file__).resolve(), source_path, decision_path,
             ...
             failed / "plan.json", failed / "reference-failure-diagnostic.json",
             PHASE / "weighted-vocal-quarter-cpu-001-stage/command.json",
             PHASE / "weighted-vocal-quarter-cpu-001-stage/execution.json",
             PHASE / "weighted-vocal-quarter-cpu-001-stage/evaluation-console.log"]
    ...
    plan = {"schema": "latency58-quarter-vocal-cpu-qualification-v1", "source_bindings": bindings,
            "reference_correction": "Include perfect-prediction absence for quiet, nonzero inactive targets; objective and all tolerances unchanged",

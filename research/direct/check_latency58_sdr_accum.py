"""Compare sequential gradient accumulation with a joint mean on the real model."""
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-sdr-accum-functional-plan-v1"
    ...
    require(model_state == parent_plan["parent"]["model_state_sha256"]
            and teacher_state == teacher_identity["model_state_sha256"], "Fixture loaded a different parent or teacher")
    ...
    for index in range(ACCUMULATION_STEPS):
        ...
        # Include absent references and both projection-mask states.
        target[index, 2].zero_()
    ...
    def objective(batch):
        ...
        terms = raw4_native_objective(output.raw, target[..., warmup:], flags, projection=True)
        return terms.total + .5 * deployed_teacher_l1(output.deployed, teacher_target)

    # Independent reference: retain the four short graphs and differentiate
    # their explicit joint mean once, without the accumulation helper.
    losses = [objective(batch) for batch in batches]
    loss_values = [float(loss.detach()) for loss in losses]
    ...
    for batch in batches:
        ...
        del loss
    require(sequential_values == loss_values and len(reference_gradients) == 21,
            "Accumulation changed the forward objectives")
    gradients = {}
    for (name, parameter), expected in zip(model.named_parameters(), reference_gradients, strict=True):
        ...
        require(actual is not None and bool(torch.isfinite(actual).all()) and bool((actual != 0).any()),
                "Missing, nonfinite or empty accumulated gradient")
        error = float((actual - expected).abs().max())
        require(torch.allclose(actual, expected, rtol=3e-5, atol=3e-7), "Accumulated gradient differs: " + name)
        gradients[name] = {"max_absolute_error": error, "reference_max_absolute": float(expected.abs().max())}
    ...
    model.zero_grad(set_to_none=True)
    verify_inputs(plan)
    result = {"schema": "latency58-sdr-accum-functional-v1", "status": "pass",
              ...
              "mean_loss": reference_value, "microbatch_loss_values": loss_values,
              ...
              "limitations": ["Short synthetic CPU FP32 gradients do not qualify full-crop GPU resource use.",
                              "The mean retains each B4 projection cap; no single-forward B16 penalty equivalence is claimed."],
              "elapsed_seconds": time.monotonic() - began}
    write(out / "result.json", result)
    print("All 21 accumulated gradients match the independent joint mean; model and teacher are unchanged.", flush=True)

"""Build a context-comparison player from authenticated existing Actions audio.
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-context-listening-plan-v1"
    ...
    require([x["id"] for x in plan["models"]] == ["working", "parent", "reset", "warm"],
            "Require the working model, common parent and matched context arms")
    import numpy as np
    ...
    for row in plan["models"]:
        ...
        _, report = load_completed(directory, evidence, canonical_baseline=row["id"] == "working")
    ...
    html = html.replace("Actions · training comparison", "5.8 ms · context comparison")
    html = html.replace(
        ...
        "Compare the working model, the C91-trained parent, and training with reset or warm state on Actions – One Minute Smile, 60–75 seconds.")
    ...
    html = html.replace(
        ...
        "All four models use the same 256-sample total-latency architecture. New checkpoints remain development comparisons. No listening verdict has been recorded.")
    ...
    write(out / "result.json", {
        "schema": "latency58-context-listening-preparation-v1", "status": "pass",
    ...
    with (out / "README.md").open("x") as stream:
        stream.write("# Context comparison at " + str(plan["step"]) + " updates\n\n")
        ...
        stream.write("The working 5.8 ms model, common C91-trained parent, reset arm and warm arm are labelled. "

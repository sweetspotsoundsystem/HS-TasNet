...
def main():
    ...
    out = PHASE / "attention-onnx-drift-diagnostic-003"
    ...
    write(out / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
          ...
          "variants": ["original_unoptimized", "precise_gru_nonlinear"],
    ...
    for transformation in (precise_nonlinear,):
    ...
    summaries = []
    for label in ("original_unoptimized", "precise_gru_nonlinear"):
        graph = copy.deepcopy(original)
        proof = precise_nonlinear(graph) if label == "precise_gru_nonlinear" else {}

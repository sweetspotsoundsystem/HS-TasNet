"""Execute saved-attention continuation through monitored rehearsal, training and full14 scoring."""
...
def launch(plan_path, previous_execution_path, *, resource):
    plan = read(plan_path)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-temporal-attention-training-plan-v1", "Wrong temporal-attention training plan")
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_temporal_attention_continuation", "--plan", str(plan_path),
...
def audit_and_score(plan_path):
    import torch
    from research.direct.latency58_temporal_attention_checkpoint import audit_saved

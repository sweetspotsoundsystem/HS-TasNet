"""One monitored resource check or bounded drum-weighted context training stage."""
...
def main():
    ...
    require(plan["schema"] == "latency58-sdr-drum-training-v3" and stage["schema"] == "latency58-sdr-drum-stage-v3"
            and stage["plan_sha256"] == args.plan_sha256 and 0 <= start < stop <= config["steps"] == 1000
            and stop in ((2,) if plan["resource_only"] else (250, 500, 1000)) and config["batch_size"] == 4 and config["workers"] == 2
    ...
    from research.direct.latency58_sdr_drum_v3_checkpoint import (
    ...
    from research.direct.latency58_drum_emphasis import (
        DRUM_WEIGHT, TEACHER_WEIGHT, VERSION, drum_emphasized_objective)

    require(plan["drum_weight"] == DRUM_WEIGHT == 2 and plan["teacher_weight"] == TEACHER_WEIGHT
            and plan["objective_version"] == VERSION, "Drum objective definition differs")
    ...
    try:
        for mixture, targets in loader:
            ...
            terms = drum_emphasized_objective(raw, deployed, targets[..., plan["warmup_samples"]:],
                                             teacher_targets, flags, drum_weight=plan["drum_weight"])
            supervised = terms.waveform_l1 + terms.projection_contribution
            auxiliary = terms.teacher_l1
            loss = terms.total
            loss.backward()
            ...
            step += 1
            row = {"step": step, "lr": lr, "loss": float(loss.detach()), "supervised_loss": float(supervised.detach()),
                   ...
                   "drum_weight": plan["drum_weight"], "objective_version": VERSION,
            ...
            del raw, deployed, output, terms, supervised, auxiliary, loss, teacher_targets
        ...
        if plan["resource_only"]:
            ...
            helpers.atomic_json(run / "resource.json", {
                "schema": "latency58-sdr-drum-resource-result-v3", "status": "pass",

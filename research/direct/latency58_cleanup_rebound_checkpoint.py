...
def bound(plan, item):
    require(plan["source_bindings"].get(item["path"]) == item["sha256"] == sha(item["path"]),
            "Unbound cleanup-rebound prerequisite")
    return read(item["path"])


def validate_recipe(plan):
    require(plan["schema"] == "latency58-cleanup-rebound-training-v1"
            and type(plan["resource_only"]) is bool
            and plan["comparison_variable"] == "matched_learning_rate_schedule"
    ...
    config = {**reference["config"], "lr": 3e-5, "min_lr": 3e-6, "seed": 20260922, "data_start": 976000}
    ...
    require(decision["status"] == "train_rebound_independent_of_deployment"
    ...
    require(not control["resource_only"] and control["config"] == {**config, "lr": 1e-5, "min_lr": 1e-6}
            and all(plan[k] == control[k] for k in RECIPE_KEYS if k not in {"config", "stop_counted_bytes"})
            and plan["parent"] == control["parent"]
            and decision["matched_control_training_plan"] == plan["matched_control_training_plan"],
            "Matched learning-rate control differs beyond its schedule")
    return reference
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="selected_cleanup_matched_higher_lr",
                 training_objective="ordinary_only_teacher_plus_controlled_deployed_truth",
                 cleanup_rebound_trial_updates=step,
                 ...
                 comparison_variable="matched_learning_rate_schedule",
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-cleanup-rebound-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-cleanup-rebound-inference-v1" and payload["step"] == step
...
def validate_journal(journal, step, plan, helpers):
    ...
    require(len(reference_rows) == 250 and len(rows) == step, "Incomplete learning-rate comparison")
    identity_keys = ("pristine_batch_sha256", "original_augmentation_sha256", "augmented_batch_sha256",
                     "teacher_targets_sha256", "augmentation_rng_before_sha256", "augmentation_rng_after_sha256",
                     "first_sample_index", "next_sample_index", "view_codes")
    for row, reference_row in zip(rows, reference_rows):
        ...
        for actual, reference_micro in zip(row["microbatches"], reference_row["microbatches"], strict=True):
            require(all(actual[k] == reference_micro[k] for k in identity_keys), "Matched input, teacher or RNG differs")
    require(rows[0]["first_update_inherited_gradient_sha256"] == reference_rows[0]["first_update_inherited_gradient_sha256"],
            "Identical starting model and loss did not produce identical first gradients")
    return rows
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-cleanup-rebound-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-cleanup-rebound-generation-v1", "step": step, "plan_sha256": plan_sha,

...
class STFT(Module):
    ...
    def __init__(
        ...
        assert isinstance(causal_current_chunk, bool)
        self.causal_current_chunk = causal_current_chunk

        window = hann_window(win_length)
        if causal_current_chunk:
            assert win_length == 2 * hop_length
            assert n_fft == win_length
            current_chunk_analysis_window = cat((
                window[:hop_length],
                torch.ones_like(window[hop_length:])
            ))
        else:
            current_chunk_analysis_window = None
        self.register_buffer('window', window)
        self.register_buffer('current_chunk_analysis_window', current_chunk_analysis_window)
    ...
    def inverse(self, spec, is_streaming = False):
        ...
        # c115: frame k observes [past, current] and directly emits current.
        # The analysis window is flat over this half, so synthesis needs neither
        # another window nor overlap-add normalization.

        if self.causal_current_chunk:
            current_frames = ifft[:, hop_length:, :]
            return current_frames.transpose(1, 2).reshape(batch, frames * hop_length)
    ...
    def current_chunk_carrier(self, audio):
        assert self.causal_current_chunk
        assert exists(self.current_chunk_analysis_window)
        return torch.stft(
            audio,
            n_fft = self.n_fft,
            win_length = self.win_length,
            hop_length = self.hop_length,
            center = False,
            window = self.current_chunk_analysis_window,
            return_complex = True
        )
...
class HSTasNet(Module):
    def __init__(
        ...
        *,
        ...
        self.causal_current_chunk = causal_current_chunk
        if causal_current_chunk:
            assert segment_len == 2 * overlap_len
            assert n_fft == segment_len
            assert decoder_hann_baked, 'the current-chunk decoder stores finalized filters directly'
        ...
        self.stft = STFT(
            ...
            causal_current_chunk = causal_current_chunk
        )
        ...
        self.conv_decode = ConvTranspose1DWithHannWindow(
            num_basis,
            audio_channels,
            overlap_len if causal_current_chunk else segment_len,
            stride = overlap_len,
    ...
    def init_stateful_transform_fn(
        ...
        def fn(audio_chunk: ndarray | Tensor):
            ...
            if self.causal_current_chunk:
                assert transformed.shape[-1] == chunk_len
                out = transformed
            else:
    ...
    def forward(
        ...
        is_streaming = False,
        return_streaming_components = False
    ):
        auto_causal_pad = default(auto_causal_pad, self.training)

        if return_streaming_components:
            assert is_streaming
            assert not exists(targets)
            assert not exists(audio_lens)
            assert not exists(return_reduced_sources)

        if self.causal_current_chunk:
            # With left causal context, each frame produces exactly one hop.  Do
            # not throw away a partial final hop: bounded right zero-padding is
            # removed from the output below.
            if not (auto_causal_pad and not is_streaming):
                assert divisible_by(audio.shape[-1], self.overlap_len)
                assert audio.shape[-1] >= self.segment_len
            assert not exists(targets) or (auto_causal_pad and not is_streaming)
        else:
        ...
        if need_audio_mask:
            mask_multiple = self.overlap_len if self.causal_current_chunk else self.segment_len
            audio_lens = round_down_to_multiple(audio_lens, mask_multiple)
            assert (audio_lens > 0).all()

            audio_mask = lens_to_mask(audio_lens, audio_len)

        # Legacy overlap-add needs symmetric padding.  The current-chunk path
        # prepends only the previous-hop context and pads the right solely to
        # complete a final hop.

        if auto_causal_pad and not is_streaming:
            causal_pad = self.causal_pad
            if self.causal_current_chunk:
                right_pad = (-audio_len) % self.overlap_len
                audio = F.pad(audio, (causal_pad, right_pad), value = 0.)
            else:
        ...
        # Preserve the c91 Hann-windowed feature distribution.  Only synthesis
        # uses the causal carrier whose past half rises with the legacy Hann and
        # whose current half is flat.

        if self.causal_current_chunk:
            synthesis_complex_spec = self.stft.current_chunk_carrier(spec_audio_input)
            synthesis_magnitude = synthesis_complex_spec.abs()
            synthesis_real_imag_spec = torch.view_as_real(synthesis_complex_spec)
        else:
            synthesis_complex_spec = complex_spec
            synthesis_magnitude = magnitude
            if self.spec_branch_use_phase:
                synthesis_real_imag_spec = real_imag_spec
        ...
        if self.spec_branch_use_phase:
            ...
            scaled_real_imag_spec = multiply('b ..., b ... t -> (b t) ...', synthesis_real_imag_spec, spec_mask)
            ...
            magnitude, phase = synthesis_magnitude, synthesis_complex_spec.angle()
        ...
        streaming_components = None
        if exists(self.output_source_scales):
            recon_audio.mul_(self.output_source_scales[None, :, None, None])
            if return_streaming_components:
                component_scales = self.output_source_scales[None, :, None, None]
                streaming_components = (
                    recon_audio_from_spec * component_scales,
                    recon_audio_from_waveform * component_scales,
                )
        elif return_streaming_components:
            streaming_components = (
                recon_audio_from_spec,
                recon_audio_from_waveform,
            )
        ...
        if audio_is_squeezed:
            ...
            if return_streaming_components:
                assert exists(streaming_components)
                streaming_components = tuple(
                    rearrange(component, 'b s 1 n -> b s n')
                    for component in streaming_components
                )

        # Remove framing-only samples without changing the requested timeline.

        if auto_causal_pad and not is_streaming:
            if self.causal_current_chunk:
                recon_audio = recon_audio[..., :audio_len]
            else:
        ...
        if return_streaming_components:
            assert exists(streaming_components)
            return recon_audio, lstm_hiddens, streaming_components

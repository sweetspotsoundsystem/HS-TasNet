"""Score an in-memory quadrature graph with reduced-range signed encoder weights on unchanged full14 music.
...
PRECISION = "Nine U8U8 projections plus one U8S8 reduced-range encoder; FP64 quantizer ancestors; FP32 phase factors, decoding and public state"
...
def main():
    ...
    from research.direct.latency58_quadrature_encoder_s8 import build
    ...
    screen_path = PHASE / "quadrature-encoder-s8-screen-001/result.json"
    long_path = PHASE / "quadrature-encoder-s8-long-001/result.json"
    screen, long = read(screen_path), read(long_path)
    prerequisite_executions = [PHASE / name / "execution.json" for name in
                              ("quadrature-encoder-s8-screen-stage-002", "quadrature-encoder-s8-long-stage-001")]
    ...
    graph, proof = build(native, original)
    ...
    paths = [ROOT / "research/direct/latency58_sdr_checkpoint.py", original_path, unsigned_path, unsigned_execution, *prerequisite_executions,
             ...
             ROOT / "research/direct/evaluate_latency58_int8.py", ROOT / "research/direct/latency58_quadrature_encoder_s8.py",
             module, *sorted((module.parent / "capi").glob("*.so*"))]
    bindings = {**long_plan["source_bindings"], **{str(p): sha(p) for p in paths}}
    out = PHASE / "quadrature-encoder-s8-full14-memory-001"
    ...
    write(out / "in-memory-graph.json", {"source_bindings": bindings, "graph_sha256": screen["graph_sha256"],
          ...
          "counted_bytes_before": counted_before, "reserved_pending_training_bytes": 0,
    ...
    plan = {"source_bindings": bindings, "checkpoint": checkpoint, "graph_saved": False, "reference_result": str(reference_path),
            "parent_float_checkpoint": parent, "unsigned_reference_result": str(unsigned_path), "c204_reference_result": str(c204_path),
    ...
    candidate = {"model": {"label": "In-memory quadrature with reduced-range signed encoder", "runtime_artifact": checkpoint,

...
def main():
    ...
    from research.direct.latency58_branch_output_int8 import build, make_reference
    ...
    out = PHASE / "branch-output-int8-screen-001"
    ...
    for optimization in ("disabled", "all"):
        session = session_for(data, baseline["interface"], optimization)
        for case in cases(model, hops=64):
            first = run_case(reference, session, case, baseline["interface"])
            replay = run_case(reference, session, case, baseline["interface"])
    ...
    write(out / "result.json", {"status": "pass" if passed else "fail", "strict_parity_passed": passed,
        ...
        "limitations": "Short synthetic strict parity to independently reconstructed fourteen-projection inference. This is not parity to the unquantized source or the PR #15 fourteen-projection graph. Full-mixture quality, counterfactual leakage, long-stream parity and target-Mac timing remain unmeasured."})

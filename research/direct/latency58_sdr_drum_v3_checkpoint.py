"""Authenticated accumulation parent and split generations for drum-weighted adaptation."""
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="normalized_drum_emphasis_fine_tune", parent_provenance=old,
                 ...
                 gradient_accumulation_steps=1, training_microbatch_size=plan["config"]["batch_size"],
                 trial_augmented_examples=step * plan["config"]["batch_size"], trial_microbatches=step,
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-sdr-drum-generation-v3"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-sdr-drum-inference-v3" and payload["step"] == step
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-sdr-drum-inference-v3", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-sdr-drum-generation-v3", "step": step, "plan_sha256": plan_sha,

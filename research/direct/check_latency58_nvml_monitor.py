...
OLD = HERE / "watch_gpu_process_finalization.py"
NEW = HERE / "watch_gpu_process_nvml.py"
OUT = PHASE / "nvml-monitor-check-001"
...
def run_case(name, *, rows=(2,), delay=.8, child_code=0, final_timeout=3, fault=None,
    ...
    monitor_out = HERE / ("nvml-cpu-control-001-" + name)
    ...
    class SimulatedNvmlTransport:
        ...
        def query(self, timeout):
            ...
            if inject and fault == "invalid_raw_memory":
                memory["free"] += 1
            return {"stdout": f"{identity}, Fixture, fixture-driver, 16384, {used}, {temperature}, N/A, N/A, N/A\n",
...
def main():
    ...
    require(sha(OLD) == "58832a30ddd9c38e603273638834546eec4978623cf3d88c25990a6d08be840e",

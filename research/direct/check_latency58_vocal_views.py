"""Check source-remix alignment and leakage metrics before scoring music."""
...
def main():
    ...
    sources = generator.normal(0, .02, (4, 2, 2304)).astype(np.float32)
    sources[2, :, 700:850] = 0
    ...
    intervals = [{"id": str(i), "reference_start": start, "reference_end": stop,
                 ...
                 for i, (start, stop) in enumerate(((33, 321), (901, 1497)))]
    parity, digests = [], []
    for group in (1, 3):
    ...
    require(digests[0] == digests[1], "Input digest depends on grouping")
    ...
    verify_inputs(plan)
    result = {"schema": "latency58-vocal-views-functional-result-v1", "status": "pass", "version": VERSION,
              ...
              "counted_bytes_after": require_space(plan, 350_000_000), "cuda_initialized": False,

...
def main():
    ...
    parser.add_argument("--output-prefix", default="branch-gru-int8-fixture-001")
    ...
    from research.direct.latency58_branch_gru_int8 import make_reference
    from research.direct.run_latency58_deployed_vocal_views import budget_snapshot
    torch.set_num_threads(1)
    torch.set_num_interop_threads(1)
    torch.use_deterministic_algorithms(True)
    short_root, long_root = PHASE / "branch-gru-int8-screen-002", PHASE / "branch-gru-int8-long-001"
    short, long = read(short_root / "result.json"), read(long_root / "result.json")
    for directory, result in ((short_root, short), (long_root, long)):
    ...
    require(short["strict_parity_passed"] and short["graph_sha256"] == long["graph_sha256"]
            == "878c74694fa4c558de1c5a75837893a0afeadcf57f6e3b860d5904cab04e9fc9", "Wrong candidate graph")
    graph_path, parent_path = Path(short["saved_graph_path"]), Path(short["parent_graph"]["path"])
    ...
    paths = [Path(__file__).resolve(), graph_path, parent_path, parent_proof_path,
    ...
    budget_plan = {**read(PHASE / "deployed-vocal-views-001/plan.json"),
                   "diagnostic_artifact_allowance_bytes": 40_000_000}
    ...
    reference, independent = make_reference(model, onnx.load(parent_path),
        read(parent_proof_path)["conversion"], onnx.load(graph_path), read(short_root / "graph.json")["conversion"])
    require(len(independent) == 14, "Incomplete independently reconstructed projections")
    ...
    out.mkdir()
    plan = {"schema": "latency58-fourteen-projection-fixture-plan-v1", "source_bindings": bindings,
    ...
    provenance = {"format": "SGRTG001: LE uint32 case count, then per case LE uint32 frames, planar input[2,T] and deployed[4,2,T] LE float32",
        ...
        "reference": "Independent CPU PyTorch declared fourteen-projection signed integer inference, reconstructed from source checkpoint weights. Eight FP32 public states; zero initial states; one zero flush; crop to real length. ONNX parsing authenticates reconstructed weights; runtime imports are actively rejected."}
    ...
    write(out / "result.json", {"status": "pass", "plan_sha256": sha(out / "plan.json"),
        ...
        "budget_before": before, "budget_after": budget_snapshot(budget_plan), "native_tests_executed": False,

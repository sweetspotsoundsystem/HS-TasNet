...
from research.direct.latency58_leader_cleanup_checkpoint_v2 import validate_recipe, require_space
from research.direct.report_latency58_sdr import load_completed
from research.direct.report_latency58_leader_cleanup import REFERENCES, reference_views, load_views
...
def main():
    ...
    prefix = "leader-cleanup-250"
    stage = PHASE / "leader-cleanup-to-000250-001"
    control_stage = PHASE / "controlled-deployed-half-to-000250-001"
    ...
    require(argv[argv.index("-m") + 1] == "research.direct.run_latency58_leader_cleanup_stage_v2"
    ...
    for suffix in ("", "_parallel", "_probes", "_views"):
        old = ROOT / ("research/direct/evaluate_latency58_controlled_deployed" + suffix + ".py")
        new = ROOT / ("research/direct/evaluate_latency58_leader_cleanup" + suffix + ".py")
        expected = old.read_text().replace("controlled_deployed", "leader_cleanup").replace(
            "latency58-controlled-deployed", "latency58-leader-cleanup").replace(
            "research.direct.latency58_leader_cleanup_checkpoint", "research.direct.latency58_leader_cleanup_checkpoint_v2")
        require(expected == new.read_text(), "Evaluation changed beyond checkpoint loading and labels")
        evidence.update({str(p): sha(p) for p in (old, new)})
    modules = ("audit_latency58_leader_cleanup_training_match", "run_latency58_leader_cleanup_quality",
               "run_latency58_leader_cleanup_views", "report_latency58_leader_cleanup",
               "queue_latency58_leader_cleanup_quality", "prepare_latency58_leader_cleanup_quality",
    ...
    for label, reference_prefix in REFERENCES.items():
        ...
        for mode in ("full14", "actions60", "probes"):
            ...
            if label == "working_cleanup":
                require(quality_plan["training_plan"] == training["reference_training_plan"],
    ...
    require(states["leader"] == training["parent"]["model_state_sha256"], "Different rollback reference")
    ...
    before = require_space(training, 400_000_000)
    reservation = {"schema": "latency58-leader-cleanup-views-reservation-v1",
    ...
    queue_plan = {"schema": "latency58-leader-cleanup-queued-quality-plan-v1", "quality_prefix": prefix,
    ...
    write(out / "result.json", {"schema": "latency58-leader-cleanup-quality-preparation-v1", "status": "pass",
          ...
          "counted_bytes_before": before, "counted_bytes_after": require_space(training, 400_000_000)})

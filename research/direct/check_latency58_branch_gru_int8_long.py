"""Long carried-state checks of the saved fourteen-projection experiment."""
...
def main():
    ...
    budget = {**baseline, "diagnostic_artifact_allowance_bytes": 10_000_000}
    before = budget_snapshot(budget)
    short_root = PHASE / "branch-gru-int8-screen-002"
    ...
    paths = [Path(__file__).resolve(), baseline_path, short_root / "result.json",
    ...
    reference, independent = make_reference(model, parent, parent_proof["conversion"], graph,
    ...
    out = PHASE / "branch-gru-int8-long-001"

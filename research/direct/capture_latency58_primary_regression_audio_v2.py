"""Capture two new Skelpolu models while reusing exact baseline/reference WAVs.
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-primary-regression-audio-plan-v2"
            and plan["track_index"] == 10 and plan["track_name"] == "Skelpolu - Human Mistakes"
            and [m["id"] for m in plan["models"]] == ["parent", "candidate"]
            and [m["kind"] for m in plan["models"]] == ["accum_candidate", "drum_candidate"]
    ...
    require(out.parent == PHASE and old.parent == PHASE and out.is_dir()
            and all(not (out / name).exists() for name in ("result.json", "index.html", "parent", "candidate")),
    ...
    require(saved["schema"] == "latency58-primary-regression-audio-v1" and saved["status"] == "pass"
    ...
    _, working = load_completed(PHASE / "teacher-half250-full14-001", evidence, canonical_baseline=True)
    prior = saved["models"]["working"]
    require(prior["model_state_sha256"] == plan["working_model_state_sha256"]
            == working["results"][0]["model"]["model_state_sha256"]
            and prior["exact_stored_track_score"] and prior["exact_stored_stream_metadata"]
            and prior["model_and_rng_unchanged"] and prior["track_score"] == working["results"][0]["tracks"][10]
            and prior["stream_metadata"] == working["results"][0]["stream_batches"][10],
            "Reused working estimate is not the exact primary capture")
    baseline = {}
    for path, row in saved["audio_files"].items():
        if row["source_id"] not in ("working", "reference", "mixture"):
            continue
        file = Path(path)
        require(file.is_relative_to(old) and plan["source_bindings"].get(path) == sha(file) == row["sha256"]
        ...
        if row["source_id"] == "working":
            require(row["stem"] in SOURCE_ORDER, "Unexpected working stem")
            baseline[index, row["stem"]] = values.T
        else:
        ...
        inventory[path] = row
        reused.append(path)
    require(len(reused) == 18 and len(baseline) == 8, "Incomplete baseline and reference inventory")
    for i in range(2):
        values = np.stack([baseline[i, stem] for stem in SOURCE_ORDER])
        require(float(np.abs(values.sum(axis=0, dtype=np.float32) - mixtures[i].astype(np.float32)).max()) <= 1e-6,
                "Reused working stems do not reconstruct the mixture")
    del baseline
    models = {"working": {**prior, "reused_from_capture_result": str(old / "result.json")}}
    sources = [{"id": "working", "label": "Working 5.8 ms", "kind": "estimate", "base": "",
                "checkpoint_sha256": prior["checkpoint"]["sha256"], "algorithmic_latency_samples": 256,
                "sample_rate": 44100, "status": "Working baseline"}]
    ...
    with (out / "progress.jsonl").open("x", buffering=1) as progress:
        for item in plan["models"]:
            ...
            if item["kind"] == "accum_candidate":
                from research.direct.evaluate_latency58_sdr_accum import load_evaluation_model
            else:
                from research.direct.evaluate_latency58_sdr_drum_v3 import load_evaluation_model
    ...
    require(len(inventory) == 34 and len(new_files) == 16
    ...
    with (out / "README.md").open("x") as stream:
        stream.write("# Skelpolu primary-excerpt comparison\n\n[Open the player](index.html). "
                     ...
                     "Parent and candidate labels identify the newly captured endpoints.\n\n"
                     "Each new model streams from sample zero and reproduces its stored complete track score "
                     "and stream metadata exactly before WAV export. All 34 files are stereo FLOAT at 44.1 kHz; "
                     "18 are reused and 16 are new. [Capture details](result.json) record hashes and per-excerpt diagnostics.\n\n"
    ...
    write(out / "result.json", {"schema": "latency58-primary-regression-audio-v2", "status": "pass",

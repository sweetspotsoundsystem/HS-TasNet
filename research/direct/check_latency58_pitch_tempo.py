"""Qualify prospective pitch/tempo augmentation on CPU and recorded training crops."""
...
from research.direct.latency58_pitch_tempo_augmentation import (
    ...
    PitchTempoCropDataset, recipe, transform_crop, transform_stem)
...
def synthetic_check():
    ...
    for source in (1, 2, 3):
        for semitones, tempo in ((0, 1.12), (2, 1.12), (-2, .88)):
            ...
            block = output[0, 44100:88200].double().numpy()
            ...
            rows.append({"source": SOURCE_NAMES[source], "semitones": semitones, "tempo": tempo,
                         "expected_hz": expected_hz, "actual_hz": float(actual_hz), "error_cents": error_cents})
    ...
    choices = [recipe(seed=20261028, sample_index=i) for i in range(10000)]
    require(choices == [recipe(seed=20261028, sample_index=i) for i in range(10000)]
...
def main():
    ...
    policy_path = PHASE / "branch-sdr-continuation-001/plan.json"
    policy = read(policy_path)
    counted = require_space(policy, 450_000_000)
    source_paths = [Path(__file__), ROOT / "research/direct/latency58_pitch_tempo_augmentation.py",
    ...
    first, stop, seed = 3964000, 3964064, 20261028
    ...
    snapshot = {"schema": "latency58-pitch-tempo-cpu-inputs-v1", "version": VERSION, "selection": selection,

"""Authenticate paired pitch/tempo raw/EMA endpoints and retain every regression."""
...
def check_journal(plan, rows, training, resource):
    ...
    from research.direct.latency58_pitch_ema_data import batch_recipes
    config = plan["config"]
    require(len(rows) == config["steps"], "Incomplete pitch/EMA journal")
    for index, row in enumerate(rows):
        ...
        require(row["step"] == row["ema_updates"] == index + 1 and row["lr"] == rate
                ...
                and len(row["microbatches"]) == 1, "Update schedule or batch geometry changed")
        ...
        require(row["pitch_tempo_recipes"] == pitch and row["augmentation_recipe"] == remix
                ...
                and terms["augmented_examples"] == 12
                and all(a + b == 16 for a, b in zip(terms["active_windows"], terms["absent_windows"], strict=True)),
                "Pitch/tempo composition, remix recipes or activity counts changed")
        ...
        if index < 4:
            ...
            require(row["before_remix_audio_sha256"] == expected["input_sha256"]
                    ...
                    "Production data prefix differs from CPU qualification")
        ...
        require(all(math.isfinite(actual) and math.isclose(actual, expected, rel_tol=2e-6, abs_tol=2e-6)
                    for actual, expected in identities) and row["loss"] == terms["loss"], "Blended loss identity changed")
        require(all(math.isfinite(row[name]) and row[name] >= 0 for name in OBSERVATION_FIELDS),
                "Invalid resource timing")
...
def main():
    ...
    from research.direct.prepare_latency58_branch_pitch_ema import RECIPE
    ...
    root = PHASE / "branch-pitch-ema-002"
    ...
    require(plan["objective_version"] == VERSION and plan["direct_sdr_weight"] == SDR_WEIGHT == .2
            ...
            and plan["parent_full_sdr_db"] == 4.453207822764789 and plan["parent_training_updates"] == 35250
            and plan["continuation_kind"] == "selected_branch_sdr_pitch_tempo_and_paired_ema"
            and plan["optimizer_initialization"] == "fresh_adam" and not plan["inference_architecture_changed"]
    ...
    for path in (root / "resource-stage/execution.json", root / "production-stage/execution.json",
                 ...
                 PHASE / "branch-pitch-ema-preparation-stage-002/execution.json",
                 PHASE / "branch-pitch-ema-preparation-stage-002/root-execution.json"):
        execution = read(path)
    ...
    require(command["actual_root_session"] == prefix["actual_root_session"] == args.actual_root_session
            ...
            and command["argv"][3] == "research.direct.run_latency58_branch_pitch_ema"
    ...
    review = {"status": "not_selected" if best_kind == "parent" else "selected_for_research",
              ...
              "interpretation": "A single-seed continuation jointly changes pitch/tempo augmentation, seed and crop range. Raw versus EMA compares one shared trajectory. Paired-track intervals exclude training-seed uncertainty and repeated selection. EMA inference has no matching raw Adam optimizer; restart requires its explicit fresh-optimizer policy or the retained raw/EMA generation.",

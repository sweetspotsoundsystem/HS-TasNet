"""One monitored rehearsal or bounded stage of accumulated drum-weighted training."""
...
def main():
    ...
    require(plan["schema"] == "latency58-sdr-drum-accum-training-v1" and stage["schema"] == "latency58-sdr-drum-accum-stage-v1"
            and stage["plan_sha256"] == args.plan_sha256 and 0 <= start < stop <= config["steps"] == 1000
            and stop in ((2,) if plan["resource_only"] else (250, 500, 1000)) and config["batch_size"] == 16 and config["workers"] == 2
    ...
    try:
        ...
        if plan["resource_only"]:
            ...
            helpers.atomic_json(run / "resource.json", {
                "schema": "latency58-sdr-drum-accum-resource-result-v1", "status": "pass",

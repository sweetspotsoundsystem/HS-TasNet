"""Training context that omits discarded warmup decoder frames."""
import torch
from torch.nn import functional as F

from research.direct.latency58_sdr_context import ContextOutput
from research.direct.latency58_long_magnitude import Latency58LongMagnitudeModel, VERSION
...
def render_scored_context(model, mixture, *, warmup_samples, carry_state):
    require(carry_state is True and type(warmup_samples) is int and warmup_samples > 0
            ...
            "Invalid fast training context")
    require(type(model) is Latency58LongMagnitudeModel, "Require the long-feature model")
    ...
    require(raw.shape == deployed.shape == (mixture.shape[0], 4, 2, count)
            and torch.equal(physical, score), "Fast context lost physical alignment")

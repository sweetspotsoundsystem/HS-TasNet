...
def main():
    ...
    for entry in Path("/proc").iterdir():
        ...
        try:
            ...
            continue
        if len(argv) > 3 and argv[:4] == [PYTHON, "-u", "-m", "research.direct.run_latency58_branch_sdr_blend"]:
    ...
    paths = [plan_path, Path(__file__).resolve(), ROOT / "research/direct/report_latency58_branch_sdr_blend.py",
             root / "resource-stage/root-execution.json", root / "resource-stage/execution.json",
             root / "resource-run/result.json", PHASE / "branch-sdr-blend-preparation-stage-001/execution.json",
             PHASE / "branch-sdr-blend-preparation-stage-001/root-execution.json"]

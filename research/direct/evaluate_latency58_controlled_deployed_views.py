"""Score the same fixed vocal controlled_deployeds for a completed controlled_deployed pilot."""
...
def initialize_worker(plan):
    ...
    spec = plan["model"]
    require(spec["kind"] == "controlled_deployed", "Require the new controlled_deployed checkpoint family")
    binding = spec["quality_plan"]
    require(sha(binding["path"]) == binding["sha256"], "Diagnostic quality plan changed")
    from research.direct.evaluate_latency58_controlled_deployed import load_evaluation_model
...
def main():
    ...
    require(sha(args.plan) == args.plan_sha256, "Vocal controlled_deployed plan changed")
    plan = read(args.plan)
    require(plan["schema"] == "latency58-controlled-deployed-views-evaluation-plan-v1" and plan["workers"] == 2

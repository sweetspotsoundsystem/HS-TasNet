"""Authenticated parent and split generations for stronger soft-capped SDR fine-tuning."""
...
def load_parent(plan):
    """Load the frozen, fully evaluated warm500 parent without its retired Adam."""
    import torch
    from research.direct.latency58_context_checkpoint import load_model as load_context

    require(not torch.cuda.is_initialized(), "Load parent before CUDA")
    parent = plan["parent"]
    require(parent["kind"] == "context_candidate", "Require the frozen warm-context parent")
    binding = parent["training_plan"]
    require(sha(binding["path"]) == binding["sha256"]
            and sha(parent["checkpoint"]["path"]) == parent["checkpoint"]["sha256"], "Parent bytes changed")
    model, receipt = load_context(parent["generation"], read(binding["path"]),
                                  expected_plan_sha=binding["sha256"])
    require(receipt["step"] == 500 and receipt["carry_state"]
            and receipt["files"]["model.pt"]["sha256"] == parent["checkpoint"]["sha256"]
            and parent["checkpoint"]["path"] == str(Path(parent["generation"]) / "model.pt"),
    ...
    require(payload["provenance"] == model.provenance == parent["provenance"]
            ...
            "Parent lineage or inference geometry differs")
    return model


def expected_provenance(plan, step, plan_sha):
    from research.direct.latency58_sdr_softcap import VERSION
    parent = plan["parent"]
    old = copy.deepcopy(parent["provenance"])
    value = copy.deepcopy(old)
    value.update(initialization="stronger_soft_capped_sdr_fine_tune", parent_provenance=old,
                 ...
                 sdr_softcap_strong_trial_updates=step, carry_state=True,
                 ...
                 training_objective="raw4_l1_plus_context_teacher_l1_plus_soft_capped_sdr_auxiliary",
                 sdr_softcap_version=VERSION, sdr_softcap_weight=plan["sdr_softcap_weight"],
                 sdr_softcap_error_ratio_floor=plan["sdr_softcap_error_ratio_floor"],
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-sdr-softcap-strong-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-sdr-softcap-strong-inference-v1" and payload["step"] == step
...
def validate_journal(journal, step, plan, helpers):
    ...
    for row in rows:
        require(row["carry_state"] == plan["carry_state"] and row["initial_state_detached"]
                ...
                and row["sdr_softcap_weight"] == plan["sdr_softcap_weight"] == 0.03
                and math.isfinite(row["sdr_softcap_loss"]) and row["sdr_softcap_loss"] >= 0
                and len(row["sdr_softcap_per_stem"]) == len(row["sdr_softcap_active_examples_per_stem"]) == 4
                and all(math.isfinite(v) and v >= 0 for v in row["sdr_softcap_per_stem"])
                and len(row["sdr_softcap_active_windows"]) == plan["config"]["batch_size"]
                and all(len(v) == 4 and all(type(n) is int and 0 <= n <= 2 for n in v)
                        for v in row["sdr_softcap_active_windows"])
                and all(type(n) is int and 0 <= n <= plan["config"]["batch_size"]
                        for n in row["sdr_softcap_active_examples_per_stem"])
                and abs(row["loss"] - row["supervised_loss"] - row["teacher_weight"] * row["teacher_l1"]
                        - row["sdr_softcap_weight"] * row["sdr_softcap_loss"]) < 1e-7
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-sdr-softcap-strong-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-sdr-softcap-strong-generation-v1", "step": step, "plan_sha256": plan_sha,

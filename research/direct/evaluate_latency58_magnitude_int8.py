...
def main():
    ...
    counted_before = require_space(source, 415_000_000)
    screen_path = PHASE / "m4-magnitude-int8-screen-001/result.json"
    long_path = PHASE / "m4-magnitude-int8-long-001/result.json"
    ...
    reference_path = PHASE / "full-magnitude-001/full14/result.json"
    ...
    require(reference["model"]["model_state_sha256_after"] == fingerprint
            and reference["checkpoint"] == parent and reference["aggregate"]["full_sdr_db"] == 4.114394535058717,
    ...
    paths = [source_path, screen_path, screen_path.parent / "plan.json", long_path, long_path.parent / "plan.json",
    ...
    out = PHASE / "m4-magnitude-int8-full14-001"
    ...
    candidate = {"model": {"label": "Full magnitude U8U8 with precise quantizer inputs", "runtime_artifact": checkpoint,

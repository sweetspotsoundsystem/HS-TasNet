...
SCHEMA = "latency58-fixed-recorded-training-gradient-probe-v2"
...
def prerequisites():
    ...
    paths.extend([ROOT / "research/direct/run_latency58_grouped_gradient_probe.py",
                  PHASE / "grouped-gradient-probe-training-001-prepare-stage/command.json",
                  PHASE / "grouped-gradient-probe-training-001-prepare-stage/preparation-execution.json",
                  PHASE / "grouped-gradient-probe-training-001-prepare-stage/preparation-console.log"])
    for path in paths:
        digest = sha(path)
...
def audio_bindings(crops, source):
    ...
    # The inherited training plan binds the sealed corpus manifest, but its
    # historical source_bindings list is not an exhaustive corpus audio list.
    # make_dataset has authenticated that manifest and selected its 301 train
    # tracks. Authenticate every actual read against those frozen file records.
    sealed = {str(file.path): file.sha256
              for tracks in crops.original.tracks_by_root.values()
              for track in tracks for file in track.files.values()}
    require(files and all(sealed.get(p) == digest for p, digest in files.items()),
            "Decoded files differ from the sealed selected-training manifest")
    require(all(p not in source["source_bindings"] or source["source_bindings"][p] == digest
                for p, digest in files.items()), "Decoded training file conflicts with an inherited binding")
...
def prepare(out):
    ...
    plan = {"schema": SCHEMA, "source_bindings": bindings, "policy": policy(),
            "prior_preparation_failure": "v1 incorrectly required every training file in the inherited partial binding list; no plan or gradients produced",
            "training_audio_provenance": "Sealed manifest selected-training file records, inherited bindings where present, and direct full-file SHA-256 verification",
...
def main():
    ...
    parser.add_argument("--output-prefix", default="grouped-gradient-probe-training-002")

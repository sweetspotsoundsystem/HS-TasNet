...
ORDER = ("working", "drum500", "original", "focused", "focused_mixer")
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-vocal-focus-listening-plan-v1"
            and plan["step"] == 250 and 3 <= len(plan["models"]) <= 5
            and [row["id"] for row in plan["models"]] == list(ORDER[:len(plan["models"])])
    ...
    for row in plan["models"]:
        ...
        if row["id"] in ORDER[2:]:
            ...
            require(summary["schema"] == "latency58-vocal-focus-quality-summary-v1" and summary["status"] == "pass"
                    and summary["source_bindings_unchanged"] and summary["arm"] == row["id"]
    ...
    replacements = {
        "Actions · training comparison": "Vocal-cleanliness pilot · Actions",
        "Compare the C91 reference, native C191 and two completed training trials on Actions – One Minute Smile, 60–75 seconds. No new candidate has been retained.":
            "Compare the working model, drum checkpoint and completed vocal-cleanliness pilots on Actions – One Minute Smile, 60–75 seconds. Use the Vocals and Other stems to check vocal separation, and the other stems to assess instrument fidelity.",
    ...
    with (out / "index.html").open("x") as stream:
        stream.write(html)
    write(out / "result.json", {"schema": "latency58-vocal-focus-listening-preparation-v1", "status": "pass",
    ...
    with (out / "README.md").open("x") as stream:
        stream.write("# Vocal-cleanliness pilot comparison\n\n[Open the player](index.html). "

...
def checker_budget_from_snapshot(budget, observed):
    """Map the existing 440 MB child guard into the standing save reservation.

    The enclosing budget continues to hold the entire save reserve before and
    after each CPU stage. Existing checker/evaluator guards remain unchanged.
    """
    require(budget["authorized_cap_bytes"] == 90_000_000_000
            ...
            "Require the complete cap and standing reservations")
    require(set(budget["counted_roots"]) == set(observed["counted_roots"])
            and budget["external_git_common_directory"] == observed["external_git_common_directory"]
            and budget["external_git_common_directory"] not in budget["counted_roots"]
            and all(budget[key] == observed[key] for key in
                ("authorized_cap_bytes", "live_training_save_reservation_bytes",
                 "other_outside_allowance_bytes", "diagnostic_artifact_allowance_bytes")),
            "Storage observation does not describe this plan")
    shared = 440_000_000
    standing_remainder = budget["live_training_save_reservation_bytes"] - shared
    outside = (observed["external_git_common_bytes"] + budget["other_outside_allowance_bytes"]
               + standing_remainder + budget["diagnostic_artifact_allowance_bytes"])
    counted = sum(observed["counted_roots"].values())
    require(counted + outside + shared == observed["conservative_total_with_reservations"],
            "Reservation mapping omitted or counted an artifact twice")
    require(observed["conservative_total_with_reservations"] + 50_000_000 < budget["authorized_cap_bytes"],
            "Reserve the full paired diagnostic before preparing it")
    return ({"counted_roots": list(budget["counted_roots"]),
             "stop_counted_bytes": budget["authorized_cap_bytes"] - outside,
             "outside_roots_reservation_bytes": outside},
            {"version": "paired-source-views-shared-standing-save-reservation-v1",
             "standing_training_save_reservation_bytes": budget["live_training_save_reservation_bytes"],
             "unchanged_child_training_guard_bytes": shared,
             "standing_save_reserve_in_outside_bytes": standing_remainder,
             "all_existing_artifact_roots_counted": True,
             "enclosing_full_budget_checked_before_and_after_each_stage": True,
             "paired_diagnostic_maximum_additional_bytes": 50_000_000})
...
def prepare(quality_root, output_prefix):
    ...
    control = PHASE / "paired-vocal-serial-launcher-check-001"
    ...
    require(check["status"] == "pass" and check["source_bindings_unchanged"]
            ...
            and check["reservation_mapping_qualified"]
            and len(check["budget_negative_cases"]) == 6
            and all(case["rejected"] for case in check["budget_negative_cases"])
            and check["inference_and_metric_function_asts_unchanged"]
    ...
    paths.append(ROOT / "research/direct/check_latency58_paired_vocal_serial_launcher.py")
    ...
    budget_path = Path(quality_root).resolve() / "plan.json"
    require("storage_budget" in records["source"], "Require the completed training plan's explicit storage budget")
    paths.append(budget_path)
    merge_bindings(bindings, {str(p): sha(p) for p in paths})
    verify_inputs({"source_bindings": bindings})
    budget = records["source"]["storage_budget"]
    before = budget_snapshot(budget)
    legacy_budget, reservation_mapping = checker_budget_from_snapshot(budget, before)
    plan = {"schema": SCHEMA, "output_directory": str(out), "source_bindings": bindings,
        ...
        "storage_budget_source": binding(budget_path), "reservation_mapping": reservation_mapping,

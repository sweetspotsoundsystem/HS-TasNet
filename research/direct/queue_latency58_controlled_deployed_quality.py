...
from research.direct.report_latency58_controlled_deployed import REFERENCES, load_views, reference_views
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-controlled-deployed-queued-quality-plan-v1"
    ...
    require(training["schema"] == "latency58-controlled-deployed-training-v1" and not training["resource_only"]
            and training["teacher_mode"] == "ordinary_only",
            "Only a production pilot may queue quality")
    prefix = plan["quality_prefix"]
    require(prefix == "controlled-deployed-half-250" and training["additional_loss_weight"] == .5, "Different quality prefix")
    waited = 0.0
    ...
    if not ((stage / "execution.json").is_file() and (stage / "audit-execution.json").is_file()):
        ...
        try:
            ...
            require(int(stat[19]) == root_identity["start_ticks"] and command == root_identity["argv"]
                    and command[command.index("-m") + 1] == "research.direct.run_latency58_controlled_deployed_stage"
    ...
    # Bind terminal evidence only after the exact production supervisor exits.
    match_dir = Path(plan["training_match_directory"])
    require(match_dir.parent == PHASE and not match_dir.exists(), "Preserve previous training match")
    match_sources = {**plan["source_bindings"], str(args.plan): args.plan_sha256}
    models = {}
    for side, side_stage, side_training in (
            ("reference", Path(plan["control_stage_directory"]), training["matched_ordinary_training_plan"]),
            ("candidate", stage, training_binding)):
        recipe = read(side_training["path"])
        match_sources.update(recipe["source_bindings"])
        models[side] = {"training_plan": side_training}
        for key, filename in (("audit", "audit.json"), ("audit_execution", "audit-execution.json"),
                              ("execution", "execution.json")):
            path = side_stage / filename
            models[side][key] = {"path": str(path), "sha256": sha(path)}
        match_sources.update({item["path"]: item["sha256"] for item in models[side].values()})
        generation = Path(read(side_stage / "audit.json")["generation"])
        paths = [Path(read(side_stage / "execution.json")["monitor_result"]),
                 Path(recipe["run_dir"]) / "status.json",
                 *(generation / name for name in ("receipt.json", "model.pt", "rng.pt", "metrics.jsonl"))]
        match_sources.update({str(path): sha(path) for path in paths})
    match_plan = {"schema": "latency58-controlled-deployed-training-match-plan-v1", "models": models,
                  "output_directory": str(match_dir), "source_bindings": match_sources,
                  "counted_roots": training["counted_roots"], "stop_counted_bytes": training["stop_counted_bytes"]}
    verify_inputs(match_plan)
    match_dir.mkdir()
    write(match_dir / "plan.json", match_plan)
    execute([PYTHON, "-u", "-m", "research.direct.audit_latency58_controlled_deployed_training_match",
             "--plan", str(match_dir / "plan.json"), "--plan-sha256", sha(match_dir / "plan.json")],
            match_dir, "evaluation", 240, match_sources, {"plan_sha256": sha(match_dir / "plan.json")})
    match_binding = {key: {"path": str(match_dir / filename), "sha256": sha(match_dir / filename)}
                     for key, filename in (("plan", "plan.json"), ("result", "result.json"),
                                           ("execution", "execution.json"))}
    load_completed_match(match_binding, {})
    commands = [
        [PYTHON, "-u", "-m", "research.direct.run_latency58_controlled_deployed_quality",
         "--training-plan", training_binding["path"], "--training-plan-sha256", training_binding["sha256"],
         "--stage-directory", str(stage), "--match-directory", str(match_dir), "--output-prefix", prefix, "--modes", "full14", "probes", "actions60"],
        [PYTHON, "-u", "-m", "research.direct.run_latency58_controlled_deployed_views", "--prefix", prefix,
    ...
    load_completed_match(match_binding, evidence)
    ...
    summary_plan = {"schema": "latency58-controlled-deployed-summary-plan-v1", "candidate_prefix": prefix,
                    "output_directory": str(summary_dir), "source_bindings": evidence, "training_match": match_binding}
    summary_dir.mkdir()
    write(summary_dir / "plan.json", summary_plan)
    execute([PYTHON, "-u", "-m", "research.direct.report_latency58_controlled_deployed",
    ...
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-controlled-deployed-queued-quality-result-v1", "status": "pass",
        "plan_sha256": args.plan_sha256, "source_bindings": plan["source_bindings"], "source_bindings_unchanged": True,
        "training_match": match_binding, "summary": {"path": str(summary_dir / "result.json"), "sha256": sha(summary_dir / "result.json")},

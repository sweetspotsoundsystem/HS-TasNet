"""Run the bounded quarter-controlled trial after a healthy serialized GPU stage."""
...
from research.direct.latency58_quarter_controlled_checkpoint import load_parent, require_space, validate_recipe
...
def stage(plan_path, previous_execution, out):
    ...
    stage_data = {"schema": "latency58-quarter-controlled-stage-v1", "plan_sha256": sha(plan_path),
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_quarter_controlled",
...
def main():
    ...
    resource_execution = stage(args.resource_plan, args.previous_execution, PHASE / "quarter-controlled-resource-001")
    ...
    production.update(resource_only=False, run_dir=str(PHASE / "quarter-controlled-b16-micro4-lr1e5-250"),
    ...
    execution = stage(path, resource_execution, PHASE / "quarter-controlled-to-000250-001")
    ...
    execute([PYTHON, "-u", "-m", "research.direct.audit_latency58_quarter_controlled", "--plan", str(path),

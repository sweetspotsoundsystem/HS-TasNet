...
def bound(plan, item):
    require(plan["source_bindings"].get(item["path"]) == item["sha256"] == sha(item["path"]),
            "Unbound cleanup-lr3e6 prerequisite")
    return read(item["path"])


def validate_recipe(plan):
    require(plan["schema"] == "latency58-cleanup-lr3e6-training-v1"
    ...
    config = {**reference["config"], "lr": 3e-6, "min_lr": 3e-7, "seed": 20260922, "data_start": 976000}
    ...
    require(decision["status"] == "train_lr3e6_independent_of_deployment"
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="selected_cleanup_matched_lower_lr",
                 training_objective="ordinary_only_teacher_plus_controlled_deployed_truth",
                 cleanup_lr3e6_trial_updates=step,
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-cleanup-lr3e6-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-cleanup-lr3e6-inference-v1" and payload["step"] == step
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-cleanup-lr3e6-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-cleanup-lr3e6-generation-v1", "step": step, "plan_sha256": plan_sha,

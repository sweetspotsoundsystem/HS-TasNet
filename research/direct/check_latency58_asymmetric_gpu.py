"""One discarded, monitored GPU resource update; no training run or checkpoint.
...
def main():
    ...
    require(plan["schema"] == "latency58-asymmetric-gpu-resource-v1" and plan["samples"] in (4096, 88064),
    ...
    require(parent_step == 2000 and model_state_sha256(parent) == plan["parent_model_state_sha256"],
            "Resource parent differs from the frozen endpoint")
    model = Latency58AsymmetricModel.from_hann_model(parent).eval()
    proof = json.loads(Path(plan["functional_result"]).read_text())
    require(proof["status"] == "pass" and model_state_sha256(model) == proof["model_state_sha256"]
            and model_state_sha256(parent) == proof["parent_model_state_sha256"],
    ...
    expected_gradient = torch.sign(raw.detach() - targets) / targets.numel()
    progress("backward")
    ...
    result = {"status": "pass", "family": model.architecture_metadata["version"],
              "parent_checkpoint": plan["parent_checkpoint"], "initial_model_state_sha256": proof["model_state_sha256"],
              "objective": "raw4_l1", "samples": samples, "batch_size": 4, "data_hops": samples // HOP,

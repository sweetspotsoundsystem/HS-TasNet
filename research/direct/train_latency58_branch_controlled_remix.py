"""Continue trained branch memories with twelve ordinary and four source-controlled views."""
...
def main():
    ...
    require_space(plan, 440_000_000)
    run.mkdir()
    ...
    from research.direct.latency58_branch_controlled_remix import AUGMENTATION, augment, recipe
    if plan["direct_sdr_weight"] == .2:
        from research.direct.latency58_branch_sdr_blend import objective, VERSION, SDR_WEIGHT
    else:
        from research.direct.latency58_attention_sdr_blend import objective, VERSION, SDR_WEIGHT
    ...
    from research.direct.latency58_branch_memory_checkpoint import audit_live, save_generation
    ...
    require(VERSION == plan["objective_version"] and plan["direct_sdr_weight"] == SDR_WEIGHT
            and SDR_WEIGHT in (.1, .2)
    ...
    require(plan["parent_kind"] == "saved_trained_branch_memory"
            ...
            "Saved branch-memory parent or continuation initialization changed")
    model = parent
    ...
    with (run / "metrics.jsonl").open("x", buffering=1) as journal:
        for mixture_cpu, truth_cpu in loader:
            ...
            require(recipe_row["view_codes"] == [2] * 12 + [0] * 2 + [1] * 2
                    and recipe_row["source_keep"] == [[True] * 4] * 12
                        + [[True, True, False, True]] * 2 + [[False, False, True, False]] * 2
                    and torch.equal(mixture_cpu[12:], truth_cpu[12:].sum(1))
                    and torch.count_nonzero(truth_cpu[12:14, 2]) == 0
                    and torch.count_nonzero(truth_cpu[14:, [0, 1, 3]]) == 0,
                    "Controlled views or full-context physical mixture changed")
            ...
            for offset in range(0, config["batch_size"], config["microbatch_size"]):
                ...
                require(terms.absent_window_counts.min().item() >= 2,
                        "Every stem must have at least two silent target windows")
            ...
            row = {"step": step, "lr": lr, "first_sample_index": config["data_start"] + (step - 1) * config["batch_size"],
                   ...
                   "branch_memory_gradient_norms": refinement_gradients}
            if step <= 2:
                prefix.append(row)

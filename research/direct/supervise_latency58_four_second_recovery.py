...
def main():
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.resume_latency58_four_second_shared", "--launch-plan", str(args.plan)]
    sources = {**plan["source_bindings"], str(args.plan): args.plan_sha256, str(Path(__file__).resolve()): sha(__file__)}
    began = time.monotonic()
    with (root / "recovery-controller-console-001.log").open("x") as output:
        child = subprocess.Popen(argv, cwd=ROOT, stdout=output, stderr=subprocess.STDOUT)
        write(root / "recovery-root-launch-001.json", {"supervisor_pid": os.getpid(), "controller_pid": child.pid,

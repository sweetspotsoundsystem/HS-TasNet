...
ORDER = ("working", "focused_control", "ordinary_only")
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-counterfactual-listening-plan-v1"
            and plan["step"] == 250 and len(plan["models"]) == 3
    ...
    for row in plan["models"]:
        ...
        if row["id"] in ORDER[1:]:
            ...
            for item in (row["summary"], row["summary_execution"], row["training_plan"]):
                require(bindings.get(item["path"]) == item["sha256"] == sha(item["path"]), "Unbound pilot review input")
            require(summary["schema"] == ("latency58-vocal-focus-quality-summary-v1" if row["id"] == "focused_control"
                                           else "latency58-counterfactual-quality-summary-v1") and summary["status"] == "pass"
                    and summary["source_bindings_unchanged"] and summary["arm"] == "focused"
                    and summary["model_state_sha256"] == fingerprint and summary["step"] == 250
                    and summary["training_plan"] == quality["training_plan"] == row["training_plan"]
                    and execution["actual_exit_code"] == 0 and not execution["timed_out"]
                    and execution["source_bindings_unchanged"] and execution["plan_sha256"] == summary["plan_sha256"],
                    "Pilot lacks its complete quality summary")
    ...
    require(match["model_states"] == {"reference": models["focused_control"]["model_state_sha256"],
                                       "candidate": models["ordinary_only"]["model_state_sha256"]}
    ...
    replacements = {
        "Actions · training comparison": "Selective teacher · Actions",
        "Compare the C91 reference, native C191 and two completed training trials on Actions – One Minute Smile, 60–75 seconds. No new candidate has been retained.":
            "Compare the working model, focused control and selective-teacher trial on Actions – One Minute Smile, 60–75 seconds. Listen for vocals in Other, unwanted sound in Vocals and changes to instruments.",
    ...
    with (out / "index.html").open("x") as stream:
        stream.write(html)
    write(out / "result.json", {"schema": "latency58-counterfactual-listening-preparation-v1", "status": "pass",
        ...
        "all_decoded_track_and_aggregate_scores_exact": True, "audio_files_copied": 0, "new_inference": False,
    ...
    with (out / "README.md").open("x") as stream:
        stream.write("# Selective teacher comparison\n\n[Open the player](index.html). "

"""Supervise one GPU child with bounded finalization after its planned final update.
...
def main():
    ...
    try:
        ...
        identities_after = {}
        for name, path in (("source", Path(__file__)), ("launch_spec", args.launch_spec), ("event_transport", TRANSPORT_SOURCE)):
            try:
                identities_after[name] = sha(path)
            except OSError as error:
                identities_after[name] = repr(error)
        identities_unchanged = identities_after == {"source": source_before, "launch_spec": args.launch_spec_sha256, "event_transport": TRANSPORT_SHA256}

param(
    ...
    [ValidateSet("skelpolu", "actions")][string]$Panel = "skelpolu",
    [int]$DebugPort = 9234
)
...
(async () => {
  ...
  require(estimates.length === 5 && data.sources.length === estimates.length + 2 && stem.options.length === 4,
    'Unexpected player inventory');
  require(JSON.stringify(estimates.map(x => x.id)) === JSON.stringify(['working', 'previous', 'parent', 'short', 'long']),
    'Unexpected comparison sources');
  const expectedName = panel === 'skelpolu' ? 'Skelpolu - Human Mistakes' : 'Actions - One Minute Smile';
  const intervals = panel === 'skelpolu' ? [[1323000, 1984500], [3307500, 3969000]] : [[2646000, 3307500]];
  ...
  const beforeHistory = audio.currentTime;
  button('short').click();
  await until(() => ready() && !audio.seeking && !audio.paused, 'history switch resume');
  const afterHistory = audio.currentTime;
  require(beforeHistory >= 4.24 && Math.abs(afterHistory - beforeHistory) < 0.25
    && button('short').getAttribute('aria-pressed') === 'true', 'History switch lost position or source');
  ...
  button('long').click(); button('short').click(); button('parent').click(); button('working').click();
  ...
  document.body.dispatchEvent(new KeyboardEvent('keydown', {key:String(data.sources.findIndex(x => x.id === 'long') + 1), bubbles:true}));
  await until(() => ready() && button('long').getAttribute('aria-pressed') === 'true', 'keyboard source');
  ...
  return {status:'pass', panel, files, source_switch:{before:beforeSource, after:afterSource},
    history_switch:{before:beforeHistory, after:afterHistory},
...
'@
    $expression = $expression.Replace('__HISTORY_PANEL__', $Panel)

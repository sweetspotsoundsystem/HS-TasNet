"""Copy completed sixteen-projection evidence into the isolated Mac test checkout."""
...
def main():
    out = PHASE / "m4-inference-diagnostics-plugin-001"
    stage = PHASE / "branch-output-int8-package-review-001"
    ...
    evidence = {}
    for key, name in (("quality", "branch-output-int8-quality-001"),
                      ("review", "branch-output-int8-review-001"),
                      ("native", "branch-output-int8-native-tests-003")):
        paths = [PHASE / name / leaf for leaf in ("plan.json", "result.json", "execution.json")]
        ...
        evidence[key] = {"directory": str(PHASE / name), "result_sha256": sha(paths[1]),
    ...
    keys = ("status", "results", "parent_fourteen_projection_comparison",
            "parent_fourteen_projection_all_track_stem_cells", "previous_deployment_comparison", "source_fp32_comparison",
    ...
    public = {"schema": "stemgenrt-sixteen-projection-deployment-quality-v1",
    ...
    plan = {"schema": "latency58-sixteen-projection-package-plan-v1", "source_bindings": bindings,
            "destination": str(destination), "destination_previous_sha256": sha(destination),
    ...
    verify_inputs(plan)
    result = {"schema": "latency58-sixteen-projection-package-v1", "status": "pass",
        ...
        "release_replaced": False, "gpu_used": False}

"""Verify long trajectories after replacing all ten quadrature integer weight matrices."""
import hashlib
import json
import os
...
def main():
    ...
    from research.direct.latency58_quadrature_all_s8 import build, make_reference
    ...
    screen_execution_path = PHASE / "quadrature-all-s8-screen-stage-001/execution.json"
    ...
    graph, proof = build(native, original, original_proof)
    reference, _ = make_reference(native, original, graph, original_proof)
    ...
    out = PHASE / "quadrature-all-s8-long-001"
    require(not out.exists(), "Preserve long diagnostics")
    out.mkdir()
    paths = [ROOT / "research/direct/latency58_temporal_pending_space.py", original_path, original_proof_path, source_path, Path(__file__).resolve(), ROOT / "research/direct/latency58_quadrature_int8.py",
             screen_path, screen_path.parent / "plan.json", screen_execution_path, Path(checkpoint["path"]), music_path]
    paths.extend(ROOT / "research/direct" / name for name in ("latency58_quadrature_all_s8.py", "latency58_quadrature_magint8.py"))
    ...
    write(out / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
          ...
          "tolerances": TOLERANCES, "pending_training_reservation_bytes": 380_000_000,
    ...
    require(all(row["existing_strict_tolerances_passed"] for row in rows), "Long streaming reference parity failed")

"""One matched rehearsal or 250-update selective-distillation pilot."""
...
def main():
    ...
    from research.direct.latency58_counterfactual_checkpoint import validate_recipe
    validate_recipe(plan)
    require(stage["schema"] == "latency58-counterfactual-stage-v1"
    ...
    from research.direct.latency58_counterfactual_journal import loss_evidence, rng_state_sha256
    ...
    try:
        for mixtures_cpu, targets_cpu in loader:
            ...
            for micro_index in range(plan["accumulation_steps"]):
                ...
                terms = counterfactual_teacher_objective(output.raw, output.deployed,
                    targets[..., plan["warmup_samples"]:], teacher_targets, flags,
                    view_codes=tuple(codes), mode=plan["teacher_mode"])
                loss = terms.total
                ...
                microbatches.append({
                    ...
                    **loss_evidence(output.raw, output.deployed, targets[..., plan["warmup_samples"]:],
                                    teacher_targets, terms, tuple(codes)),
                ...
                del mixture, targets, flags, output, terms, loss, teacher_targets, rng
        ...
        if plan["resource_only"]:
            ...
            if plan["teacher_mode"] == "all_views":
                ...
                old_resource = read(control["full_resource"]["path"])
                require(fingerprint == old_resource["final_model_state_sha256"],
                        "Existing-loss replay did not reproduce the original two-update model")
            receipt = {"model_state_sha256": fingerprint}
            helpers.atomic_json(run / "resource.json", {
                "schema": "latency58-counterfactual-resource-result-v1", "status": "pass",
                ...
                "existing_control_prefix_exact": plan["teacher_mode"] == "all_views",

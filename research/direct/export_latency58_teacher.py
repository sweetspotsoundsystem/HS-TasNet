...
def metadata(plan, model, step, fingerprint, shapes):
    ...
    return {
        ...
        "hs_tasnet.training_objective": model.provenance["training_objective"],
        "hs_tasnet.teacher_weight": str(model.provenance["teacher_weight"]),
        "hs_tasnet.teacher_model_state_sha256": model.provenance["teacher_model_state_sha256"],
        "hs_tasnet.teacher_used_in_inference": "false",
        "hs_tasnet.checkpoint_loader_sha256": sha(ROOT / "research/direct/latency58_teacher_checkpoint_v2.py"),
    }
...
def main():
    ...
    require(plan["schema"] == "latency58-teacher-onnx-export-plan-v1" and Path.cwd() == ROOT,
            "Export schema or working directory differs")
    bindings = plan["source_bindings"]
    for relative in ("research/direct/export_latency58_teacher.py", "research/direct/latency58_asymmetric.py",
                     "research/direct/latency58_asymmetric_checkpoint.py", "research/direct/latency58_teacher_checkpoint_v2.py",
                     "research/direct/train_latency58.py", "research/direct/latency58_evaluate.py",
    ...
    correction = json.loads(Path(plan["canonicalization_receipt"]["path"]).read_text())
    require(bindings.get(plan["canonicalization_receipt"]["path"]) == plan["canonicalization_receipt"]["sha256"]
            and correction["status"] == "pass" and correction["every_model_tensor_bit_exact"]
            and correction["source_bindings_unchanged"] and correction["output"] == plan["checkpoint"]
            and correction["model_state_sha256"] == plan["model_state_sha256"], "Canonical lineage proof differs")
    for name in ("quality_panel", "quality_panel_execution", "canonicalization_execution"):
        require(bindings.get(plan[name]["path"]) == plan[name]["sha256"], "Missing completed evidence binding: " + name)
    panel = json.loads(Path(plan["quality_panel"]["path"]).read_text())
    quality_execution = json.loads(Path(plan["quality_panel_execution"]["path"]).read_text())
    correction_execution = json.loads(Path(plan["canonicalization_execution"]["path"]).read_text())
    identity = panel["results"][0]["model"]
    require(len(panel["results"][0]["tracks"]) == 14 and panel["inputs_unchanged"]
            and panel["results"][0]["checkpoint"] == correction["input"]
            and identity["model_state_sha256_before"] == identity["model_state_sha256_after"]
            == correction["model_state_sha256"]
            and all(value["actual_exit_code"] == 0 and not value["timed_out"]
                    and value["source_bindings_unchanged"] for value in (quality_execution, correction_execution)),
            "Full-panel weights or observed quality/canonicalization execution differs")
    ...
    from research.direct.latency58_teacher_checkpoint_v2 import load_model_state, make_model
    ...
    report = {"schema": "latency58-teacher-onnx-verification-v1", "plan_sha256": args.plan_sha256,

...
training reconstruction loss and SDR, verifies the four upstream states are unchanged,
recomputes the two synthesis tails for each variant, and never writes a model
or evaluates the held-out validation panel.
"""
...
UPSTREAM_STATES = ("audio_history", "fusion_hidden", "attention_keys", "attention_values")
SYNTHESIS_STATES = ("spectral_numerator_tail", "waveform_tail")
...
def main():
    ...
    failed_root = PHASE / "attention-parent-ablation-stage-001"
    failed_execution = read(failed_root / "execution.json")
    require(failed_execution["actual_exit_code"] == 1 and not failed_execution["timed_out"]
            and failed_execution["source_bindings_unchanged"], "Preserve the failed six-state premise")
    ...
    out = PHASE / "attention-parent-ablation-002"
    ...
    paths.extend((failed_root / "plan.json", failed_root / "execution.json",
                  failed_root / "evaluation-console.log",
                  ROOT / "research/direct/diagnose_latency58_attention_ablation.py"))
    ...
    plan = {"source_bindings": bindings, "training_steps": steps, "examples_per_batch": 16,
            ...
            "supersedes_failed_diagnostic": str(failed_root),
            "correction": "Attention changes the two synthesis overlap buffers. Recompute each variant's full warmup and compare only the four upstream states for equality; record the synthesis-tail differences."}
    ...
    with torch.no_grad(), (out / "progress.jsonl").open("x", buffering=1) as progress:
        for logged in selected:
            ...
            baseline_state, baseline_output, baseline_warm = None, None, None
            variants = {}
            for variant in VARIANTS:
                if variant != "saved_attention":
                    model.attention = ablated_attention(variant)
                try:
                    # Attention is upstream of both synthesis tails. Recompute
                    # warmup with this variant so its first scored hop has the
                    # same semantics as its subsequent streaming output.
                    state = model.warm_state(mixture[..., :88064]).detached()
                    if baseline_warm is None:
                        baseline_warm = {name: value.clone() for name, value in state._asdict().items()}
                    require(all(torch.equal(baseline_warm[name], getattr(state, name)) for name in UPSTREAM_STATES),
                            "An ablation changed an upstream warmup state")
                    initial_state_digest = state_sha256(state._asdict())
                    rendered = model.render(F.pad(physical, (0, 128)), state)
                    require(state_sha256(state._asdict()) == initial_state_digest,
                            "Rendering mutated a variant's warmup state")
                finally:
                    if variant != "saved_attention":
                        del model.attention
                ...
                if baseline_state is None:
                    baseline_state = {name: value.clone() for name, value in rendered.state._asdict().items()}
                    baseline_output = deployed.clone()
                require(all(torch.equal(baseline_state[name], getattr(rendered.state, name)) for name in UPSTREAM_STATES)
                        and all(bool(torch.isfinite(value).all()) for value in rendered.state),
                        "An ablation changed an upstream state or produced a nonfinite state")
                ...
                row = {"reconstruction_loss": float(terms.total), "waveform_loss": float(terms.waveform),
                       ...
                       "all_four_upstream_states_bit_exact_after_warmup_and_scored_audio": True,
                       "warmup_synthesis_tail_max_abs_change": {
                           name: float((getattr(state, name) - baseline_warm[name]).abs().max()) for name in SYNTHESIS_STATES},
                       "final_synthesis_tail_max_abs_change": {
                           name: float((getattr(rendered.state, name) - baseline_state[name]).abs().max()) for name in SYNTHESIS_STATES},
            ...
            del inputs, mixture, truth, changed, factors, state, physical, targets, baseline_state, baseline_output, baseline_warm
    ...
    write(out / "result.json", {"status": "pass", "source_bindings": bindings, "source_bindings_unchanged": True,
          ...
          "training_examples_covered_by_batch_hashes": 64, "all_four_upstream_states_unchanged_across_variants": True,
          "synthesis_tails_recomputed_for_each_variant": True,

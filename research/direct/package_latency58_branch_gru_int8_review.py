"""Copy completed fourteen-projection evidence into the isolated Mac test checkout."""
...
def main():
    out = PHASE / "branch-gru-int8-plugin-001"
    stage = PHASE / "branch-gru-int8-package-review-001"
    ...
    evidence = {}
    for key, name in (("quality", "branch-gru-int8-quality-001"),
                      ("review", "branch-gru-int8-review-001"),
                      ("native", "branch-gru-int8-native-tests-002")):
    ...
    require(quality["track_count"] == 14 and quality["excerpt_count"] == 28
            ...
            and len(native["passed"]) == 162 and native["total"] == 170,
            "Incomplete panel or correctness suite")
    keys = ("status", "results", "previous_deployment_comparison", "source_fp32_comparison",
    ...
    public = {"schema": "stemgenrt-fourteen-projection-deployment-quality-v1",
    ...
    plan = {"schema": "latency58-fourteen-projection-package-plan-v1", "source_bindings": bindings,
    ...
    verify_inputs(plan)
    result = {"schema": "latency58-fourteen-projection-package-v1", "status": "pass",

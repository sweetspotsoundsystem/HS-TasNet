param(
    ...
    [ValidateSet("leader_cleanup")][string]$LastArm = "leader_cleanup",
    [int]$DebugPort = 9234
)
...
(async () => {
  const panel = 'actions';
  const lastArm = '__VOCAL_LAST_ARM__';
  const allIds = ['working', 'leader', 'working_cleanup', 'leader_cleanup'];
  const expectedIds = allIds.slice(0, allIds.indexOf(lastArm) + 1);
  const priorId = expectedIds[expectedIds.length - 2];
  ...
  await until(() => document.getElementById('sources')?.children.length >= 6, 'page controls');
  ...
  require(estimates.length === expectedIds.length && data.sources.length === estimates.length + 2 && stem.options.length === 4,
    'Unexpected player inventory');
  require(JSON.stringify(estimates.map(x => x.id)) === JSON.stringify(expectedIds),
    'Unexpected comparison sources');
  const expectedName = 'Actions - One Minute Smile';
  const intervals = [[2646000, 3307500]];
  require(data.tracks.length === 1 && data.tracks[0].name === expectedName
    && excerpt.options.length === intervals.length && data.tracks[0].clips.length === intervals.length,
    'Unexpected primary excerpts');
  require(JSON.stringify(data.tracks[0].clips.map(x => [x.reference_start, x.reference_end]))
    === JSON.stringify(intervals), 'Primary intervals differ');
  const expectedFiles = (estimates.length * 4 + 5) * intervals.length;
  require(data.human_listening_verdict === null, 'Player contains a listening verdict');
  const files = [];
  for (const clipIndex of intervals.keys()) {
    audio.pause();
    excerpt.value = String(clipIndex);
    ...
    for (const source of data.sources) {
      for (const name of source.kind === 'mixture' ? ['bass'] : ['drums', 'bass', 'vocals', 'other']) {
        ...
        audio.pause();
        files.push({clip_index:clipIndex, source:source.id, stem:source.kind === 'mixture' ? null : name,
  ...
  const beforeArm = audio.currentTime;
  button(priorId).click();
  await until(() => ready() && !audio.seeking && !audio.paused, 'arm switch resume');
  const afterArm = audio.currentTime;
  require(beforeArm >= 4.24 && Math.abs(afterArm - beforeArm) < 0.25
    && button(priorId).getAttribute('aria-pressed') === 'true', 'Arm switch lost position or source');
  ...
  button(lastArm).click(); button(priorId).click(); button('leader').click(); button('working').click();
  ...
  document.body.dispatchEvent(new KeyboardEvent('keydown', {key:String(data.sources.findIndex(x => x.id === lastArm) + 1), bubbles:true}));
  await until(() => ready() && button(lastArm).getAttribute('aria-pressed') === 'true', 'keyboard source');
  ...
  return {status:'pass', panel, last_arm:lastArm, files, source_switch:{before:beforeSource, after:afterSource},
    arm_switch:{before:beforeArm, after:afterArm},
    stem_switch:{before:beforeStem, after:afterStem}, rapid_source_switch:{before:beforeRapid, after:afterRapid},
    strict_nonzero_seek_verified:true, expected_physical_intervals_verified:true,
    verified_clip_count:intervals.length,
...
'@
    $expression = $expression.Replace('__VOCAL_LAST_ARM__', $LastArm)
    ...
    $report = @{schema="latency58-leader-cleanup-listening-browser-check-v1"; status="pass";
    ...
    $report = @{schema="latency58-leader-cleanup-listening-browser-check-v1"; status="failed";

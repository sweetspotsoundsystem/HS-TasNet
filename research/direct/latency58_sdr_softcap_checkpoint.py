"""Authenticated parent and split generations for soft-capped SDR fine-tuning."""
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="soft_capped_sdr_fine_tune", parent_provenance=old,
                 ...
                 sdr_softcap_trial_updates=step, carry_state=True,
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-sdr-softcap-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-sdr-softcap-inference-v1" and payload["step"] == step
...
def validate_journal(journal, step, plan, helpers):
    ...
    for row in rows:
        require(row["carry_state"] == plan["carry_state"] and row["initial_state_detached"]
                ...
                and row["sdr_softcap_weight"] == plan["sdr_softcap_weight"] == 0.003
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-sdr-softcap-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-sdr-softcap-generation-v1", "step": step, "plan_sha256": plan_sha,

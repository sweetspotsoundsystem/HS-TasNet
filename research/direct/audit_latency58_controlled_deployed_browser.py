...
from research.direct.prepare_latency58_controlled_deployed_listening import ORDER
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-controlled-deployed-browser-inventory-plan-v1"
    ...
    expected_schema = "latency58-controlled-deployed-listening-preparation-v1"
    require(saved["status"] == browser["status"] == "pass"
            and saved["schema"] == expected_schema
            and browser["schema"] == "latency58-controlled-deployed-listening-browser-check-v1"
    ...
    verify_inputs(plan)
    write(Path(plan["output"]), {"schema": "latency58-controlled-deployed-browser-inventory-v1",

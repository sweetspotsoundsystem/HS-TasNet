// Native CPU1 timing for the nine-I/O, eight-state branch ABI. No DAW queue.
...
constexpr size_t kTensorCount = 9;
constexpr std::array<const char*, kTensorCount> inputs{
...
constexpr std::array<const char*, kTensorCount> outputs{
    "separated_chunk", "next_audio_history", "next_fusion_hidden",
    "next_spectral_numerator_tail", "next_waveform_tail", "next_attention_keys",
    "next_attention_values", "next_spec_memory_hidden", "next_waveform_memory_hidden"};
const std::array<std::vector<int64_t>, kTensorCount> inputShapes{
    std::vector<int64_t>{1, 2, 128}, {1, 2, 896}, {2, 1, 1000},
    {1, 4, 2, 128}, {1, 4, 2, 128}, {1, 31, 64}, {1, 31, 128},
    {1, 1, 500}, {1, 1, 500}};
const std::array<std::vector<int64_t>, kTensorCount> outputShapes{
...
int main(int argc, char** argv) {
  try {
    ...
    require(std::string(OrtGetApiBase()->GetVersionString()) == "1.26.0",
            "Require the shipping ORT 1.26.0 runtime");
    disableDenormals();
    ...
    require(session.GetInputCount() == kTensorCount && session.GetOutputCount() == kTensorCount,
            "Expected nine inputs and nine outputs");
    auto memory = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
    std::array<std::vector<float>, kTensorCount> in, out;
    ...
    for (int hop = 0; hop < warmup + measured; ++hop) {
      ...
      session.Run(runOptions, inputs.data(), inputValues.data(), kTensorCount,
                  outputs.data(), outputValues.data(), kTensorCount);
      const auto runEnd = Clock::now();
      for (size_t channel = 0; channel < 2; ++channel) {
        for (size_t sample = 0; sample < 896; ++sample) {
          const float expected = sample < 768
              ? in[1][channel * 896 + sample + 128]
              : in[0][channel * 128 + sample - 768];
          require(out[1][channel * 896 + sample] == expected,
                  "Exact physical history shift changed");
        }
      }
      for (size_t i = 0; i < kTensorCount; ++i) {
        require(outputValues[i].GetTensorMutableData<float>() == out[i].data(),
                "ORT replaced a preallocated public output");

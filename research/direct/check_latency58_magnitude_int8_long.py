...
def main():
    ...
    require(screen["strict_parity_passed"] and screen["float_export_parity_passed"]
            and screen["source_bindings_unchanged"], "Short independent screen incomplete")
    ...
    out = PHASE / "m4-magnitude-int8-long-001"
    require(not out.exists(), "Preserve long diagnostics")
    out.mkdir()
    paths = [source_path, Path(__file__).resolve(), ROOT / "research/direct/latency58_magnitude_int8.py",
             screen_path, screen_path.parent / "plan.json", Path(checkpoint["path"]), music_path]

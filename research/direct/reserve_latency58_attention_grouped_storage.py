"""Reserve the grouped trial after review, preserving models, audio and selected optimizers.

Prefer remaining generated plugin objects when sufficient. If necessary, retire
only the authenticated historical OLA step-1225 intermediate optimizer, then the
...
RESERVE, MANIFEST_MARGIN = 400_000_000, 5_000_000
OLA_STEP = 1225
OLA_RESUME_SHA = "1adbed8948e5fa707d38ae0f50cece20d4e36b92fabaa4deaa066fb2641619df"
...
def main():
    ...
    previous_intent_path = PHASE / "attention-plugin-build-object-retirement-001/intent.json"
    ...
    paths.extend(ROOT / "research/direct" / name for name in (
        "reclaim_latency58_attention_build_objects.py", "prepare_latency58_attention_grouped.py",
    ...
    out = PHASE / "attention-grouped-storage-001"
    ...
    write(out / "intent.json", {"targets": chosen, "preserved_bindings": protected,
          ...
          "limitation": "If selected, the historical step-1225 intermediate optimizer/RNG snapshot will no longer support exact restart; its separate inference model, journals, receipt and step-2000 terminal resume remain unchanged. Generated objects are rebuildable from retained sources."})
    verify_inputs({"source_bindings": protected})
    for item in chosen:
        path = Path(item["path"])
        ...
        if item["kind"] == "generated_object":
            ...
            require(old is not None and path == old / "checkpoints/step-001225/resume.pt", "Invalid historical target")
        path.unlink()
    verify_inputs({"source_bindings": protected})

...
def bound(plan, item):
    require(plan["source_bindings"].get(item["path"]) == item["sha256"] == sha(item["path"]),
            "Unbound cleanup-followup prerequisite")
    return read(item["path"])


def validate_recipe(plan):
    require(plan["schema"] == "latency58-cleanup-followup-training-v1"
            and type(plan["resource_only"]) is bool
            and plan["comparison_variable"] == "fresh_adam_followup_with_new_batches"
            and plan["optimizer_initialization"] == "fresh_adam"
            and plan["checkpoint_and_quality_reserve_bytes"] == 400_000_000,
            "Different bounded followup recipe")
    reference = bound(plan, plan["reference_training_plan"])
    from research.direct.latency58_cleanup_successor_checkpoint_v2 import validate_recipe as validate_parent
    ...
    require(all(plan[k] == reference[k] for k in RECIPE_KEYS if k not in excluded),
            "Followup changed the fixed model, objective, teacher or resource geometry")
    ...
    require(plan["config"] == config and plan["stop_counted_bytes"] == 79_500_000_000
            ...
            and plan["parent"]["kind"] == "cleanup_successor"
            and plan["initialized_model_state_sha256"] == plan["parent"]["model_state_sha256"],
            "Followup initialization, schedule or allowance differs")
    decision = bound(plan, plan["preparation_decision"])
    require(decision["status"] == "train_followup_independent_of_deployment"
            ...
            and decision["parent_selected_by"] == "prospective_trajectory_endpoint"
            and decision["quality_promotion_claimed"] is False,
            "Different prospective followup decision")
    return reference


def load_parent(plan):
    import torch
    from research.direct.latency58_cleanup_successor_checkpoint_v2 import load_model
    validate_recipe(plan)
    require(not torch.cuda.is_initialized(), "Load the audited parent on CPU before CUDA")
    parent = plan["parent"]
    training = bound(plan, parent["training_plan"])
    require(parent["training_plan"] == plan["reference_training_plan"], "Parent recipe differs")
    audit = bound(plan, parent["audit"])
    audit_execution = bound(plan, parent["audit_execution"])
    execution = bound(plan, parent["execution"])
    monitor = bound(plan, parent["monitor"])
    require(audit["status"] == "pass" and audit["step"] == 250
            and audit["plan_sha256"] == audit_execution["plan_sha256"] == execution["plan_sha256"]
            == parent["training_plan"]["sha256"]
            and audit["source_bindings_unchanged"] and audit_execution["source_bindings_unchanged"]
            and execution["source_bindings_unchanged"] and not audit_execution["timed_out"]
            and audit_execution["actual_exit_code"] == execution["actual_exit_code"] == monitor["child_exit_code"] == 0
            and monitor["status"] == monitor["supervisor_health"] == "pass"
            and monitor["post_exit_quiet_completed"] and audit["generation"] == parent["generation"]
            and audit["model_state_sha256"] == parent["model_state_sha256"],
            "Parent is not an audited complete healthy training endpoint")
    for name in ("model.pt", "receipt.json", "rng.pt", "metrics.jsonl"):
        path = str(Path(parent["generation"]) / name)
        require(plan["source_bindings"].get(path) == sha(path), "Unbound parent generation")
    model, receipt = load_model(parent["generation"], training, expected_plan_sha=parent["training_plan"]["sha256"])
    require(state_sha256(model.state_dict()) == parent["model_state_sha256"] == receipt["model_state_sha256"]
            ...
            and not torch.cuda.is_initialized(), "Parent identity or geometry differs")
    return model
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="cleanup_trajectory_fresh_adam_followup",
                 training_objective="ordinary_only_teacher_plus_controlled_deployed_truth",
                 cleanup_followup_trial_updates=step,
                 ...
                 comparison_variable="fresh_adam_followup_with_new_batches",
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-cleanup-followup-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-cleanup-followup-inference-v1" and payload["step"] == step
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-cleanup-followup-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-cleanup-followup-generation-v1", "step": step, "plan_sha256": plan_sha,

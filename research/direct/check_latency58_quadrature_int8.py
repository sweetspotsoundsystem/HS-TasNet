"""Check precise integer execution of the saved 4.2277 quadrature pilot."""
...
def main():
    ...
    from research.direct.latency58_quadrature_int8 import build, make_reference
    ...
    out = PHASE / "m4-quadrature-int8-screen-001"
    ...
    write(out / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
          ...
          "magnitude_projection_kept_floating": True, "phase_factors_kept_fp32": True,
          "native_host_qualified": False, "timing_performed": False})
    import shutil
    for name in ("latency58_quadrature_int8.py", Path(__file__).name):
    ...
    from research.direct.latency58_quadrature_onnx import run_float_case as _run_case
    ...
    measurements = []  # Execution fidelity only; no timing during training.
    ...
    result = {"status": "diagnostic_complete", "strict_parity_passed": all(row["passed"] for row in cases),
          "cases": cases, "timings": measurements, "timing_performed": False, "source_bindings_unchanged": True, "graph_sha256": candidate_sha,

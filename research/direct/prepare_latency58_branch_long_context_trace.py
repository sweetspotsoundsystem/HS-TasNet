"""Diagnose repeated event-query timeouts without changing the qualified learning recipe."""
import json
...
def main():
    ...
    old = PHASE / "branch-long-context-004"
    ...
    qualification = PHASE / "branch-long-context-telemetry-trace-001"
    ...
    require(qualified["status"] == "pass" and qualified["source_bindings_unchanged"]
            ...
            and qualified["only_five_stderr_lines_added"] and qualified["original_and_traced_event_payloads_match"]
            and qualified["sentinel_and_coverage_and_fault_detection_unchanged"]
            and qualified["query_deadline_seconds"] == 10, "Event-query instrumentation is not qualified")
    recovery = PHASE / "branch-long-context-telemetry-recovery-005"
    ...
    require(watchdog.name == "watch_gpu_process_trace.py"
            and watchdog.with_name("watch_gpu_process.py") == Path(original["watchdog_source"]),
            "Trace monitor lineage differs")
    paths = [Path(__file__).resolve(), original_path, old / "resource-stage/root-execution.json",
             ...
             ROOT / "research/direct/observe_latency58_branch_long_context_trace_prefix.py",
             ROOT / "research/direct/report_latency58_branch_long_context_trace.py"]
    ...
    for name in ("execution.json", "root-execution.json"):
        path = PHASE / "branch-long-context-preparation-stage-004" / name
        previous = read(path)
        ...
        paths.append(path)
    out = PHASE / "branch-long-context-005"
    ...
    plan = {**original, "name": out.name, "output_directory": str(out), "retry_of": str(original_path),
            "retry_reason": "Repeated Windows event-query timeouts; add qualified stderr phase markers to locate a further timeout",
    ...
    write(out / "preparation.json", {"status": "pass", "original_plan_sha256": sha(original_path),
          ...
          "supervision_logic_unchanged_except_stderr_timing_markers": True, "all_supervision_limits_unchanged": True,
          ...
          "limitation": "Tracing identifies completed PowerShell phases if a timeout recurs. It does not fix or establish the cause of the timeout, prove future host stability, or measure model quality."})
    ...
    print(json.dumps({"status": "pass", "plan": str(out / "plan.json"), "plan_sha256": sha(out / "plan.json"),
                      "learning_recipe_unchanged": True, "event_query_phase_tracing": True}), flush=True)

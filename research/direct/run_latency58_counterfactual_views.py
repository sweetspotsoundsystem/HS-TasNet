...
def main():
    ...
    require(quality_plan["schema"] == "latency58-counterfactual-parallel-music-plan-v1"
    ...
    require(sha(training_binding["path"]) == training_binding["sha256"]
            and training["schema"] == "latency58-counterfactual-training-v1" and not training["resource_only"],
    ...
    for path in (template_path, template_path.parent / "execution.json", args.reservation,
                 Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_counterfactual_views.py",
                 ROOT / "research/direct/evaluate_latency58_counterfactual.py",
                 ROOT / "research/direct/latency58_vocal_views.py"):
        bindings[str(path)] = sha(path)
    plan = copy.deepcopy(template)
    plan.update(schema="latency58-counterfactual-views-evaluation-plan-v1", output_directory=str(out),
                model={"kind": "counterfactual", "label": args.prefix, "arm": training["arm"], "teacher_mode": training["teacher_mode"],
    ...
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_counterfactual_views",

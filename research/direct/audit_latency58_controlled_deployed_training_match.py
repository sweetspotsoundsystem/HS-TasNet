"""Match every deployed-truth training example and RNG stream to the ordinary-only control."""
...
from research.direct.latency58_controlled_deployed_checkpoint import SHARED_CONTROL_KEYS, IDENTITY_KEYS, require_space
...
def load_completed_match(binding, evidence):
    ...
    require(plan["schema"] == "latency58-controlled-deployed-training-match-plan-v1"
            and result["schema"] == "latency58-controlled-deployed-training-match-v1" and result["status"] == "pass"
            ...
            and result["additional_loss_weight"] == .5
...
def main():
    ...
    import torch
    from research.direct import latency58_counterfactual_checkpoint as old
    from research.direct import latency58_controlled_deployed_checkpoint as new
    ...
    require(candidate["matched_ordinary_training_plan"] == plan["models"]["reference"]["training_plan"]
            and all(reference[k] == candidate[k] for k in (*SHARED_CONTROL_KEYS, "teacher_mode", "counterfactual_version", "matched_control_training_plan")), "Different matched recipes")
    ...
    require(receipts["reference"]["model_state_sha256"] != receipts["candidate"]["model_state_sha256"],
            "The added controlled supervision produced the same final model")
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-controlled-deployed-training-match-v1", "status": "pass",
          ...
          "teacher_mode": "ordinary_only", "additional_loss_weight": .5, "quality_selected": False, "training_updates_executed": 0,

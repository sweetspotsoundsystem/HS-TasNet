"""Run one monitored counterfactual rehearsal or audited 250-update trial."""
...
def main():
    ...
    parser.add_argument("--stop-step", type=int, choices=(2, 250), required=True)
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-counterfactual-training-v1", "Unknown training plan")
    from research.direct.latency58_counterfactual_checkpoint import require_space, validate_recipe
    validate_recipe(plan)
    require_space(plan, 10_000_000 if plan["resource_only"] else 380_000_000)
    ...
    require(decision["schema"] == "latency58-vocal-focus-three-arm-decision-v1"
            and decision["status"] == "prepare_selective_distillation"
            and decision["all_three_quality_reviews_complete"]
            and decision["all_original_pilots_closed_at_updates"] == 250
            and decision["next_trial"]["parent_model_state_sha256"] == plan["parent"]["model_state_sha256"]
            and decision["next_trial"]["config"] == plan["config"]
            and decision["next_trial"]["new_production_updates_maximum"] == 250
            and not decision["quality_selected"] and not decision["goal_complete"],
            "The new loss lacks the completed three-arm research decision")
    if plan["resource_only"]:
        ...
        require(resource["schema"] == "latency58-counterfactual-resource-result-v1" and resource["status"] == "pass"
        ...
        require(pairing["schema"] == "latency58-counterfactual-resource-pairing-v1" and pairing["status"] == "pass"
                ...
                and pairing["resource_results"][plan["teacher_mode"]] == plan["full_resource"]
    ...
    stage = {"schema": "latency58-counterfactual-stage-v1", "plan_sha256": args.plan_sha256,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_counterfactual", "--plan", str(args.plan.resolve()),
    ...
    if plan["resource_only"]:
        ...
        write(out / "resource-qualification.json", {
            "schema": "latency58-counterfactual-resource-qualification-v1", "status": "pass",
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.audit_latency58_counterfactual", "--plan", str(args.plan.resolve()),

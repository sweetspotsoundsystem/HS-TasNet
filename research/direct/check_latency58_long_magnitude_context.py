...
def compare(model, mixture, warmup):
    ...
    from research.direct.latency58_long_magnitude_context import render_scored_context as fast
    ...
    require(len(gradients) == 23 and all(row["reference_norm"] > 0 for row in gradients.values()), "Unexercised neural tensor")
    ...
    return {"status": "pass", "microbatch": mixture.shape[0], "warmup_samples": warmup,
            ...
            "all_23_gradient_errors": gradients, "loss": first["loss"], "warmup_input_gradient_zero": True,

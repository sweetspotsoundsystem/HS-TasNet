"""Score two fixed vocal counterfactuals on all original development excerpts."""
...
def initialize_worker(plan):
    ...
    if spec["kind"] == "working":
        ...
        require(spec["kind"] in ("drum_accum", "history"), "Unsupported diagnostic model")
        binding = spec["quality_plan"]
        require(sha(binding["path"]) == binding["sha256"], "Diagnostic quality plan changed")
        module = importlib.import_module("research.direct.evaluate_latency58_sdr_" + spec["kind"])
        model, _ = module.load_evaluation_model(read(binding["path"]))
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-vocal-views-evaluation-plan-v1" and plan["workers"] == 2
    ...
    require(allocation["new_artifact_allowance_bytes"] == 10_000_000
            and allocation["history_reserve_bytes"] == 350_000_000
            ...
            "Diagnostic lacks a reservation separate from history completion")
    out = Path(plan["output_directory"])
    require(out.is_dir() and not (out / "result.json").exists(), "Preserve diagnostic result")
    # History can legitimately consume its reserved checkpoint space while a
    # read-only diagnostic is running. Do not require that same space twice.

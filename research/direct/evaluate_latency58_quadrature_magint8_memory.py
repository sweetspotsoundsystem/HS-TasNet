"""Score an in-memory ten-projection quadrature graph on unchanged full14 music.
...
PRECISION = "Ten U8U8 projections including magnitude; FP64 quantizer ancestors; FP32 phase factors, decoding and public state"
...
def main():
    ...
    counted_before = require_cpu_space(source, 5_000_000)
    screen_path = PHASE / "m4-quadrature-magint8-screen-001/result.json"
    long_path = PHASE / "m4-quadrature-magint8-long-001/result.json"
    ...
    require(screen["advance_to_long_check"] and screen["exploratory_speed_gate_passed"]
            and screen["strict_parity_passed"] and screen["float_export_parity_passed"]
    ...
    paths = [ROOT / "research/direct/latency58_pending_training_space.py", *prerequisite_executions,
             ...
             ROOT / "research/direct/evaluate_latency58_int8.py", ROOT / "research/direct/latency58_quadrature_magint8.py",
             module, *sorted((module.parent / "capi").glob("*.so*"))]
    bindings = {**long_plan["source_bindings"], **{str(p): sha(p) for p in paths}}
    out = PHASE / "m4-quadrature-magint8-full14-memory-001"
    ...
    write(out / "in-memory-graph.json", {"source_bindings": bindings, "graph_sha256": screen["graph_sha256"],
          ...
          "counted_bytes_before": counted_before, "reserved_pending_training_bytes": 380000000,
    ...
    candidate = {"model": {"label": "In-memory quadrature with ten U8U8 projections", "runtime_artifact": checkpoint,

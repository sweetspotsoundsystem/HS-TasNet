"""Execute a frozen fusion_refinement plan through monitored rehearsal, training and full14 scoring."""
...
def launch(plan_path, previous_execution_path, *, resource):
    plan = read(plan_path)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-fusion-refinement-training-plan-v1", "Wrong fusion-refinement training plan")
    ...
    if not resource:
        ...
        result = read(path)
        parity_path = root / "resource-run/fusion_refinement-context-gpu-parity.json"
        ...
        require(result["status"] == "pass" and result["updates"] == 2 and not result["checkpoint_written"]
                ...
                and len(parity["all_26_gradients"]) == 26,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_fusion_refinement", "--plan", str(plan_path),
...
def audit_and_score(plan_path):
    import torch
    from research.direct.latency58_fusion_refinement_checkpoint import audit_saved
    ...
    require(training["status"] == "pass" and training["checkpoint_written"]
            ...
            "Require completed fusion-refinement training before scoring")
    ...
    require(audit["step"] == training["updates"]
            ...
            "Saved fusion_refinement model differs from the training endpoint")
    verify_inputs(plan)
    ...
    quality_plan = {"schema": "latency58-fusion-refinement-full14-plan-v1", "label": plan["name"], "checkpoint": checkpoint,
    ...
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_fusion_refinement", "--plan", str(quality / "plan.json"),

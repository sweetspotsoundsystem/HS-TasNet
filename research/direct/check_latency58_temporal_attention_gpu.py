"""Resource-stage check of temporal_attention warmup at the full training batch size."""
import torch

from research.direct.check_latency58_temporal_attention import compare_context
...
def check_gpu(model):
    ...
    require(type(model) is Latency58TemporalAttentionModel and parameter.device.type == "cuda"
            and model.training and model.training_precision == "bf16"
            and all(p.requires_grad and p.grad is None for p in model.parameters()),
            "Require a fresh BF16 temporal-attention training model before its first optimizer update")
    fingerprint = state_sha256(model.state_dict())
    saved_expansion = model.temporal_output.weight.detach().clone()
    generator = torch.Generator(device=parameter.device).manual_seed(202609147)
    ...
    try:
        with torch.random.fork_rng(devices=[parameter.device.index]), torch.no_grad():
            model.temporal_output.weight.copy_(.002 * torch.randn(
                model.temporal_output.weight.shape, device=parameter.device, generator=generator))
        ...
        with torch.no_grad():
            model.temporal_output.weight.copy_(saved_expansion)

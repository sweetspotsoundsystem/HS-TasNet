"""Screen higher-precision arithmetic around integer projections at unchanged tolerances."""
...
def main():
    ...
    from research.direct.latency58_int8_precise_float import rewrite, make_reference
    ...
    original_graph = onnx.load(binding["path"], load_external_data=False)
    import copy
    graph = rewrite(copy.deepcopy(original_graph))
    ...
    reference, proof = make_reference(native, original_graph)
    ...
    out = PHASE / "m4-int8-precise-float-002"
    require(not out.exists(), "Preserve earlier screens")
    out.mkdir()
    paths = [source_path, Path(__file__).resolve(), ROOT / "research/direct/latency58_int8_precise_float.py",
    ...
    write(out / "plan.json", {"source_bindings": bindings, "parent_graph": binding, "graph_sha256": candidate_sha,
          "graph_saved": False, "independent_integer_weight_proofs": proof,
          "arithmetic": "FP64 surrounding arithmetic; original U8U8 projection blocks with FP32 dequantization/bias; FP32 public tensors",
    ...
    for cycle, order in enumerate((("dynamic", "precise"), ("precise", "dynamic"),
                                   ("precise", "dynamic"), ("dynamic", "precise"))):
        for name in order:

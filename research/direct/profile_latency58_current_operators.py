"""Bounded CPU profile of the current fused-QKV graph during monitored training."""
...
OUT = ROOT / "research/m4_followup_20260916/current-operator-profile-001"
PHASE = ROOT / "research/direct/runs/latency58"
GRAPH = ROOT / "research/m4_followup_20260916/attention-qkv-int8-screen-001/model.onnx"
GRAPH_SHA = "08424ca91feae8d4746442a35ebf70489dea70ea6e81401b39483cf02d497748"
...
def main():
    ...
    require(ort.__version__ == "1.26.0", "Use the released runtime version")
    began = time.monotonic()
    before = snapshot()
    require(not OUT.exists(), "Preserve earlier profile evidence")
    reference_plan = GRAPH.parent / "plan.json"
    contract = json.loads(reference_plan.read_text())["interface"]
    data = GRAPH.read_bytes()
    require(hashlib.sha256(data).hexdigest() == GRAPH_SHA and len(data) == 37_532_574,
            "Current graph identity changed")
    ...
    write(OUT / "plan.json", {"graph": str(GRAPH), "graph_sha256": GRAPH_SHA,
        ...
        "plain_calls": 256, "plain_warmup": 64, "profile_calls": 128, "profile_warmup": 32,
    ...
    audio = np.random.default_rng(20260916).normal(0, .03, (256, 1, 2, 128)).astype(np.float32)
    ...
    plain = session(False)
    durations, plain_closure = execute(plain, 256)
    del plain
    traced = session(True)
    _, traced_closure = execute(traced, 128)
    ...
    require(len(runs) == 128, "Incomplete profile calls")
    start, stop = runs[32]["ts"], runs[-1]["ts"] + runs[-1]["dur"]
    ...
    require(total > 0 and all(len(v) == 96 for v in groups.values()), "Incomplete kernel coverage")
    operators = sorted([{"name": k, **attributes[k], "calls": len(v),
        ...
        key=lambda v: v["median_us"], reverse=True)
    median_total = sum(v["median_us"] for v in operators)
    for row in operators:
        row["percent_summed_kernel_medians"] = 100 * row["median_us"] / median_total
    by_type = defaultdict(float)
    for row in operators:
        by_type[row["op_name"]] += row["percent_kernel_time"]
    require(all(sha(p) == h for p, h in bindings.items()), "Profile inputs changed")
    timings = durations[64:]
    result = {"status": "pass", "graph_sha256": GRAPH_SHA,
        ...
        "kleidiai_disabled": True, "operator_ranking": "median of 96 warmed calls; means retained",
        ...
        "limitations": "Synthetic-noise operator diagnosis on Linux during GPU training. Tracing and Python output allocation add cost; per-operator medians are a ranking diagnostic, not an additive end-to-end latency measure. No callback, M4 timing, numerical-reference or quality acceptance is established."}

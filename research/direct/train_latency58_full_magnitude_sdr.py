"""Train all magnitude-model parameters with direct deployed-output SDR loss."""
...
def main():
    ...
    from research.direct.latency58_wave_spectral import AUGMENTATION, augment
    from research.direct.latency58_full_magnitude_checkpoint import load_model, audit_live, save_generation
    from research.direct.latency58_fast_context import render_scored_context
    ...
    model.provenance = {**model.provenance, "direct_sdr_current_stage_corpus": "301 recorded MUSDB18-HQ and MoisesDB training tracks",
                        "direct_sdr_current_stage_augmentation": AUGMENTATION,
    ...
    if stage["resource_only"]:
        from research.direct.check_latency58_fast_context16 import check_gpu
        write(run / "fast-context-gpu-parity.json", check_gpu(model))
    ...
    with (run / "metrics.jsonl").open("x", buffering=1) as journal:
        for mixture_cpu, truth_cpu in loader:
            ...
            for offset in range(0, config["batch_size"], config["microbatch_size"]):
                ...
                require(all(p.grad is not None for p in model.parameters()), "An inherited parameter lacks a gradient")
                micro_rows.append({"loss": float(terms.total.detach()), "negative_sdr_db": float(terms.negative_sdr_db.detach()),
                    ...
                    "relative_l1_anchor": float(terms.relative_l1_anchor.detach()), "augmented_examples": int(changed.sum()),

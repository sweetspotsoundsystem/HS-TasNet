"""Screen three small FP32 products against the current fused-QKV graph."""
...
from research.direct.latency58_four_second_storage import ROOT, POLICY, snapshot
from research.direct.run_latency58_quality import require, read, sha, write

PHASE = ROOT / "research/direct/runs/latency58"
OUT = ROOT / "research/m4_followup_20260916/small-projections-fp32-screen-001"
...
def main():
    ...
    import torch
    from research.direct.latency58_small_projections_fp32 import build, PARENT_SHA, digest
    from research.direct.latency58_attention_qkv_int8_reference import make_reference as make_qkv_reference
    from research.direct.latency58_small_projections_fp32_reference import SmallProjectionFP32Reference
    ...
    sixteen_root = PHASE / "branch-output-int8-screen-003"
    sixteen_result = read(sixteen_root / "result.json")
    sixteen_path = PHASE / "m4-inference-diagnostics-plugin-001/model/model.onnx"
    short_root = ROOT / "research/m4_followup_20260916/attention-qkv-int8-screen-001"
    parent_result = read(short_root / "result.json")
    parent_execution_path = short_root.parent / "attention-qkv-int8-execution.json"
    parent_execution = read(parent_execution_path)
    ...
    profile_path = profile_root / "current-operator-profile-001/result.json"
    profile_execution_path = profile_root / "current-operator-profile-001/execution.json"
    ...
    fourteen_path = Path(sixteen_result["parent_graph"]["path"])
    ...
    source_paths = [Path(__file__).resolve(), POLICY,
        ROOT / "research/direct/latency58_four_second_storage.py",
        ROOT / "research/direct/latency58_small_projections_fp32.py",
        ROOT / "research/direct/latency58_small_projections_fp32_reference.py",
        ROOT / "research/direct/latency58_attention_qkv_int8.py",
        ROOT / "research/direct/latency58_attention_qkv_fusion.py",
        ROOT / "research/direct/latency58_attention_qkv_int8_reference.py",
        ROOT / "research/direct/latency58_branch_output_int8_reference.py",
        baseline_path, short_root / "result.json", parent_execution_path, short_root / "graph.json",
        sixteen_path, sixteen_root / "result.json", sixteen_root / "graph.json",
    ...
    write(OUT / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
        ...
        "purpose": "Three measured FP32 projections; preserve all weights and surrounding FP64 arithmetic",
        ...
        "local_speed_gate": {"minimum_median_block_p50_reduction": 0.03,
            "minimum_faster_pairs": 3, "pairs": 4, "order": "ABBAABBA", "warmup_hops": 64, "measured_hops": 192},
    ...
    parent_reference, independent = make_qkv_reference(model, onnx.load(ten_path), ten_proof["conversion"],
        onnx.load(fourteen_path), read(fourteen_proof_path)["conversion"], onnx.load(sixteen_path),
        read(sixteen_root / "graph.json")["conversion"], parent, read(short_root / "graph.json")["conversion"])
    for row in conversion["projections"]:
        weight = getattr(model, row["module"]).weight.detach().cpu().float().numpy().T
        require(digest(weight) == row["weight_sha256"], "FP32 projection differs from native training weights")
    reference = SmallProjectionFP32Reference(parent_reference.model).eval().requires_grad_(False)
    write(OUT / "independent-reference.json", {"integer_projections": independent,
        "fp32_products": conversion["projections"], "source": "Native saved training tensors, not ORT outputs"})
    reports = []
    ...
    if passed:
        ...
        signal = np.random.default_rng(20260916).normal(0, .03, (256, 1, 2, 128)).astype(np.float32)
        for repeat in range(2):
            for label in ("parent", "candidate", "candidate", "parent"):
                ...
                for audio in signal:
                    ...
                    states = values[1:]
                measured = durations[64:]
                timings.append({"repeat": repeat, "label": label, "warmup_hops": 64, "measured_hops": 192,
    ...
    write(OUT / "result.json", {"status": "pass" if passed else "fail", "strict_parity_passed": passed,
        ...
        "limitations": "Independent three-small-FP32-product synthetic parity only. Rounding changes model arithmetic; trained weight bytes are unchanged. Long-stream numerical checks, full14 and source-view quality, native timing and physical M4 acceptance remain required. Linux Python timings include allocation and concurrent GPU training load."})
    print(json.dumps({"status": "pass" if passed else "fail", "local_speed_gate": gate}), flush=True)
    require(passed, "Small FP32 candidate failed independent numerical checks")

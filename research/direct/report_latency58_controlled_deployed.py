"""Compare deployed-truth quality with its ordinary-only control and retained baselines."""
...
REFERENCES = {"working": "teacher-half250", "drum500": "sdr-drum-accum-500",
              ...
              "ordinary_control": "counterfactual-teacher-ordinary-only-250"}


def reference_views(label):
    if label == "ordinary_control":
        return PHASE / "counterfactual-teacher-ordinary-only-250-views-001"
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-controlled-deployed-summary-plan-v1", "Unexpected summary plan")
    ...
    require(reference_states["ordinary_control"] == match["model_states"]["reference"],
    ...
    write(out / "result.json", {
        "schema": "latency58-controlled-deployed-quality-summary-v1", "status": "pass", "plan_sha256": args.plan_sha256,

...
from research.direct.latency58_cleanup_lr3e6_checkpoint import load_parent, require_space, validate_recipe
...
def stage(plan_path, previous_execution, out):
    ...
    stage_data = {"schema": "latency58-cleanup-lr3e6-stage-v1", "plan_sha256": sha(plan_path),
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_cleanup_lr3e6",
...
def main():
    ...
    resource_execution = stage(args.resource_plan, args.previous_execution, PHASE / "cleanup-lr3e6-resource-001")
    ...
    production.update(resource_only=False, run_dir=str(PHASE / "cleanup-lr3e6-b16-micro4-lr3e6-250"),
    ...
    execution = stage(path, resource_execution, PHASE / "cleanup-lr3e6-to-000250-001")
    ...
    execute([PYTHON, "-u", "-m", "research.direct.audit_latency58_cleanup_lr3e6", "--plan", str(path),

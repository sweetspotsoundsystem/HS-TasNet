"""Train all 30 temporal_attention tensors from an authenticated fusion-refinement parent."""
...
def main():
    ...
    require(state_sha256(parent.state_dict()) == plan["parent_model_state_sha256"], "Fusion-refinement parent changed")
    ...
    with (run / "metrics.jsonl").open("x", buffering=1) as journal:
        for mixture_cpu, truth_cpu in loader:
            ...
            if step < 2:
                require(refinement_gradients["temporal_output.weight"] > 0
                        and all((refinement_gradients[name] == 0 if step == 0 else refinement_gradients[name] > 0)
                                for name in ADAPTERS if name != "temporal_output.weight"),
                        "Attention projections did not activate from zero output")

"""Read every required watchdog health field, omitting supplemental counters.

Power and utilization are explicitly not queried. Their CSV slots remain as
annotated strings because the existing parser retains those fields verbatim.
...
OPTIONAL = frozenset()
SAMPLING_POLICY = 'latency58-nvml-required-health-getters-v1'
OMITTED_SUPPLEMENTAL = ('nvmlDeviceGetPowerUsage', 'nvmlDeviceGetPowerManagementLimit',
...
class ReadOnlyNvml:
    ...
    def sample(self):
        ...
        values.update({'power.draw': 'not_queried', 'power.limit': 'not_queried',
                       'utilization.gpu': 'not_queried'})
        ...
        return {'values': values, 'memory_bytes': memory_bytes, 'api_timings': list(self.calls),
                ...
                'sampling_policy': SAMPLING_POLICY, 'omitted_supplemental_getters': list(OMITTED_SUPPLEMENTAL)}

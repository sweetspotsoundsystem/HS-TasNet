"""Authenticated parent and split generations for weak log-relative fine-tuning."""
...
def expected_provenance(plan, step, plan_sha):
    from research.direct.latency58_log_relative import VERSION
    parent = plan["parent"]
    old = copy.deepcopy(parent["provenance"])
    value = copy.deepcopy(old)
    value.update(initialization="weak_log_relative_error_fine_tune", parent_provenance=old,
                 ...
                 log_relative_trial_updates=step, carry_state=True,
                 ...
                 training_objective="raw4_l1_plus_context_teacher_l1_plus_weak_log_relative_error",
                 log_relative_version=VERSION, log_relative_weight=plan["log_relative_weight"],
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-log-relative-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-log-relative-inference-v1" and payload["step"] == step
...
def validate_journal(journal, step, plan, helpers):
    ...
    for row in rows:
        require(row["carry_state"] == plan["carry_state"] and row["initial_state_detached"]
                ...
                and row["log_relative_weight"] == plan["log_relative_weight"] == 0.01
                and math.isfinite(row["log_relative_loss"]) and row["log_relative_loss"] >= 0
                and len(row["log_relative_per_stem"]) == len(row["log_relative_active_examples_per_stem"]) == 4
                and all(math.isfinite(v) and v >= 0 for v in row["log_relative_per_stem"])
                and len(row["log_relative_active_windows"]) == plan["config"]["batch_size"]
                and all(len(v) == 4 and all(type(n) is int and 0 <= n <= 2 for n in v)
                        for v in row["log_relative_active_windows"])
                and all(type(n) is int and 0 <= n <= plan["config"]["batch_size"]
                        for n in row["log_relative_active_examples_per_stem"])
                and abs(row["loss"] - row["supervised_loss"] - row["teacher_weight"] * row["teacher_l1"]
                        - row["log_relative_weight"] * row["log_relative_loss"]) < 1e-7
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-log-relative-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-log-relative-generation-v1", "step": step, "plan_sha256": plan_sha,

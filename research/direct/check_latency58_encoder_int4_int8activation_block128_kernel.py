"""Screen larger blocks in the four-bit encoder kernel with integer activations."""
...
def build(native, saved_graph):
    ...
    config = DefaultWeightOnlyQuantConfig(block_size=128, is_symmetric=True, accuracy_level=4, bits=4)
    packed, scales, unused_zeros = DefaultWeightOnlyQuantizer(config).qbits_block_quant(weight)
    require(packed.shape == (3000, 16, 64) and scales.shape == (3000, 16)
            and np.isfinite(scales).all() and np.all(scales != 0), "Invalid four-bit encoder blocks")
    unpacked = np.stack((packed & 15, packed >> 4), axis=-1).reshape(3000, 16, 128)
    decoded = np.ascontiguousarray(((unpacked.astype(np.float32) - np.float32(8)) *
                                   scales[..., None]).reshape(3000, 2048).T)
    normalized_error = np.abs(weight.T.reshape(3000, 16, 128) - decoded.T.reshape(3000, 16, 128)) / np.abs(scales[..., None])
    ...
    nodes4 = [h.make_node("MatMulNBits", ["x", "w", "s"], ["linear"], name="matrix_int4",
                         domain="com.microsoft", K=2048, N=3000, block_size=128, bits=4, accuracy_level=4),
              h.make_node("Add", ["linear", "bias"], ["y"], name="bias")]
    models = {"u8u8": graph(nodes8, [nh.from_array(v, k) for k, v in arrays8.items()]),
              "int4_int8_activation": graph(nodes4, [nh.from_array(v, k) for k, v in arrays4.items()])}
    proof = {"weight_shape": list(weight.shape), "source_matrix_sha256": fingerprint(weight),
             ...
             "block_size": 128, "accuracy_level": 4, "default_zero_point": 8,
...
def block_reference(inputs, decoded, bias):
    blocks = inputs.reshape(16, 16, 128)
    ...
    quantized = np.clip(np.rint(blocks * inverse), -127, 127).astype(np.int8)
    dequantized = (quantized.astype(np.float32) * scale).reshape(16, 2048)
    return dequantized.astype(np.float64) @ decoded.astype(np.float64) + bias.astype(np.float64)
...
def references(inputs, weights):
    ...
    return {"u8u8": np.stack(integer_reference),
            "int4_int8_activation": block_reference(inputs, decoded, bias),
...
def main():
    ...
    previous_root = PHASE / "encoder-int4-int8activation-kernel-screen-001"
    previous = read(previous_root / "result.json")
    execution_path = PHASE / "encoder-int4-int8activation-kernel-screen-stage-001/execution.json"
    execution = read(execution_path)
    require(previous["status"] == "pass" and previous["int4_over_u8_ratio"] > 1
            and execution["actual_exit_code"] == 0 and execution["source_bindings_unchanged"],
            "Require the closed slower block-32 integer-activation screen")
    verify_inputs(read(previous_root / "plan.json"))
    paths = (previous_root / "plan.json", previous_root / "result.json", execution_path,
    ...
    out = PHASE / "encoder-int4-int8activation-block128-kernel-screen-001"
    ...
    write(out / "plan.json", {"source_bindings": bindings, "source_checkpoint": parent_binding,
          ...
          "recipe": {"bits": 4, "block_size": 128, "symmetric": True, "accuracy_level": 4},
    ...
    with tempfile.TemporaryDirectory(prefix="latency58-encoder-kernel-", dir=PHASE) as temporary:
        ...
        for cycle, label in enumerate(("u8u8", "int4_int8_activation", "int4_int8_activation", "u8u8"), 1):
    ...
    write(out / "result.json", {"status": "pass", "source_bindings_unchanged": True,
          ...
          "median_of_process_p50_us": medians, "int4_over_u8_ratio": medians["int4_int8_activation"] / medians["u8u8"],

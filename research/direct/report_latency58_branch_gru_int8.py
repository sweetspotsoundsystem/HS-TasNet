"""Review completed fourteen-projection quality, vocal views and native cost.
...
STAGES = {"full_reference": "branch-plugin-full14-001",
          "vocal_reference": "deployed-vocal-views-001",
          "candidate": "branch-gru-int8-quality-001",
          "native": "branch-gru-int8-native-001"}
...
def main():
    ...
    parser.add_argument("--output-prefix", default="branch-gru-int8-review-001")
    ...
    require_cpu()
    paths = [Path(__file__).resolve(), ROOT / "research/evaluate.py", ROOT / "research/metrics.py"]
    ...
    stages = {key: load_completed(stage, bindings) for key, stage in STAGES.items()}
    fplan, original = stages["full_reference"]
    vplan, baseline = stages["vocal_reference"]
    ...
    for index, (a, b) in enumerate(zip(reference["tracks"], candidate["tracks"], strict=True)):
        ...
        for key, expected in (("vocal_reference", baseline["tracks"][index]), ("candidate", None)):
            ...
            if key == "candidate":
                ...
                require(track == expected, "Baseline per-track evidence differs from final report")
    ...
    budget_plan = {**cplan, "diagnostic_artifact_allowance_bytes": 35_000_000}
    before = budget_snapshot(budget_plan)
    verify_inputs({"source_bindings": bindings})
    out.mkdir()
    plan = {"schema": "latency58-branch-gru-int8-review-plan-v1", "source_bindings": bindings,
            "input_stages": STAGES, "budget_before": before, "quality_selection": False,
            "output_allowance_bytes": 5_000_000}
    write(out / "plan.json", plan)
    result = {"schema": "latency58-branch-gru-int8-review-v1", "status": "pass",
        ...
        "native_cost": {k: native[k] for k in ("median_of_block_p50_ms", "candidate_over_released_ratio",
            "system", "cpu", "background_load_declared", "limitations")},

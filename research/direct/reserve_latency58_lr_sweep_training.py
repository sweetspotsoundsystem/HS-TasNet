...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-lr-sweep-training-storage-plan-v1"
            and plan["steps"] == [800]
    ...
    write(out / "intent.json", {
        "schema": "latency58-lr-sweep-training-storage-intent-v1", "status": "audited",
        "plan_sha256": args.plan_sha256, "remove": remove, "protected_bindings": protected,
        "reason": "Reserve the current training checkpoint, the queued matched learning-rate training block, and CPU quality artifacts under the existing cap. Retire only the obsolete intermediate full resume at step 800; preserve every corresponding inference tensor and the final step-2000 full resume. Same-Adam/RNG restart at this old intermediate step is intentionally retired.",

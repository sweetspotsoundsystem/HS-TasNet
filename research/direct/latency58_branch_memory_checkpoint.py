"""Authenticated inference and optimizer snapshots for branch_memory training."""
...
from research.direct.latency58_branch_memory import Latency58BranchMemoryModel, VERSION

SCHEMA = "latency58-branch-memory-inference-v1"
GENERATION_SCHEMA = "latency58-branch-memory-generation-v1"
RESERVE_BYTES = 440_000_000
...
def load_model(binding):
    ...
    require(path.is_file() and not path.is_symlink() and sha(path) == binding["sha256"],
            "Branch-memory checkpoint bytes changed")
...
def load_payload(payload):
    ...
    require(payload["schema"] == SCHEMA and type(payload["step"]) is int and payload["step"] > 0,
            "Unknown branch_memory checkpoint or invalid step")
    with torch.random.fork_rng(devices=[]), torch.device("cpu"):
        model = Latency58BranchMemoryModel()
    names = [name for name, _ in model.named_parameters()]
    require(payload["architecture"] == model.architecture_metadata
            and payload["parameter_names"] == names and len(names) == 40,
            "Branch-memory geometry or learned tensor inventory changed")
    ...
    require(set(payload["model"]) == set(model.state_dict())
            ...
            "Invalid branch_memory inference tensors")
    ...
    require(state_sha256(dict(model.named_buffers())) == payload["fixed_buffers_sha256"]
            ...
            and provenance["branch_memory_version"] == VERSION
            and provenance["branch_memory_updates"] == payload["step"]
            and provenance["training_updates"] == provenance["branch_memory_parent_updates"] + payload["step"]
            and provenance["branch_memory_training_plan_sha256"] == payload["plan_sha256"]
            and provenance["branch_memory_all_neural_parameters_trained"] is True,
            "Branch-memory fixed buffers or lineage changed")
...
def audit_live(model, optimizer, step, frozen):
    ...
    require(type(model) is Latency58BranchMemoryModel and type(step) is int and step >= 0
            and len(parameters) == 40 and all(p.requires_grad for p in parameters)
            ...
            "Require all 40 learned tensors and one ordered optimizer group")
...
def make_payloads(model, optimizer, step, plan, plan_sha):
    ...
    require(type(step) is int and step > 0 and step == plan["config"]["steps"]
            ...
            and model.provenance["branch_memory_parent_model_state_sha256"] == plan["parent_model_state_sha256"],
            "Save only the declared branch_memory endpoint with unchanged fixed buffers")
    audit_live(model, optimizer, step, dict(model.named_buffers()))
    tensors = cpu_tree(model.state_dict())
    names = [name for name, _ in model.named_parameters()]
    provenance = {**model.provenance, "branch_memory_version": VERSION,
                  "branch_memory_parent_checkpoint": plan["parent_checkpoint"],
                  "branch_memory_parent_model_state_sha256": plan["parent_model_state_sha256"],
                  "branch_memory_parent_updates": plan["parent_training_updates"],
                  "branch_memory_updates": step, "training_updates": plan["parent_training_updates"] + step,
                  "branch_memory_all_neural_parameters_trained": True,
                  "branch_memory_training_plan_sha256": plan_sha,
                  "branch_memory_objective_version": plan["objective_version"],
...
def save_generation(model, optimizer, step, plan, plan_sha, run):
    ...
    require(not pending.exists() and not final.exists(), "Preserve existing branch-memory checkpoints")
    pending.mkdir()
...
def audit_saved(binding, plan, plan_sha):
    ...
    require(receipt["schema"] == GENERATION_SCHEMA and set(receipt["files"]) == {"model.pt", "optimizer.pt"}
            ...
            and payload["provenance"]["branch_memory_parent_checkpoint"] == plan["parent_checkpoint"]
            and payload["provenance"]["branch_memory_parent_model_state_sha256"] == plan["parent_model_state_sha256"]
            and payload["provenance"]["branch_memory_parent_updates"] == plan["parent_training_updates"],
            "Saved branch_memory endpoint or parent differs from its plan")
    ...
    with torch.inference_mode():
        ...
        require(closure < 1e-6 and model.algorithmic_latency_samples == 256,
                "Saved branch_memory closure or physical delay differs")
    return {"status": "pass", "step": payload["step"], "checkpoint": binding,
            ...
            "optimizer_sha256": receipt["files"]["optimizer.pt"]["sha256"], "saved_optimizer_tensor_count": 40,
...
def audit_resume(model, payload, resume, plan, plan_sha):
    ...
    require(set(states) == set(range(40)) and len(groups) == 1 and groups[0]["params"] == list(range(40)),

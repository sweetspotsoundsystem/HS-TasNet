...
def main():
    ...
    require(verified["schema"] == "latency58-teacher-onnx-verification-v1"
    ...
    require(metadata["hs_tasnet.architecture_version"] == "cropped1024-asymmetric256-hop128-v1"
    ...
    if "tail_updates" in provenance:
        require(type(provenance["tail_updates"]) is int and provenance["tail_updates"] == int(step)
                ...
                and provenance["training_objective"] in ("raw4_l1", "raw4_l1_plus_teacher_l1")
    ...
    require(int(step) == provenance["tail_updates"] == provenance["teacher_trial_updates"] == 250
            and provenance["asymmetric_training_updates"] == 750
            and provenance["parent_training_updates"] == provenance["parent_provenance"]["training_updates"] == 4750
            and provenance["initialization"] == "matched_trial_from_authenticated_asymmetric500"
            ...
            and metadata["hs_tasnet.teacher_weight"] == str(provenance["teacher_weight"])
            and metadata["hs_tasnet.teacher_used_in_inference"] == "false",
            "Canonical matched trial lineage differs")
    ...
    replacements = {
        ...
        '"ola-cropped1024-hann512-hop256-v1"': '"cropped1024-asymmetric256-hop128-v1"',
        ...
        'expect("hs_tasnet.kind", "cropped1024_ola");': 'expect("hs_tasnet.kind", "cropped1024_asymmetric_hop128");',
    ...
    out.mkdir()
    cpp = out / "latency58_teacher_native_async_qualifier.cpp"
    cpp.write_text(source)
    require(sha(BASE) == BASE_SHA and sha(args.verification) == args.verification_sha256
            and sha(args.model) == verified["onnx_sha256"], "Preparation inputs changed")
    receipt = {"schema": "latency58-teacher-native-source-preparation-v1", "source": {"path": str(cpp), "sha256": sha(cpp)},

"""Train causal branch memories from the saved SDR-blend parent with the same blended loss."""
...
def main():
    ...
    with (run / "metrics.jsonl").open("x", buffering=1) as journal:
        for mixture_cpu, truth_cpu in loader:
            ...
            if step < 2:
                expected_nonzero = ADAPTERS if step else ADAPTERS[-2:]
                require(all(refinement_gradients[name] > 0 for name in expected_nonzero),
                        "Both new output projections must learn first, then all private GRU tensors")
                if step == 0:
                    require(all(refinement_gradients[name] == 0 for name in ADAPTERS[:-2]),
                            "Zero output projections must initially block private GRU gradients")

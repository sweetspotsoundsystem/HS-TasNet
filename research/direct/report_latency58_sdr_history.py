"""Compare a history endpoint with its parent, working model and SDR leader."""
...
def main():
    ...
    require(training["schema"] == "latency58-sdr-history-training-v1", "Unknown accumulated-gradient training plan")
    ...
    leader_prefix = "sdr-drum-accum-500"
    leader_comparisons, leader_states = {}, set()
    for mode in ("full14", "actions60", "probes"):
        _, leader = load_completed(PHASE / (leader_prefix + "-" + mode + "-001"), bindings)
        leader_states.add(leader["model_state_sha256"] if mode == "probes"
                          else leader["results"][0]["model"]["model_state_sha256"])
        leader_comparisons[mode] = probe_comparison(leader, candidates[mode]) if mode == "probes" else compare(
            leader["results"][0], candidates[mode]["results"][0])
    require(len(leader_states) == 1, "SDR-leader comparisons contain different models")
    require(all(sha(p) == s for p, s in bindings.items()), "A report input changed")
    report = {
        "schema": "latency58-sdr-history-quality-summary-v1", "prefix": args.prefix, "step": plans["full14"]["step"],
        "model_state_sha256": states["full14"], "source_bindings": bindings,
        "versus_working_baseline": comparisons,
        "versus_sdr_leader": leader_comparisons, "sdr_leader_prefix": leader_prefix,
        "sdr_leader_model_state_sha256": next(iter(leader_states)),
        ...
        "student_history_samples": training["warmup_samples"], "teacher_history_samples": training["teacher_history_samples"],
        ...
        "limitations": ["Primary development-panel results; confirmation intervals remain outside selection.",
                        "This parent-relative report cannot isolate student-history effects; use the matched-arm comparison.",

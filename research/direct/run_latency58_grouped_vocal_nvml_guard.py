...
def launch(plan_path, previous_execution_path, *, resource, prefix_only=False):
    ...
    out.mkdir()
    stop = 2 if resource else 153 if prefix_only else plan["config"]["steps"]
    ...
    if not resource:
        ...
        if not prefix_only:
            ...
            require(previous_execution_path.resolve() == (root / "resume-prefix-stage/execution.json").resolve()
                    ...
                    and prefix["plan_sha256"] == sha(plan_path) and prefix["resumed_from_step"] == 150
                    and prefix["updates"] == prefix["ema_updates"] == 153 and prefix["resume_prefix_only"]
                    and prefix["full_schedule_steps"] == 500 and not prefix["checkpoint_written"]
                    and prefix["replayed_steps_matching_retained_raw_ema_data_loss_gradients"] == [151, 152, 153],
    ...
    spec = {"schema": "gpu-watchdog-launch-finalization-v1", "expected_final_step": stop,
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_grouped_vocal_nvml_guard", "--plan", str(plan_path),

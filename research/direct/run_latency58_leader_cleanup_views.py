...
def main():
    ...
    require(quality_plan["schema"] == "latency58-leader-cleanup-parallel-music-plan-v1"
    ...
    require(sha(training_binding["path"]) == training_binding["sha256"]
            and training["schema"] == "latency58-leader-cleanup-training-v1" and not training["resource_only"],
    ...
    for path in (template_path, template_path.parent / "execution.json", args.reservation,
                 Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_leader_cleanup_views.py",
                 ROOT / "research/direct/evaluate_latency58_leader_cleanup.py",
                 ROOT / "research/direct/latency58_vocal_views.py"):
        bindings[str(path)] = sha(path)
    plan = copy.deepcopy(template)
    plan.update(schema="latency58-leader-cleanup-views-evaluation-plan-v1", output_directory=str(out),
                model={"kind": "leader_cleanup", "label": args.prefix, "arm": training["arm"], "teacher_mode": training["teacher_mode"],
    ...
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_leader_cleanup_views",

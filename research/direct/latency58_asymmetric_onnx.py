"""Asymmetric hop128 ONNX export copy and independent recurrent numerical verifier.
...
STATE_NAMES = tuple(AsymmetricState._fields)
...
def _run_case(model, wrapper, session, case):
    ...
    native_state = AsymmetricState(*(torch.from_numpy(value.copy()) for value in initial))

...
from research.direct.report_latency58_quarter_controlled import REFERENCES, load_views, reference_views
...
def main():
    ...
    require(prospective["schema"] == "latency58-quarter-controlled-quality-preparation-v1"
    ...
    training_terminal = PHASE / "quarter-controlled-queue-001/result.json"
    reference_terminal = PHASE / "cleanup-lr3e6-quality-prep-001/result.json"
    waited = {
    ...
    require(completed["actual_exit_code"] == 0 and completed["source_bindings_unchanged"]
            and reference_completed["actual_exit_code"] == 0 and reference_completed["source_bindings_unchanged"],
            "Training or reference quality did not finish successfully")
    verify_inputs(prospective)
    training_path = PHASE / "quarter-controlled-prep-001/training-plan.json"
    ...
    stage = PHASE / "quarter-controlled-to-000250-001"
    ...
    queue = PHASE / "quarter-controlled-250-queue-001"
    require(not queue.exists(), "Preserve previous followup scoring")
    queue.mkdir()
    reservation = {
        "schema": "latency58-quarter-controlled-views-reservation-v1",
        "evaluation_directories": [str(PHASE / "quarter-controlled-250-views-001")],
    ...
    plan = {
        "schema": "latency58-quarter-controlled-queued-quality-plan-v1", "maximum_wait_seconds": 9000,
        ...
        "quality_prefix": "quarter-controlled-250", "supervisor": prospective["training_supervisor"],
    ...
    command = [PYTHON, "-u", "-m", "research.direct.queue_latency58_quarter_controlled_quality",

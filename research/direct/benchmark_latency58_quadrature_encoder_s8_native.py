...
def main():
    ...
    require_quiet()
    quality_root = PHASE / "quadrature-encoder-s8-full14-memory-001"
    review_path = quality_root / "selection-review.json"
    review, quality = read(review_path), read(quality_root / "result.json")
    execution_path = PHASE / "quadrature-encoder-s8-full14-memory-stage-001/execution.json"
    ...
    paths = [source_path, review_path, execution_path, quality_root / "result.json", quality_root / "plan.json",
             screen_path, cpp, c204, unsigned_path, Path(__file__).resolve(), Path(compiler), *libraries, *headers]
    ...
    paths.extend(ROOT / "research/direct" / name for name in (
        "latency58_quadrature_encoder_s8.py", "benchmark_latency58_quadrature_memory_native.py",
    ...
    out = PHASE / "quadrature-encoder-s8-quiet-native-001"

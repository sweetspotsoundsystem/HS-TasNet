...
def main():
    ...
    require(type(prefix_only) is bool and not (prefix_only and stage["resource_only"])
            and config["steps"] == 500 and plan["resume_checkpoint"]["step"] == 150
            and plan["resume_prefix_qualification"] == {"start_step": 150, "stop_step": 153,
    ...
    require(plan["schema"] == "latency58-grouped-vocal-training-plan-v1" and stage["plan_sha256"] == args.plan_sha256
            and stage["stop_step"] == (2 if stage["resource_only"] else 153 if prefix_only else config["steps"])
    ...
    require(len(replay_rows) == 171 and [row["step"] for row in replay_rows] == list(range(1, 172)),
            "Require the complete retained 171-update journal")
    replayed_steps = []
    ...
    if not stage["resource_only"] and plan.get("resume_checkpoint"):
        ...
        require(start_step == 150 and [json.loads(line) for line in previous_journal.splitlines()]
                == replay_rows[:150], "Recovery journal differs from the retained first 150 updates")
    ...
    if prefix_only:
        require(start_step == 150 and step == 153 and replayed_steps == [151, 152, 153]

"""One monitored resource check or bounded accumulated-gradient context training stage."""
...
def main():
    ...
    require(plan["schema"] == "latency58-sdr-accum-training-v1" and stage["schema"] == "latency58-sdr-accum-stage-v1"
    ...
    from research.direct.latency58_sdr_accum_checkpoint import (
    ...
    try:
        for mixtures_cpu, targets_cpu in loader:
            ...
            for micro_index in range(plan["accumulation_steps"]):
                ...
                terms = raw4_native_objective(output.raw, targets[..., plan["warmup_samples"]:], flags, projection=True)
                ...
                microbatches.append({
                    ...
                    "teacher_l1": float(auxiliary.detach()), "deranged_examples": int(flags.sum()),
                ...
                del mixture, targets, flags, output, terms, auxiliary, loss, teacher_targets, rng, value
            ...
            step += 1
            means = {k: sum(m[k] for m in microbatches) / plan["accumulation_steps"] for k in
                     ("loss", "supervised_loss", "waveform_l1", "projection", "projection_contribution", "teacher_l1")}
        ...
        if plan["resource_only"]:
            ...
            helpers.atomic_json(run / "resource.json", {
                "schema": "latency58-sdr-accum-resource-result-v1", "status": "pass",

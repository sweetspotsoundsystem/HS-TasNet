...
(async () => {
  ...
  require(estimates.length === 4 && data.sources.length === estimates.length + 2 && stem.options.length === 4,
    'Unexpected player inventory');
  require(estimates[0].id === 'working' && estimates.at(-1).id === 'candidate' && estimates.some(x => x.id === 'parent') && estimates.some(x => x.id === 'previous'),
    'Unexpected comparison sources');
  require(data.tracks.length === 1 && data.tracks[0].name === 'Skelpolu - Human Mistakes'
    && excerpt.options.length === 2 && data.tracks[0].clips.length === 2, 'Unexpected primary excerpts');
  require(JSON.stringify(data.tracks[0].clips.map(x => [x.reference_start, x.reference_end]))
    === JSON.stringify([[1323000, 1984500], [3307500, 3969000]]), 'Primary intervals differ');
  const expectedFiles = (estimates.length * 4 + 5) * 2;
  ...
  return {status:'pass', files, source_switch:{before:beforeSource, after:afterSource},
    ...
    strict_nonzero_seek_verified:true, both_primary_excerpts_verified:true,

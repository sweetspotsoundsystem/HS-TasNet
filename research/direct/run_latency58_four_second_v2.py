...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    require(previous_execution_path == (Path(plan["previous_execution_for_resource"]) if resource
                                        else root / "resource-stage-002/execution.json"), "Wrong GPU continuity predecessor")
    if not resource:
        enclosing = read(root / "resource-root-execution-002.json")
        require(type(enclosing["actual_exit_code"]) is int and enclosing["actual_exit_code"] == 0
                ...
                and enclosing["stage_execution_sha256"] == sha(root / "resource-stage-002/execution.json")
    ...
    (require_original_monitor_closed if resource else require_monitor_closed)(previous_execution, previous)
    name = "resource" if resource else "production"
    out, run = root / (name + ("-stage-002" if resource else "-stage")), root / (name + "-run")
    ...
    monitor_out = ARTIFACT_ROOT / "monitors" / (root.name + "-" + name + ("-002" if resource else ""))

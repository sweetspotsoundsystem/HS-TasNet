"""Run a reviewed fresh grouped continuation with the qualified NVML supervisor."""
...
def require_recovery_check(result, *, expected_steps=500, legacy=False):
    require(result["status"] == "pass" and result["parent_weights_unchanged"]
            ...
            and ((legacy and expected_steps == 500 and result["training_schedule_still_500"])
                 or (not legacy and result["training_schedule_steps"] == expected_steps))
            and result["ephemeral_tensor_files_removed"]
...
def require_resource_result(result, parity, plan, plan_sha):
    ...
    require_recovery_check(recovery, expected_steps=plan["config"]["steps"])
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    require(not plan.get("resume_checkpoint") and plan["optimizer_initialization"] == "fresh_adam"
            and plan["continuation_review"]["complete_parent_and_candidate_views_reviewed"],
            "Require a reviewed fresh continuation, with no recovered-prefix shortcut")
    ...
    require_recovery_check(recovery_result, expected_steps=500, legacy=True)
    ...
    require(cpu_result["status"] == "pass" and cpu_result["source_bindings_unchanged"]
            ...
            and cpu_plan["fixture_model_state_sha256"] == plan["canonical_cpu_control_model_state_sha256"]
    ...
    qualified, qualified_execution = read(schedule_cpu / "result.json"), read(schedule_cpu / "execution.json")
    require_recovery_check(qualified, expected_steps=plan["config"]["steps"])
    require(qualified["device"] == "cpu" and not qualified["gpu_used"]
            and qualified["source_bindings_unchanged"] and qualified_execution["actual_exit_code"] == 0
            and qualified_execution["source_bindings_unchanged"] and not qualified_execution["timed_out"]
            and qualified["plan_sha256"] == qualified_execution["plan_sha256"] == sha(schedule_cpu / "plan.json")
            and qualified_execution["result_sha256"] == sha(schedule_cpu / "result.json")
            and qualified["parent_checkpoint"] == plan["parent_checkpoint"]
            and plan["schedule_recovery_cpu_qualification"] == binding(schedule_cpu / "result.json"),
            "Planned schedule and selected parent lack complete CPU recovery qualification")
    validate_recipe(plan)
    verify_inputs(plan)
    observed_budget = budget_snapshot(plan["storage_budget"])
    require(observed_budget["headroom_bytes"] > (1_250_000_000 if resource else 1_200_000_000),
            "Reserve both new persistent generations and diagnostics before GPU work")
    ...
    spec = {"schema": "gpu-watchdog-launch-finalization-v1", "expected_final_step": stop,
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_grouped_continuation", "--plan", str(plan_path),

"""Audit a controlled deployed-truth model, Adam state, RNG and source-error journal."""
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-quarter-controlled-training-v1", "Unknown training plan")
    from research.direct.latency58_quarter_controlled_checkpoint import load_model, read_generation, validate_journal, validate_recipe
    ...
    require(loaded == receipt and receipt["step"] == plan["config"]["steps"] == 250, "Loaded generation differs")
    ...
    require(len(names) == count and receipt["optimizer_parameter_names"] == names, "Parameter order differs")
    ...
    require(set(optimizer) == {"state", "param_groups"} and len(optimizer["param_groups"]) == 1
            and set(optimizer["state"]) == set(range(count)), "Adam inventory differs")
    helpers.validate_adam_group(optimizer["param_groups"][0], list(range(count)), step, config)
    ...
    result = {"schema": "latency58-quarter-controlled-audit-v1", "status": "pass", "step": step,
              ...
              "parameter_tensors": count, "buffer_tensors": 6, "adam_state_pairs": count,
              "arm": plan["arm"], "focused_augmentation": plan["focused_augmentation"],
              "local_mask_mixer": plan["local_mask_mixer"], "augmentation_version": plan["augmentation_version"],
              "matched_input_journal_verified": True,
              "matched_input_scope": "all pristine crops, original augmentation and RNG; shared-view teacher targets",
              "controlled_examples_per_update": 4, "ordinary_examples_per_update": 12,
              "shared_input_and_teacher_microbatches": step * 2,
              "restored_ordinary_microbatches": step * 2,
              "first_gradient_identity_required": False,
              "batch_identity_rows": len(rows), "fixed_buffers_unchanged": True,
              "microbatch_identity_rows": sum(len(row["microbatches"]) for row in rows),
              "microbatch_size": plan["microbatch_size"], "accumulation_steps": plan["accumulation_steps"],
              "drum_weight": plan["drum_weight"], "objective_version": plan["objective_version"],
              "normalized_drum_objective_journal_verified": True,
              "selected_teacher_participation_and_batch_divisor_verified": True,
              ...
              "controlled_deployed_truth_and_full_batch_divisor_verified": True,
              "augmented_examples": step * config["batch_size"],

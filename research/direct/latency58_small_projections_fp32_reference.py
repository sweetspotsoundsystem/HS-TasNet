"""Independent FP32 products from the authenticated native training weights."""
import torch
...
def fp32_linear(x, weight):
    return F.linear(x.float(), weight.float()).double()


class SmallProjectionFP32Reference(nn.Module):
    ...
    def forward(self, audio, history, hidden, spectral_tail, waveform_tail, past_keys, past_values,
        ...
        refined = fused + fp32_linear(F.silu(fp32_linear(fused, m.fusion_refine_reduce.weight)),
        ...
        fused_spec, fused_wave = (refined + fp32_linear(attended, m.temporal_output.weight)).chunk(2, -1)

...
from research.direct.prepare_latency58_leader_cleanup_listening import ORDER
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-leader-cleanup-browser-inventory-plan-v1"
            and plan["last_arm"] == ORDER[-1], "Unexpected audit scope")
    verify_inputs(plan)
    ...
    expected_schema = "latency58-leader-cleanup-listening-preparation-v1"
    require(saved["status"] == browser["status"] == "pass"
            and saved["schema"] == expected_schema
            and browser["schema"] == "latency58-leader-cleanup-listening-browser-check-v1"
    ...
    verify_inputs(plan)
    write(Path(plan["output"]), {"schema": "latency58-leader-cleanup-browser-inventory-v1",

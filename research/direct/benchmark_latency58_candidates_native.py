"""Compare C204 FP32 and both independently checked precise U8U8 candidates with the plugin's native ORT 1.26 CPU SDK.
...
MAGNITUDE_INT8_SHA = "291b32f4d47aa446913edf93d46d121e58d223bb3949de8b3b8303e528cc9faf"
...
def main():
    ...
    parser.add_argument("--magnitude-int8", type=Path, required=True, help="Saved precise magnitude integer graph")
    ...
    models = {"fp32_c204": args.baseline.resolve(), "int8_c204": args.c204_int8.resolve(),
              "int8_magnitude": args.magnitude_int8.resolve()}
    require(sha(models["fp32_c204"]) == BASELINE_SHA and sha(models["int8_c204"]) == C204_INT8_SHA
            and sha(models["int8_magnitude"]) == MAGNITUDE_INT8_SHA,
    ...
    plan = {"schema": "latency58-native-cpu1-three-model-comparison-v1", "source_bindings": bindings,
            ...
            "candidate_precision": "Nine U8U8 projections; FP64 quantizer ancestors and magnitude projection; FP32 decoding and public state"}
    ...
    result = {"status": "pass", "plan_sha256": sha(args.output / "plan.json"), "cycles": rows,
              ...
              "magnitude_over_c204_integer_ratio": medians["int8_magnitude"] / medians["int8_c204"],
    ...
    print(json.dumps({key: result[key] for key in ("status", "median_of_cycle_p50_ms", "ratios_to_fp32_c204", "magnitude_over_c204_integer_ratio")}), flush=True)

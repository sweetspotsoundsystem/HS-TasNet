"""Exercise the complex-mask serializer and optimizer restart in RAM only."""
import hashlib
import io
import json
import os
...
def check(source, evidence):
    ...
    import random
    import torch
    from research.direct.latency58_complex_mask import Latency58ComplexMaskModel
    from research.direct.latency58_complex_mask_checkpoint import make_payloads, load_payload, audit_resume, audit_live
    ...
    model = Latency58ComplexMaskModel.from_parent(parent).train().requires_grad_(True)
    ...
    generator = torch.Generator().manual_seed(202609131)
    ...
    plan = {**source, "schema": "latency58-complex-mask-training-plan-v1",
    ...
    invalid = {**recovered, "model": {**recovered["model"],
               "amplitude_expand.weight": recovered["model"]["amplitude_expand.weight"].double()}}
    wrong_step = {**loaded["optimizer"], "step": 3}
    for name, operation, message in (
        ("wrong_tensor_dtype", lambda: load_payload(invalid), "Invalid complex_mask inference tensors"),
    ...
    return {"status": "pass", "serialized_in_memory": serialized, "all_26_optimizer_states_checked": True,
...
def main():
    ...
    functional = read(PHASE / "complex-mask-functional-001/result.json")
    execution = read(PHASE / "complex-mask-functional-stage-001/execution.json")
    ...
    paths = [source_path, Path(__file__).resolve(), PHASE / "complex-mask-functional-001/result.json",
             PHASE / "complex-mask-functional-stage-001/execution.json",
             ROOT / "research/direct/latency58_complex_mask_checkpoint.py"]
    bindings = {**read(PHASE / "complex-mask-functional-001/plan.json")["source_bindings"],
                **{str(path): sha(path) for path in paths}}
    verify_inputs({"source_bindings": bindings})
    out = PHASE / "complex-mask-checkpoint-memory-functional-001"
    ...
    result.update(source_bindings_unchanged=True, elapsed_seconds=time.monotonic() - began,
                  plan_sha256=sha(out / "plan.json"), counted_bytes_after=require_cpu_space(source, 0))

...
def main():
    ...
    for entry in Path("/proc").iterdir():
        ...
        try:
            ...
            continue
        if len(argv) > 3 and argv[:4] == [PYTHON, "-u", "-m", "research.direct.run_latency58_branch_long_context_b4"]:
    ...
    paths = [plan_path, Path(__file__).resolve(), ROOT / "research/direct/report_latency58_branch_long_context_persistent.py",
             root / "resource-stage/root-execution.json", root / "resource-stage/execution.json",
             root / "resource-run/result.json", PHASE / "branch-long-context-preparation-stage-006/execution.json",

"""Score unchanged continuous vocal counterfactuals for a saved branch-memory endpoint."""
...
def initialize_worker(plan):
    ...
    spec = plan["model"]
    require(spec["kind"] == "branch_memory", "Require a saved branch-memory checkpoint")
    from research.direct.latency58_branch_memory_checkpoint import load_model
    model, _ = load_model(spec["checkpoint"])
...
def main():
    ...
    require(sha(args.plan) == args.plan_sha256, "Vocal counterfactual plan changed")
    plan = read(args.plan)
    require(plan["schema"] == "latency58-branch-vocal-views-evaluation-plan-v1" and plan["workers"] == 2
    ...
    require(qualification["checkpoint"] == plan["model"]["checkpoint"]
            and qualification["model_state_sha256"] == plan["model"]["model_state_sha256"]
            and qualification["public_stream_states"] == 8
            and qualification["tested_group_hops"] == [1, 3, 64],
            "Source views were not qualified on this exact checkpoint")
    require(plan["diagnostic_artifact_allowance_bytes"] == 10_000_000
            and plan["concurrent_training_reservation_bytes"] == 440_000_000
            and plan["stop_counted_bytes"] + plan["outside_roots_reservation_bytes"] == 90_000_000_000,
            "Diagnostic or concurrent training reservation changed")
    out = Path(plan["output_directory"])
    require(out.is_dir() and not (out / "result.json").exists(), "Preserve diagnostic result")
    # Keep conservative space for the active trainer even if it has already
    # published its checkpoint by the time this diagnostic starts.
    before = require_space(plan, plan["concurrent_training_reservation_bytes"] + 5_000_000)
    ...
    result = {"schema": "latency58-vocal-views-evaluation-result-v1", "status": "pass", "version": VERSION,
              ...
              "concurrent_training_reservation_bytes": plan["concurrent_training_reservation_bytes"],
    ...
    current_bytes = sum(p.stat().st_size for p in out.rglob("*") if p.is_file())
    require(current_bytes + len(json.dumps(result, indent=2, allow_nan=False).encode("utf-8")) + 1_000_000
            < plan["diagnostic_artifact_allowance_bytes"], "Diagnostic artifact allowance exceeded")

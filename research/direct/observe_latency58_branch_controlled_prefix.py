...
def main():
    ...
    for entry in Path("/proc").iterdir():
        ...
        try:
            ...
            continue
        if len(argv) > 3 and argv[:4] == [PYTHON, "-u", "-m", "research.direct.run_latency58_branch_controlled_remix"]:
    ...
    paths = [plan_path, Path(__file__).resolve(), ROOT / "research/direct/report_latency58_branch_controlled_remix.py",
             root / "resource-stage/root-execution.json", root / "resource-stage/execution.json",
             root / "resource-run/result.json", PHASE / "branch-controlled-remix-preparation-stage-001/execution.json",
             PHASE / "branch-controlled-remix-preparation-stage-001/root-execution.json"]
    ...
    while True:
        ...
        time.sleep(1)
    normalized = [{k: v for k, v in row.items() if k not in ("elapsed_seconds", "peak_vram_gib")} for row in rows]
    ...
    write(root / "production-prefix-independent-check.json", {"status": "pass", "normalized_fields_equal": True,
          "excluded_observation_fields": ["elapsed_seconds", "peak_vram_gib"], "production_first_two_rows": rows,
          "resource_first_two_updates": resource["matching_production_updates"],
          "resource_step2_model_state_sha256": resource["final_model_state_sha256"],

...
def combine_reports(reports, *, indices, elapsed):
    from research import evaluate as legacy
    ordered = [reports[i] for i in indices]
    first = ordered[0]
    for report in ordered:
        require(len(report["results"]) == 1 and len(report["results"][0]["tracks"]) == 1
                and report["threads"] == report["interop_threads"] == report["batch_size"] == 1
                and report["deterministic_algorithms"] and report["cpu_rng_unchanged"]
                and not report["cuda_initialized"] and not report["retained_audio"],
                "Worker scope or CPU invariants differ")
        for key in ("manifest_sha256", "config_sha256", "metric_source_sha256", "output_policy", "metrics",
                    "precision", "source_bindings_before", "source_bindings_after", "torch_version",
                    "unroll_hops", "io_block_hops", "excerpts"):
            require(report[key] == first[key], "Worker protocols differ: " + key)
        require(report["results"][0]["model"] == first["results"][0]["model"], "Worker models differ")
    result = copy.deepcopy(first)
    result.pop("parallel_worker_pid")
    manifest = read(ROOT / "research/manifests/valid.json")
    expected_names = [manifest["tracks"][i]["name"] for i in indices]
    rows = [r["results"][0]["tracks"][0] for r in ordered]
    require([row["name"] for row in rows] == expected_names, "Restored track order differs from the manifest")
    result.update(panel="full" if indices == list(range(len(manifest["tracks"]))) else "custom-development",
                  track_names=expected_names, excerpt_count=sum(r["excerpt_count"] for r in ordered))
    candidate = result["results"][0]
    candidate.update(tracks=rows, aggregate=legacy._aggregate_tracks(rows),
                     stream_batches=[r["results"][0]["stream_batches"][0] for r in ordered],
                     reconstruction_max_abs=max(r["results"][0]["reconstruction_max_abs"] for r in ordered),
                     evaluation_seconds=elapsed)
    result["evaluator_kind"] = "latency58_independent_tracks_cpu1"
    result["parallel_track_execution"] = {
        "worker_processes": 2, "threads_per_worker": 1, "interop_threads_per_worker": 1,
        "observed_worker_pids": sorted({r["parallel_worker_pid"] for r in ordered}),
        "start_method": "spawn", "batch_size_per_stream": 1,
        "worker_evaluation_seconds_sum": sum(r["results"][0]["evaluation_seconds"] for r in ordered),
        "worker_evaluator_kind": first["evaluator_kind"],
        "aggregation": "Unchanged legacy._aggregate_tracks after restoring manifest order",
        "state": "Every track independently streams from sample zero through real prefixes and gaps",
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-sdr-parallel-music-plan-v1" and plan["workers"] == 2
            and plan["model_kind"] in ("working_baseline", "sdr_candidate"), "Unsupported parallel evaluation")

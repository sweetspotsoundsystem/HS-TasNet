...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-leader-cleanup-gain-error-plan-v1"
            and set(plan["references"]) == {"working", "working_cleanup", "leader"}, "Different comparison scope")
    verify_inputs(plan)
    ...
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-leader-cleanup-gain-error-v1", "status": "pass",

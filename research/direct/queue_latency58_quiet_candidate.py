...
def main():
    ...
    require(plan["schema"] == "latency58-quiet-candidate-queue-plan-v1"
            and plan["maximum_wait_seconds"] == 9000 and plan["evaluation_timeout_seconds"] == 2400
            and set(plan["owners"]) == {"primary", "focused"}, "Different bounded quiet work")
    out = Path(plan["output_directory"])
    require(out.parent == PHASE and out.is_dir() and not (out / "result.json").exists(), "Preserve queue")
    primary = PHASE / "counterfactual-teacher-ordinary-only-250-full14-001"
    focused = PHASE / "quiet-wanted-focused-001"
    destination = PHASE / "quiet-wanted-ordinary-only-001"
    require(plan["primary_directory"] == str(primary) and plan["focused_directory"] == str(focused)
            and plan["evaluation_directory"] == str(destination) and not destination.exists(), "Different or existing evaluation")
    terminals = {"primary": primary / "execution.json", "focused": focused / "execution.json"}
    ...
    control = load_quiet(focused, evidence)
    training = read(quality["training_plan"]["path"])
    require(quality["schema"] == "latency58-counterfactual-parallel-music-plan-v1" and quality["step"] == 250
            and training["teacher_mode"] == "ordinary_only"
            and quality["training_plan"] == plan["training_plan"]
            and control["model"]["kind"] == "focused", "Different primary endpoint or control")
    ...
    quiet_plan = {"schema": "latency58-quiet-wanted-evaluation-plan-v1", "workers": 2, "track_indices": list(range(14)),
                  "model": {"kind": "ordinary_only", "prefix": "counterfactual-teacher-ordinary-only-250",
    ...
    write(out / "wait-complete.json", {"waited_seconds": time.monotonic() - began,
                                       ...
                                       "focused_execution": binding(terminals["focused"]), "training_match": quality["training_match"]})
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_quiet_wanted",
    ...
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-quiet-candidate-queue-v1", "status": "pass",

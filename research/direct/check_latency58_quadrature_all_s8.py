"""Check reduced-range signed weights in all ten existing integer projections."""
...
def main():
    ...
    from research.direct.latency58_quadrature_all_s8 import build, make_reference, BASELINE_SHA
    ...
    training_root = PHASE / "temporal-attention-001"
    ...
    prior_path = PHASE / "quadrature-encoder-s8-quiet-native-001/analysis.json"
    prior = read(prior_path)
    require(prior["status"] == "pass" and prior["actual_root_exit_code"] == 0
            and prior["decision"] == "do_not_publish_encoder_only_graph", "Encoder-only investigation is incomplete")
    verify_inputs(prior)
    paths = [prior_path, Path(__file__).resolve(), ROOT / "research/direct/latency58_quadrature_all_s8.py", source_path,
             ROOT / "research/direct/latency58_temporal_pending_space.py",
    ...
    out = PHASE / "quadrature-all-s8-screen-001"
    ...
    write(out / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
          ...
          "oracle": "Independent reconstruction of all ten reduced-range signed matrices and CPU int32 products",
    ...
    graph, conversion = build(native, original, original_proof)
    encoder = next(v for v in conversion["projections"] if v["module"] == "conv_encode")
    require(encoder["s8_weights_sha256"] == kernel["weight_proof"]["s8_weights_sha256"]
            and encoder["s8_scales_sha256"] == kernel["weight_proof"]["s8_scales_sha256"],
    ...
    require(result["strict_parity_passed"], "All-signed streaming reference parity failed")

param(
    ...
    [ValidateSet("controlled_deployed")][string]$LastArm = "controlled_deployed",
    [int]$DebugPort = 9234
)
...
(async () => {
  ...
  const allIds = ['working', 'ordinary_control', 'controlled_deployed'];
  ...
  button(lastArm).click(); button(priorId).click(); button('controlled_deployed').click(); button('working').click();
...
'@
    ...
    $report = @{schema="latency58-controlled-deployed-listening-browser-check-v1"; status="pass";
    ...
    $report = @{schema="latency58-controlled-deployed-listening-browser-check-v1"; status="failed";

...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="accumulated_normalized_drum_emphasis_fine_tune", parent_provenance=old,
                 ...
                 accum_trial_updates=step, drum_accum_trial_updates=step,
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-sdr-drum-accum-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-sdr-drum-accum-inference-v1" and payload["step"] == step
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-sdr-drum-accum-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-sdr-drum-accum-generation-v1", "step": step, "plan_sha256": plan_sha,

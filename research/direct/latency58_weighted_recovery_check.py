...
from research.direct.latency58_lossless_recovery_codec_v2 import pack_snapshot, unpack_snapshot
from research.direct.latency58_lossless_recovery_files_v3 import publish_snapshot, read_snapshot, CURRENT, PENDING, RECEIPTS
...
def exercise_recovery(parent, source, out, *, disk_directory=None, progress=None):
    ...
    require(disk_directory is None or device.type == "cpu", "The optional filesystem wrapper fixture is CPU-only")
    ...
    try:
        ...
        require(fixture["config"]["steps"] == source["config"]["steps"] == 1000,
        ...
        require(count == packing["tensor_count"] == 216 + (device.type == "cuda")
                and recovered["planned_stop_step"] == recovered["training_config"]["steps"] == 1000
        ...
        result = {"status": "pass", "device": str(device), "precision": precision,
                  ...
                  "all_rng_streams_replayed": True, "training_schedule_steps": 1000,

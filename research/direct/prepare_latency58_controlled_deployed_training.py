"""Freeze 250 controlled deployed-truth updates after exact GPU resource pairing."""
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-controlled-deployed-training-preparation-plan-v1", "Different preparation")
    ...
    require(paired["schema"] == "latency58-controlled-deployed-resource-pairing-v1" and paired["status"] == "pass"
    ...
    require(matched["schema"] == "latency58-controlled-deployed-protocol-v1"
            and matched["production_additional_loss_weights"] == [.5] and matched["maximum_production_updates"] == 250
            and matched["quality_endpoints"] == [250] and not matched["automatic_continuation"],
            "Different bounded production schedule")
    resource_binding = paired["resource_results"]["half"]
    require(paired["training_plans"]["half"] == plan["resource_plan"]
    ...
    require(resource_plan["resource_only"] and resource_plan["teacher_mode"] == "ordinary_only"
            and resource_plan["additional_loss_weight"] == resource["additional_loss_weight"] == .5
            and resource_plan["additional_loss_version"] == resource["additional_loss_version"]
    ...
    before = require_space(resource_plan, 450_000_000)
    candidate = copy.deepcopy(resource_plan)
    candidate.update(resource_only=False, run_dir=str(PHASE / "controlled-deployed-half-b16-micro4-lr3e5-250"),
    ...
    path = out / "half-training-plan.json"
    write(path, candidate)
    write(out / "training-preparation.json", {"schema": "latency58-controlled-deployed-training-preparation-v1", "status": "pass",
          ...
          "checkpoint_and_quality_reserve_bytes": 450_000_000, "quality_selected": False,

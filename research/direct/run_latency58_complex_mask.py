"""Execute a frozen complex_mask plan through monitored rehearsal, training and full14 scoring."""
...
def launch(plan_path, previous_execution_path, *, resource):
    plan = read(plan_path)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-complex-mask-training-plan-v1", "Wrong complex-mask training plan")
    ...
    if not resource:
        ...
        result = read(path)
        parity_path = root / "resource-run/complex_mask-context-gpu-parity.json"
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_complex_mask", "--plan", str(plan_path),
...
def audit_and_score(plan_path):
    import torch
    from research.direct.latency58_complex_mask_checkpoint import audit_saved
    ...
    require(training["status"] == "pass" and training["checkpoint_written"]
            ...
            "Require completed complex-mask training before scoring")
    ...
    require(audit["step"] == training["updates"]
            ...
            "Saved complex_mask model differs from the training endpoint")
    verify_inputs(plan)
    ...
    quality_plan = {"schema": "latency58-complex-mask-full14-plan-v1", "label": plan["name"], "checkpoint": checkpoint,
    ...
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_complex_mask", "--plan", str(quality / "plan.json"),

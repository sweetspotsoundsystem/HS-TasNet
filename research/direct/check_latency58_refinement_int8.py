"""Screen two integer refinement products against the exact PR #17 parent."""
...
OUT = ROOT / "research/m4_followup_20260916/refinement-int8-screen-001"
...
def main():
    ...
    import torch
    from research.direct.latency58_refinement_int8 import build, PARENT_SHA
    from research.direct.latency58_refinement_int8_reference import make_reference
    ...
    source_paths = [Path(__file__).resolve(), POLICY,
        ...
        ROOT / "research/direct/latency58_refinement_int8.py",
        ROOT / "research/direct/latency58_refinement_int8_reference.py",
    ...
    write(OUT / "plan.json", {"source_bindings": bindings, "checkpoint": checkpoint,
        ...
        "purpose": "Test only the two refinement matrix products; retain all attention operations",
    ...
    reference, independent = make_reference(model, onnx.load(ten_path), ten_proof["conversion"],
        ...
        read(short_root / "graph.json")["conversion"], graph, conversion)
    ...
    write(OUT / "result.json", {"status": "pass" if passed else "fail", "strict_parity_passed": passed,
        ...
        "limitations": "Independent eighteen-projection synthetic parity only. Quantization changes model arithmetic. Long-stream numerical checks, full14 and source-view quality, native timing and physical M4 acceptance remain required. Linux Python timings include allocation and concurrent GPU training load."})
    print(json.dumps({"status": "pass" if passed else "fail", "local_speed_gate": gate}), flush=True)
    require(passed, "Refinement candidate failed independent numerical checks")

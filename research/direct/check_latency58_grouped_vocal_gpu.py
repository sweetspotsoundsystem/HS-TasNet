"""Prospective grouped-loss resource checks; callable CPU math, GPU entry gate."""
...
def compare_group_gradients(model, mixture_cpu, targets_cpu, *, warmup_samples, progress=None):
    """Compare B4/B2 model gradients with independent whole-group output VJPs.

    Each forward/backward finishes before another microbatch is rendered.
    Reference output coordinates and their derivatives are detached from the
    model. No B16 neural activation graph is kept in memory.
    """
    ...
    for group, microbatch in (("ordinary", 4), ("auxiliary", 2)):
        ...
        value = whole_objective(raw, deployed, truth[..., warmup_samples:], audio[..., warmup_samples:]).total
        if group == "auxiliary":
            value = value * AUXILIARY_WEIGHT
        reference_losses[group] = float(value.detach())
    ...
    gradients, totals, counts = {}, {}, {}
    try:
        for mode in ("independent_whole_group_vjp", "grouped_contributions"):
            model.zero_grad(set_to_none=True)
            losses, activity = {}, {}
            for group, microbatch in (("ordinary", 4), ("auxiliary", 2)):
                audio, truth = inputs[group]
                total = 0.
                active = torch.zeros(4, device=device, dtype=torch.int64)
                ...
                for offset in range(0, len(audio), microbatch):
                    ...
                    require(torch.equal(result.raw, outputs[group][0][offset:end])
                            and torch.equal(result.deployed, outputs[group][1][offset:end]),
                            "Grouped gradient passes evaluated different outputs")
                    if mode == "independent_whole_group_vjp":
                        torch.autograd.backward((result.raw, result.deployed),
                            tuple(v[offset:end] for v in derivatives[group]))
                    else:
                        value, terms = contribution(group, result.raw, result.deployed, truth[offset:end, ..., warmup_samples:],
                            audio[offset:end, ..., warmup_samples:], groups)
                        value.backward()
                        total += float(value.detach())
                        active += terms.active_window_counts; absent += terms.absent_window_counts
                        del value, terms
                    require(physical.grad is not None and bool(torch.isfinite(physical.grad).all())
                            ...
                            and torch.count_nonzero(physical.grad[..., warmup_samples:]) > 0,
                            "Grouped warmup or scored-input gradients changed")
                    del result, physical
                    if progress is not None:
                        progress(mode, group, offset)
                losses[group] = total
                activity[group] = {"active": active.cpu().tolist(), "absent": absent.cpu().tolist()}
            require(all(p.grad is not None and bool(torch.isfinite(p.grad).all()) and float(p.grad.norm()) > 0
                        for p in model.parameters()), "Grouped GPU rehearsal missed a parameter gradient")
            gradients[mode] = {name: p.grad.detach().cpu().clone() for name, p in model.named_parameters()}
            totals[mode], counts[mode] = losses, activity
        errors = {}
        for name, expected in gradients["independent_whole_group_vjp"].items():
            actual = gradients["grouped_contributions"][name]
            relative_l2 = float((actual - expected).double().norm() / expected.double().norm())
            require(torch.allclose(actual, expected, atol=1e-7, rtol=1e-4) and relative_l2 < 5e-5,
                    "Grouped parameter gradient differs from whole-group VJP: " + name)
            errors[name] = {"maximum_absolute_error": float((actual - expected).abs().max()),
                            "relative_l2_error": relative_l2, "reference_norm": float(expected.norm())}
        require(len(errors) == 40, "Wrong grouped parameter inventory")
        for group in inputs:
            actual = totals["grouped_contributions"][group]
            reduction = getattr(groups, group)
            require(abs(actual - reference_losses[group]) < 3e-6
                    and counts["grouped_contributions"][group] == {
                        "active": reduction.active.cpu().tolist(), "absent": reduction.absent.cpu().tolist()},
                    "Grouped scalar reduction or activity differs")
        require(state_sha256(model.state_dict()) == fingerprint, "Gradient qualification changed weights")
        return {"status": "pass", "model_state_sha256": fingerprint, "device": str(device),
            ...
            "all_40_gradients": errors, "reference_losses": reference_losses,
            "actual_losses": totals["grouped_contributions"], "activity": counts["grouped_contributions"],
...
def check_restart(parent, plan, *, progress=None):
    ...
    from research.direct.check_latency58_grouped_vocal_restart import exercise, fingerprint

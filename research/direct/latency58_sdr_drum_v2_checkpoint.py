...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-sdr-drum-generation-v2"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-sdr-drum-inference-v2" and payload["step"] == step
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-sdr-drum-inference-v2", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-sdr-drum-generation-v2", "step": step, "plan_sha256": plan_sha,

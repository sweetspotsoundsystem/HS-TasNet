...
def main():
    ...
    require(verified["schema"] == "latency58-asymmetric-onnx-verification-v1"
    ...
    require(provenance["asymmetric_training_updates"] == provenance["tail_updates"] == int(step)
            and provenance["parent_training_updates"] == 4250
            and provenance["training_objective"] == "raw4_l1",
            "Asymmetric adaptation lineage differs")
    ...
    out.mkdir()
    cpp = out / "latency58_asymmetric_native_async_qualifier.cpp"
    cpp.write_text(source)
    require(sha(BASE) == BASE_SHA and sha(args.verification) == args.verification_sha256
            and sha(args.model) == verified["onnx_sha256"], "Preparation inputs changed")
    receipt = {"schema": "latency58-asymmetric-native-source-preparation-v1", "source": {"path": str(cpp), "sha256": sha(cpp)},

...
def main():
    ...
    require(training["schema"] == "latency58-counterfactual-training-v1"
            and training["teacher_mode"] == "ordinary_only" and not training["resource_only"],
    ...
    paths = [audit_path, audit_exec_path, stage / "execution.json", stage / "stage.json", monitor_path,
             ...
             Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_counterfactual.py",
             ROOT / "research/direct/audit_latency58_counterfactual_training_match.py",
             ROOT / "research/direct/compare_latency58_sdr.py", ROOT / "research/direct/compare.py"]
    if "probes" in args.modes:
        paths.append(ROOT / "research/direct/evaluate_latency58_counterfactual_probes.py")
    if "full14" in args.modes:
        ...
        paths += [proof_path, ROOT / "research/direct/evaluate_latency58_counterfactual_parallel.py"]
    ...
    for mode in args.modes:
        ...
        plan.update(schema="latency58-counterfactual-" + ("probe" if mode == "probes" else "music") + "-plan-v1",
        ...
        if mode == "full14":
            plan.update(schema="latency58-counterfactual-parallel-music-plan-v1", workers=2,
                        model_kind="counterfactual_candidate", track_indices=list(range(14)),
    ...
    for mode, out, plan in prepared:
        ...
        module = "research.direct.evaluate_latency58_counterfactual" + ("_probes" if mode == "probes" else "_parallel" if mode == "full14" else "")

param(
    ...
    [ValidateSet("focused_mixer")][string]$LastArm = "focused_mixer",
    [int]$DebugPort = 9234
)
...
try {
    ...
    $expression = @'
(async () => {
  const panel = 'skelpolu';
  const lastArm = '__VOCAL_LAST_ARM__';
  const allIds = ['working', 'drum500', 'original', 'focused', 'focused_mixer'];
  ...
  const expectedName = 'Skelpolu - Human Mistakes';
  const intervals = [[1323000, 1984500], [3307500, 3969000]];
  ...
  button(lastArm).click(); button(priorId).click(); button('drum500').click(); button('working').click();
...
'@
    ...
    $report = @{schema="latency58-vocal-focus-skelpolu-browser-check-v1"; status="pass";
    ...
    $report = @{schema="latency58-vocal-focus-skelpolu-browser-check-v1"; status="failed";

"""Build and run the isolated sixteen-projection plugin's Release correctness suite."""
...
def main():
    ...
    target, prep = (PHASE / name for name in ("m4-inference-diagnostics-plugin-001", "branch-output-int8-plugin-preparation-002"))
    ...
    out = PHASE / "branch-output-int8-native-tests-002"
    require(not out.exists(), "Preserve prior native build/test evidence")
    sdk = PHASE / "branch-gru-int8-plugin-001/libs/onnxruntime"
    ...
    require(destination.resolve() == sdk.resolve() and not (target / "build-release").exists(), "Require retained SDK and fresh build")
    ...
    require("PKG_CONFIG_LIBDIR" not in os.environ, "Do not hide system zlib metadata")
    ...
    paths.append(target / "cmake/VerifyParityFixture.cmake")
    paths.append(PHASE / "branch-output-int8-native-preflight-001/execution.json")
    paths.append(ROOT / "research/direct/run_latency58_branch_output_int8_native_tests.py")
    ...
    out.mkdir()
    plan = {"schema": "latency58-sixteen-projection-native-tests-v1", "source_bindings": bindings,
        ...
        "runtime": "1.26.0 CPU", "native_host_qualified": False, "environment": environment,

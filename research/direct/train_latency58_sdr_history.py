"""Matched two/eight-second student histories with shared eight-second C91 targets."""
...
def main():
    ...
    require(plan["schema"] == "latency58-sdr-history-training-v1" and stage["schema"] == "latency58-sdr-history-stage-v1"
            and stage["plan_sha256"] == args.plan_sha256 and 0 <= start < stop <= config["steps"] == 500
            and stop in ((2,) if plan["resource_only"] else (250, 500)) and config["batch_size"] == 16 and config["workers"] == 2
            and plan["microbatch_size"] == 4 and plan["accumulation_steps"] == 4
            and config["crop_samples"] == 440320 and plan["warmup_samples"] in (88064, 352256)
            and plan["teacher_history_samples"] == 352256 and plan["scored_samples"] == 88064
            and plan["optimizer_initialization"] == "fresh_adam"
            and config["seed"] == 20260916 and config["data_start"] == 964000
    ...
    from research.direct.latency58_sdr_history_checkpoint import (
        load_parent, load_model, read_generation, require_space, save_generation, validate_journal, LOSS_KEYS)
    ...
    require(plan["history_version"] == HISTORY_VERSION, "History implementation differs")
    ...
    try:
        for mixtures_cpu, targets_cpu in loader:
            ...
            for micro_index in range(plan["accumulation_steps"]):
                ...
                teacher_targets = physical_context_teacher(teacher, mixture, kind=plan["teacher_kind"],
                                                           warmup_samples=plan["teacher_history_samples"])
                ...
                output = render_history_view(model, mixture, student_history_samples=plan["warmup_samples"],
                ...
                require(torch.equal(output.physical_mixture, mixture[..., plan["teacher_history_samples"]:])
                        and output.initial_state_detached, "Context training lost alignment or detached state")
                terms = drum_emphasized_objective(output.raw, output.deployed,
                    targets[..., plan["teacher_history_samples"]:], teacher_targets, flags)
                loss = terms.total
                ...
                microbatches.append({
                    ...
                    "unweighted_waveform_l1": float(terms.unweighted_waveform_l1.detach()),
                    "unweighted_teacher_l1": float(terms.unweighted_teacher_l1.detach()),
                    "raw_drum_l1": float(terms.raw_drum_l1.detach()),
                    "teacher_drum_l1": float(terms.teacher_drum_l1.detach()),
                    "augmented_batch_sha256": digest.hexdigest(), "teacher_targets_sha256": teacher_digest,
                    "data_hops": output.data_hops, "flush_hops": output.flush_hops,
                    "initial_state_detached": output.initial_state_detached,
                })
                del mixture, targets, flags, output, terms, loss, teacher_targets, rng, value
            ...
            identities = {k: hashlib.sha256("".join(m[k] for m in microbatches).encode("ascii")).hexdigest()
                          for k in ("augmented_batch_sha256", "teacher_targets_sha256")}
            row = {"step": step, "lr": lr, **means, **identities,
                   ...
                   "teacher_history_samples": plan["teacher_history_samples"], "history_version": HISTORY_VERSION,
                   "student_view_start": plan["teacher_history_samples"] - plan["warmup_samples"],
        ...
        if plan["resource_only"]:
            ...
            helpers.atomic_json(run / "resource.json", {
                "schema": "latency58-sdr-history-resource-result-v1", "status": "pass",
                ...
                "teacher_history_samples": plan["teacher_history_samples"], "history_version": HISTORY_VERSION,
                "optimizer_initialization": plan["optimizer_initialization"],

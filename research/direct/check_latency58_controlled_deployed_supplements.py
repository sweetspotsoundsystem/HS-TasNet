...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-controlled-deployed-supplements-check-plan-v1", "Different supplement check")
    ...
    new = functions(root / "evaluate_latency58_quiet_deployed.py")
    ...
    require(functions(root / "report_latency58_vocal_gain_error.py")["compare_gain"]
                == functions(root / "report_latency58_controlled_deployed_gain_error.py")["compare_gain"],
            "Gain supplement changes comparison arithmetic")
    from research.direct.evaluate_latency58_quiet_deployed import check_window_metrics, PREFIXES
    ...
    require(checked == old_check() and PREFIXES["controlled_deployed"] == "controlled-deployed-half-250",
            "Analytic quiet fidelity or new checkpoint prefix differs")
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-controlled-deployed-supplements-check-v1", "status": "pass",

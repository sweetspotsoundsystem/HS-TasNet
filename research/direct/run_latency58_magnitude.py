"""Prepare, monitor, audit and score a frozen-parent spectral-magnitude adapter."""
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_magnitude", "--plan", str(plan_path),
...
def prepare(args):
    import torch
    from research.direct.latency58_direct_sdr import check as objective_check, VERSION
    from research.direct.check_latency58_magnitude import check
    ...
    functional = {"model": check(), "objective": objective_check()}
    ...
    paths = [Path(__file__).resolve(), ROOT / "research/direct/latency58_direct_sdr.py",
             ...
             ROOT / "research/direct/latency58_magnitude_checkpoint.py", ROOT / "research/direct/train_latency58_magnitude.py",
             ROOT / "research/direct/latency58_magnitude.py", ROOT / "research/direct/check_latency58_magnitude.py",
             ROOT / "research/direct/reserve_latency58_magnitude.py", PHASE / "magnitude-storage-001/receipt.json",
             ROOT / "research/direct/evaluate_latency58_magnitude.py", ROOT / "research/direct/reserve_latency58_direct_sdr.py",
    ...
    config = {**template["config"], "steps": args.steps, "seed": args.seed, "data_start": args.data_start,
              ...
              "root_weights": {"musdb18hq_train": 1.0}, "augmentation": "ordinary_original_mixture"}
    plan = {"schema": "latency58-direct-sdr-plan-v1", "name": args.name, "output_directory": str(out), "config": config,
            ...
            "objective_version": VERSION, "model_version": MODEL_VERSION, "trainable_parameter_names": [ADAPTER], "teacher_used": False, "fixed_residual_share": 1 / 16,
    ...
    counted = require_space(plan, 180_000_000)
    verify_inputs(plan)
    ...
    write(out / "preparation.json", {"status": "pass", "parent_model_state_sha256": plan["parent_model_state_sha256"],
          ...
          "counted_bytes_before": counted, "forecast_including_outside_and_pilot": counted + 800_000_000 + 180_000_000,
          "rationale": "Train a zero-initialized normalized spectral-magnitude projection while freezing every inherited neural tensor; use direct deployed SDR on 83 frozen recorded MUSDB training tracks and preserve 256 samples of algorithmic delay.",
...
def audit_and_score(plan_path):
    ...
    parameters = [p for name, p in model.named_parameters() if name in plan["trainable_parameter_names"]]
    require(len(parameters) == 1 and resume["trainable_parameter_names"] == plan["trainable_parameter_names"],
            "Saved optimizer must contain only the magnitude projection")
    ...
    write(out / "checkpoint-audit.json", {"status": "pass", "step": payload["step"], "checkpoint": checkpoint,
          ...
          "inherited_tensors_unchanged": True, "trainable_parameter_names": plan["trainable_parameter_names"],
    ...
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_magnitude", "--plan", str(quality / "plan.json"),
...
def main():
    ...
    parser.add_argument("--name", default="magnitude-sdr-001")
    parser.add_argument("--steps", type=int, default=1000)
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--seed", type=int, default=20260928)
    parser.add_argument("--data-start", type=int, default=1_020_000)
    ...
    resource = launch(plan_path, PHASE / "direct-sdr-musdb-001/production-stage/execution.json", resource=True)

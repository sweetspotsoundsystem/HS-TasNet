"""CPU functional checks for the untrained temporal-attention architecture."""
...
def compare_context(model, mixture, warmup):
    ...
    from research.direct.latency58_temporal_attention_context import render_scored_context as fast
    ...
    require(all(v == 0 for v in errors.values()) and first["loss"] == second["loss"]
            and len(gradients) == 30 and all(v["maximum_error"] == 0 and v["reference_norm"] > 0 for v in gradients.values()),
    ...
    model.zero_grad(set_to_none=True)
    return {"status": "pass", "output_and_input_gradient_errors": errors, "all_30_gradients": gradients,
...
def check(parent):
    ...
    from research.direct.latency58_fusion_refinement import FusionRefinementState
    from research.direct.latency58_temporal_attention import (
        ADAPTERS, WINDOW, KEY, VALUE, Latency58TemporalAttentionModel, TemporalAttentionState,
    )
    generator = torch.Generator().manual_seed(202609141)
    parent_sha = state_sha256(parent.state_dict())
    model = Latency58TemporalAttentionModel.from_parent(parent)
    ...
    with torch.inference_mode():
        for signal, samples in (("noise", audio), ("silence", torch.zeros_like(audio))):
            for nonzero in (False, True):
                ...
                if nonzero:
                    state = FusionRefinementState(*(torch.randn(v.shape, generator=generator) *
                        (.01 * PUBLIC_FUSION_SCALE if i == 1 else .001) for i, v in enumerate(state)))
                extra = model.initial_state(2)[4:]
                if nonzero:
                    extra = tuple(.01 * torch.randn(v.shape, generator=generator) for v in extra)
                first, second = parent.render(samples, state), model.render(samples, TemporalAttentionState(*state, *extra))
                require(all(bits_equal(getattr(first, k), getattr(second, k)) for k in
                        ...
                        and all(bits_equal(a, b) for a, b in zip(first.state, second.state[:4], strict=True)),
        ...
        model.temporal_output.weight.copy_(.002 * torch.randn(model.temporal_output.weight.shape, generator=generator))
        features = torch.randn(2, 7, 1000, generator=generator)
        past_keys = .01 * torch.randn(2, WINDOW - 1, KEY, generator=generator)
        past_values = .01 * torch.randn(2, WINDOW - 1, VALUE, generator=generator)
        # Independent NumPy FP64 projections and explicit causal frame loops.
        array = features.numpy().astype(np.float64)
        project = lambda name: array @ getattr(model, "temporal_" + name).weight.numpy().astype(np.float64).T
        queries = project("query")
        keys = np.concatenate((past_keys.numpy(), project("key")), axis=1)
        values = np.concatenate((past_values.numpy(), project("value")), axis=1)
        expected = np.empty_like(array)
        for frame in range(array.shape[1]):
            logits = np.sum(queries[:, frame, None] * keys[:, frame:frame + WINDOW], axis=-1) * KEY ** -.5
            weights = np.exp(logits - logits.max(axis=-1, keepdims=True))
            weights /= weights.sum(axis=-1, keepdims=True)
            mixed = (weights[..., None] * values[:, frame:frame + WINDOW]).sum(axis=1)
            expected[:, frame] = mixed @ model.temporal_output.weight.numpy().astype(np.float64).T
        actual, next_keys, next_values = model.attention(features, past_keys, past_values)
        numpy_error = float(np.max(np.abs(actual.numpy() - expected)))
        require(numpy_error < 2e-6, "Attention differs from independent causal expression")
        require(np.max(np.abs(next_keys.numpy() - keys[:, -(WINDOW - 1):])) < 2e-6
                and np.max(np.abs(next_values.numpy() - values[:, -(WINDOW - 1):])) < 2e-6,
                "Attention cache differs from independent history")
        mutated = features.clone()
        mutated[:, 4:] *= -3
        require(torch.equal(actual[:, :4], model.attention(mutated, past_keys, past_values)[0][:, :4]),
                "Attention admitted a future frame")
        nonlinearity = float((model.attention(2 * features, past_keys, past_values)[0] - 2 * actual).abs().max())
        require(nonlinearity > 1e-5, "Attention unexpectedly reduces to a fixed linear projection")
        whole = model.render(audio)
        require(not torch.equal(whole.deployed, parent.render(audio).deployed), "Nonzero attention has no waveform effect")
        ...
        buffer.seek(0)
        restored = Latency58TemporalAttentionModel().eval().requires_grad_(False)
    ...
    try:
        ...
        require(str(error) == "Use the distinct temporal-attention state family", "Unexpected state rejection reason")
    else:
    ...
    with torch.no_grad():
        model.temporal_output.weight.zero_()
    ...
    for step in range(2):
        ...
        loss.backward()
        attention_gradient_norms.append({name: float(dict(model.named_parameters())[name].grad.norm()) for name in ADAPTERS})
        require(attention_gradient_norms[-1]["temporal_output.weight"] > 0
                and all((attention_gradient_norms[-1][name] == 0 if step == 0 else
                         attention_gradient_norms[-1][name] > 0) for name in ADAPTERS if name != "temporal_output.weight"),
                "Zero initialization does not activate all attention projections")
    ...
    require(len(updated) == len(optimizer.state) == 30 and all(torch.equal(v, frozen[k]) for k, v in model.named_buffers()),
    ...
    return {"status": "pass", "parent_model_state_sha256": parent_sha, "zero_correction_cases": zero_cases,
            "causal_attention_numpy_max_abs": numpy_error,
            ...
            "all_30_neural_tensors_updated": updated, "discarded_functional_adam_updates": 2,
            "attention_gradient_norms_from_zero_output": attention_gradient_norms,
            "fixed_buffers_unchanged": True, "additional_parameters": model.architecture_metadata["temporal_attention_parameters"],
...
def main():
    ...
    parser.add_argument("--name", default="temporal-attention-functional-001")
    ...
    require(review["status"] == "selected_for_research" and review["actual_root_exit_code"] == 0
            and review["best_full_sdr_db"] == 4.266897232064164,
            "Require the reviewed best fusion-refinement parent")
    ...
    require(state_sha256(parent.state_dict()) == "437179545299aac9a079da9e4eac6f29dd138ac68bfddb573696f77c110d8bf0",
            "Wrong preserved fusion-refinement parent")
    out = PHASE / args.name
    ...
    paths.extend(ROOT / "research/direct" / name for name in
                 ("latency58_temporal_attention.py", "latency58_temporal_attention_context.py", "latency58_sdr_context.py"))
    ...
    write(out / "plan.json", {"source_bindings": bindings, "parent_checkpoint": parent_binding,
          ...
          "counted_bytes_before": counted, "gpu_training_active": False, "independent_cpu_quality_scoring_may_be_active": True,
          "candidate_weights_written": False, "zero_output_initialization_seed": 202609141,
          "hypothesis": "Causal attention revisits the last 32 received fused-feature frames using 64-channel keys and 128-channel values. Two history caches add no future input or audio buffering; the output projection starts at zero.",

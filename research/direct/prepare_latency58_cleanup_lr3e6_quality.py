...
def main():
    ...
    require(prospective["schema"] == "latency58-cleanup-lr3e6-quality-preparation-v1"
    ...
    training_terminal = PHASE / "cleanup-lr-sweep-queue-001/result.json"
    reference_terminal = PHASE / "cleanup-rebound-250-queue-001/result.json"
    waited = {
    ...
    verify_inputs(prospective)
    training_path = PHASE / "cleanup-lr3e6-prep-001/training-plan.json"
    ...
    stage = PHASE / "cleanup-lr3e6-to-000250-001"
    ...
    queue = PHASE / "cleanup-lr3e6-250-queue-001"
    require(not queue.exists(), "Preserve previous followup scoring")
    queue.mkdir()
    reservation = {
        "schema": "latency58-cleanup-lr3e6-views-reservation-v1",
        "evaluation_directories": [str(PHASE / "cleanup-lr3e6-250-views-001")],
    ...
    plan = {
        "schema": "latency58-cleanup-lr3e6-queued-quality-plan-v1", "maximum_wait_seconds": 9000,
        ...
        "quality_prefix": "cleanup-lr3e6-250", "supervisor": prospective["training_supervisor"],
    ...
    command = [PYTHON, "-u", "-m", "research.direct.queue_latency58_cleanup_lr3e6_quality",

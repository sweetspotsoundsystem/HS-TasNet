"""Match every leader-cleanup example and RNG stream to the completed working-parent recipe."""
...
from research.direct.latency58_leader_cleanup_checkpoint_v2 import RECIPE_KEYS, IDENTITY_KEYS, require_space
...
def load_completed_match(binding, evidence):
    ...
    require(plan["schema"] == "latency58-leader-cleanup-training-match-plan-v1"
            and result["schema"] == "latency58-leader-cleanup-training-match-v1" and result["status"] == "pass"
            ...
            and result["additional_loss_weight"] == .5 and result["comparison_variable"] == "training_parent"
            and not result["matched_loss_effect_from_leader_claimed"]
...
def main():
    ...
    import torch
    from research.direct import latency58_controlled_deployed_checkpoint as old
    from research.direct import latency58_leader_cleanup_checkpoint_v2 as new
    ...
    for side, spec in plan["models"].items():
        ...
        if side == "candidate":
            require(training["teacher_mode"] == audit["teacher_mode"] == "ordinary_only"
                    ...
                    and training["additional_loss_weight"] == audit["additional_loss_weight"] == .5, "Different selected teacher policy")
    ...
    require(candidate["reference_training_plan"] == plan["models"]["reference"]["training_plan"]
            and all(reference[k] == candidate[k] for k in RECIPE_KEYS), "Different matched recipes")
    ...
    require(receipts["reference"]["model_state_sha256"] != receipts["candidate"]["model_state_sha256"],
            "Parent transfer produced the same final model")
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-leader-cleanup-training-match-v1", "status": "pass",
          ...
          "teacher_mode": "ordinary_only", "additional_loss_weight": .5, "comparison_variable": "training_parent",
          "matched_loss_effect_from_leader_claimed": False, "quality_selected": False, "training_updates_executed": 0,

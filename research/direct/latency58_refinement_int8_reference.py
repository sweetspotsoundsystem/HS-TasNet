"""Independent eighteen-projection reconstruction; no runtime-output oracle."""
import torch
...
def make_reference(native, ten_graph, ten_conversion, fourteen_graph, fourteen_conversion,
    ...
    require(set(prefixes) == {'fusion_refine_reduce', 'fusion_refine_expand'},
            'Unexpected refinement projections')
    for name in ('fusion_refine_reduce', 'fusion_refine_expand'):
        original = getattr(native, name)
        require(original.bias is None, 'Refinement bias policy changed')
        projection = SignedProjection(original.weight, None, stored, prefixes[name])
        setattr(reference.model, name, projection)
        proof.append({'module': name, **projection.proof, 'bias': None})
    require(len(proof) == 18, 'Expected eighteen independent integer projections')
    return RefinementIntegerReference(reference.model).eval().requires_grad_(False), proof


class RefinementIntegerReference(nn.Module):
    ...
    def forward(self, audio, history, hidden, spectral_tail, waveform_tail, past_keys, past_values,
        ...
        refined = fused + m.fusion_refine_expand(F.silu(m.fusion_refine_reduce(fused)))

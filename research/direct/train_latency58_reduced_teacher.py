"""Fine-tune C204 with reduced ordinary-mixture teacher supervision."""
...
def main():
    ...
    from research.direct.latency58_reduced_teacher_checkpoint import validate_recipe
    validate_recipe(plan)
    require(stage["schema"] == "latency58-reduced-teacher-stage-v1"
    ...
    from research.direct.latency58_reduced_teacher_checkpoint import (
    ...
    from research.direct.latency58_reduced_teacher_loss import objective as controlled_deployed_objective, VERSION as REDUCED_VERSION
    from research.direct.latency58_counterfactual_journal import rng_state_sha256
    from research.direct.latency58_reduced_teacher_loss import loss_evidence
    ...
    from research.direct.latency58_reduced_teacher_checkpoint import compare_inputs
    reference_rows = [json.loads(line) for line in Path(plan["matched_positive_journal"]["path"]).read_text().splitlines()]
    ...
    try:
        for mixtures_cpu, targets_cpu in loader:
            ...
            for micro_index in range(plan["accumulation_steps"]):
                ...
                controlled_terms = controlled_deployed_objective(output.raw, output.deployed,
                    ...
                    view_codes=tuple(codes), weight=plan["additional_loss_weight"], teacher_weight=plan["teacher_weight"])
                ...
                microbatches.append({
                    ...
                    **loss_evidence(output.raw, output.deployed, targets[..., plan["warmup_samples"]:],
                                    teacher_targets, controlled_terms, tuple(codes), plan["additional_loss_weight"], plan["teacher_weight"]),
            ...
            row = {"step": step, "lr": lr, **means, **identities,
                   ...
                   "reduced_teacher_version": REDUCED_VERSION, "teacher_weight": plan["teacher_weight"], "teacher_kind": plan["teacher_kind"],
            ...
            compare_inputs(row, reference_rows[step - 1])
        ...
        if plan["resource_only"]:
            ...
            helpers.atomic_json(run / "resource.json", {
                "schema": "latency58-reduced-teacher-resource-result-v1", "status": "pass",
                ...
                "comparison_variable": "teacher_weight",
                "focused_augmentation": plan["focused_augmentation"],
                "local_mask_mixer": plan["local_mask_mixer"],
                "augmentation_version": plan["augmentation_version"],
                "reduced_teacher_version": REDUCED_VERSION, "teacher_weight": plan["teacher_weight"], "teacher_model_state_sha256": plan["teacher_model_state_sha256"],
                "drum_weight": plan["drum_weight"], "objective_version": plan["objective_version"],

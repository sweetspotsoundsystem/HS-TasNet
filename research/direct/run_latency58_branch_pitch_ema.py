"""Monitor one SDR/EMA trajectory and score both saved endpoints on unchanged full14."""
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    from research.direct.latency58_pitch_ema_data import policy as data_policy
    ...
    if not resource:
        ...
        result = read(path)
        parity_path = root / "resource-run/branch-memory-context-gpu-parity.json"
        ...
        require(result["status"] == "pass" and result["updates"] == 2 and not result["checkpoint_written"]
                ...
                and parity["status"] == "pass" and parity["temporary_nonzero_projections_restored"]
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_branch_pitch_ema", "--plan", str(plan_path),
...
def audit_and_score(plan_path):
    ...
    require(training["status"] == "pass" and training["checkpoint_written"]
            ...
            "Require completed raw/EMA training before scoring")
    checkpoint = training["checkpoint"]
    audit = audit_saved(checkpoint, plan, sha(plan_path))
    require(audit["step"] == training["updates"] == training["ema_updates"]
            and audit["model_state_sha256"] == training["final_model_state_sha256"]
            and audit["raw_model_state_sha256"] == training["final_raw_model_state_sha256"],
            "Saved raw or averaged model differs from the training endpoint")
    verify_inputs(plan)
    write(out / "checkpoint-audit.json", {**audit, "source_bindings_unchanged": True})
    generation = Path(checkpoint["path"]).parent
    checkpoints = {"raw": binding(generation / "raw-model.pt"), "ema": checkpoint}
    paths = [out / name for name in ("checkpoint-audit.json", "production-run/result.json", "production-stage/execution.json")]
    paths.extend(generation.iterdir())
    bindings = {**plan["source_bindings"], str(plan_path): sha(plan_path), **{str(p): sha(p) for p in paths}}
    reports = {}
    for kind, endpoint in checkpoints.items():
        quality = out / ("full14-" + kind)
        quality.mkdir()
        quality_plan = {"schema": "latency58-branch-memory-full14-plan-v1", "label": plan["name"] + "-" + kind,
                        "checkpoint": endpoint, "reference_result": plan["reference_result"], "workers": 2,
        ...
        reports[kind] = read(quality / "result.json")
        require(reports[kind]["status"] == "pass" and reports[kind]["track_count"] == 14
                and reports[kind]["excerpt_count"] == 28 and reports[kind]["results"][0]["checkpoint"] == endpoint,
                "Incomplete raw or averaged full14 evaluation")
    raw, averaged = (reports[kind]["results"][0] for kind in ("raw", "ema"))
    ...
    terminal = {"status": "training_audit_and_paired_full14_complete", "checkpoints": checkpoints,
                "full_sdr_db": {kind: report["results"][0]["aggregate"]["full_sdr_db"] for kind, report in reports.items()},
                "target_full_sdr_db": 5.0, "target_reached": any(report["target_reached"] for report in reports.values()),
                "quality_results": {kind: binding(out / ("full14-" + kind) / "result.json") for kind in reports},
                "paired_comparison": binding(out / "paired-full14-comparison.json"),
                "quality_selected": False, "plugin_replaced": False}

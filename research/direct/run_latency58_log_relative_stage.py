"""Run a monitored resource rehearsal or an audited weak log-relative stage."""
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-log-relative-training-v1", "Unknown training plan")
    ...
    require(proof["status"] == "pass" and proof["source_bindings_unchanged"]
            and proof["version"] == plan["log_relative_version"] and proof["auxiliary_weight"] == plan["log_relative_weight"]
    ...
    if plan["resource_only"]:
        ...
        shared = ("config", "parent", "warmup_samples", "scored_samples", "carry_state",
                  ...
                  "precision_policy", "torch_version", "environment", "log_relative_weight", "log_relative_version",
                  "helper_source", "watchdog_source", "manifest_sha256", "functional_proof", "functional_execution")
        require(resource["schema"] == "latency58-log-relative-resource-result-v1" and resource["status"] == "pass"
    ...
    stage = {"schema": "latency58-log-relative-stage-v1", "plan_sha256": args.plan_sha256,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_log_relative", "--plan", str(args.plan.resolve()),
    ...
    if plan["resource_only"]:
        ...
        write(out / "resource-qualification.json", {
            "schema": "latency58-log-relative-resource-qualification-v1", "status": "pass",
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.audit_latency58_log_relative", "--plan", str(args.plan.resolve()),
    ...
    if previous_generation is not None:
        ...
        retirement = {"schema": "latency58-log-relative-replaced-optimizer-v1", "path": str(old_optimizer),

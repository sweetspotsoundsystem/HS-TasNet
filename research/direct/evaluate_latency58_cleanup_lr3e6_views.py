"""Score the same fixed vocal cleanup_lr3e6s for a completed cleanup_lr3e6 pilot."""
...
def initialize_worker(plan):
    ...
    spec = plan["model"]
    require(spec["kind"] == "cleanup_lr3e6", "Require the new cleanup_lr3e6 checkpoint family")
...
def main():
    ...
    require(sha(args.plan) == args.plan_sha256, "Vocal cleanup_lr3e6 plan changed")
    plan = read(args.plan)
    require(plan["schema"] == "latency58-cleanup-lr3e6-views-evaluation-plan-v1" and plan["workers"] == 2

...
def completed_endpoint(plan):
    ...
    from research.direct.latency58_counterfactual_checkpoint import read_generation, validate_recipe
    ...
    require(summary["schema"] == "latency58-counterfactual-quality-summary-v1" and summary["status"] == "pass"
            ...
            and set(summary["comparisons"]) == {"working", "drum500", "drum1000", "focused_control"}
...
def completed_supplements(plan, summary, receipt):
    """Require the actual matched training result and both complete quiet comparisons."""
    ...
    require(matched["model_states"] == {"reference": summary["reference_model_states"]["focused_control"],
                                        "candidate": receipt["model_state_sha256"]},
            "Review does not describe the authenticated matched pair")
    require(set(plan["quiet_comparisons"]) == {"working", "focused_control"}, "Incomplete quiet reference scope")
    ...
    gain_paths = [gain_directory / name for name in ("plan.json", "result.json", "supplement-execution.json")]
    ...
    require(gain_result["schema"] == "latency58-vocal-gain-error-v1" and gain_result["status"] == "pass"
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-counterfactual-review-plan-v1", "Unexpected review plan")
    verify_inputs(plan)
    ...
    review_fields = ("full_mixture", "per_track_stem_bands_and_absence", "probes_and_dc", "actions",
                     ...
                     "local_windows", "quiet_wanted_sources", "matched_teacher_change", "interpretation", "limitations")
    ...
    from research.direct.latency58_counterfactual_checkpoint import require_space
    counted = require_space(training, 3_000_000)
    verify_inputs(plan)
    write(out / "result.json", {
        "schema": "latency58-counterfactual-review-v1", "status": "pass", "plan_sha256": args.plan_sha256,

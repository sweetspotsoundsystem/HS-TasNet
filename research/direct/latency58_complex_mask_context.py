"""Detached warmup with final-frame decoding for the complex_mask prototype."""
import torch
...
def render_scored_context(model, mixture, *, warmup_samples, carry_state):
    require(carry_state is True and type(warmup_samples) is int and warmup_samples > 0
            ...
            "Invalid complex_mask training context")
    require(type(model) is Latency58ComplexMaskModel, "Require the complex_mask model")
    ...
    require(raw.shape == deployed.shape == (mixture.shape[0], 4, 2, count)
            and torch.equal(physical, score), "Complex-mask context lost physical alignment")

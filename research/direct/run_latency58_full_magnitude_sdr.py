"""Run a saved-checkpoint full-model magnitude/direct-SDR training experiment."""
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_full_magnitude_sdr", "--plan", str(plan_path),
...
def prepare(args):
    import torch
    from research.direct.latency58_direct_sdr import check, VERSION
    from research.direct.latency58_wave_spectral import AUGMENTATION
    ...
    from research.direct.check_latency58_full_magnitude import check as check_full_training
    from research.direct.check_latency58_fast_context import check_cpu
    functional = {"objective": check(), "full_training": check_full_training(), "fast_context": check_cpu()}
    ...
    paths = [Path(__file__).resolve(), ROOT / "research/direct/latency58_direct_sdr.py",
             ROOT / "research/direct/latency58_full_magnitude_checkpoint.py", ROOT / "research/direct/train_latency58_full_magnitude_sdr.py",
             ROOT / "research/direct/latency58_wave_spectral.py",
             ROOT / "research/direct/latency58_magnitude.py",
             ROOT / "research/direct/check_latency58_full_magnitude.py",
             ROOT / "research/direct/latency58_direct_sdr_checkpoint.py", ROOT / "research/direct/reserve_latency58_full_magnitude.py",
             PHASE / "full-magnitude-storage-001/receipt.json",
             ROOT / "research/direct/evaluate_latency58_full_magnitude.py", ROOT / "research/direct/reserve_latency58_direct_sdr.py",
             Path(parent["path"]), reference_path, PHASE / "direct-sdr-storage-001/receipt.json", PHASE / "direct-sdr-storage-002/receipt.json",
             ROOT / "research/direct/latency58_recorded301_data.py", ROOT / "research/direct/reserve_latency58_direct_sdr_followup.py",
    ...
    paths.extend(ROOT / "research/direct" / name for name in (
        "latency58_fast_context.py", "latency58_state_warmup.py", "check_latency58_fast_context.py", "check_latency58_fast_context16.py"))
    paths.extend(PHASE / name for name in ("rehearsal-storage-001/receipt.json", "native-object-storage-001/receipt.json"))
    ...
    config = {**template["config"], "steps": args.steps, "seed": args.seed, "data_start": args.data_start,
              "crop_samples": 132224, "lr": args.lr, "min_lr": args.lr / 10, "warmup": 50,
              "microbatch_size": 16, "checkpoint_every": args.steps,
              "source_corpus_root_weights": template["config"]["root_weights"],
              "root_weights": ROOT_WEIGHTS, "augmentation": AUGMENTATION}
    plan = {"schema": "latency58-direct-sdr-plan-v1", "name": args.name, "output_directory": str(out), "config": config,
            ...
            "augmentation_probability": .5, "source_gain_bounds_db": [-3., 3.], "spectral_fft_sizes": [],
            "supervision": "negative active-window SDR plus 0.5 absent-window leakage dB plus 0.05 relative raw-head L1",
            "warmup_samples": 88064, "scored_samples": 44160, "accumulation_steps": 1,
            "target_full_sdr_db": 5.0, "quality_endpoints": [args.steps], "automatic_plugin_replacement": False,
            "training_context_implementation": "state-only warmup plus unchanged scored native render",
    ...
    counted = require_space(plan, 370_000_000)
    verify_inputs(plan)
    ...
    write(out / "preparation.json", {"status": "pass", "parent_model_state_sha256": plan["parent_model_state_sha256"],
          ...
          "counted_bytes_before": counted, "forecast_including_outside_and_pilot": counted + 800_000_000 + 370_000_000,
          "rationale": "Train all 22 neural tensors from the saved 4.114394535 dB magnitude checkpoint using the evaluation-matched scale-dependent one-second SDR loss, 0.5 absent-window leakage penalty and 0.05 relative raw-head L1 anchor. Use the same 301 recorded training tracks, two-second detached warmup, one-second scored suffix, batch sixteen and same-song augmentation. Run 2000 updates at peak Adam LR 3e-5 on fresh counter-addressed crops. The preceding waveform/spectral 4000-update continuation scored 4.099680774 dB and is not the parent. This changes training supervision, not inference geometry or validation.",
...
def audit_and_score(plan_path):
    ...
    execute([PYTHON, "-u", "-m", "research.direct.evaluate_latency58_full_magnitude", "--plan", str(quality / "plan.json"),
...
def main():
    ...
    parser.add_argument("--name", default="full-magnitude-sdr-001")
    parser.add_argument("--steps", type=int, default=2000)
    parser.add_argument("--lr", type=float, default=3e-5)
    parser.add_argument("--seed", type=int, default=20261010)
    parser.add_argument("--data-start", type=int, default=2_200_000)
    ...
    resource = (plan_path.parent / "resource-stage/execution.json" if args.after_resource else
                launch(plan_path, PHASE / "full-magnitude-fast16-001/production-stage/execution.json", resource=True))

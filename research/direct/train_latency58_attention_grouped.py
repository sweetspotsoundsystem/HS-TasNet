"""Train the saved attention model with distinct backbone and attention Adam rates."""
...
def main():
    ...
    verify_inputs(stage)
    config = plan["config"]
    require(plan["schema"] == "latency58-attention-grouped-training-plan-v1" and stage["plan_sha256"] == args.plan_sha256
    ...
    from research.direct.latency58_temporal_attention import Latency58TemporalAttentionModel, ADAPTERS
    from research.direct.latency58_attention_grouped_checkpoint import (
        audit_live, save_generation, make_optimizer, set_learning_rate, scheduled_backbone_lr,
        optimizer_configuration, OPTIMIZER_SCHEMA, SCHEDULE_SCHEMA,
    )
    from research.direct.latency58_temporal_attention_context import render_scored_context
    ...
    require(plan["parent_kind"] == "saved_temporal_attention"
            and plan["optimizer_initialization"] == "fresh_adam"
            and plan["optimizer_schema"] == OPTIMIZER_SCHEMA
            and plan["optimizer_schedule"] == SCHEDULE_SCHEMA
            and type(parent) is Latency58TemporalAttentionModel
    ...
    model.provenance = {**model.provenance, "temporal_attention_current_stage_corpus": "301 recorded MUSDB18-HQ and MoisesDB training tracks",
                        "temporal_attention_current_stage_augmentation": AUGMENTATION,
                        "temporal_attention_online_teacher_used": False,
                        "temporal_attention_teacher_generated_targets_in_current_stage": False,
                        "temporal_attention_training_selection": plan["training_selection"]}
    ...
    audit_live(model, optimizer, 0, frozen, attention_lr_multiplier=plan["attention_lr_multiplier"])
    require(config["microbatch_size"] == 16 and plan["accumulation_steps"] == 1
            ...
            "Temporal-attention training geometry changed")
    if stage["resource_only"]:
        from research.direct.check_latency58_temporal_attention_gpu import check_gpu
        write(run / "temporal_attention-context-gpu-parity.json", check_gpu(model))
    ...
    with (run / "metrics.jsonl").open("x", buffering=1) as journal:
        for mixture_cpu, truth_cpu in loader:
            ...
            lr = scheduled_backbone_lr(config, step)
            ...
            for offset in range(0, config["batch_size"], config["microbatch_size"]):
                ...
                require(all(p.grad is not None for p in model.parameters()), "A temporal_attention parameter lacks a gradient")
            ...
            if step < 2:
                require(all(refinement_gradients[name] > 0 for name in ADAPTERS),
                        "Every trained attention projection must receive a nonzero gradient")
            norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 5., error_if_nonfinite=True, foreach=False)
            optimizer.step()
            torch.cuda.synchronize()
            step += 1
            row = {"step": step, "lr": lr, "attention_lr": optimizer.param_groups[1]["lr"],
                   ...
                   "temporal_attention_gradient_norms": refinement_gradients}
            if step <= 2:
                prefix.append(row)
                if rehearsal is not None:
                    require(row == rehearsal["matching_production_updates"][step - 1], "Production prefix differs from resource rehearsal")
                if step == 2:
                    audit_live(model, optimizer, step, frozen, attention_lr_multiplier=plan["attention_lr_multiplier"])
    ...
    verify_inputs(plan)
    verify_inputs(stage)
    summary = {"status": "pass", "updates": step, "optimizer_group_count": 2,
               "optimizer_configuration": optimizer_configuration(plan["attention_lr_multiplier"]),
               "final_model_state_sha256": state_sha256(model.state_dict()),

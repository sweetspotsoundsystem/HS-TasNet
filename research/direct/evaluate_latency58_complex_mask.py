"""Unchanged full14 music scoring for saved complex_mask inference checkpoints."""
...
def initialize(plan):
    ...
    import torch
    from research.direct.latency58_complex_mask_checkpoint import load_model
    from research.direct.latency58_evaluate import model_state_sha256
    torch.set_num_threads(1)
    torch.set_num_interop_threads(1)
    torch.use_deterministic_algorithms(True)
    _MODEL, _ = load_model(plan["checkpoint"])
...
def main():
    ...
    require(sha(args.plan) == args.plan_sha256, "Complex-mask scoring plan changed")
    plan = read(args.plan)
    require(plan["schema"] == "latency58-complex-mask-full14-plan-v1" and plan["track_indices"] == list(range(14))
    ...
    write(out / "result.json", {"schema": "latency58-complex-mask-full14-result-v1", "status": "pass",

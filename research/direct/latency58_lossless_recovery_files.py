...
from research.direct.latency58_lossless_recovery_codec import (
...
def production_preflight(directory, plan, serialized_bytes):
    ...
    reservation = plan["packed_storage_reservation"]
    require(reservation["retained_file_bytes"] == MAX_FILE_BYTES
            and reservation["additional_metadata_bytes"] >= 30_000_000
            and plan["storage_budget"]["live_training_save_reservation_bytes"] >= MAX_FILE_BYTES
            and 0 < serialized_bytes <= MAX_FILE_BYTES, "Unprepared packed storage reservation")
    directory = Path(directory).resolve(strict=True)
    require(any(directory.is_relative_to(Path(root).resolve(strict=True))
                for root in plan["storage_budget"]["counted_roots"]), "Packed destination is not counted")
    budget = budget_snapshot(plan["storage_budget"])
    current = directory / CURRENT
    occupied = current.stat().st_size if current.exists() else 0
    # The measured total already includes the current generation and the full
    # 600 MB transient reserve. Reserve one permanent generation at its ceiling;
    # that standing transient reserve covers the pending file during replacement.
    forecast = (budget["conservative_total_with_reservations"] - occupied + MAX_FILE_BYTES
                + reservation["additional_metadata_bytes"])
    require(forecast < budget["authorized_cap_bytes"], "Packed generation and metadata exceed the artifact cap")
    return {"budget": budget, "current_generation_bytes_already_counted": occupied,
            "forecast_with_permanent_ceiling_and_all_standing_reserves": forecast}

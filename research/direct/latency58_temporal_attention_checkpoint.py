"""Authenticated inference and optimizer snapshots for temporal_attention training."""
...
from research.direct.latency58_temporal_attention import Latency58TemporalAttentionModel, VERSION

SCHEMA = "latency58-temporal-attention-inference-v1"
GENERATION_SCHEMA = "latency58-temporal-attention-generation-v1"
...
def load_model(binding):
    ...
    require(path.is_file() and not path.is_symlink() and sha(path) == binding["sha256"],
            "Temporal-attention checkpoint bytes changed")
...
def load_payload(payload):
    ...
    require(payload["schema"] == SCHEMA and type(payload["step"]) is int and payload["step"] > 0,
            "Unknown temporal_attention checkpoint or invalid step")
    with torch.random.fork_rng(devices=[]), torch.device("cpu"):
        model = Latency58TemporalAttentionModel()
    names = [name for name, _ in model.named_parameters()]
    require(payload["architecture"] == model.architecture_metadata
            and payload["parameter_names"] == names and len(names) == 30,
            "Temporal-attention geometry or learned tensor inventory changed")
    ...
    require(set(payload["model"]) == set(model.state_dict())
            ...
            "Invalid temporal_attention inference tensors")
    ...
    require(state_sha256(dict(model.named_buffers())) == payload["fixed_buffers_sha256"]
            ...
            and provenance["temporal_attention_version"] == VERSION
            and provenance["temporal_attention_updates"] == payload["step"]
            and provenance["training_updates"] == provenance["temporal_attention_parent_updates"] + payload["step"]
            and provenance["temporal_attention_training_plan_sha256"] == payload["plan_sha256"]
            and provenance["temporal_attention_all_neural_parameters_trained"] is True,
            "Temporal-attention fixed buffers or lineage changed")
...
def audit_live(model, optimizer, step, frozen):
    ...
    require(type(model) is Latency58TemporalAttentionModel and type(step) is int and step >= 0
            and len(parameters) == 30 and all(p.requires_grad for p in parameters)
            ...
            "Require all 30 learned tensors and one ordered optimizer group")
...
def audit_resume(model, payload, resume, plan, plan_sha):
    ...
    require(set(states) == set(range(30)) and len(groups) == 1 and groups[0]["params"] == list(range(30)),

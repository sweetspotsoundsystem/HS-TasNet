...
def completed_endpoint(plan):
    ...
    from research.direct.latency58_leader_cleanup_checkpoint_v2 import read_generation, validate_recipe
    ...
    require(summary["schema"] == "latency58-leader-cleanup-quality-summary-v1" and summary["status"] == "pass"
            ...
            and set(summary["comparisons"]) == {"working", "leader", "working_cleanup"}
            and summary["comparison_variable"] == training["comparison_variable"] == "training_parent"
            and summary["matched_loss_effect_from_leader_claimed"] is False
            and summary["reference_model_states"]["leader"] == training["parent"]["model_state_sha256"]
...
def completed_supplements(plan, summary, receipt):
    ...
    require(matched["model_states"] == {"reference": summary["reference_model_states"]["working_cleanup"],
                                        "candidate": receipt["model_state_sha256"]},
            "Review does not describe the authenticated matched pair")
    require(set(plan["quiet_comparisons"]) == {"working", "leader", "working_cleanup"}, "Incomplete quiet reference scope")
    ...
    require(gain_result["schema"] == "latency58-leader-cleanup-gain-error-v1" and gain_result["status"] == "pass"
...
def completed_playback(plan, receipt):
    ...
    require(workflow["schema"] == "latency58-leader-cleanup-listening-workflow-v1"
    ...
    require(player["status"] == browser["status"] == inventory["status"] == "pass"
            and player["models"]["leader_cleanup"]["model_state_sha256"] == receipt["model_state_sha256"]
            and player["all_decoded_track_and_aggregate_scores_exact"] and player["shared_reference_samples_exact"]
            and len(inventory["files"]) == len(player["audio_files"]) == 21
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-leader-cleanup-review-plan-v1", "Unexpected review plan")
    verify_inputs(plan)
    ...
    review_fields = ("full_mixture", "per_track_stem_bands_and_absence", "probes_and_dc", "actions",
                     ...
                     "local_windows", "quiet_wanted_sources", "matched_parent_transfer", "native_actions_playback", "interpretation", "limitations")
    ...
    from research.direct.latency58_leader_cleanup_checkpoint_v2 import require_space
    counted = require_space(training, 3_000_000)
    verify_inputs(plan)
    write(out / "result.json", {
        "schema": "latency58-leader-cleanup-review-v1", "status": "pass", "plan_sha256": args.plan_sha256,
        ...
        "reference_training_plan": training["reference_training_plan"],
        ...
        "decision": "Close this pilot at its original 250-update horizon; retain its inference model for quality and deployment qualification.",

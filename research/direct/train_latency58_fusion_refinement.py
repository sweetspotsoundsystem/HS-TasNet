"""Train all 26 fusion_refinement tensors from an authenticated quadrature parent."""
...
def main():
    ...
    verify_inputs(stage)
    config = plan["config"]
    require(plan["schema"] == "latency58-fusion-refinement-training-plan-v1" and stage["plan_sha256"] == args.plan_sha256
    ...
    from research.direct.latency58_fusion_refinement import Latency58FusionRefinementModel, ADAPTERS
    from research.direct.latency58_fusion_refinement_checkpoint import audit_live, save_generation
    from research.direct.latency58_fusion_refinement_context import render_scored_context
    ...
    model.provenance = {**model.provenance, "fusion_refinement_current_stage_corpus": "301 recorded MUSDB18-HQ and MoisesDB training tracks",
                        "fusion_refinement_current_stage_augmentation": AUGMENTATION,
                        "fusion_refinement_online_teacher_used": False,
                        "fusion_refinement_teacher_generated_targets_in_current_stage": False,
                        "fusion_refinement_training_selection": plan["training_selection"]}
    ...
    require(config["microbatch_size"] == 16 and plan["accumulation_steps"] == 1
            ...
            "Fusion-refinement training geometry changed")
    if stage["resource_only"]:
        from research.direct.check_latency58_fusion_refinement_gpu import check_gpu
        write(run / "fusion_refinement-context-gpu-parity.json", check_gpu(model))
    ...
    with (run / "metrics.jsonl").open("x", buffering=1) as journal:
        for mixture_cpu, truth_cpu in loader:
            ...
            for offset in range(0, config["batch_size"], config["microbatch_size"]):
                ...
                require(all(p.grad is not None for p in model.parameters()), "A fusion_refinement parameter lacks a gradient")
            ...
            if step < 2:
                require(refinement_gradients["fusion_refine_expand.weight"] > 0
                        and (refinement_gradients["fusion_refine_reduce.weight"] == 0 if step == 0 else
                             refinement_gradients["fusion_refine_reduce.weight"] > 0),
                        "Fusion-refinement factors did not activate from zero expansion")
            ...
            row = {"step": step, "lr": lr, "first_sample_index": config["data_start"] + (step - 1) * config["batch_size"],
                   ...
                   "fusion_refinement_gradient_norms": refinement_gradients}
            if step <= 2:
                prefix.append(row)

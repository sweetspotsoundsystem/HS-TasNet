...
def validate_selection(plan):
    ...
    require(summary["schema"] in ("latency58-counterfactual-quality-summary-v1",
                                  "latency58-controlled-deployed-quality-summary-v1",
                                  "latency58-leader-cleanup-quality-summary-v1")
            and summary["status"] == "pass" and summary["source_bindings_unchanged"]
            and summary["all_metrics_compared"]
            and summary["training_plan"] == primary_plan["training_plan"]
            and summary_execution["plan_sha256"] == summary["plan_sha256"],
            "Selected summary does not authenticate the completed source-view quality bundle")
    summary_plan = Path(selection["primary_summary"]["path"]).parent / "plan.json"
    require(plan["source_bindings"].get(str(summary_plan)) == summary["plan_sha256"] == sha(summary_plan),
            "Selected summary plan is changed or unbound")
    from research.direct.audit_latency58_counterfactual_training_match import load_completed_match as load_teacher_match
    from research.direct.audit_latency58_controlled_deployed_training_match import load_completed_match as load_deployed_match
    from research.direct.audit_latency58_leader_cleanup_training_match import load_completed_match as load_leader_match
    matching_evidence = {}
    loader = {"latency58-counterfactual-quality-summary-v1": load_teacher_match,
              "latency58-controlled-deployed-quality-summary-v1": load_deployed_match,
              "latency58-leader-cleanup-quality-summary-v1": load_leader_match}[summary["schema"]]
    matched = loader(summary["training_match"], matching_evidence)
    require(matched["training_plans"]["candidate"] == summary["training_plan"]
            and matched["model_states"]["candidate"] == summary["model_state_sha256"]
            and all(plan["source_bindings"].get(p) == v for p, v in matching_evidence.items()),
            "Selected endpoint lacks bound full training pairing")

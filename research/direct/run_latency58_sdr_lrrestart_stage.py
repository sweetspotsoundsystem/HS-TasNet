"""Run a monitored resource rehearsal or an audited higher-LR context stage."""
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-sdr-lrrestart-training-v1", "Unknown training plan")
    from research.direct.latency58_sdr_lrrestart_checkpoint import read_generation, require_space
    ...
    if plan["resource_only"]:
        ...
        require(resource["schema"] == "latency58-sdr-lrrestart-resource-result-v1" and resource["status"] == "pass"
    ...
    if start:
        ...
        require(decision["schema"] == "latency58-sdr-lrrestart-continuation-v1"
    ...
    stage = {"schema": "latency58-sdr-lrrestart-stage-v1", "plan_sha256": args.plan_sha256,
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_sdr_lrrestart", "--plan", str(args.plan.resolve()),
    ...
    if plan["resource_only"]:
        ...
        write(out / "resource-qualification.json", {
            "schema": "latency58-sdr-lrrestart-resource-qualification-v1", "status": "pass",
    ...
    argv = [PYTHON, "-u", "-m", "research.direct.audit_latency58_sdr_lrrestart", "--plan", str(args.plan.resolve()),
    ...
    if previous_generation is not None:
        ...
        retirement = {"schema": "latency58-sdr-lrrestart-replaced-optimizer-v1", "path": str(old_optimizer),

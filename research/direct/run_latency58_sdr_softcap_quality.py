...
def main():
    ...
    require(training["schema"] == "latency58-sdr-softcap-training-v1", "Require a soft-capped SDR trial plan")
    ...
    paths = [audit_path, audit_exec_path, stage / "execution.json", stage / "stage.json", monitor_path,
             ...
             Path(__file__).resolve(), ROOT / "research/direct/evaluate_latency58_sdr_softcap.py",
             ROOT / "research/direct/compare_latency58_sdr.py", ROOT / "research/direct/compare.py"]
    if "probes" in args.modes:
        paths.append(ROOT / "research/direct/evaluate_latency58_sdr_softcap_probes.py")
    if "full14" in args.modes:
        ...
        paths += [proof_path, ROOT / "research/direct/evaluate_latency58_sdr_softcap_parallel.py"]
    ...
    for mode in args.modes:
        ...
        plan.update(schema="latency58-sdr-softcap-" + ("probe" if mode == "probes" else "music") + "-plan-v1",
        ...
        if mode == "full14":
            plan.update(schema="latency58-sdr-softcap-parallel-music-plan-v1", workers=2,
                        model_kind="sdr_softcap_candidate", track_indices=list(range(14)),
    ...
    for mode, out, plan in prepared:
        ...
        module = "research.direct.evaluate_latency58_sdr_softcap" + ("_probes" if mode == "probes" else "_parallel" if mode == "full14" else "")

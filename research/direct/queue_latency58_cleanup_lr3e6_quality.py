...
from research.direct.report_latency58_cleanup_lr3e6 import REFERENCES, load_views, reference_views
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-cleanup-lr3e6-queued-quality-plan-v1"
    ...
    require(prefix == "cleanup-lr3e6-250" and training["additional_loss_weight"] == .5, "Different quality prefix")
    waited = 0.0
    ...
    if not ((stage / "execution.json").is_file() and (stage / "audit-execution.json").is_file()):
        ...
        try:
            ...
            require(int(stat[19]) == root_identity["start_ticks"] and command == root_identity["argv"]
                    and command[command.index("-m") + 1] == "research.direct.run_latency58_cleanup_lr3e6"
    ...
    # of plugin timing. The final summary separately verifies the matched learning-rate journal.
    commands = [
        [PYTHON, "-u", "-m", "research.direct.run_latency58_cleanup_lr3e6_quality",
        ...
        [PYTHON, "-u", "-m", "research.direct.run_latency58_cleanup_lr3e6_views", "--prefix", prefix,
    ...
    summary_plan = {"schema": "latency58-cleanup-lr3e6-summary-plan-v1", "candidate_prefix": prefix,
    ...
    execute([PYTHON, "-u", "-m", "research.direct.report_latency58_cleanup_lr3e6",
    ...
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-cleanup-lr3e6-queued-quality-result-v1", "status": "pass",

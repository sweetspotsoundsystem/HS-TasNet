"""Reserve and bind quality work to the currently running selective supervisor."""
...
from research.direct.report_latency58_counterfactual import REFERENCES, reference_views, load_views
...
def main():
    ...
    prefix = "counterfactual-teacher-ordinary-only-250"
    stage = PHASE / "counterfactual-teacher-ordinary-only-to-000250-001"
    control_stage = PHASE / "vocal-focus-focused-to-000250-001"
    ...
    require(argv[argv.index("-m") + 1] == "research.direct.run_latency58_counterfactual_stage"
    ...
    for suffix in ("", "_parallel", "_probes", "_views"):
        old = ROOT / ("research/direct/evaluate_latency58_vocal_focus" + suffix + ".py")
        new = ROOT / ("research/direct/evaluate_latency58_counterfactual" + suffix + ".py")
        require(old.read_text().replace("vocal_focus", "counterfactual").replace("vocal-focus", "counterfactual")
                == new.read_text(), "Evaluation changed beyond checkpoint loading and labels")
        evidence.update({str(p): sha(p) for p in (old, new)})
    modules = ("audit_latency58_counterfactual_training_match", "run_latency58_counterfactual_quality",
               "run_latency58_counterfactual_views", "report_latency58_counterfactual",
               "queue_latency58_counterfactual_quality", "prepare_latency58_counterfactual_quality",
    ...
    for label, reference_prefix in REFERENCES.items():
        ...
        for mode in ("full14", "actions60", "probes"):
            ...
            if label == "focused_control":
                require(quality_plan["training_plan"] == training["matched_control_training_plan"],
    ...
    before = require_space(training, 380_000_000)
    reservation = {"schema": "latency58-counterfactual-views-reservation-v1",
    ...
    queue_plan = {"schema": "latency58-counterfactual-queued-quality-plan-v1", "quality_prefix": prefix,
    ...
    write(out / "result.json", {"schema": "latency58-counterfactual-quality-preparation-v1", "status": "pass",
          ...
          "counted_bytes_before": before, "counted_bytes_after": require_space(training, 380_000_000)})

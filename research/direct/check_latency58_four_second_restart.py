"""Qualify B2 weighted updates and exact packed disk restart for the 2000-step plan.
    ...
    try:
        with patch("torch.nn.utils.clip_grad_norm_", wraps=torch.nn.utils.clip_grad_norm_) as clips, \
            ...
            require(result["raw_model_state_sha256"] != before["model"]
                    ...
                    and len(result["groups"]["ordinary"]["microbatches"]) == 8
...
def run_fixture(parent, plan, out, progress):
    ...
    fixture = {'schema': 'latency58-four-second-proposal-short-cpu-restart-fixture-v1',
        ...
        **{key: decision[key] for key in ('parent_checkpoint', 'parent_model_state_sha256',
        ...
        'precision_policy': 'CPU FP32 synthetic weighted B2 restart fixture',
    ...
    require(fixture['config']['steps'] == 2000, 'Retain the selected complete training schedule')
    ...
    reject(rejected, 'noncontiguous_step', lambda: grouped_update(model, optimizer, ema, *inputs,
        step=2, warmup_samples=512, ordinary_microbatch=2, auxiliary_microbatch=2))
    ...
    require(continuous == replay and fingerprint(model, optimizer, ema) == fingerprint(restored, adam, average),
            'Weighted B2 update or raw/Adam/EMA endpoint differs after disk restart')
    ...
    return {'status': 'pass', 'parent_model_state_sha256': parent_sha, 'ordinary_microbatch': 2,
        'auxiliary_microbatch': 2, 'logical_batch_size': 16, 'warmup_samples': 512, 'scored_samples': 44160,
        ...
        'all_tensor_and_metadata_bytes_exact': True, 'third_weighted_b2_update_and_accounting_bit_exact': True,

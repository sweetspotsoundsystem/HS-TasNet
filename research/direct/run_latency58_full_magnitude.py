"""Train all layers with phase-invariant magnitude features on 301 recorded tracks."""
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_full_magnitude", "--plan", str(plan_path),
...
def prepare(args):
    ...
    functional = {"objective": check(), "full_training": check_full_training()}
    ...
    paths = [Path(__file__).resolve(), ROOT / "research/direct/latency58_direct_sdr.py",
             ROOT / "research/direct/latency58_full_magnitude_checkpoint.py", ROOT / "research/direct/train_latency58_full_magnitude.py",
    ...
    config = {**template["config"], "steps": args.steps, "seed": args.seed, "data_start": args.data_start,
              "crop_samples": 176384, "lr": args.lr, "min_lr": args.lr / 10, "warmup": 50,
    ...
    write(out / "preparation.json", {"status": "pass", "parent_model_state_sha256": plan["parent_model_state_sha256"],
          ...
          "rationale": "Train all 22 neural tensors with a zero-initialized magnitude projection from the preserved best checkpoint, on 301 recorded training tracks; half original mixtures and half same-song source gains within +/-3 dB with shared polarity. Magnitude features add no audio buffering; native runtime qualification remains pending quality success.",
...
def main():
    ...
    parser.add_argument("--name", default="full-magnitude-001")
    parser.add_argument("--steps", type=int, default=2000)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--seed", type=int, default=20261007)
    parser.add_argument("--data-start", type=int, default=2_000_000)
    ...
    if args.prepare_only:
        ...
        return
    resource = launch(plan_path, PHASE / "wave-spectral-001/production-stage/execution.json", resource=True)

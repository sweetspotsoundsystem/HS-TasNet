"""One bounded stage of a matched GPU SDR recovery fine-tune."""
...
def main():
    ...
    require(plan["schema"] == "latency58-sdr-training-v1" and stage["schema"] == "latency58-sdr-stage-v1"
            and stage["plan_sha256"] == args.plan_sha256 and 0 <= start < stop <= config["steps"] == 1000
            and stop in (2, 25, 250, 500, 1000) and config["batch_size"] == 4 and config["workers"] == 2
            and config["crop_samples"] == 88064 and config["precision"] == "bf16"
    ...
    from research.direct.latency58_sdr_checkpoint import load_model, read_generation, require_space, save_generation
    ...
    from research.direct.latency58_sdr_teacher import STUDENT_STATE_SHA256, load_initial_student, load_teacher, physical_targets
    ...
    if start == 0:
        ...
        require(state_sha256(model.state_dict()) == STUDENT_STATE_SHA256, "Fresh student differs")
    else:
        ...
        helpers.validate_journal(journal, start, config)
    ...
    try:
        for mixture, targets in loader:
            ...
            teacher_targets = physical_targets(teacher, mixture, kind=plan["teacher_kind"])
            require(torch.equal(rng, torch.cuda.get_rng_state()), "Teacher changed matched augmentation RNG")
            output = model.render(torch.nn.functional.pad(mixture, (0, 128)))
            require(torch.equal(output.delayed_mixture[..., 128:128 + config["crop_samples"]], mixture),
                    "Training lost physical audio alignment")
            raw, deployed = (v[..., 128:128 + config["crop_samples"]] for v in (output.raw, output.deployed))
            ...
            row = {"step": step, "lr": lr, "loss": float(loss.detach()), "supervised_loss": float(terms.total.detach()),
                   ...
                   "data_hops": 688, "flush_hops": 1, "stage_start_step": start,

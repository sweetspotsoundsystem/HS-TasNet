"""Prepare a complex-mask trial only after the quadrature continuation is reviewed."""
...
def prepare(args):
    ...
    from research.direct.latency58_complex_mask import Latency58ComplexMaskModel, ComplexMaskState
    ...
    require(Path.cwd() == ROOT and os.environ.get("CUDA_VISIBLE_DEVICES") == ""
            ...
            and 0 < args.warmup < args.steps and args.data_start >= 2_964_000
            and args.seed > 20261014 and bool(args.rationale.strip()), "Invalid CPU preparation or recipe")
    torch.set_num_threads(1)
    torch.set_num_interop_threads(1)
    torch.use_deterministic_algorithms(True)
    previous_root = PHASE / "quadrature-continuation-001"
    ...
    require(terminal["status"] == "training_audit_and_full14_complete"
            ...
            "The quadrature continuation must finish scoring and receive a recorded review")
    ...
    functional_paths = (PHASE / "complex-mask-functional-001/result.json",
                        PHASE / "complex-mask-checkpoint-memory-functional-001/result.json")
    ...
    require(functional["status"] == checkpoint_check["status"] == "pass"
            ...
            and not checkpoint_check["disk_generation_path_exercised"], "Complex-mask CPU prerequisites failed")
    prerequisite_paths = []
    for root, stage in (("complex-mask-functional-001", "complex-mask-functional-stage-001"),
                        ("complex-mask-checkpoint-memory-functional-001", "complex-mask-checkpoint-memory-functional-stage-001")):
    ...
    with torch.inference_mode():
        audio = .03 * torch.randn(2, 2, 19 * 128, generator=torch.Generator().manual_seed(202609134))
        state = parent.initial_state(2)
        first, second = parent.render(audio, state), model.render(audio, ComplexMaskState(*state))
        require(all(torch.equal(getattr(first, key).view(torch.int32), getattr(second, key).view(torch.int32))
                ...
                "Zero amplitude initialization changed the selected parent")
    out = PHASE / args.name
    ...
    paths.extend(ROOT / "research/direct" / name for name in (
        "latency58_complex_mask.py", "latency58_complex_mask_context.py", "latency58_complex_mask_checkpoint.py",
        "check_latency58_complex_mask.py", "check_latency58_complex_mask_gpu.py",
        "check_latency58_complex_mask_checkpoint_memory.py", "train_latency58_complex_mask.py",
        "run_latency58_complex_mask.py", "evaluate_latency58_complex_mask.py"))
    ...
    plan = {**source, "schema": "latency58-complex-mask-training-plan-v1", "name": args.name,
            "output_directory": str(out), "config": config,
            "source_bindings": {**source["source_bindings"], **{str(p): sha(p) for p in paths}},
            ...
            "training_context_implementation": "complex-mask final-frame warmup and physically aligned scored render",
            "cpu_fast_context_bit_exact": True, "gpu_fast_context_bit_exact_required_before_updates": True,
            "complex_mask_gpu_fixture_requires_exact_original_weight_restoration": True,
    ...
    write(out / "preparation.json", {"status": "pass", "parent_full_sdr_db": best_score,
          "previous_continuation_full_sdr_db": terminal["full_sdr_db"], "target_full_sdr_db": 5.0,
          ...
          "additional_parameters": model.architecture_metadata["amplitude_head_parameters"],
          ...
          "cuda_initialized": False, "quality_measured_for_complex_mask": False,

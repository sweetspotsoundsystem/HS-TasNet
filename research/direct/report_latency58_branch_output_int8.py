"""Review completed sixteen-projection quality, vocal views and native cost.
...
RELEASED = "878c74694fa4c558de1c5a75837893a0afeadcf57f6e3b860d5904cab04e9fc9"
CANDIDATE = "c7ea50ac67bf4bfddf1f5ff41c6eb419af00fe420ce1a0b0eaeef11a1861cd61"
STAGES = {"full_reference": "branch-gru-int8-quality-001",
          "vocal_reference": "branch-gru-int8-quality-001",
          "candidate": "branch-output-int8-quality-001",
          "native": "branch-output-int8-native-pair-001"}
...
def missing_inputs():
    return [str(PHASE / stage / name) for stage in STAGES.values()
            for name in ("plan.json", "result.json", "execution.json")
            if not (PHASE / stage / name).is_file()]


def load_completed(stage, bindings):
    paths = [PHASE / stage / name for name in ("plan.json", "result.json", "execution.json")]
...
def main():
    ...
    parser.add_argument("--output-prefix", default="branch-output-int8-review-001")
    ...
    require_cpu()
    load_correction_path = PHASE / "branch-output-int8-native-pair-001/load-scope-correction.json"
    load_correction = read(load_correction_path)
    require(load_correction["timings_unchanged"], "Unexpected native timing correction")
    paths = [load_correction_path, Path(__file__).resolve(), ROOT / "research/evaluate.py", ROOT / "research/metrics.py"]
    ...
    require_close(full, changed["parent_fourteen_projection_comparison"], "stored parent comparison")
    require_close(cells, changed["parent_fourteen_projection_all_track_stem_cells"], "stored parent cells")
    ...
    budget_plan = {**cplan, "diagnostic_artifact_allowance_bytes": 250_000_000}
    before = budget_snapshot(budget_plan)
    verify_inputs({"source_bindings": bindings})
    out.mkdir()
    plan = {"schema": "latency58-branch-output-int8-review-plan-v1", "source_bindings": bindings,
            "input_stages": STAGES, "budget_before": before, "quality_selection": False,
            "output_allowance_bytes": 5_000_000}
    write(out / "plan.json", plan)
    result = {"schema": "latency58-branch-output-int8-review-v1", "status": "pass",
        ...
        "native_cost": {**{k: native[k] for k in ("median_of_block_p50_ms", "candidate_over_released_ratio",
            "system", "cpu", "background_load_declared", "limitations")}, "load_scope_correction": load_correction},
        ...
        "budget_before": before, "budget_after": budget_snapshot(budget_plan),

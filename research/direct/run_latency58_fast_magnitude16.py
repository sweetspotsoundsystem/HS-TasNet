"""Continue magnitude training with a faster microbatch and detached warmup path."""
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_fast_magnitude16", "--plan", str(plan_path),
...
def prepare(args):
    ...
    paths = [Path(__file__).resolve(), ROOT / "research/direct/latency58_direct_sdr.py",
             ROOT / "research/direct/latency58_full_magnitude_checkpoint.py", ROOT / "research/direct/train_latency58_fast_magnitude16.py",
    ...
    paths.append(PHASE / "rehearsal-storage-001/receipt.json")
    ...
    write(out / "preparation.json", {"status": "pass", "parent_model_state_sha256": plan["parent_model_state_sha256"],
          ...
          "rationale": "Continue all 22 neural tensors from the saved 4.114394535 dB checkpoint on the same 301 recorded training tracks. Keep two seconds of detached warmup; score one second per crop with one batch of sixteen and one backward pass. Omit unused warmup decoder frames, with exact CPU and required GPU scored-waveform/gradient checks. Four thousand updates expose approximately the same scored duration as the previous two thousand. Inference geometry and validation remain unchanged.",
...
def main():
    ...
    parser.add_argument("--name", default="full-magnitude-fast16-001")
    parser.add_argument("--steps", type=int, default=4000)
    parser.add_argument("--lr", type=float, default=6e-5)
    parser.add_argument("--seed", type=int, default=20261009)
    parser.add_argument("--data-start", type=int, default=2_100_000)
    ...
    resource = (plan_path.parent / "resource-stage/execution.json" if args.after_resource else
                launch(plan_path, PHASE / "full-magnitude-fast-001/resource-stage/execution.json", resource=True))

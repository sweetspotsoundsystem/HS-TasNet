param(
    ...
    [ValidateSet("ordinary_only")][string]$LastArm = "ordinary_only",
    [int]$DebugPort = 9234
)
...
(async () => {
  ...
  const allIds = ['working', 'focused_control', 'ordinary_only'];
  ...
  button(lastArm).click(); button(priorId).click(); button('ordinary_only').click(); button('working').click();
...
'@
    ...
    $report = @{schema="latency58-counterfactual-listening-browser-check-v1"; status="pass";
    ...
    $report = @{schema="latency58-counterfactual-listening-browser-check-v1"; status="failed";

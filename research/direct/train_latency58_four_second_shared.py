...
def runtime_policy():
    return {"version": "four-second-shared-b16-b2-weighted-packed-save-runtime-v1", "resource_max_seconds": 1800,
            "production_seconds_per_update": 100, "production_fixed_allowance_seconds": 600,
            "save_seconds_per_generation": 60, "progress_timeout_seconds": 240,
            "finalization_timeout_seconds": 180, "resource_startup_grace_seconds": 900,
            "production_startup_grace_seconds": 240}
...
def validate_recipe(plan):
    ...
    require(sha(decision_binding["path"]) == decision_binding["sha256"]
            == "4f4f32d9ad3c97ba208a72e78504df664d3ee2271f5f043a1dd690ccce4c0140",
    ...
    require(config["batch_size"] == 16 and config["microbatch_size"] == 16
            and config["auxiliary_microbatch_size"] == 2 and plan["accumulation_steps"] == 1
...
def main():
    ...
    require(run == Path(plan["output_directory"]) / ("resource-run-005" if stage["resource_only"] else "production-run")
    ...
    model.provenance = {**model.provenance, "branch_memory_current_stage_corpus": "301 recorded MUSDB18-HQ and MoisesDB training tracks",
                        ...
                        "branch_memory_current_stage_training_context": {"warmup_samples": plan["warmup_samples"],
                            "scored_samples": plan["scored_samples"], "logical_batch_size": 16, "microbatch_size": 16,
    ...
    if stage["resource_only"]:
        from research.direct.latency58_four_second_shared_qualification import require_gpu_evidence
        qualified = require_gpu_evidence(plan)
        require(qualified["parent_model_state_sha256"] == state_sha256(model.state_dict()),
                "Current resource GPU parent differs from the completed geometry qualification")
        write(run / "grouped-vocal-gpu-parity.json", qualified)

"""Prepare fused-feature refinement from the selected fusion-refinement checkpoint."""
...
def prepare(args):
    import torch
    from research.direct.latency58_fusion_refinement_checkpoint import load_model, audit_saved
    from research.direct.latency58_temporal_attention import Latency58TemporalAttentionModel, TemporalAttentionState
    ...
    require(Path.cwd() == ROOT and os.environ.get("CUDA_VISIBLE_DEVICES") == ""
            ...
            and args.seed > 20261016 and bool(args.rationale.strip()), "Invalid CPU preparation or recipe")
    torch.set_num_threads(1)
    torch.set_num_interop_threads(1)
    torch.use_deterministic_algorithms(True)
    previous_root = PHASE / "fusion-refinement-001"
    ...
    require(terminal["status"] == "training_audit_and_full14_complete"
            ...
            and review["status"] == "selected_for_research",
            "The fusion-refinement trial must finish scoring and receive its selection review")
    ...
    require(reference["status"] == "pass" and reference["track_count"] == 14
            ...
            "Use the best saved fusion-refinement parent while the quality target remains unmet")
    ...
    require(parent_audit["status"] == "pass" and parent_audit["saved_optimizer_tensor_count"] == 26
            and payload["plan_sha256"] == sha(parent_plan_path), "Selected parent generation audit failed")
    functional_paths = (PHASE / "temporal-attention-functional-001/result.json",
                        PHASE / "temporal-attention-checkpoint-memory-functional-001/result.json")
    ...
    require(functional["status"] == checkpoint_check["status"] == "pass"
            ...
            and len(functional["all_30_neural_tensors_updated"]) == 30
            and checkpoint_check["all_30_optimizer_states_checked"]
            ...
            and not checkpoint_check["disk_generation_path_exercised"], "Temporal-attention CPU prerequisites failed")
    prerequisite_paths = []
    prerequisite_bindings = {}
    for root, stage in (("temporal-attention-functional-001", "temporal-attention-functional-stage-001"),
                        ("temporal-attention-checkpoint-memory-functional-001", "temporal-attention-checkpoint-memory-functional-stage-001")):
    ...
    require(read(PHASE / "temporal-attention-functional-001/plan.json")["parent_checkpoint"] == parent_binding
    ...
    with torch.inference_mode():
        audio = .03 * torch.randn(2, 2, 19 * 128, generator=torch.Generator().manual_seed(202609148))
        state = parent.initial_state(2)
        first, second = parent.render(audio, state), model.render(audio, TemporalAttentionState(*state, *model.initial_state(2)[4:]))
        require(all(torch.equal(getattr(first, key).view(torch.int32), getattr(second, key).view(torch.int32))
                ...
                and all(torch.equal(a.view(torch.int32), b.view(torch.int32))
                        for a, b in zip(first.state, second.state[:4], strict=True)),
    ...
    paths.extend(ROOT / "research/direct" / name for name in (
        "latency58_temporal_attention.py", "latency58_temporal_attention_context.py", "latency58_temporal_attention_checkpoint.py",
        "check_latency58_temporal_attention.py", "check_latency58_temporal_attention_gpu.py",
        "check_latency58_temporal_attention_checkpoint_memory.py", "train_latency58_temporal_attention.py",
    ...
    plan = {**source, "schema": "latency58-temporal-attention-training-plan-v1", "name": args.name,
            ...
            "parent_kind": "saved_fusion_refinement", "optimizer_initialization": "fresh_adam",
            ...
            "quality_endpoints": [args.steps], "all_neural_parameter_tensors_trained": 30,
            "training_context_implementation": "temporal-attention final-frame warmup and physically aligned scored render",
            "cpu_fast_context_bit_exact": True, "gpu_fast_context_bit_exact_required_before_updates": True,
            "temporal_attention_gpu_fixture_requires_exact_original_weight_restoration": True,
    ...
    plan.pop("fusion_refinement_gpu_fixture_requires_exact_original_weight_restoration", None)
    ...
    write(out / "preparation.json", {"status": "pass", "parent_full_sdr_db": best_score,
          "previous_fusion_refinement_full_sdr_db": terminal["full_sdr_db"], "target_full_sdr_db": 5.0,
          ...
          "additional_parameters": model.architecture_metadata["temporal_attention_parameters"],
          "additional_state_tensors": 2, "additional_audio_buffering_samples": 0,
          "rationale": args.rationale, "counted_bytes_before": counted,
          "forecast_including_outside_and_run": counted + 800_000_000 + 390_000_000,
          "cuda_initialized": False, "quality_measured_for_temporal_attention": False,

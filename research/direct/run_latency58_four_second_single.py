...
from research.direct.train_latency58_four_second_single import validate_recipe, runtime_policy, budget_snapshot as storage_snapshot
...
def require_cpu_evidence(plan):
    entries = ((ARTIFACT_ROOT / "single-model-cpu-001", "four_second_model_cpu_qualification"),
    ...
    require(model["fixture_model_state_sha256"] == plan["parent_model_state_sha256"]
            ...
            and model["ordinary_microbatch"] == gradients["ordinary_microbatch"] == 1
    ...
    require(len(model["auxiliary_context_comparisons"]) == 2 and all(
        context["status"] == "pass" and context["scored_samples"] == 176512
        and len(context["all_40_gradients"]) == 40
        and all(row["maximum_error"] == 0 and row["reference_norm"] > 0
                for row in context["all_40_gradients"].values())
        for context in (model["context_comparison"], *model["auxiliary_context_comparisons"])),
        "B1 ordinary or individual auxiliary CPU context differs")
    ...
    require(recovered["status"] == "pass" and recovered["parent_model_state_sha256"] == plan["parent_model_state_sha256"]
            and recovered["ordinary_microbatch"] == recovered["auxiliary_microbatch"] == 1
            ...
            and recovered["third_weighted_b1_update_and_accounting_bit_exact"]
            ...
            "Selected B1 weighted update or new packed disk recovery is not qualified")
...
def require_base_resource_result(result, parity, plan, plan_sha):
    ...
    ordinary, auxiliary = parity["ordinary_context"], parity["auxiliary_contexts"]
    require(set(auxiliary) == {"instrumental", "vocals_only"}, "Require both complete auxiliary context checks")
    for check in (ordinary, *auxiliary.values()):
        require(check["status"] == "pass" and check["scored_samples"] == 176512 and check["microbatch_size"] == 1
    ...
    require(ordinary["microbatch_size"] == 1 and ordinary["logical_batch_loss"]["status"] == "pass",
            "Ordinary B16/B1 loss qualification failed")
    ...
    require(gradients["status"] == "pass" and gradients["device"].startswith("cuda")
            ...
            and gradients["ordinary_microbatch"] == 1 and gradients["auxiliary_microbatch"] == 1
    ...
    require(restart["status"] == "pass" and restart["device"].startswith("cuda") and restart["precision"] == "bf16"
            ...
            and restart["ordinary_microbatch"] == restart["auxiliary_microbatch"] == 1
...
def require_resource_result(result, parity, plan, plan_sha):
    ...
    recovered = read(Path(plan["output_directory"]) / "resource-run-004/recovery-gpu-parity.json")
    ...
    allocator = read(root / "resource-run-004/allocator-after-qualification.json")
    ...
    rows = [json.loads(line) for line in (root / "resource-run-004/metrics.jsonl").read_text().splitlines()]
    ...
    require(len(rows) == 2 and slowest_update < runtime_policy()["production_seconds_per_update"]
            ...
            "Measured B1 update and checkpoint save exceed the prepared production allowances")


def require_failed_resource_closed(execution, monitor):
    require(execution["actual_exit_code"] == 1 and execution["source_bindings_unchanged"]
            and monitor["status"] == "child_failed" and monitor["child_exit_code"] == 1
            and monitor["supervisor_health"] == "pass" and monitor["post_exit_quiet_completed"]
            and monitor["identities_unchanged"] and monitor["source_sha256"] == monitor["source_sha256_after"] == WATCHDOG_SHA
            and monitor["latest_completed_step_seen"] is None and monitor["expected_final_step"] == 2
            and not Path("/proc", str(monitor["child_pid"])).exists(), "OOM resource did not close with healthy telemetry")
    for key in ("event_worker_close", "gpu_worker_close"):
        value = monitor[key]
        require(value["closed"] and value["actual_exit_code"] == 0 and not value["forced"]
                and not Path("/proc", str(value["linux_pid"])).exists(), "OOM telemetry worker remains live")
    require(monitor["gpu_worker_close"]["identities_unchanged"], "OOM telemetry identity changed")
...
def launch(plan_path, previous_execution_path, *, resource):
    ...
    require(previous_execution_path == (Path(plan["previous_execution_for_resource"]) if resource
                                        else root / "resource-stage-004/execution.json"), "Wrong GPU continuity predecessor")
    if not resource:
        enclosing = read(root / "resource-root-execution-004.json")
        require(type(enclosing["actual_exit_code"]) is int and enclosing["actual_exit_code"] == 0
                ...
                and enclosing["stage_execution_sha256"] == sha(root / "resource-stage-004/execution.json")
                and enclosing["result_sha256"] == sha(root / "resource-run-004/result.json"),
    ...
    (require_failed_resource_closed if resource else require_monitor_closed)(previous_execution, previous)
    name = "resource" if resource else "production"
    out, run = root / (name + ("-stage-004" if resource else "-stage")), root / (name + ("-run-004" if resource else "-run"))
    ...
    if not resource:
        result_path, parity_path = root / "resource-run-004/result.json", root / "resource-run-004/grouped-vocal-gpu-parity.json"
        require_resource_result(read(result_path), read(parity_path), plan, sha(plan_path))
        stage["resource_result"] = binding(result_path)
        for path in (result_path, parity_path, root / "resource-run-004/recovery-gpu-parity.json"):
    ...
    spec = {"schema": "gpu-watchdog-launch-finalization-v1", "expected_final_step": stop,
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_four_second_single", "--plan", str(plan_path),
                     "--plan-sha256", sha(plan_path), "--stage", str(out / "stage.json"),
                     "--stage-sha256", sha(out / "stage.json")]}
    write(out / "watchdog-spec.json", spec)
    monitor_out = ARTIFACT_ROOT / "monitors" / (root.name + "-" + name + ("-004" if resource else ""))

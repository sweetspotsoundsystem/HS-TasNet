"""Measure long integer-vs-Torch drift without changing qualification tolerances."""
import hashlib
import json
import os
...
def main():
    ...
    reference, _ = make_reference(native, onnx.load(graph_binding["path"], load_external_data=False))
    session = session_for(Path(graph_binding["path"]).read_bytes())
    ...
    cases = [next(verification_cases(1024, [])),
             {"name": "recorded_training_music_30_seconds", "audio": np.ascontiguousarray(music.T), "initial_states": _initial_states()}]
    out = PHASE / "m4-int8-long-reference-001"
    require(not out.exists(), "Preserve long diagnostics")
    out.mkdir()
    paths = [source_path, Path(__file__).resolve(), ROOT / "research/direct/latency58_int8_reference.py",
             Path(graph_binding["path"]), music_path]
    bindings = {**source["source_bindings"], **{str(path): sha(path) for path in paths}}
    write(out / "plan.json", {"source_bindings": bindings, "graph": graph_binding, "ort_version": ort.__version__,

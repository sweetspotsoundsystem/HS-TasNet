"""Authenticated inference and optimizer snapshots for complex_mask training."""
...
from research.direct.latency58_complex_mask import Latency58ComplexMaskModel, VERSION

SCHEMA = "latency58-complex-mask-inference-v1"
GENERATION_SCHEMA = "latency58-complex-mask-generation-v1"
...
def load_model(binding):
    ...
    require(path.is_file() and not path.is_symlink() and sha(path) == binding["sha256"],
            "Complex-mask checkpoint bytes changed")
...
def load_payload(payload):
    ...
    require(payload["schema"] == SCHEMA and type(payload["step"]) is int and payload["step"] > 0,
            "Unknown complex_mask checkpoint or invalid step")
    with torch.random.fork_rng(devices=[]), torch.device("cpu"):
        model = Latency58ComplexMaskModel()
    names = [name for name, _ in model.named_parameters()]
    require(payload["architecture"] == model.architecture_metadata
            and payload["parameter_names"] == names and len(names) == 26,
            "Complex-mask geometry or learned tensor inventory changed")
    ...
    require(set(payload["model"]) == set(model.state_dict())
            ...
            "Invalid complex_mask inference tensors")
    ...
    require(state_sha256(dict(model.named_buffers())) == payload["fixed_buffers_sha256"]
            ...
            and provenance["complex_mask_version"] == VERSION
            and provenance["complex_mask_updates"] == payload["step"]
            and provenance["training_updates"] == provenance["complex_mask_parent_updates"] + payload["step"]
            and provenance["complex_mask_training_plan_sha256"] == payload["plan_sha256"]
            and provenance["complex_mask_all_neural_parameters_trained"] is True,
            "Complex-mask fixed buffers or lineage changed")
...
def audit_live(model, optimizer, step, frozen):
    ...
    require(type(model) is Latency58ComplexMaskModel and type(step) is int and step >= 0
...
def make_payloads(model, optimizer, step, plan, plan_sha):
    ...
    require(type(step) is int and step > 0 and step == plan["config"]["steps"]
            ...
            and model.provenance["complex_mask_parent_model_state_sha256"] == plan["parent_model_state_sha256"],
            "Save only the declared complex_mask endpoint with unchanged fixed buffers")
    audit_live(model, optimizer, step, dict(model.named_buffers()))
    tensors = cpu_tree(model.state_dict())
    names = [name for name, _ in model.named_parameters()]
    provenance = {**model.provenance, "complex_mask_version": VERSION,
                  "complex_mask_parent_checkpoint": plan["parent_checkpoint"],
                  "complex_mask_parent_model_state_sha256": plan["parent_model_state_sha256"],
                  "complex_mask_parent_updates": plan["parent_training_updates"],
                  "complex_mask_updates": step, "training_updates": plan["parent_training_updates"] + step,
                  "complex_mask_all_neural_parameters_trained": True,
                  "complex_mask_training_plan_sha256": plan_sha,
                  "complex_mask_objective_version": plan["objective_version"],
...
def save_generation(model, optimizer, step, plan, plan_sha, run):
    ...
    require(not pending.exists() and not final.exists(), "Preserve existing complex-mask checkpoints")
    pending.mkdir()
...
def audit_saved(binding, plan, plan_sha):
    ...
    require(receipt["schema"] == GENERATION_SCHEMA and set(receipt["files"]) == {"model.pt", "optimizer.pt"}
            ...
            and payload["provenance"]["complex_mask_parent_checkpoint"] == plan["parent_checkpoint"]
            and payload["provenance"]["complex_mask_parent_model_state_sha256"] == plan["parent_model_state_sha256"]
            and payload["provenance"]["complex_mask_parent_updates"] == plan["parent_training_updates"],
            "Saved complex_mask endpoint or parent differs from its plan")
    ...
    with torch.inference_mode():
        ...
        require(closure < 1e-6 and model.algorithmic_latency_samples == 256,
                "Saved complex_mask closure or physical delay differs")

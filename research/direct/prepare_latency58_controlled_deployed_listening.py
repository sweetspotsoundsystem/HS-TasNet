...
ORDER = ("working", "ordinary_control", "controlled_deployed")
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-controlled-deployed-listening-plan-v1"
    ...
    for row in plan["models"]:
        ...
        if row["id"] in ORDER[1:]:
            ...
            require(summary["schema"] == ("latency58-counterfactual-quality-summary-v1" if row["id"] == "ordinary_control"
                                           else "latency58-controlled-deployed-quality-summary-v1") and summary["status"] == "pass"
    ...
    require(match["model_states"] == {"reference": models["ordinary_control"]["model_state_sha256"],
                                       "candidate": models["controlled_deployed"]["model_state_sha256"]}
    ...
    replacements = {
        "Actions · training comparison": "Vocal cleanup · Actions",
        "Compare the C91 reference, native C191 and two completed training trials on Actions – One Minute Smile, 60–75 seconds. No new candidate has been retained.":
            "Compare the working model and two vocal-cleanup trials on Actions – One Minute Smile, 60–75 seconds. Listen for vocals in Other, unwanted sound in Vocals and changes to instruments.",
    ...
    with (out / "index.html").open("x") as stream:
        stream.write(html)
    write(out / "result.json", {"schema": "latency58-controlled-deployed-listening-preparation-v1", "status": "pass",

...
def runtime_policy():
    return {"version": "four-second-b1-weighted-packed-save-runtime-v1", "resource_max_seconds": 1800,
            "production_seconds_per_update": 150, "production_fixed_allowance_seconds": 600,
...
def validate_recipe(plan):
    ...
    require(sha(decision_binding["path"]) == decision_binding["sha256"]
            == "a978ab2716f5764956b632a9decbb699895aca93dc309eb3f2d06d8c2355e6c2",
    ...
    require(config["batch_size"] == 16 and config["microbatch_size"] == 1
            and config["auxiliary_microbatch_size"] == 1 and plan["accumulation_steps"] == 16
...
def main():
    ...
    require(run == Path(plan["output_directory"]) / ("resource-run-004" if stage["resource_only"] else "production-run")
    ...
    model.provenance = {**model.provenance, "branch_memory_current_stage_corpus": "301 recorded MUSDB18-HQ and MoisesDB training tracks",
                        ...
                        "branch_memory_current_stage_training_context": {"warmup_samples": plan["warmup_samples"],
                            "scored_samples": plan["scored_samples"], "logical_batch_size": 16, "microbatch_size": 1,
                            "logical_batch_loss": reduction_policy(), "grouped_vocal_loss": applied_policy(),
                            "auxiliary_examples": 2, "auxiliary_microbatch_size": 1,
    ...
    require(config["microbatch_size"] == 1 and plan["accumulation_steps"] == 16
    ...
    if stage["resource_only"]:
        from research.direct.check_latency58_four_second_single_device import check_gpu

...
def main():
    ...
    from research.direct.check_latency58_best_onnx_arithmetic_memory import selected_endpoint, selected_variant
    ...
    screen_root = PHASE / "best-model-onnx-arithmetic-memory-001"
    screen, screen_plan = read(screen_root / "result.json"), read(screen_root / "plan.json")
    execution_path = PHASE / "best-model-onnx-arithmetic-memory-stage-001/execution.json"
    ...
    for direction, actual in (("input", session.get_inputs()), ("output", session.get_outputs())):
        require(tuple(value.name for value in actual) == contract[direction + "_names"]
                ...
                and all(value.type == "tensor(float)" for value in actual), "Selected-model ABI differs")
    ...
    paths = [Path(__file__).resolve(), path, *diagnostic_paths, screen_root / "result.json", screen_root / "plan.json", execution_path,
             ROOT / "research/direct/check_latency58_best_onnx_long_v2.py",
             PHASE / "best-model-onnx-long-stage-002/execution.json"]
    bindings = {**screen_plan["source_bindings"], **{str(p): sha(p) for p in paths}}
    verify_inputs({"source_bindings": bindings})
    out = PHASE / "best-model-onnx-arithmetic-long-001"

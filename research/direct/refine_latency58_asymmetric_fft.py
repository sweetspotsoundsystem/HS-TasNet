"""Test FP64 for the forward FFT while retaining the public FP32 graph ABI.

This is a measured arithmetic hypothesis, not a quality or runtime promotion.
The learned tensors and inverse synthesis transform are preserved exactly.
"""
...
def main():
    ...
    require(plan["schema"] == "latency58-asymmetric-fft-refinement-plan-v1"
    ...
    output = out / "asymmetric500-forward-fft64.onnx"
    ...
    for node in graph.graph.node:
        ...
        if node.op_type == "DFT" and attrs.get("inverse", 0) == 0:
            require(node.name == "/DFT" and attrs == {"axis": 1, "inverse": 0, "onesided": 1}
                    and len(node.input) == 2 and len(node.output) == 1, "Unexpected forward transform")
            original_output, original_input = node.output[0], node.input[0]
            node.input[0] = original_input + "_fft64"
            node.output[0] = original_output + "_fft64"
            nodes.extend((onnx.helper.make_node("Cast", [original_input], [node.input[0]],
                          name="/ForwardFFTToDouble", to=onnx.TensorProto.DOUBLE), node,
                          onnx.helper.make_node("Cast", [node.output[0]], [original_output],
                          name="/ForwardFFTToFloat", to=onnx.TensorProto.FLOAT)))
            changed.append(node.name)
        else:
            nodes.append(node)
    require(changed == ["/DFT"], "Change exactly one forward FFT")
    del graph.graph.node[:]
    graph.graph.node.extend(nodes)
    props = {v.key: v.value for v in graph.metadata_props}
    props.update({"hs_tasnet.forward_fft_precision": "float64_internal_float32_io",
    ...
    result = {"status": "passed_short_fixtures_only" if verification["passed"] else "failed_not_qualified",
              ...
              "graph_transform": "Only forward DFT input cast FP64 and output cast FP32; inverse DFT unchanged",

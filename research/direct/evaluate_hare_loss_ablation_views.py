"""Score the same fixed vocal hare_loss_ablations for a completed hare_loss_ablation pilot."""
...
def initialize_worker(plan):
    ...
    spec = plan["model"]
    require(spec["kind"] == "hare_loss_ablation", "Require the new hare_loss_ablation checkpoint family")
    binding = spec["quality_plan"]
    require(sha(binding["path"]) == binding["sha256"], "Diagnostic quality plan changed")
    from research.direct.evaluate_hare_loss_ablation import load_evaluation_model
...
def main():
    ...
    require(sha(args.plan) == args.plan_sha256, "Vocal hare_loss_ablation plan changed")
    plan = read(args.plan)
    require(plan["schema"] == "hare-loss-ablation-views-evaluation-plan-v1" and plan["workers"] == 2

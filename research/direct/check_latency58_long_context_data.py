"""Check real ten-second crops and aligned two/eight-second history views."""
...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-long-context-data-v1" and plan["warmup_samples"] == 352256
            and plan["scored_samples"] == 88064 and plan["batch_size"] == 4
            and plan["sample_indices"] == list(range(963000, 963016))
    ...
    require(minimum >= length, "A corpus track is too short for a real ten-second crop")
    options = dict(root_weights=config["sampling"]["root_weights"], seed=config["seed"], crop_samples=length,
                   vocal_active_probability=config["sampling"]["vocal_active_probability"], final_sample_index=963016)
    left, right = (production.CounterAddressedCropDataset(tracks, **options) for _ in range(2))
    rows = []
    for start in range(963000, 963016, 4):
        ...
        pieces = []
        for region in (slice(0, 352256), slice(352256, None)):
        ...
        # Both student histories can share the exact final two seconds and
        # augmentation draws. Drawing a second, shorter dataset crop would not
        # preserve those coordinates, so take a view of this same long crop.
        short_start = 352256 - 88064
        torch.set_rng_state(rng)
        short = experiment._augment_training_distribution(
            mixture=mixture[..., short_start:], targets=targets[..., short_start:])
        require(torch.equal(torch.get_rng_state(), after)
                and torch.equal(short[0], full[0][..., short_start:])
                and torch.equal(short[1], full[1][..., short_start:])
                and torch.equal(short[2], full[2])
                and torch.equal(short[0][..., 88064:], full[0][..., 352256:])
                and torch.equal(short[1][..., 88064:], full[1][..., 352256:]),
                "Two/eight-second views lose matched augmentation or scored suffix")
        torch.set_rng_state(after)
        rows.append({"first_sample_index": start, "next_sample_index": start + 4,
                     ...
                     "two_second_history_view_start": short_start,
                     "short_view_augmentation_exact": True, "scored_suffix_exact": True,
                     "shared_scored_suffix_sha256_cpu": digest((("mixture", full[0][..., 352256:]),
                                                               ("targets", full[1][..., 352256:])))})
    ...
    result = {"schema": "latency58-long-context-data-result-v1", "status": "pass",
              "plan_sha256": args.plan_sha256, "manifest_sha256": manifest_sha, "minimum_track_frames": minimum,
              "corpus_tracks": len(tracks), "full_crop_samples": length, "warmup_samples": 352256,
              ...
              "training_recipe_frozen": False, "optimizer_updates": 0, "quality_improvement_established": False,
              "scope": "Data geometry only; no parent, objective, teacher-history choice or training run selected. CPU hashes are not GPU augmentation proof.",

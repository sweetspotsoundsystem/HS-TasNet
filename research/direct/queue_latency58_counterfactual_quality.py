...
from research.direct.report_latency58_counterfactual import REFERENCES, load_views, reference_views
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-counterfactual-queued-quality-plan-v1"
    ...
    require(prefix == "counterfactual-teacher-" + training["teacher_mode"].replace("_", "-") + "-250", "Different quality prefix")
    waited = 0.0
    ...
    if not ((stage / "execution.json").is_file() and (stage / "audit-execution.json").is_file()):
        ...
        try:
            ...
            require(int(stat[19]) == root_identity["start_ticks"] and command == root_identity["argv"]
                    and command[command.index("-m") + 1] == "research.direct.run_latency58_counterfactual_stage"
    ...
    for side, side_stage, side_training in (
            ("reference", Path(plan["control_stage_directory"]), training["matched_control_training_plan"]),
    ...
    match_plan = {"schema": "latency58-counterfactual-training-match-plan-v1", "models": models,
    ...
    execute([PYTHON, "-u", "-m", "research.direct.audit_latency58_counterfactual_training_match",
    ...
    commands = [
        [PYTHON, "-u", "-m", "research.direct.run_latency58_counterfactual_quality",
        ...
        [PYTHON, "-u", "-m", "research.direct.run_latency58_counterfactual_views", "--prefix", prefix,
    ...
    summary_plan = {"schema": "latency58-counterfactual-summary-plan-v1", "candidate_prefix": prefix,
    ...
    execute([PYTHON, "-u", "-m", "research.direct.report_latency58_counterfactual",
    ...
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-counterfactual-queued-quality-result-v1", "status": "pass",

"""Authenticated inference and optimizer snapshots for quadrature training."""
...
from research.direct.latency58_quadrature import Latency58QuadratureModel, VERSION

SCHEMA = "latency58-quadrature-inference-v1"
GENERATION_SCHEMA = "latency58-quadrature-generation-v1"
...
def load_model(binding):
    ...
    require(payload["schema"] == SCHEMA and type(payload["step"]) is int and payload["step"] > 0,
            "Unknown quadrature checkpoint or invalid step")
    with torch.random.fork_rng(devices=[]), torch.device("cpu"):
        model = Latency58QuadratureModel()
    names = [name for name, _ in model.named_parameters()]
    require(payload["architecture"] == model.architecture_metadata
            and payload["parameter_names"] == names and len(names) == 24,
            "Quadrature geometry or learned tensor inventory changed")
    ...
    require(set(payload["model"]) == set(model.state_dict())
            ...
            "Invalid quadrature inference tensors")
    ...
    require(state_sha256(dict(model.named_buffers())) == payload["fixed_buffers_sha256"]
            ...
            and provenance["quadrature_version"] == VERSION
            and provenance["quadrature_updates"] == payload["step"]
            and provenance["training_updates"] == provenance["quadrature_parent_updates"] + payload["step"]
            and provenance["quadrature_training_plan_sha256"] == payload["plan_sha256"]
            and provenance["quadrature_all_neural_parameters_trained"] is True,
            "Quadrature fixed buffers or lineage changed")
...
def audit_live(model, optimizer, step, frozen):
    ...
    require(type(model) is Latency58QuadratureModel and type(step) is int and step >= 0
            and len(parameters) == 24 and all(p.requires_grad for p in parameters)
            ...
            "Require all 24 learned tensors and one ordered optimizer group")
...
def save_generation(model, optimizer, step, plan, plan_sha, run):
    ...
    require(type(step) is int and step > 0 and step == plan["config"]["steps"]
            ...
            and model.provenance["quadrature_parent_model_state_sha256"] == plan["parent_model_state_sha256"],
            "Save only the declared quadrature endpoint with unchanged fixed buffers")
    audit_live(model, optimizer, step, dict(model.named_buffers()))
    pending, final = run / "checkpoint.pending", run / "checkpoint"
    require(not pending.exists() and not final.exists(), "Preserve existing quadrature checkpoints")
    pending.mkdir()
    tensors = cpu_tree(model.state_dict())
    names = [name for name, _ in model.named_parameters()]
    provenance = {**model.provenance, "quadrature_version": VERSION,
                  "quadrature_parent_checkpoint": plan["parent_checkpoint"],
                  "quadrature_parent_model_state_sha256": plan["parent_model_state_sha256"],
                  "quadrature_parent_updates": plan["parent_training_updates"],
                  "quadrature_updates": step, "training_updates": plan["parent_training_updates"] + step,
                  "quadrature_all_neural_parameters_trained": True,
                  "quadrature_training_plan_sha256": plan_sha,
                  "quadrature_objective_version": plan["objective_version"],
...
def audit_saved(binding, plan, plan_sha):
    ...
    require(receipt["schema"] == GENERATION_SCHEMA and set(receipt["files"]) == {"model.pt", "optimizer.pt"}
            ...
            and payload["provenance"]["quadrature_parent_checkpoint"] == plan["parent_checkpoint"]
            and payload["provenance"]["quadrature_parent_model_state_sha256"] == plan["parent_model_state_sha256"]
            and payload["provenance"]["quadrature_parent_updates"] == plan["parent_training_updates"],
    ...
    require(set(states) == set(range(24)) and len(groups) == 1 and groups[0]["params"] == list(range(24)),
    ...
    with torch.inference_mode():
        ...
        require(closure < 1e-6 and model.algorithmic_latency_samples == 256,
                "Saved quadrature closure or physical delay differs")
    return {"status": "pass", "step": payload["step"], "checkpoint": binding,
            ...
            "optimizer_sha256": receipt["files"]["optimizer.pt"]["sha256"], "saved_optimizer_tensor_count": 24,

...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-quarter-controlled-queued-quality-plan-v1"
    ...
    require(prefix == "quarter-controlled-250" and training["additional_loss_weight"] == .5, "Different quality prefix")
    waited = 0.0
    ...
    if not ((stage / "execution.json").is_file() and (stage / "audit-execution.json").is_file()):
        ...
        try:
            ...
            require(int(stat[19]) == root_identity["start_ticks"] and command == root_identity["argv"]
                    and command[command.index("-m") + 1] == "research.direct.run_latency58_quarter_controlled"
    ...
    # of plugin timing. The final summary separately verifies the matched view-frequency journal.
    commands = [
        [PYTHON, "-u", "-m", "research.direct.run_latency58_quarter_controlled_quality",
        ...
        [PYTHON, "-u", "-m", "research.direct.run_latency58_quarter_controlled_views", "--prefix", prefix,
    ...
    summary_plan = {"schema": "latency58-quarter-controlled-summary-plan-v1", "candidate_prefix": prefix,
    ...
    execute([PYTHON, "-u", "-m", "research.direct.report_latency58_quarter_controlled",
    ...
    verify_inputs(plan)
    write(out / "result.json", {"schema": "latency58-quarter-controlled-queued-quality-result-v1", "status": "pass",

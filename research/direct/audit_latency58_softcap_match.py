"""Authenticate matched data and RNG for the two auxiliary recipes at 250."""
...
def main():
    ...
    import numpy as np
    import torch
    from research.direct import latency58_log_relative_checkpoint as reference_loader
    from research.direct import latency58_sdr_softcap_checkpoint as candidate_loader
    ...
    arms = (("log_relative", "log-relative", args.reference_plan, reference_loader),
            ("sdr_softcap", "sdr-softcap", args.candidate_plan, candidate_loader))
    ...
    shared = ("parent", "parent_prefix", "config", "warmup_samples", "scored_samples", "carry_state",
              ...
              "manifest_sha256", "geometry", "functional_proofs")
    require(all(plans["log_relative"][k] == plans["sdr_softcap"][k] for k in shared),
            "Parent, data, optimizer schedule, teacher or geometry differs")
    require(plans["log_relative"]["log_relative_weight"] == 0.01
            and plans["sdr_softcap"]["sdr_softcap_weight"] == 0.003
            and plans["sdr_softcap"]["sdr_softcap_error_ratio_floor"] == 0.01,
    ...
    for left, right in zip(journals["log_relative"], journals["sdr_softcap"], strict=True):
        require(all(left[k] == right[k] for k in keys)
                and left["log_relative_active_windows"] == right["sdr_softcap_active_windows"]
                and left["log_relative_active_examples_per_stem"]
                == right["sdr_softcap_active_examples_per_stem"],
                "Actual matched condition differs at step " + str(left["step"]))
    left, right = rngs["log_relative"], rngs["sdr_softcap"]
    ...
    write(args.output, {
        "schema": "latency58-softcap-matched-batches-v1", "status": "pass", "step": step,
        ...
        "initial_model_state_sha256": plans["log_relative"]["parent"]["model_state_sha256"],
        "different_trained_model_states": states, "quality_conclusion": None,
        "treatment": "Joint change in auxiliary shape (0 dB to 20 dB soft cap) and coefficient (0.01 to 0.003)",

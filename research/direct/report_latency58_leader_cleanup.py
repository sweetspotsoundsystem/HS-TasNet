"""Compare the leader-cleanup endpoint with both parents and the completed cleanup recipe."""
...
REFERENCES = {"working": "teacher-half250", "leader": "sdr-drum-accum-500",
              "working_cleanup": "controlled-deployed-half-250"}


def reference_views(label):
    return PHASE / {"working": "vocal-views-working-001", "leader": "vocal-views-drum500-001",
                    "working_cleanup": "controlled-deployed-half-250-views-001"}[label]
...
def main():
    ...
    plan = read(args.plan)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-leader-cleanup-summary-plan-v1", "Unexpected summary plan")
    ...
    require(reference_states["working_cleanup"] == match["model_states"]["reference"],
            "Matched control state differs from scored control")
    require(reference_states["leader"] == training["parent"]["model_state_sha256"], "Leader reference differs from parent")
    require(all(plan["source_bindings"].get(p) == s for p, s in evidence.items()), "Unbound completed score evidence")
    verify_inputs(plan)
    write(out / "result.json", {
        "schema": "latency58-leader-cleanup-quality-summary-v1", "status": "pass", "plan_sha256": args.plan_sha256,
        ...
        "additional_loss_version": training["additional_loss_version"], "training_match": plan["training_match"],
        ...
        "all_metrics_compared": True, "comparison_variable": "training_parent",
        "matched_loss_effect_from_leader_claimed": False, "quality_selected": False, "human_listening_completed": False,

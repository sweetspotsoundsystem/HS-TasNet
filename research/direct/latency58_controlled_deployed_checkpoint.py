"""Authenticated generations for matched controlled deployed-source supervision."""
...
from research.direct.latency58_counterfactual_journal import (
    UNIFORM_KEYS, compare_control_prefix, rng_state_sha256)

from research.direct.latency58_controlled_deployed_journal import EXTRA_LOSS_KEYS, validate_microbatch

LOSS_KEYS = (*LOSS_KEYS, *UNIFORM_KEYS, *EXTRA_LOSS_KEYS)

SHARED_CONTROL_KEYS = (
    "config", "parent", "initialized_model_state_sha256", "architecture", "geometry",
    "warmup_samples", "scored_samples", "carry_state", "microbatch_size", "accumulation_steps",
    "teacher_kind", "teacher_weight", "teacher", "teacher_model_state_sha256",
    "drum_weight", "objective_version", "augmentation_version", "accumulation_version",
    "optimizer_initialization", "mixer_initialization_seed", "arm", "focused_augmentation",
    "local_mask_mixer", "parameter_tensors", "environment", "precision_policy", "torch_version",
    "helper_source", "watchdog_source", "manifest_sha256", "automatic_continuation",
    "counted_roots", "stop_counted_bytes", "quality_endpoints")
...
def base_plan(plan):
    return {**plan, "schema": "latency58-counterfactual-training-v1"}


def validate_recipe(plan):
    from research.direct.latency58_counterfactual_checkpoint import validate_recipe as validate_base
    from research.direct.latency58_controlled_deployed_loss import VERSION
    require(plan["schema"] == "latency58-controlled-deployed-training-v1"
            and plan["teacher_mode"] == "ordinary_only"
            and plan["additional_loss_version"] == VERSION
            and type(plan["additional_loss_weight"]) in (int, float)
            and plan["additional_loss_weight"] in (0, .5)
            and (plan["resource_only"] or plan["additional_loss_weight"] == .5), "Different deployed supervision recipe")
    validate_base(base_plan(plan))
    item = plan["matched_ordinary_training_plan"]
    require(plan["source_bindings"].get(item["path"]) == item["sha256"] == sha(item["path"]), "Unbound ordinary control")
    control = read(item["path"])
    require(control["schema"] == "latency58-counterfactual-training-v1" and not control["resource_only"]
            and all(plan[k] == control[k] for k in (*SHARED_CONTROL_KEYS, "teacher_mode",
                "counterfactual_version", "matched_control_training_plan")),
            "The controlled deployed recipe changes more than its added term")
    decision_item = plan["preparation_decision"]
    require(plan["source_bindings"].get(decision_item["path"]) == decision_item["sha256"] == sha(decision_item["path"]),
            "Unbound deployed truth decision")
    decision = read(decision_item["path"])
    require(decision["status"] == "prepare_matched_controlled_deployed_truth_pilot"
            and decision["matched_control_training_plan"] == item
            and decision["training_parent"] == plan["parent"] and decision["config"] == plan["config"]
            and decision["reference_resource"] == plan["reference_resource"]
            and decision["maximum_production_updates"] == 250
            and not decision["automatic_continuation"] and not decision["quality_selected"], "Different fixed experiment")
    item = plan["reference_resource"]
    require(plan["source_bindings"].get(item["path"]) == item["sha256"] == sha(item["path"]), "Unbound zero replay reference")


def load_parent(plan):
    from research.direct.latency58_counterfactual_checkpoint import load_parent as load_base
    validate_recipe(plan)
    return load_base(base_plan(plan))
...
def expected_provenance(plan, step, plan_sha):
    ...
    value.update(initialization="matched_counterfactual_teacher_fine_tune",
                 training_objective="mean_four_b4_normalized_drum_raw4_and_selected_context_teacher_l1",
                 counterfactual_trial_updates=step, counterfactual_version=plan["counterfactual_version"],
                 teacher_mode=plan["teacher_mode"], teacher_batch_divisor="all_four_examples",
                 matched_control_training_plan=plan["matched_control_training_plan"])
    value.update(initialization="matched_controlled_deployed_truth_fine_tune",
                 training_objective="ordinary_only_teacher_plus_controlled_deployed_truth",
                 controlled_deployed_trial_updates=step,
                 ...
                 matched_ordinary_training_plan=plan["matched_ordinary_training_plan"])
...
def read_generation(directory, *, expected_plan_sha=None, require_optimizer=True):
    ...
    require(receipt["schema"] == "latency58-controlled-deployed-generation-v1"
...
def load_model(directory, plan, *, expected_plan_sha):
    ...
    step = receipt["step"]
    require(payload["schema"] == "latency58-controlled-deployed-inference-v1" and payload["step"] == step
...
def validate_journal(journal, step, plan, helpers):
    import math
    from research.direct.latency58_counterfactual_checkpoint import validate_journal as validate_base
    validate_recipe(plan)
    ...
    for actual, base in zip(rows, projected, strict=True):
        ...
        for micro in actual["microbatches"]:
            validate_microbatch(micro, plan["additional_loss_weight"])
...
def save_generation(run, model, teacher, optimizer, step, plan, plan_sha, helpers):
    ...
    payload = {"schema": "latency58-controlled-deployed-inference-v1", "step": step, "model": tensors,
    ...
    with (pending / "metrics.jsonl").open("xb") as stream:
        ...
        stream.flush()
        os.fsync(stream.fileno())
    receipt = {"schema": "latency58-controlled-deployed-generation-v1", "step": step, "plan_sha256": plan_sha,

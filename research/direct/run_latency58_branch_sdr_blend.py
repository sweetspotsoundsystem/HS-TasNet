"""Execute the trained-branch 0.2 SDR blend with monitored GPU stages and unchanged full14 scoring."""
...
def launch(plan_path, previous_execution_path, *, resource):
    plan = read(plan_path)
    verify_inputs(plan)
    require(plan["schema"] == "latency58-branch-memory-training-plan-v1", "Wrong branch-memory training plan")
    ...
    spec = {"schema": "gpu-watchdog-launch-v1", "cwd": str(ROOT), "environment": plan["environment"],
            ...
            "argv": [PYTHON, "-u", "-m", "research.direct.train_latency58_branch_sdr_blend", "--plan", str(plan_path),
...
def audit_and_score(plan_path):
    import torch
    from research.direct.latency58_branch_memory_checkpoint import audit_saved
    ...
    require(training["status"] == "pass" and training["checkpoint_written"]
            ...
            "Require completed branch-memory training before scoring")
    ...
    require(audit["step"] == training["updates"]
            ...
            "Saved branch-memory model differs from the training endpoint")
    verify_inputs(plan)
    ...
    quality_plan = {"schema": "latency58-branch-memory-full14-plan-v1", "label": plan["name"], "checkpoint": checkpoint,

"""Self-contained inference snapshots for direct SDR fine-tuning."""
...
SCHEMA = "latency58-direct-sdr-inference-v1"
...
def load_model(binding):
    ...
    if payload["schema"] == VERSION:
        return load_checkpoint(binding["path"], binding["sha256"])
    ...
    require(model.provenance["direct_sdr_updates"] == payload["step"]
...
def save_generation(model, optimizer, step, plan, plan_sha, run):
    ...
    write(pending / "receipt.json", {"schema": "latency58-direct-sdr-generation-v1", "step": step,

"""Independent CPU reconstruction with sixteen signed integer projections.

The parent streaming reference is reproduced with exactly the two branch
output calls routed through independently reconstructed integer projections.
No runtime graph outputs are used as an oracle.
"""
import torch
...
def make_reference(*args):
    from research.direct.latency58_branch_output_int8 import make_reference as reconstruct
    original, proof = reconstruct(*args)
    return BranchOutputIntegerReference(original.model).eval().requires_grad_(False), proof


class BranchOutputIntegerReference(nn.Module):

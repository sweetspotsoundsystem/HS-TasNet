...
def main():
    ...
    plan = read(args.plan)
    require(plan["schema"] == "latency58-plugin-build-storage-plan-v1"
            and plan["steps"] == [525, 550, 575, 600, 625]
            and plan["reserve_after_bytes"] == 1_600_000_000, "Different bounded retirement")
    verify_inputs(plan)
    import torch
    ...
    write(out / "intent.json", {
        "schema": "latency58-plugin-build-storage-intent-v1", "status": "audited",
        "plan_sha256": args.plan_sha256, "remove": remove, "protected_bindings": protected,
        "reason": "Reserve candidate plugin build, fixture, LFS and Mac handoff space under the existing cap. Retire only five obsolete intermediate full resumes; preserve every corresponding inference tensor and the final step-2000 full resume. Same-Adam/RNG restart at these five old intermediate steps is intentionally retired.",

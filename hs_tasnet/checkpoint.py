...
snapshots are lossless and self-contained. They do not depend on trial paths,
machine-bound plans, external XOR parents or historical monitoring receipts.
"""
...
def _native_payload_model(payload):
    _require(payload["schema"] == NATIVE_SCHEMA and type(payload["step"]) is int and payload["step"] > 0,
...
def save_training_checkpoint(path, model, optimizer, ema, *, step, next_sample_index,
    ...
    envelope = {"schema": SCHEMA, "codec": CODEC if compressed else None,
